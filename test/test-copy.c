...
#include <libscatola-private/scatola-progress-info.h>
...
static void
changed_cb (ScatolaProgressInfo *info,
	    gpointer data)
{
	g_print ("Changed: %s -- %s\n",
		 scatola_progress_info_get_status (info),
		 scatola_progress_info_get_details (info));
}

static void
progress_changed_cb (ScatolaProgressInfo *info,
		     gpointer data)
{
	g_print ("Progress changed: %f\n",
		 scatola_progress_info_get_progress (info));
}

static void
finished_cb (ScatolaProgressInfo *info,
	     gpointer data)
{
	g_print ("Finished\n");
	gtk_main_quit ();
...
main (int argc, char* argv[])
{
	...
	int i;
	GList *infos;
	ScatolaProgressInfo *progress_info;
	...
	scatola_file_operations_copy (sources,
				       NULL /* GArray *relative_item_points */,
				       dest,
				       GTK_WINDOW (window),
				       copy_done, NULL);

	infos = scatola_get_all_progress_info ();

	if (infos == NULL) {
		return 0;
	}

	progress_info = SCATOLA_PROGRESS_INFO (infos->data);
