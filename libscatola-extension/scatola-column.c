/*
 *  scatola-column.c - Info columns exported by ScatolaColumnProvider
 *                      objects.
 *
...
G_DEFINE_TYPE (ScatolaColumn, scatola_column, G_TYPE_OBJECT);

/**
 * SECTION:scatola-column
 * @title: ScatolaColumn
 * @short_description: List view column descriptor object
 * @include: libscatola-extension/scatola-column.h
 *
 * #ScatolaColumn is an object that describes a column in the file manager
 * list view. Extensions can provide #ScatolaColumn by registering a
 * #ScatolaColumnProvider and returning them from
 * scatola_column_provider_get_columns(), which will be called by the main
 ...
 *
 * Returns: a newly created #ScatolaColumn
 */
ScatolaColumn *
scatola_column_new (const char *name,
                 ...
                 const char *description)
{
    ...
    column = g_object_new (SCATOLA_TYPE_COLUMN,
                           "name", name,
                           "attribute", attribute,
                           "label", label,
...
static void
scatola_column_get_property (GObject *object,
                          guint param_id,
                          GValue *value,
                          GParamSpec *pspec)
{
...
static void
scatola_column_set_property (GObject *object,
                          guint param_id,
                          const GValue *value,
                          GParamSpec *pspec)
{
...
static void
scatola_column_finalize (GObject *object)
{
    ScatolaColumn *column;
    ...
    G_OBJECT_CLASS (scatola_column_parent_class)->finalize (object);
}

static void
scatola_column_init (ScatolaColumn *column)
{
    column->details = g_new0 (ScatolaColumnDetails, 1);
    column->details->xalign = 0.0;
}

static void
scatola_column_class_init (ScatolaColumnClass *class)
{
    G_OBJECT_CLASS (class)->finalize = scatola_column_finalize;
    G_OBJECT_CLASS (class)->get_property = scatola_column_get_property;
    G_OBJECT_CLASS (class)->set_property = scatola_column_set_property;
