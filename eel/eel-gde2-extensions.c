...
/* eel-gde2-extensions.c - implementation of new functions that operate on
                            gde2 classes. Perhaps some of these should be
  			    rolled into gde2 someday.
...
char *
eel_gde2_make_terminal_command (const char *command)
{
...
void
eel_gde2_open_terminal_on_screen (const char *command,
                                  GdkScreen  *screen)
{
    char *command_line;
    ...
    command_line = eel_gde2_make_terminal_command (command);
    ...
    gde2_gdk_spawn_command_line_on_screen(screen, command_line, NULL);
    g_free (command_line);
}
