...
bookmark_holder_new (ScatolaBookmark *bookmark,
                     ScatolaWindow *window,
...
activate_bookmark_in_menu_item (GtkAction *action, gpointer user_data)
{
    ...
    if (scatola_bookmark_uri_known_not_to_exist (holder->bookmark))
    {
        holder->failed_callback (holder->window, holder->bookmark);
    }
    else
    {
        location = scatola_bookmark_get_location (holder->bookmark);
        slot = scatola_window_get_active_slot (holder->window);
        scatola_window_slot_go_to (slot,
                                location,
...
void
scatola_menus_append_bookmark_to_menu (ScatolaWindow *window,
                                    ScatolaBookmark *bookmark,
...
action_connect_to_server_callback (GtkAction *action,
                                   gpointer user_data)
{
    ScatolaWindow *window = SCATOLA_WINDOW (user_data);
    GtkWidget *dialog;

    dialog = scatola_connect_server_dialog_new (window);
                      ...
                      gpointer user_data)
{
    ...
    scatola_window_slot_stop_loading (slot);
}
                      ...
                      gpointer user_data)
{
    ...
    scatola_window_slot_go_home (slot,
                              should_open_in_new_tab ());
}
                                 ...
                                 gpointer user_data)
{
    ...
    path = scatola_get_templates_directory ();
...
action_reload_callback (GtkAction *action,
                        gpointer user_data)
{
    scatola_window_reload (SCATOLA_WINDOW (user_data));
}
...
action_zoom_in_callback (GtkAction *action,
                         gpointer user_data)
{
    scatola_window_zoom_in (SCATOLA_WINDOW (user_data));
}
...
action_zoom_out_callback (GtkAction *action,
                          gpointer user_data)
{
    scatola_window_zoom_out (SCATOLA_WINDOW (user_data));
}
...
action_zoom_normal_callback (GtkAction *action,
                             gpointer user_data)
{
    scatola_window_zoom_to_default (SCATOLA_WINDOW (user_data));
}
                                   ...
                                   gpointer callback_data)
{
    ...
    {
        mode = SCATOLA_WINDOW_SHOW_HIDDEN_FILES_DISABLE;
    }

    scatola_window_info_set_hidden_files_mode (window, mode);
}
...
show_hidden_files_preference_callback (gpointer callback_data)
{
    ...
    if (window->details->show_hidden_files_mode == SCATOLA_WINDOW_SHOW_HIDDEN_FILES_DEFAULT)
    {
        ...
        /* inform views */
        scatola_window_info_set_hidden_files_mode (window, SCATOLA_WINDOW_SHOW_HIDDEN_FILES_DEFAULT);
                             ...
                             gpointer user_data)
{
    ...
    scatola_file_management_properties_dialog_show (G_CALLBACK (preferences_respond_callback), window);
}
        ...
        gpointer user_data)
{
    ...
    scatola_property_browser_show (gtk_window_get_screen (window));
}

static void
action_about_scatola_callback (GtkAction *action,
                            gpointer user_data)
{
    const gchar *authors[] =
    {
    ...
    const gchar *license[] =
    {
        N_("Scatola is free software; you can redistribute it and/or modify "
        "it under the terms of the GNU General Public License as published by "
        "the Free Software Foundation; either version 2 of the License, or "
        "(at your option) any later version."),
        N_("Scatola is distributed in the hope that it will be useful, "
        "but WITHOUT ANY WARRANTY; without even the implied warranty of "
        "MERCHANTABILITY or FITNESS FOR A PARTICULAR PURPOSE.  See the "
        "GNU General Public License for more details."),
        N_("You should have received a copy of the GNU General Public License "
        "along with Scatola; if not, write to the Free Software Foundation, Inc., "
    ...
    gtk_show_about_dialog (GTK_WINDOW (user_data),
                           "program-name", _("Scatola"),
                           "version", VERSION,
                           "comments", _("Scatola lets you organize "
                                         "files and folders, both on "
                                         "your computer and online."),
                           "copyright", _("Copyright \xC2\xA9 1999-2009 The Nautilus authors\n"
                                          "Copyright \xC2\xA9 2011-2016 The Scatola authors"),
...
action_up_callback (GtkAction *action,
                    gpointer user_data)
{
    scatola_window_go_up (SCATOLA_WINDOW (user_data), FALSE, should_open_in_new_tab ());
}

static void
action_scatola_manual_callback (GtkAction *action,
                             gpointer user_data)
{
    ScatolaWindow *window;
    GError *error;
    ...
    gtk_show_uri (gtk_window_get_screen (GTK_WINDOW (window)),
                   SCATOLA_IS_DESKTOP_WINDOW (window)
                      ? "help:gde2-user-guide"
                      : "help:gde2-user-guide/gosscatola-1",
                        ...
                        ScatolaWindow *window)
{
    ...
    gicon = scatola_trash_monitor_get_icon ();
...
static void
scatola_window_initialize_trash_icon_monitor (ScatolaWindow *window)
{
    ScatolaTrashMonitor *monitor;

    monitor = scatola_trash_monitor_get ();
...
static const GtkActionEntry main_entries[] =
{
    ...
    },
    {
        ...
        N_("Prefere_nces"),
        NULL, N_("Edit Scatola preferences"),
    ...
    /* name, stock id */         { "Scatola Manual", GTK_STOCK_HELP,
        /* label, accelerator */       N_("_Contents"), "F1",
        /* tooltip */                  N_("Display Scatola help"),
        G_CALLBACK (action_scatola_manual_callback)
    },
    /* name, stock id */         { "About Scatola", GTK_STOCK_ABOUT,
        /* label, accelerator */       N_("_About"), NULL,
        /* tooltip */                  N_("Display credits for the creators of Scatola"),
        G_CALLBACK (action_about_scatola_callback)
    },
    ...
    /* name, stock id */         { "Go to Network", SCATOLA_ICON_NETWORK,
    ...
    /* name, stock id */         { "Go to Templates", SCATOLA_ICON_TEMPLATE,
    ...
    /* name, stock id */         { "Go to Trash", SCATOLA_ICON_TRASH,
 ...
 */
void
scatola_window_initialize_menus (ScatolaWindow *window)
{
    GtkActionGroup *action_group;
    ...
    action = gtk_action_group_get_action (action_group, SCATOLA_ACTION_UP);
    g_object_set (action, "short_label", _("_Up"), NULL);

    action = gtk_action_group_get_action (action_group, SCATOLA_ACTION_HOME);
    g_object_set (action, "short_label", _("_Home"), NULL);

    action = gtk_action_group_get_action (action_group, SCATOLA_ACTION_SHOW_HIDDEN_FILES);
    g_signal_handlers_block_by_func (action, action_show_hidden_files_callback, window);
    gtk_toggle_action_set_active (GTK_TOGGLE_ACTION (action),
                                  g_settings_get_boolean (scatola_preferences, SCATOLA_PREFERENCES_SHOW_HIDDEN_FILES));
    g_signal_handlers_unblock_by_func (action, action_show_hidden_files_callback, window);
    ...
    g_signal_connect_swapped (scatola_preferences, "changed::" SCATOLA_PREFERENCES_SHOW_HIDDEN_FILES,
    ...
    ui = scatola_ui_string_get ("scatola-shell-ui.xml");
    gtk_ui_manager_add_ui_from_string (ui_manager, ui, -1, NULL);

    scatola_window_initialize_trash_icon_monitor (window);
}

void
scatola_window_finalize_menus (ScatolaWindow *window)
{
    ScatolaTrashMonitor *monitor;
...
static GList *
get_extension_menus (ScatolaWindow *window)
{
    ScatolaWindowSlot *slot;
    GList *providers;
    GList *items;
    ...
    for (l = providers; l != NULL; l = l->next)
    {
        ...
        file_items = scatola_menu_provider_get_background_items (provider,
                     GTK_WIDGET (window),
                     slot->viewed_file);
...
static void
add_extension_menu_items (ScatolaWindow *window,
                          guint merge_id,
                          GtkActionGroup *action_group,
                          GList *menu_items,
                          const char *subdirectory)
{
    ...
    for (l = menu_items; l; l = l->next)
    {
        ...
        action = scatola_action_from_menu_item (item);
...
void
scatola_window_load_extension_menus (ScatolaWindow *window)
{
    GtkActionGroup *action_group;
    GList *items;
    guint merge_id;
