...
typedef struct
{
    ...
    gboolean mounting;
    ScatolaWindowSlotInfo *go_to_after_mount_slot;
    ScatolaWindowOpenFlags go_to_after_mount_flags;

    GtkTreePath *eject_highlight_path;
} ScatolaPlacesSidebar;

typedef struct
{
    GtkScrolledWindowClass parent;
} ScatolaPlacesSidebarClass;

typedef struct
{
    GObject parent;
} ScatolaPlacesSidebarProvider;

typedef struct
{
    GObjectClass parent;
} ScatolaPlacesSidebarProviderClass;
...
static void  scatola_places_sidebar_iface_init        (ScatolaSidebarIface         *iface);
static void  sidebar_provider_iface_init               (ScatolaSidebarProviderIface *iface);
static GType scatola_places_sidebar_provider_get_type (void);
static void  open_selected_bookmark                    (ScatolaPlacesSidebar        *sidebar,
        GtkTreeModel                 *model,
        GtkTreePath                  *path,
        ScatolaWindowOpenFlags flags);

#if GTK_CHECK_VERSION (3, 0, 0)
static void  scatola_places_sidebar_style_updated         (GtkWidget                    *widget);
#else
static void  scatola_places_sidebar_style_set             (GtkWidget                    *widget,
        GtkStyle                     *previous_style);
#endif
static gboolean eject_or_unmount_bookmark              (ScatolaPlacesSidebar *sidebar,
        GtkTreePath *path);
static gboolean eject_or_unmount_selection             (ScatolaPlacesSidebar *sidebar);
...
static void bookmarks_check_popup_sensitivity          (ScatolaPlacesSidebar *sidebar);
...
static const GtkTargetEntry scatola_shortcuts_source_targets[] =
{
    { "GTK_TREE_MODEL_ROW", GTK_TARGET_SAME_WIDGET, GTK_TREE_MODEL_ROW }
};

/* Target types for dropping into the shortcuts list */
static const GtkTargetEntry scatola_shortcuts_drop_targets [] =
{
    { "GTK_TREE_MODEL_ROW", GTK_TARGET_SAME_WIDGET, GTK_TREE_MODEL_ROW },
...
typedef struct
{
    ...
    ScatolaPlacesSidebar *sidebar;
} ScatolaShortcutsModelFilter;

typedef struct
{
    GtkTreeModelFilterClass parent_class;
} ScatolaShortcutsModelFilterClass;

#define SCATOLA_SHORTCUTS_MODEL_FILTER_TYPE (_scatola_shortcuts_model_filter_get_type ())
#define SCATOLA_SHORTCUTS_MODEL_FILTER(obj) (G_TYPE_CHECK_INSTANCE_CAST ((obj), SCATOLA_SHORTCUTS_MODEL_FILTER_TYPE, ScatolaShortcutsModelFilter))

GType _scatola_shortcuts_model_filter_get_type (void);
static void scatola_shortcuts_model_filter_drag_source_iface_init (GtkTreeDragSourceIface *iface);

G_DEFINE_TYPE_WITH_CODE (ScatolaShortcutsModelFilter,
                         _scatola_shortcuts_model_filter,
                         GTK_TYPE_TREE_MODEL_FILTER,
                         G_IMPLEMENT_INTERFACE (GTK_TYPE_TREE_DRAG_SOURCE,
                                 scatola_shortcuts_model_filter_drag_source_iface_init));

static GtkTreeModel *scatola_shortcuts_model_filter_new (ScatolaPlacesSidebar *sidebar,
        GtkTreeModel          *child_model,
        GtkTreePath           *root);

G_DEFINE_TYPE_WITH_CODE (ScatolaPlacesSidebar, scatola_places_sidebar, GTK_TYPE_SCROLLED_WINDOW,
                         G_IMPLEMENT_INTERFACE (SCATOLA_TYPE_SIDEBAR,
                                 scatola_places_sidebar_iface_init));

G_DEFINE_TYPE_WITH_CODE (ScatolaPlacesSidebarProvider, scatola_places_sidebar_provider, G_TYPE_OBJECT,
...
{
    GdkPixbuf *eject;
    ScatolaIconInfo *eject_icon_info;
    int icon_size;

    icon_size = scatola_get_icon_size_for_stock_size (GTK_ICON_SIZE_MENU);

    eject_icon_info = scatola_icon_info_lookup_from_name ("media-eject", icon_size);
    eject = scatola_icon_info_get_pixbuf_at_size (eject_icon_info, icon_size);
...
static gboolean
is_built_in_bookmark (ScatolaFile *file)
{
    gboolean built_in;
    gint idx;
    ...
    for (idx = 0; idx < G_USER_N_DIRECTORIES; idx++) {
        ...
        if (scatola_file_is_user_special_directory (file, idx)) {
...
static GtkTreeIter
add_heading (ScatolaPlacesSidebar *sidebar,
...
static void
check_heading_for_section (ScatolaPlacesSidebar *sidebar,
               SectionType section_type)
{
    switch (section_type) {
...
static GtkTreeIter
add_place (ScatolaPlacesSidebar *sidebar,
           ...
           const char *tooltip)
{
    ...
    icon_size = scatola_get_icon_size_for_stock_size (GTK_ICON_SIZE_MENU);
    icon_info = scatola_icon_info_lookup (icon, icon_size);
...
static void
compare_for_selection (ScatolaPlacesSidebar *sidebar,
...
static void
update_places (ScatolaPlacesSidebar *sidebar)
{
    ...
    if (strcmp (g_get_home_dir(), desktop_path) != 0) {
        ...
        mount_uri = scatola_get_home_directory_uri ();
        display_name = g_filename_display_basename (g_get_home_dir ());
        icon = g_themed_icon_new (SCATOLA_ICON_HOME);
    ...
    icon = g_themed_icon_new (SCATOLA_ICON_DESKTOP);
    last_iter = add_place (sidebar, PLACES_BUILT_IN,
                           SECTION_COMPUTER,
                           _("Desktop"), icon,
                           mount_uri, NULL, NULL, NULL, 0,
    ...
    icon = g_themed_icon_new (SCATOLA_ICON_FILESYSTEM);
    last_iter = add_place (sidebar, PLACES_BUILT_IN,
                           SECTION_COMPUTER,
                           _("File System"), icon,
                           mount_uri, NULL, NULL, NULL, 0,
    ...
    for (index = 0; index < G_USER_N_DIRECTORIES; index++) {
        ...
        icon = scatola_user_special_directory_get_gicon (index);
    ...
    /* add bookmarks */
    bookmark_count = scatola_bookmark_list_length (sidebar->bookmarks);

    for (index = 0; index < bookmark_count; ++index) {
        bookmark = scatola_bookmark_list_item_at (sidebar->bookmarks, index);

        if (scatola_bookmark_uri_known_not_to_exist (bookmark)) {
            continue;
        }

        root = scatola_bookmark_get_location (bookmark);
        file = scatola_file_get (root);
        ...
        mount_uri = scatola_bookmark_get_uri (bookmark);
        tooltip = g_file_get_parse_name (root);
    ...
    icon = g_themed_icon_new (SCATOLA_ICON_NETWORK);
    last_iter = add_place (sidebar, PLACES_BUILT_IN,
                           SECTION_NETWORK,
...
static gboolean
over_eject_button (ScatolaPlacesSidebar *sidebar,
                   gint x,
                   gint y,
                   GtkTreePath **path)
{
   ...
   if (gtk_tree_view_get_path_at_pos (sidebar->tree_view,
        ...
        eject_button_size = scatola_get_icon_size_for_stock_size (GTK_ICON_SIZE_MENU);
...
static gboolean
clicked_eject_button (ScatolaPlacesSidebar *sidebar,
                      GtkTreePath **path)
{
...
desktop_location_changed_callback (gpointer user_data)
{
    ...
    sidebar = SCATOLA_PLACES_SIDEBAR (user_data);
...
static void
free_drag_data (ScatolaPlacesSidebar *sidebar)
{
    sidebar->drag_data_received = FALSE;

    if (sidebar->drag_list)
    {
        scatola_drag_destroy_selection_list (sidebar->drag_list);
        sidebar->drag_list = NULL;
    }
}

static gboolean
can_accept_file_as_bookmark (ScatolaFile *file)
{
    return (scatola_file_is_directory (file) &&
...
can_accept_items_as_bookmarks (const GList *items)
{
    ...
    {
        uri = ((ScatolaDragSelectionItem *)items->data)->uri;
    ...
    else
    {
        ...
        else
        {
            ...
            gtk_tree_model_get (GTK_TREE_MODEL (sidebar->filter_model),
                                ...
                                PLACES_SIDEBAR_COLUMN_URI, &uri,
                                -1);
            scatola_drag_default_drop_action_for_icons (context, uri,
                    sidebar->drag_list,
                    &action);
            g_free (uri);
        }
...
static void
bookmarks_drop_uris (ScatolaPlacesSidebar *sidebar,
                     ...
                     int                    position)
{
    ...
    for (i = 0; uris[i]; i++)
    {
        ...
        uri = scatola_file_get_drop_target_uri (file);
        location = g_file_new_for_uri (uri);
        scatola_file_unref (file);

        name = scatola_compute_title_for_location (location);
        icon = g_themed_icon_new (SCATOLA_ICON_FOLDER);
        bookmark = scatola_bookmark_new (location, name, TRUE, icon);

        if (!scatola_bookmark_list_contains (sidebar->bookmarks, bookmark))
        {
            scatola_bookmark_list_insert_item (sidebar->bookmarks, bookmark, position++);
        }
...
build_selection_list (const char *data)
{
    ...
    {
        uri = uris[i];
        item = scatola_drag_selection_item_new ();
...
static gboolean
get_selected_iter (ScatolaPlacesSidebar *sidebar,
                   GtkTreeIter *iter)
{
    GtkTreeSelection *selection;
...
static void
reorder_bookmarks (ScatolaPlacesSidebar *sidebar,
                   int                new_position)
{
    GtkTreeIter iter;
    PlaceType type;
    ...
    if (type != PLACES_BOOKMARK ||
            old_position < 0 ||
            old_position >= scatola_bookmark_list_length (sidebar->bookmarks))
    {
        return;
    }

    scatola_bookmark_list_move_item (sidebar->bookmarks, old_position,
                                  new_position);
}
                             ...
                             ScatolaPlacesSidebar *sidebar)
{
        ...
        tree_pos == GTK_TREE_VIEW_DROP_AFTER)
    {
        ...
        if (section_type == SECTION_NETWORK && place_type == PLACES_HEADING &&
            ...
            position = scatola_bookmark_list_length (sidebar->bookmarks);
        }
    ...
    else
    {
        ...
        {
            real_action =
                scatola_drag_drop_action_ask (GTK_WIDGET (tree_view),
        ...
        if (real_action > 0)
        {
            ...
            case TEXT_URI_LIST:
                ...
                scatola_file_operations_copy_move (uris, NULL, drop_uri,
                                                real_action, GTK_WIDGET (tree_view),
                                                NULL, NULL);
                scatola_drag_destroy_selection_list (selection_list);
                g_list_free (uris);
                success = TRUE;
                break;
            case GTK_TREE_MODEL_ROW:
                                ...
                                GtkMenu   *menu)
{
    ...
    sidebar = SCATOLA_PLACES_SIDEBAR (attach_widget);
    g_assert (SCATOLA_IS_PLACES_SIDEBAR (sidebar));
...
static void
bookmarks_check_popup_sensitivity (ScatolaPlacesSidebar *sidebar)
{
    GtkTreeIter iter;
    PlaceType type;
    GDrive *drive = NULL;
    ...
    gtk_widget_set_sensitive (sidebar->popup_menu_empty_trash_item, !scatola_trash_monitor_is_empty ());
                   ...
                   GObject *user_data)
{
    ...
    if (mount != NULL)
    {
        ...
        {
            if ((sidebar->go_to_after_mount_flags & SCATOLA_WINDOW_OPEN_FLAG_NEW_WINDOW) == 0)
            {
                scatola_window_slot_info_open_location (sidebar->go_to_after_mount_slot, location,
                                                     SCATOLA_WINDOW_OPEN_ACCORDING_TO_MODE,
                                                     sidebar->go_to_after_mount_flags, NULL);
            }
            else
            {
                ScatolaWindow *cur, *new;

                cur = SCATOLA_WINDOW (sidebar->window);
                new = scatola_application_create_navigation_window (cur->application,
                        NULL,
                        gtk_window_get_screen (GTK_WINDOW (cur)));
                scatola_window_go_to (new, location);
            }
        }
...
static void
open_selected_bookmark (ScatolaPlacesSidebar   *sidebar,
                        GtkTreeModel        *model,
                        GtkTreePath         *path,
                        ScatolaWindowOpenFlags  flags)
{
    ...
    if (uri != NULL)
    {
        ...
        if ((flags & SCATOLA_WINDOW_OPEN_FLAG_NEW_WINDOW) == 0)
        {
            slot = scatola_window_info_get_active_slot (sidebar->window);
    ...
    else
    {
        ...
        if (volume != NULL && !sidebar->mounting)
        {
            ...
            scatola_file_operations_mount_volume_full (NULL, volume, FALSE,
                                                    volume_mounted_cb,
                                                    G_OBJECT (sidebar));
        }
...
static void
open_shortcut_from_menu (ScatolaPlacesSidebar   *sidebar,
                         ScatolaWindowOpenFlags  flags)
{
    GtkTreeModel *model;
    GtkTreePath *path;
...
open_shortcut_in_new_window_cb (GtkMenuItem       *item,
                                ScatolaPlacesSidebar *sidebar)
{
    open_shortcut_from_menu (sidebar, SCATOLA_WINDOW_OPEN_FLAG_NEW_WINDOW);
}
...
open_shortcut_in_new_tab_cb (GtkMenuItem       *item,
                             ScatolaPlacesSidebar *sidebar)
{
    open_shortcut_from_menu (sidebar, SCATOLA_WINDOW_OPEN_FLAG_NEW_TAB);
}

/* Rename the selected bookmark */
static void
rename_selected_bookmark (ScatolaPlacesSidebar *sidebar)
{
    GtkTreeIter iter;
    GtkTreePath *path;
    GtkTreeViewColumn *column;
...
static void
remove_selected_bookmarks (ScatolaPlacesSidebar *sidebar)
{
    GtkTreeIter iter;
    PlaceType type;
    int index;
    ...
    scatola_bookmark_list_delete_item_at (sidebar->bookmarks, index);
}
                   ...
                   ScatolaPlacesSidebar *sidebar)
{
    ...
    if (volume != NULL)
    {
        scatola_file_operations_mount_volume (NULL, volume, FALSE);
        g_object_unref (volume);
    }
}
...
unmount_done (gpointer data)
{
    ...
    window = data;
    scatola_window_info_set_initiated_unmount (window, FALSE);
            ...
            ScatolaPlacesSidebar *sidebar)
{
    if (mount != NULL)
    {
        scatola_window_info_set_initiated_unmount (sidebar->window, TRUE);
        scatola_file_operations_unmount_mount_full (NULL, mount, FALSE, TRUE,
                unmount_done,
...
static void
do_unmount_selection (ScatolaPlacesSidebar *sidebar)
{
    GtkTreeIter iter;
    GMount *mount;
...
static gboolean
eject_or_unmount_bookmark (ScatolaPlacesSidebar *sidebar,
                           GtkTreePath *path)
{
    GtkTreeModel *model;
    GtkTreeIter iter;
...
static gboolean
eject_or_unmount_selection (ScatolaPlacesSidebar *sidebar)
{
    GtkTreeIter iter;
    GtkTreePath *path;
    gboolean ret;
...
empty_trash_cb (GtkMenuItem           *item,
                ScatolaPlacesSidebar *sidebar)
{
    scatola_file_operations_empty_trash (GTK_WIDGET (sidebar->window));
}
                              ...
                              ScatolaPlacesSidebar *sidebar)
{
    ...
    GtkTreeModel *model;
    GtkTreePath *path;
    ScatolaWindowOpenFlags flags = 0;
...
static void
bookmarks_build_popup_menu (ScatolaPlacesSidebar *sidebar)
{
    GtkWidget *item;
...
static void
bookmarks_update_popup_menu (ScatolaPlacesSidebar *sidebar)
{
    bookmarks_build_popup_menu (sidebar);
}

static void
bookmarks_popup_menu (ScatolaPlacesSidebar *sidebar,
...
static void
update_eject_buttons (ScatolaPlacesSidebar *sidebar,
                      GtkTreePath         *path)
{
    GtkTreeIter iter;
                                 ...
                                 ScatolaPlacesSidebar *sidebar)
{
    ...
    else if (event->button == 2)
    {
        ...
        open_selected_bookmark (sidebar, model, path,
                                ...
                                SCATOLA_WINDOW_OPEN_FLAG_NEW_WINDOW :
                  ...
                  ScatolaPlacesSidebar *sidebar)
{
    ...
    if (bookmark != NULL)
    {
        scatola_bookmark_set_name (bookmark, new_text);
    }
}
...
static void
trash_state_changed_cb (ScatolaTrashMonitor    *trash_monitor,
                        gboolean             state,
                        gpointer             data)
{
    ScatolaPlacesSidebar *sidebar;

    sidebar = SCATOLA_PLACES_SIDEBAR (data);
...
static void
scatola_places_sidebar_init (ScatolaPlacesSidebar *sidebar)
{
    GtkTreeView       *tree_view;
    ...
    gtk_tree_view_column_set_max_width (GTK_TREE_VIEW_COLUMN (col), SCATOLA_ICON_SIZE_SMALLER);
    ...
    sidebar->filter_model = scatola_shortcuts_model_filter_new (sidebar,
    ...
    gtk_tree_view_enable_model_drag_source (GTK_TREE_VIEW (tree_view),
                                            GDK_BUTTON1_MASK,
                                            scatola_shortcuts_source_targets,
                                            G_N_ELEMENTS (scatola_shortcuts_source_targets),
                                            GDK_ACTION_MOVE);
    gtk_drag_dest_set (GTK_WIDGET (tree_view),
                       0,
                       scatola_shortcuts_drop_targets, G_N_ELEMENTS (scatola_shortcuts_drop_targets),
...
static void
scatola_places_sidebar_dispose (GObject *object)
{
    ScatolaPlacesSidebar *sidebar;

    sidebar = SCATOLA_PLACES_SIDEBAR (object);
    ...
    G_OBJECT_CLASS (scatola_places_sidebar_parent_class)->dispose (object);
}

static void
scatola_places_sidebar_class_init (ScatolaPlacesSidebarClass *class)
{
    G_OBJECT_CLASS (class)->dispose = scatola_places_sidebar_dispose;

#if GTK_CHECK_VERSION (3, 0, 0)
    GTK_WIDGET_CLASS (class)->style_updated = scatola_places_sidebar_style_updated;
#else
    GTK_WIDGET_CLASS (class)->style_set = scatola_places_sidebar_style_set;
#endif
}

static const char *
scatola_places_sidebar_get_sidebar_id (ScatolaSidebar *sidebar)
{
    return SCATOLA_PLACES_SIDEBAR_ID;
}

static char *
scatola_places_sidebar_get_tab_label (ScatolaSidebar *sidebar)
{
    return g_strdup (_("Places"));
}

static char *
scatola_places_sidebar_get_tab_tooltip (ScatolaSidebar *sidebar)
{
    return g_strdup (_("Show Places"));
}

static GdkPixbuf *
scatola_places_sidebar_get_tab_icon (ScatolaSidebar *sidebar)
{
    return NULL;
}

static void
scatola_places_sidebar_is_visible_changed (ScatolaSidebar *sidebar,
                                        gboolean         is_visible)
{
    /* Do nothing */
}
...
static void
scatola_places_sidebar_iface_init (ScatolaSidebarIface *iface)
{
    iface->get_sidebar_id = scatola_places_sidebar_get_sidebar_id;
    iface->get_tab_label = scatola_places_sidebar_get_tab_label;
    iface->get_tab_tooltip = scatola_places_sidebar_get_tab_tooltip;
    iface->get_tab_icon = scatola_places_sidebar_get_tab_icon;
    iface->is_visible_changed = scatola_places_sidebar_is_visible_changed;
}

static void
scatola_places_sidebar_set_parent_window (ScatolaPlacesSidebar *sidebar,
                                       ScatolaWindowInfo *window)
{
    ScatolaWindowSlotInfo *slot;
    ...
    sidebar->bookmarks = scatola_bookmark_list_new ();
    sidebar->uri = scatola_window_slot_info_get_current_location (slot);
...
static void
#if GTK_CHECK_VERSION (3, 0, 0)
scatola_places_sidebar_style_updated (GtkWidget *widget)
#else
scatola_places_sidebar_style_set (GtkWidget *widget,
                               GtkStyle  *previous_style)
#endif
{
    ScatolaPlacesSidebar *sidebar;

    sidebar = SCATOLA_PLACES_SIDEBAR (widget);

    update_places (sidebar);
}

static ScatolaSidebar *
scatola_places_sidebar_create (ScatolaSidebarProvider *provider,
                            ScatolaWindowInfo *window)
{
    ScatolaPlacesSidebar *sidebar;

    sidebar = g_object_new (scatola_places_sidebar_get_type (), NULL);
    scatola_places_sidebar_set_parent_window (sidebar, window);
...
{
    iface->create = scatola_places_sidebar_create;
}

static void
scatola_places_sidebar_provider_init (ScatolaPlacesSidebarProvider *sidebar)
{
}

static void
scatola_places_sidebar_provider_class_init (ScatolaPlacesSidebarProviderClass *class)
{
}

void
scatola_places_sidebar_register (void)
{
    scatola_module_add_type (scatola_places_sidebar_provider_get_type ());
}

/* Drag and drop interfaces */

static void
_scatola_shortcuts_model_filter_class_init (ScatolaShortcutsModelFilterClass *class)
{
}

static void
_scatola_shortcuts_model_filter_init (ScatolaShortcutsModelFilter *model)
{
    model->sidebar = NULL;
}
...
static gboolean
scatola_shortcuts_model_filter_row_draggable (GtkTreeDragSource *drag_source,
                                           GtkTreePath       *path)
{
    GtkTreeModel *model;
    GtkTreeIter iter;
...
static void
scatola_shortcuts_model_filter_drag_source_iface_init (GtkTreeDragSourceIface *iface)
{
    iface->row_draggable = scatola_shortcuts_model_filter_row_draggable;
}

static GtkTreeModel *
scatola_shortcuts_model_filter_new (ScatolaPlacesSidebar *sidebar,
                                 GtkTreeModel          *child_model,
                                 GtkTreePath           *root)
{
    ScatolaShortcutsModelFilter *model;

    model = g_object_new (SCATOLA_SHORTCUTS_MODEL_FILTER_TYPE,
