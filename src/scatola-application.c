...
#include "scatola-places-sidebar.h"
#include "scatola-notes-viewer.h"
#include "scatola-emblem-sidebar.h"
#include "scatola-image-properties-page.h"
#include <fcntl.h>
#include <string.h>
#include <unistd.h>
...
#include <libscatola-private/scatola-desktop-link-monitor.h>
#include <libscatola-private/scatola-directory-private.h>
...
#define SCATOLA_ACCEL_MAP_SAVE_DELAY 30

/* Keeps track of all the desktop windows. */
static GList *scatola_application_desktop_windows;

/* Keeps track of all the scatola windows. */
static GList *scatola_application_window_list;

/* Keeps track of all the object windows */
static GList *scatola_application_spatial_window_list;

/* The saving of the accelerator map was requested  */
static gboolean save_of_accel_map_requested = FALSE;

/* File Manager DBus Interface */
static ScatolaFreedesktopDBus *fdb_manager = NULL;
...
static void     drive_listen_for_eject_button      (GDrive *drive,
        ScatolaApplication *application);
static void     scatola_application_load_session     (ScatolaApplication *application);
static char *   scatola_application_get_session_data (void);

G_DEFINE_TYPE (ScatolaApplication, scatola_application, G_TYPE_OBJECT);
...
GList *
scatola_application_get_window_list (void)
{
    return scatola_application_window_list;
}

GList *
scatola_application_get_spatial_window_list (void)
{
    return scatola_application_spatial_window_list;
}

unsigned int
scatola_application_get_n_windows (void)
{
    return g_list_length (scatola_application_window_list) +
           g_list_length (scatola_application_desktop_windows);
}
...
static void
automount_all_volumes (ScatolaApplication *application)
{
    GList *volumes, *l;
    GMount *mount;
    GVolume *volume;

    if (g_settings_get_boolean (scatola_media_preferences, SCATOLA_PREFERENCES_MEDIA_AUTOMOUNT))
    {
                        ...
                        ScatolaApplication *application)
{
    ...
    data = scatola_application_get_session_data ();
    if (data)
    {
...
smclient_quit_cb (EggSMClient   *client,
                  ScatolaApplication *application)
{
    scatola_main_event_loop_quit (TRUE);
}

static void
scatola_application_init (ScatolaApplication *application)
{
    application->unique_app = unique_app_new_with_commands ("org.gde2.Scatola", NULL,
    ...
    /* register sidebars */
    scatola_places_sidebar_register ();
    scatola_information_panel_register ();
    fm_tree_view_register ();
    scatola_history_sidebar_register ();
    scatola_notes_viewer_register (); /* also property page */
    scatola_emblem_sidebar_register ();

    /* register property pages */
    scatola_image_properties_page_register ();

    /* initialize search path for custom icons */
    gtk_icon_theme_append_search_path (gtk_icon_theme_get_default (),
                                       SCATOLA_DATADIR G_DIR_SEPARATOR_S "icons");
}

ScatolaApplication *
scatola_application_new (void)
{
    return g_object_new (SCATOLA_TYPE_APPLICATION, NULL);
}

static void
scatola_application_finalize (GObject *object)
{
    ScatolaApplication *application;

    application = SCATOLA_APPLICATION (object);
    ...
    G_OBJECT_CLASS (scatola_application_parent_class)->finalize (object);
}

static gboolean
check_required_directories (ScatolaApplication *application)
{
    char *user_directory;
    ...
    g_assert (SCATOLA_IS_APPLICATION (application));

    ret = TRUE;

    user_directory = scatola_get_user_directory ();
    desktop_directory = scatola_get_desktop_directory ();
    ...
    if (directories != NULL)
    {
        ...
        if (failed_count == 1)
        {
            error_string = g_strdup_printf (_("Scatola could not create the required folder \"%s\"."),
                                            directories_as_string->str);
            detail_string = _("Before running Scatola, please create the following folder, or "
                              "set permissions such that Scatola can create it.");
        }
        else
        {
            error_string = g_strdup_printf (_("Scatola could not create the following required folders: "
                                              "%s."), directories_as_string->str);
            detail_string = _("Before running Scatola, please create these folders, or "
                              "set permissions such that Scatola can create them.");
        }

        dialog = eel_show_error_dialog (error_string, detail_string, NULL);
        /* We need the main event loop so the user has a chance to see the dialog. */
#if GTK_CHECK_VERSION (3, 0, 0)
        scatola_main_event_loop_register (GTK_WIDGET (dialog));
#else
        scatola_main_event_loop_register (GTK_OBJECT (dialog));
#endif
...
static void
menu_provider_items_updated_handler (ScatolaMenuProvider *provider, GtkWidget* parent_window, gpointer data)
{
...
menu_provider_init_callback (void)
{
    ...
    {
        ScatolaMenuProvider *provider = SCATOLA_MENU_PROVIDER (l->data);
...
{
    ScatolaApplication *application = SCATOLA_APPLICATION (data);
...
mark_desktop_files_trusted (void)
{
    ...
    f = scatola_get_desktop_location ();
    ...
    while ((info = g_file_enumerator_next_file (e, NULL, NULL)) != NULL)
    {
                ...
                !g_file_info_get_attribute_boolean (info, G_FILE_ATTRIBUTE_ACCESS_CAN_EXECUTE))
        {
            ...
            scatola_file_mark_desktop_file_trusted (c,
                                                 NULL, FALSE,
                                                 NULL, NULL);
            g_object_unref (c);
        }
...
static void
check_volume_queue (ScatolaApplication *application)
{
        GList *l, *next;
        GVolume *volume;
		...
		scatola_file_operations_mount_volume (NULL, volume, TRUE);
...
#define SCREENSAVER_NAME "org.gde2.ScreenSaver"
#define SCREENSAVER_PATH "/org/gde2/ScreenSaver"
#define SCREENSAVER_INTERFACE "org.gde2.ScreenSaver"
...
screensaver_signal_callback (GDBusProxy *proxy,
                             ...
                             gpointer user_data)
{
	ScatolaApplication *application = user_data;
...
static void
do_initialize_screensaver (ScatolaApplication *application)
{
...
static void
do_upgrades_once (ScatolaApplication *application,
                  gboolean no_desktop)
{
    ...
    metafile_dir = g_build_filename(g_get_user_config_dir(), "scatola", "metafiles", NULL);
    ...
    if (g_file_test (metafile_dir, G_FILE_TEST_IS_DIR))
    {
        ...
        {
            g_spawn_command_line_async (LIBEXECDIR "/scatola-convert-metadata --quiet", NULL);
...
static void
finish_startup (ScatolaApplication *application,
                gboolean no_desktop)
{
    GList *drives;
    ...
    scatola_desktop_link_monitor_get ();

    /* Initialize GDE2 screen saver listener to control automount
	 * permission */
...
static void
open_window (ScatolaApplication *application,
             ...
             gboolean browser_window)
{
    ...
    scatola_window_go_to (window, location);
...
static void
open_windows (ScatolaApplication *application,
              const char *startup_id,
              char **uris,
              GdkScreen *screen,
              const char *geometry,
...
void
scatola_application_open_location (ScatolaApplication *application,
                                GFile *location,
                                GFile *selection,
                                const char *startup_id)
{
    ...
    window = scatola_application_create_navigation_window (application, startup_id, gdk_screen_get_default ());

    if (selection != NULL) {
        sel_list = g_list_prepend (NULL, g_object_ref (selection));
    }

    scatola_window_slot_open_location_full (scatola_window_get_active_slot (window), location,
                                         0, SCATOLA_WINDOW_OPEN_FLAG_NEW_WINDOW, sel_list, NULL, NULL);

    if (sel_list != NULL) {
        scatola_file_list_free (sel_list);
    }
}
                     ...
                     gpointer           user_data)
{
    ...
    case COMMAND_START_DESKTOP:
        scatola_application_open_desktop (application);
        break;
    case COMMAND_STOP_DESKTOP:
        scatola_application_close_desktop ();
        break;
    default:
...
gboolean
scatola_application_save_accel_map (gpointer data)
{
    if (save_of_accel_map_requested)
    {
        char *accel_map_filename;
        accel_map_filename = scatola_get_accel_map_file ();
                               ...
                               gpointer user_data)
{
    if (!save_of_accel_map_requested)
    {
        ...
        g_timeout_add_seconds (SCATOLA_ACCEL_MAP_SAVE_DELAY,
                               scatola_application_save_accel_map, NULL);
    }
}

static gboolean
desktop_changed_callback_connect (ScatolaApplication *application)
{
...
init_css (void)
{
    ...
    gtk_css_provider_load_from_path (provider,
				SCATOLA_DATADIR G_DIR_SEPARATOR_S "scatola.css", &error);

    if (error != NULL) {
        g_warning ("Can't parse Scatola' CSS custom description: %s\n", error->message);
        g_error_free (error);
    } else {
...
/* add our desktop CSS provider,  ensures the desktop background does not get covered */
    ...
    gtk_css_provider_load_from_path (provider, SCATOLA_DATADIR G_DIR_SEPARATOR_S "scatola-desktop.css", &error);
#else
    gtk_css_provider_load_from_path (provider, SCATOLA_DATADIR G_DIR_SEPARATOR_S "scatola-desktop-3.14.css", &error);
#endif
...
void
scatola_application_startup (ScatolaApplication *application,
                          gboolean kill_shell,
                          ...
                          char **urls)
{
    ...
    /* Check the user's ~/.config/scatola directories and post warnings
    ...
    else
    {
        ...
        if (!no_desktop &&
            !g_settings_get_boolean (gde2_background_preferences, GDE2_BG_KEY_SHOW_DESKTOP))
        {
            no_desktop = TRUE;
        }
        ...
        fdb_manager = scatola_freedesktop_dbus_new (application);

        /* Monitor the preference to show or hide the desktop */
        g_signal_connect_swapped (gde2_background_preferences,
                                  "changed::" GDE2_BG_KEY_SHOW_DESKTOP,
        ...
        scatola_application_load_session (application);
                          ...
                          ScatolaDesktopWindow *window)
{
    ...
    scatola_application_desktop_windows =
        g_list_remove (scatola_application_desktop_windows, window);

    return TRUE;
}

static void
scatola_application_create_desktop_windows (ScatolaApplication *application)
{
    ...
    g_return_if_fail (scatola_application_desktop_windows == NULL);
    g_return_if_fail (SCATOLA_IS_APPLICATION (application));
    ...
    {
        window = scatola_desktop_window_new (application, gdk_display_get_default_screen (display));
        ...
        /* We realize it immediately so that the SCATOLA_DESKTOP_WINDOW_ID
           property is set so gde2-settings-daemon doesn't try to set the
        ...
        scatola_application_desktop_windows =
            g_list_prepend (scatola_application_desktop_windows, window);
    }
...
void
scatola_application_open_desktop (ScatolaApplication *application)
{
    if (scatola_application_desktop_windows == NULL)
    {
        scatola_application_create_desktop_windows (application);
    }
}

void
scatola_application_close_desktop (void)
{
    if (scatola_application_desktop_windows != NULL)
    {
        g_list_free_full (scatola_application_desktop_windows, (GDestroyNotify) gtk_widget_destroy);
        scatola_application_desktop_windows = NULL;
    }
}

void
scatola_application_close_all_navigation_windows (void)
{
    GList *list_copy;
    GList *l;

    list_copy = g_list_copy (scatola_application_window_list);
    /* First hide all window to get the feeling of quick response */
    for (l = list_copy; l != NULL; l = l->next)
    {
        ScatolaWindow *window;

        window = SCATOLA_WINDOW (l->data);
...
static ScatolaSpatialWindow *
scatola_application_get_existing_spatial_window (GFile *location)
{
    GList *l;
    ScatolaWindowSlot *slot;
    GFile *window_location;

    for (l = scatola_application_get_spatial_window_list ();
            l != NULL; l = l->next) {
        slot = SCATOLA_WINDOW (l->data)->details->active_pane->active_slot;
        ...
        if (window_location != NULL) {
        	if (g_file_equal (location, window_location)) {
            	return SCATOLA_SPATIAL_WINDOW (l->data);
            }
        }
    }
...
static ScatolaSpatialWindow *
find_parent_spatial_window (ScatolaSpatialWindow *window)
{
    ScatolaFile *file;
    ScatolaFile *parent_file;
    ScatolaWindowSlot *slot;
    GFile *location;
    ...
    parent_file = scatola_file_get_parent (file);
    scatola_file_unref (file);
    while (parent_file)
    {
        ScatolaSpatialWindow *parent_window;

        location = scatola_file_get_location (parent_file);
        parent_window = scatola_application_get_existing_spatial_window (location);
        g_object_unref (location);

        /* Stop at the desktop directory if it's not explicitely opened
         * in a spatial window of its own.
         */
        if (scatola_file_is_desktop_directory (parent_file) && !parent_window)
        {
            scatola_file_unref (parent_file);
            return NULL;
        }
...
void
scatola_application_close_parent_windows (ScatolaSpatialWindow *window)
{
    ScatolaSpatialWindow *parent_window;
    ScatolaSpatialWindow *new_parent_window;

    g_return_if_fail (SCATOLA_IS_SPATIAL_WINDOW (window));
    ...
    while (parent_window)
    {
        ...
        scatola_window_close (SCATOLA_WINDOW (parent_window));
        parent_window = new_parent_window;
    }
}

void
scatola_application_close_all_spatial_windows (void)
{
    GList *list_copy;
    GList *l;

    list_copy = g_list_copy (scatola_application_spatial_window_list);
    /* First hide all window to get the feeling of quick response */
    for (l = list_copy; l != NULL; l = l->next)
    {
        ScatolaWindow *window;
...
static void
#if GTK_CHECK_VERSION (3, 0, 0)
scatola_application_destroyed_window (GtkWidget *object, ScatolaApplication *application)
#else
scatola_application_destroyed_window (GtkObject *object, ScatolaApplication *application)
#endif
{
    scatola_application_window_list = g_list_remove (scatola_application_window_list, object);
}

static gboolean
scatola_window_delete_event_callback (GtkWidget *widget,
                                   GdkEvent *event,
                                   gpointer user_data)
{
    ScatolaWindow *window;
...
create_window (ScatolaApplication *application,
               GType window_type,
               const char *startup_id,
               GdkScreen *screen)
{
    ...
    g_return_val_if_fail (SCATOLA_IS_APPLICATION (application), NULL);

    window = SCATOLA_WINDOW (gtk_widget_new (window_type,
                                          "app", application,
                                          "screen", screen,
                                          NULL));
    ...
    g_signal_connect_data (window, "delete_event",
                           G_CALLBACK (scatola_window_delete_event_callback), NULL, NULL,
                           G_CONNECT_AFTER);

    g_signal_connect_object (window, "destroy",
                             G_CALLBACK (scatola_application_destroyed_window), application, 0);

    scatola_application_window_list = g_list_prepend (scatola_application_window_list, window);
...
{
    scatola_application_spatial_window_list = g_list_remove (scatola_application_spatial_window_list, window);

}

ScatolaWindow *
scatola_application_get_spatial_window (ScatolaApplication *application,
                                    ...
                                    gboolean        *existing)
{
    ...
    g_return_val_if_fail (SCATOLA_IS_APPLICATION (application), NULL);
    window = SCATOLA_WINDOW
    		(scatola_application_get_existing_spatial_window (location));
    ...
    window = create_window (application, SCATOLA_TYPE_SPATIAL_WINDOW, startup_id, screen);
    ...
    scatola_application_spatial_window_list = g_list_prepend (scatola_application_spatial_window_list, window);
...
ScatolaWindow *
scatola_application_create_navigation_window (ScatolaApplication *application,
        const char          *startup_id,
        GdkScreen           *screen)
{
    ScatolaWindow *window;
    ...
    window = create_window (application, SCATOLA_TYPE_NAVIGATION_WINDOW, startup_id, screen);

    maximized = g_settings_get_boolean (scatola_window_state,
    ...
    geometry_string = g_settings_get_string (scatola_window_state,
            ...
            geometry_string[0] != 0)
    {
        ...
        (GTK_WINDOW (window),
         geometry_string,
         SCATOLA_NAVIGATION_WINDOW_MIN_WIDTH,
         SCATOLA_NAVIGATION_WINDOW_MIN_HEIGHT,
         TRUE);
    }
...
desktop_location_changed_callback (gpointer user_data)
{
    if (scatola_application_desktop_windows != NULL)
    {
        g_list_foreach (scatola_application_desktop_windows,
                        (GFunc) scatola_desktop_window_update_directory, NULL);
    }
}
...
desktop_changed_callback (gpointer user_data)
{
    ...
    application = SCATOLA_APPLICATION (user_data);
    if (g_settings_get_boolean (gde2_background_preferences, GDE2_BG_KEY_SHOW_DESKTOP))
    {
        scatola_application_open_desktop (application);
    }
    else
...
static gboolean
window_can_be_closed (ScatolaWindow *window)
{
...
static void
check_screen_lock_and_mount (ScatolaApplication *application,
                             GVolume *volume)
{
        if (application->screensaver_active)
        {
                       ...
                       ScatolaApplication *application)
{
    ...
    {
        /* Allow scatola_autorun() to run. When the mount is later
         * added programmatically (i.e. for a blank CD),
         * scatola_autorun() will be called by mount_added_callback(). */
        scatola_allow_autorun_for_volume (volume);
        scatola_allow_autorun_for_volume_finish (volume);
    }
}
...
static void
drive_listen_for_eject_button (GDrive *drive, ScatolaApplication *application)
{
                      ...
                      ScatolaApplication *application)
{
    ...
    directory = scatola_directory_get_existing (root);
    g_object_unref (root);
    if (directory != NULL)
    {
    ...
    scatola_autorun (mount, autorun_show_window, application);
}
...
get_first_navigation_slot (GList *slot_list)
{
    ...
    {
        if (SCATOLA_IS_NAVIGATION_WINDOW_SLOT (l->data))
        {
            return l->data;
        }
    }
...
static gboolean
should_close_slot_with_mount (ScatolaWindow *window,
                              ScatolaWindowSlot *slot,
                              GMount *mount)
{
    if (SCATOLA_IS_SPATIAL_WINDOW (window))
    ...
    return scatola_navigation_window_slot_should_close_with_mount (SCATOLA_NAVIGATION_WINDOW_SLOT (slot),
            mount);
}
                        ...
                        ScatolaApplication *application)
{
    ...
    ScatolaWindow *window;
    ScatolaWindowSlot *slot;
    ScatolaWindowSlot *force_no_close_slot;
    GFile *root, *computer;
    ...
    window_list = scatola_application_get_window_list ();

    root = g_mount_get_root (mount);
    /* Construct a list of windows to be closed. Do not add the non-closable windows to the list. */
    for (node = window_list; node != NULL; node = node->next)
    {
        window = SCATOLA_WINDOW (node->data);
        if (window != NULL && window_can_be_closed (window))
        {
            ...
            for (lp = window->details->panes; lp != NULL; lp = lp->next)
            {
                ...
                pane = (ScatolaWindowPane*) lp->data;
    ...
    for (node = close_list; node != NULL; node = node->next)
    {
        ...
        else
        {
            ...
            scatola_window_slot_go_to (slot, computer, FALSE);
...
static char *
scatola_application_get_session_data (void)
{
    xmlDocPtr doc;
    ...
    n_processed = 0;
    for (l = scatola_get_history_list (); l != NULL; l = l->next)
    {
        ...
        tmp = scatola_bookmark_get_name (bookmark);
        ...
        tmp = scatola_bookmark_get_uri (bookmark);
        xmlNewProp (bookmark_node, "uri", tmp);
        g_free (tmp);

        if (scatola_bookmark_get_has_custom_name (bookmark))
        {
    ...
    for (l = scatola_application_window_list; l != NULL; l = l->next)
    {
        xmlNodePtr win_node, slot_node;
        ScatolaWindow *window;
        ScatolaWindowSlot *slot, *active_slot;
        GList *slots, *m;
        char *tmp;
        ...
        xmlNewProp (win_node, "type", SCATOLA_IS_NAVIGATION_WINDOW (window) ? "navigation" : "spatial");

        if (SCATOLA_IS_NAVIGATION_WINDOW (window))   /* spatial windows store their state as file metadata */
        {
        ...
        slots = scatola_window_get_slots (window);
        active_slot = scatola_window_get_active_slot (window);

        /* store one slot as window location. Otherwise
         * older Scatola versions will bail when reading the file. */
        tmp = scatola_window_slot_get_location_uri (active_slot);
        xmlNewProp (win_node, "location", tmp);
        g_free (tmp);

        for (m = slots; m != NULL; m = m->next)
        {
            slot = SCATOLA_WINDOW_SLOT (m->data);

            slot_node = xmlNewChild (win_node, NULL, "slot", NULL);

            tmp = scatola_window_slot_get_location_uri (slot);
...
void
scatola_application_load_session (ScatolaApplication *application)
{
    xmlDocPtr doc;
    gboolean bail;
        ...
        for (node = root_node->children; node != NULL; node = node->next)
        {
            ...
            else if (g_strcmp0 (node->name, "history") == 0)
            {
                ...
                for (bookmark_node = node->children; bookmark_node != NULL; bookmark_node = bookmark_node->next)
                {
                    ...
                    else if (g_strcmp0 (bookmark_node->name, "bookmark") == 0)
                    {
                        ...
                        emit_change |= scatola_add_to_history_list_no_notify (location, name, has_custom_name, icon);
            ...
            else if (g_strcmp0 (node->name, "window") == 0)
            {
                ...
                if (g_strcmp0 (type, "navigation") == 0)
                {
                    ...
                    window = scatola_application_create_navigation_window (application, NULL, gdk_screen_get_default ());
                    ...
                    for (i = 0, slot_node = node->children; slot_node != NULL; slot_node = slot_node->next)
                    {
                        if (g_strcmp0 (slot_node->name, "slot") == 0)
                        {
                            ...
                            if (slot_uri != NULL)
                            {
                                ...
                                {
                                    slot = scatola_window_open_slot (window->details->active_pane, SCATOLA_WINDOW_OPEN_SLOT_APPEND);
                                }

                                location = g_file_new_for_uri (slot_uri);
                                scatola_window_slot_open_location (slot, location, FALSE);
                    ...
                    if (i == 0)
                    {
                        ...
                        scatola_window_slot_open_location (window->details->active_pane->active_slot, location, FALSE);
                        g_object_unref (location);
                    }
                }
                ...
                {
                    location = g_file_new_for_uri (location_uri);
                    window = scatola_application_get_spatial_window (application, NULL, NULL, 
...
static void
scatola_application_class_init (ScatolaApplicationClass *class)
{
    GObjectClass *object_class;

    object_class = G_OBJECT_CLASS (class);
    object_class->finalize = scatola_application_finalize;
}
