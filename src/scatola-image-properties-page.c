...
struct ScatolaImagePropertiesPageDetails
{
...
{
    GObject parent;
} ScatolaImagePropertiesPageProvider;

typedef struct
{
    GObjectClass parent;
} ScatolaImagePropertiesPageProviderClass;


static GType scatola_image_properties_page_provider_get_type (void);
static void  property_page_provider_iface_init                (ScatolaPropertyPageProviderIface *iface);

#if GTK_CHECK_VERSION (3, 0, 0)
G_DEFINE_TYPE (ScatolaImagePropertiesPage, scatola_image_properties_page, GTK_TYPE_BOX);
#else
G_DEFINE_TYPE (ScatolaImagePropertiesPage, scatola_image_properties_page, GTK_TYPE_VBOX);
#endif

G_DEFINE_TYPE_WITH_CODE (ScatolaImagePropertiesPageProvider, scatola_image_properties_page_provider, G_TYPE_OBJECT,
                         G_IMPLEMENT_INTERFACE (SCATOLA_TYPE_PROPERTY_PAGE_PROVIDER,
                                 property_page_provider_iface_init));

static void
scatola_image_properties_page_finalize (GObject *object)
{
    ScatolaImagePropertiesPage *page;

    page = SCATOLA_IMAGE_PROPERTIES_PAGE (object);
    ...
    G_OBJECT_CLASS (scatola_image_properties_page_parent_class)->finalize (object);
}
                     ...
                     gpointer      data)
{
    ...
    page = SCATOLA_IMAGE_PROPERTIES_PAGE (data);
...
static gboolean
append_tag_value_pair (ScatolaImagePropertiesPage *page,
                       ExifData *data,
                       ExifTag   tag,
                       char     *description)
{
...
static void
append_exifdata_string (ExifData *exifdata, ScatolaImagePropertiesPage *page)
{
...
#ifdef HAVE_EXEMPI
static void
append_xmp_value_pair (ScatolaImagePropertiesPage *page,
                       XmpPtr      xmp,
                       const char *ns,
                       const char *propname,
                       char       *descr)
...
static void
append_xmpdata_string (XmpPtr xmp, ScatolaImagePropertiesPage *page)
{
    if (xmp != NULL)
    {
...
static void
load_finished (ScatolaImagePropertiesPage *page)
{
                        ...
                        gpointer         callback_data)
{
    ...
    page = SCATOLA_IMAGE_PROPERTIES_PAGE (callback_data);
...
static void
load_location (ScatolaImagePropertiesPage *page,
               const char                  *location)
{
    GFile *file;

    g_assert (SCATOLA_IS_IMAGE_PROPERTIES_PAGE (page));
...
#ifdef HAVE_EXEMPI
    {
        /* Current Exempi does not support setting custom IO to be able to use Gde2-vfs */
...
static void
scatola_image_properties_page_class_init (ScatolaImagePropertiesPageClass *class)
{
    GObjectClass *object_class;

    object_class = G_OBJECT_CLASS (class);

    object_class->finalize = scatola_image_properties_page_finalize;

    g_type_class_add_private (object_class, sizeof(ScatolaImagePropertiesPageDetails));
}

static void
scatola_image_properties_page_init (ScatolaImagePropertiesPage *page)
{
    page->details = G_TYPE_INSTANCE_GET_PRIVATE (page,
                    SCATOLA_TYPE_IMAGE_PROPERTIES_PAGE,
                    ScatolaImagePropertiesPageDetails);
                    ...
                    GList *files)
{
    ...
    if (!
            (scatola_file_info_is_mime_type (file, "image/x-bmp") ||
             scatola_file_info_is_mime_type (file, "image/x-ico") ||
             scatola_file_info_is_mime_type (file, "image/jpeg") ||
             scatola_file_info_is_mime_type (file, "image/gif") ||
             scatola_file_info_is_mime_type (file, "image/png") ||
             scatola_file_info_is_mime_type (file, "image/pnm") ||
             scatola_file_info_is_mime_type (file, "image/ras") ||
             scatola_file_info_is_mime_type (file, "image/tga") ||
             scatola_file_info_is_mime_type (file, "image/tiff") ||
             scatola_file_info_is_mime_type (file, "image/wbmp") ||
             scatola_file_info_is_mime_type (file, "image/x-xbitmap") ||
             scatola_file_info_is_mime_type (file, "image/x-xpixmap")))
    {
        return NULL;
    }
    ...
    page = g_object_new (scatola_image_properties_page_get_type (), NULL);
    load_location (page, uri);

    g_free (uri);

    real_page = scatola_property_page_new
                ("ScatolaImagePropertiesPage::property_page",
...
static void
scatola_image_properties_page_provider_init (ScatolaImagePropertiesPageProvider *sidebar)
{
}

static void
scatola_image_properties_page_provider_class_init (ScatolaImagePropertiesPageProviderClass *class)
{
}

void
scatola_image_properties_page_register (void)
{
    scatola_module_add_type (scatola_image_properties_page_provider_get_type ());
}
