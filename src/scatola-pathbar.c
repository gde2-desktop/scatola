...
#define SCATOLA_PATH_BAR_ICON_SIZE 16
...
G_DEFINE_TYPE (ScatolaPathBar,
               scatola_path_bar,
               GTK_TYPE_CONTAINER);

static void     scatola_path_bar_finalize                 (GObject         *object);
static void     scatola_path_bar_dispose                  (GObject         *object);
#if GTK_CHECK_VERSION(3,0,0)
static void     scatola_path_bar_get_preferred_width      (GtkWidget        *widget,
        						gint             *minimum,
        						gint             *natural);
static void     scatola_path_bar_get_preferred_height     (GtkWidget        *widget,
        						gint             *minimum,
        						gint             *natural);
#else
static void     scatola_path_bar_size_request             (GtkWidget       *widget,
        						GtkRequisition  *requisition);
#endif
static void     scatola_path_bar_unmap                    (GtkWidget       *widget);
static void     scatola_path_bar_size_allocate            (GtkWidget       *widget,
        GtkAllocation   *allocation);
static void     scatola_path_bar_add                      (GtkContainer    *container,
        GtkWidget       *widget);
static void     scatola_path_bar_remove                   (GtkContainer    *container,
        GtkWidget       *widget);
static void     scatola_path_bar_forall                   (GtkContainer    *container,
        gboolean         include_internals,
        GtkCallback      callback,
        gpointer         callback_data);
static void     scatola_path_bar_scroll_up                (ScatolaPathBar *path_bar);
static void     scatola_path_bar_scroll_down              (ScatolaPathBar *path_bar);
static gboolean scatola_path_bar_scroll                   (GtkWidget       *path_bar,
        GdkEventScroll  *scroll);
static void     scatola_path_bar_stop_scrolling           (ScatolaPathBar *path_bar);
static gboolean scatola_path_bar_slider_button_press      (GtkWidget       *widget,
        GdkEventButton  *event,
        ScatolaPathBar *path_bar);
static gboolean scatola_path_bar_slider_button_release    (GtkWidget       *widget,
        GdkEventButton  *event,
        ScatolaPathBar *path_bar);
static void     scatola_path_bar_grab_notify              (GtkWidget       *widget,
        gboolean         was_grabbed);
static void     scatola_path_bar_state_changed            (GtkWidget       *widget,
        GtkStateType     previous_state);
#if GTK_CHECK_VERSION (3, 0, 0)
static void     scatola_path_bar_style_updated            (GtkWidget       *widget);
#else
static void     scatola_path_bar_style_set                (GtkWidget       *widget,
        GtkStyle        *previous_style);
#endif
static void     scatola_path_bar_screen_changed           (GtkWidget       *widget,
        GdkScreen       *previous_screen);
static void     scatola_path_bar_check_icon_theme         (ScatolaPathBar *path_bar);
static void     scatola_path_bar_update_button_appearance (ButtonData      *button_data);
static void     scatola_path_bar_update_button_state      (ButtonData      *button_data,
        gboolean         current_dir);
static gboolean scatola_path_bar_update_path              (ScatolaPathBar *path_bar,
        GFile           *file_path,
        gboolean         emit_signal);

#if GTK_CHECK_VERSION(3,0,0)
static GtkWidget *
get_slider_button (ScatolaPathBar  *path_bar,
...
static void
update_button_types (ScatolaPathBar *path_bar)
{
    GList *list;
    GFile *path = NULL;
    ...
    if (path != NULL)
    {
        scatola_path_bar_update_path (path_bar, path, TRUE);
    }
}
...
desktop_location_changed_callback (gpointer user_data)
{
    ...
    path_bar = SCATOLA_PATH_BAR (user_data);

    g_object_unref (path_bar->desktop_path);
    g_object_unref (path_bar->home_path);
    path_bar->desktop_path = scatola_get_desktop_location ();
...
static void
trash_state_changed_cb (ScatolaTrashMonitor *monitor,
                        gboolean state,
                        ScatolaPathBar *path_bar)
{
    GFile *file;
    ...
    for (list = path_bar->button_list; list; list = list->next)
    {
        ...
        if (g_file_equal (file, button_data->path))
        {
            ...
            icon = scatola_trash_monitor_get_icon ();
            icon_info = scatola_icon_info_lookup (icon, SCATOLA_PATH_BAR_ICON_SIZE);
            pixbuf = scatola_icon_info_get_pixbuf_at_size (icon_info, SCATOLA_PATH_BAR_ICON_SIZE);
...
slider_timeout (gpointer user_data)
{
    ...
    if (gtk_widget_get_visible (GTK_WIDGET (path_bar)))
    {
        ...
        {
            scatola_path_bar_scroll_up (path_bar);
        }
        else
        {
            scatola_path_bar_scroll_down (path_bar);
        }
    }
...
static void
scatola_path_bar_slider_drag_motion (GtkWidget      *widget,
...
static void
scatola_path_bar_slider_drag_leave (GtkWidget      *widget,
                                 GdkDragContext *context,
                                 unsigned int    time,
                                 gpointer        user_data)
{
...
static void
scatola_path_bar_init (ScatolaPathBar *path_bar)
{
    char *p;
#if GTK_CHECK_VERSION(3, 0, 0)
    ...
    gtk_style_context_add_class (context, "scatola-pathbar");
#endif
...
#endif
    path_bar->icon_size = SCATOLA_PATH_BAR_ICON_SIZE;

    p = scatola_get_desktop_directory ();
    ...
    g_signal_connect_swapped (path_bar->up_slider_button, "clicked", G_CALLBACK (scatola_path_bar_scroll_up), path_bar);
    g_signal_connect_swapped (path_bar->down_slider_button, "clicked", G_CALLBACK (scatola_path_bar_scroll_down), path_bar);

    g_signal_connect (path_bar->up_slider_button, "button_press_event", G_CALLBACK (scatola_path_bar_slider_button_press), path_bar);
    g_signal_connect (path_bar->up_slider_button, "button_release_event", G_CALLBACK (scatola_path_bar_slider_button_release), path_bar);
    g_signal_connect (path_bar->down_slider_button, "button_press_event", G_CALLBACK (scatola_path_bar_slider_button_press), path_bar);
    g_signal_connect (path_bar->down_slider_button, "button_release_event", G_CALLBACK (scatola_path_bar_slider_button_release), path_bar);
    ...
    g_signal_connect (path_bar->up_slider_button,
                      "drag-motion",
                      G_CALLBACK (scatola_path_bar_slider_drag_motion),
                      path_bar);
    g_signal_connect (path_bar->up_slider_button,
                      "drag-leave",
                      G_CALLBACK (scatola_path_bar_slider_drag_leave),
                      path_bar);
    ...
    g_signal_connect (scatola_trash_monitor_get (),
                      "trash_state_changed",
...
static void
scatola_path_bar_class_init (ScatolaPathBarClass *path_bar_class)
{
    ...
    gobject_class->finalize = scatola_path_bar_finalize;
    gobject_class->dispose = scatola_path_bar_dispose;

#if GTK_CHECK_VERSION(3,0,0)
    widget_class->get_preferred_height = scatola_path_bar_get_preferred_height;
    widget_class->get_preferred_width = scatola_path_bar_get_preferred_width;
#else
    widget_class->size_request = scatola_path_bar_size_request;
#endif
    widget_class->unmap = scatola_path_bar_unmap;
    widget_class->size_allocate = scatola_path_bar_size_allocate;
#if GTK_CHECK_VERSION (3, 0, 0)
    widget_class->style_updated = scatola_path_bar_style_updated;
#else
    widget_class->style_set = scatola_path_bar_style_set;
#endif
    widget_class->screen_changed = scatola_path_bar_screen_changed;
    widget_class->grab_notify = scatola_path_bar_grab_notify;
    widget_class->state_changed = scatola_path_bar_state_changed;
    widget_class->scroll_event = scatola_path_bar_scroll;

    container_class->add = scatola_path_bar_add;
    container_class->forall = scatola_path_bar_forall;
    container_class->remove = scatola_path_bar_remove;

    path_bar_signals [PATH_CLICKED] =
        g_signal_new ("path-clicked",
                      G_OBJECT_CLASS_TYPE (path_bar_class),
                      G_SIGNAL_RUN_FIRST,
                      G_STRUCT_OFFSET (ScatolaPathBarClass, path_clicked),
                      NULL, NULL,
    ...
    path_bar_signals [PATH_SET] =
        g_signal_new ("path-set",
                      G_OBJECT_CLASS_TYPE (path_bar_class),
                      G_SIGNAL_RUN_FIRST,
                      G_STRUCT_OFFSET (ScatolaPathBarClass, path_set),
                      NULL, NULL,
                      g_cclosure_marshal_VOID__OBJECT,
                      G_TYPE_NONE, 1,
                      G_TYPE_FILE);
...
static void
scatola_path_bar_finalize (GObject *object)
{
    ScatolaPathBar *path_bar;

    path_bar = SCATOLA_PATH_BAR (object);

    scatola_path_bar_stop_scrolling (path_bar);
    ...
    g_signal_handlers_disconnect_by_func (scatola_trash_monitor_get (),
                                          trash_state_changed_cb, path_bar);
    g_signal_handlers_disconnect_by_func (scatola_preferences,
                                          desktop_location_changed_callback,
                                          path_bar);

    G_OBJECT_CLASS (scatola_path_bar_parent_class)->finalize (object);
}

/* Removes the settings signal handler.  It's safe to call multiple times */
static void
remove_settings_signal (ScatolaPathBar *path_bar,
                        GdkScreen  *screen)
{
...
static void
scatola_path_bar_dispose (GObject *object)
{
    remove_settings_signal (SCATOLA_PATH_BAR (object), gtk_widget_get_screen (GTK_WIDGET (object)));

    G_OBJECT_CLASS (scatola_path_bar_parent_class)->dispose (object);
}
...
#if GTK_CHECK_VERSION(3,0,0)
static void
scatola_path_bar_get_preferred_width (GtkWidget *widget,
    			       gint      *minimum,
    			       gint      *natural)
{
    ButtonData *button_data;
    ...
    path_bar = SCATOLA_PATH_BAR (widget);
...
static void
scatola_path_bar_get_preferred_height (GtkWidget *widget,
    				gint      *minimum,
    				gint      *natural)
{
    ButtonData *button_data;
...
static void
scatola_path_bar_size_request (GtkWidget      *widget,
                            GtkRequisition *requisition)
{
    ButtonData *button_data;
    ScatolaPathBar *path_bar;
...
static void
scatola_path_bar_update_slider_buttons (ScatolaPathBar *path_bar)
{
    if (path_bar->button_list)
    {
...
static void
scatola_path_bar_unmap (GtkWidget *widget)
{
    scatola_path_bar_stop_scrolling (SCATOLA_PATH_BAR (widget));

    GTK_WIDGET_CLASS (scatola_path_bar_parent_class)->unmap (widget);
}

/* This is a tad complicated */
static void
scatola_path_bar_size_allocate (GtkWidget     *widget,
                             GtkAllocation *allocation)
{
    GtkWidget *child;
    ScatolaPathBar *path_bar;
    ...
    if (need_sliders)
    {
        ...
        scatola_path_bar_update_slider_buttons (path_bar);
    }
    else
    {
...
static void
#if GTK_CHECK_VERSION (3, 0, 0)
scatola_path_bar_style_updated (GtkWidget *widget)
{
    if (GTK_WIDGET_CLASS (scatola_path_bar_parent_class)->style_updated)
    {
        GTK_WIDGET_CLASS (scatola_path_bar_parent_class)->style_updated (widget);
#else
scatola_path_bar_style_set (GtkWidget *widget,	GtkStyle  *previous_style)
{
    if (GTK_WIDGET_CLASS (scatola_path_bar_parent_class)->style_set)
    {
        GTK_WIDGET_CLASS (scatola_path_bar_parent_class)->style_set (widget, previous_style);
#endif
    }

    scatola_path_bar_check_icon_theme (SCATOLA_PATH_BAR (widget));
}

static void
scatola_path_bar_screen_changed (GtkWidget *widget,
                              GdkScreen *previous_screen)
{
    if (GTK_WIDGET_CLASS (scatola_path_bar_parent_class)->screen_changed)
    {
        GTK_WIDGET_CLASS (scatola_path_bar_parent_class)->screen_changed (widget, previous_screen);
    }
    /* We might nave a new settings, so we remove the old one */
    if (previous_screen)
    {
        remove_settings_signal (SCATOLA_PATH_BAR (widget), previous_screen);
    }
    scatola_path_bar_check_icon_theme (SCATOLA_PATH_BAR (widget));
}

static gboolean
scatola_path_bar_scroll (GtkWidget      *widget,
...
static void
scatola_path_bar_add (GtkContainer *container,
                   GtkWidget    *widget)
{
...
static void
scatola_path_bar_remove_1 (GtkContainer *container,
                        GtkWidget    *widget)
{
...
static void
scatola_path_bar_remove (GtkContainer *container,
                      GtkWidget    *widget)
{
    ScatolaPathBar *path_bar;
    GList *children;

    path_bar = SCATOLA_PATH_BAR (container);

    if (widget == path_bar->up_slider_button)
    {
        scatola_path_bar_remove_1 (container, widget);
...
static void
scatola_path_bar_forall (GtkContainer *container,
                      gboolean      include_internals,
                      GtkCallback   callback,
                      gpointer      callback_data)
{
...
static void
scatola_path_bar_scroll_down (ScatolaPathBar *path_bar)
{
    GList *list;
    GList *down_button;
    GList *up_button;
...
static void
scatola_path_bar_scroll_up (ScatolaPathBar *path_bar)
{
    GList *list;
...
static gboolean
scatola_path_bar_scroll_timeout (ScatolaPathBar *path_bar)
{
    ...
    if (path_bar->timer)
    {
        ...
        if (path_bar->need_timer)
        {
            ...
            path_bar->timer = g_timeout_add (SCROLL_TIMEOUT,
                                             (GSourceFunc)scatola_path_bar_scroll_timeout,
                                             path_bar);
...
static void
scatola_path_bar_stop_scrolling (ScatolaPathBar *path_bar)
{
    if (path_bar->timer)
    {
...
static gboolean
scatola_path_bar_slider_button_press (GtkWidget       *widget,
                                   GdkEventButton  *event,
                                   ScatolaPathBar *path_bar)
{
...
static gboolean
scatola_path_bar_slider_button_release (GtkWidget      *widget,
                                     GdkEventButton *event,
                                     ScatolaPathBar     *path_bar)
{
...
static void
scatola_path_bar_grab_notify (GtkWidget *widget,
                           gboolean   was_grabbed)
{
    if (!was_grabbed)
    {
...
static void
scatola_path_bar_state_changed (GtkWidget    *widget,
                             GtkStateType  previous_state)
{
...
static void
reload_icons (ScatolaPathBar *path_bar)
{
    GList *list;
    ...
    for (list = path_bar->button_list; list; list = list->next)
    {
        ...
        {
            scatola_path_bar_update_button_appearance (button_data);
        }
...
static void
change_icon_theme (ScatolaPathBar *path_bar)
{
    path_bar->icon_size = SCATOLA_PATH_BAR_ICON_SIZE;
...
static void
scatola_path_bar_check_icon_theme (ScatolaPathBar *path_bar)
{
    GtkSettings *settings;
...
void
scatola_path_bar_clear_buttons (ScatolaPathBar *path_bar)
{
                   ...
                   gpointer   data)
{
    ...
    path_bar = SCATOLA_PATH_BAR (gtk_widget_get_parent (button));
...
get_custom_user_icon_info (ButtonData *button_data)
{
    ...
    {
        custom_icon_uri = scatola_file_get_custom_icon (button_data->file);
...
get_type_icon_info (ButtonData *button_data)
{
    ...
    {
    case ROOT_BUTTON:
        return scatola_icon_info_lookup_from_name (SCATOLA_ICON_FILESYSTEM,
                                                SCATOLA_PATH_BAR_ICON_SIZE);

    case HOME_BUTTON:
        return scatola_icon_info_lookup_from_name (SCATOLA_ICON_HOME,
                                                SCATOLA_PATH_BAR_ICON_SIZE);

    case DESKTOP_BUTTON:
        return scatola_icon_info_lookup_from_name (SCATOLA_ICON_DESKTOP,
                                                SCATOLA_PATH_BAR_ICON_SIZE);

    case NORMAL_BUTTON:
        if (button_data->is_base_dir)
        {
            return scatola_file_get_icon (button_data->file,
                                       SCATOLA_PATH_BAR_ICON_SIZE,
                                       SCATOLA_FILE_ICON_FLAGS_NONE);
        }
...
button_data_free (ButtonData *button_data)
{
    ...
    if (button_data->file != NULL)
    {
        ...
        scatola_file_monitor_remove (button_data->file, button_data);
        scatola_file_unref (button_data->file);
    }
...
static void
scatola_path_bar_update_button_appearance (ButtonData *button_data)
{
    ScatolaIconInfo *icon_info;
    GdkPixbuf *pixbuf;
    const gchar *dir_name = get_dir_name (button_data);
...
static void
scatola_path_bar_update_button_state (ButtonData *button_data,
                                   gboolean    current_dir)
{
    if (button_data->label != NULL)
    {
    ...
    for (l = mounts; l != NULL; l = l->next)
    {
        ...
        if (g_file_equal (location, root))
        {
            ...
            if (button_data)
            {
                ...
                if (icon == NULL)
                {
                    icon = g_themed_icon_new (SCATOLA_ICON_FOLDER);
                }
                info = scatola_icon_info_lookup (icon, SCATOLA_PATH_BAR_ICON_SIZE);
                g_object_unref (icon);
                button_data->custom_icon = scatola_icon_info_get_pixbuf_at_size (info, SCATOLA_PATH_BAR_ICON_SIZE);
                g_object_unref (info);
                         ...
                         gpointer            user_data)
{
    ...
    if (info == SCATOLA_ICON_DND_GDE2_ICON_LIST)
    {
    ...
    else if (info == SCATOLA_ICON_DND_URI_LIST)
    {
...
setup_button_drag_source (ButtonData *button_data)
{
    ...
    gtk_target_list_add_uri_targets (target_list, SCATOLA_ICON_DND_URI_LIST);
...
static void
button_data_file_changed (ScatolaFile *file,
                          ButtonData *button_data)
{
    ...
    path_bar = (ScatolaPathBar *) gtk_widget_get_ancestor (button_data->button,
               SCATOLA_TYPE_PATH_BAR);
    if (path_bar == NULL)
    {
        return;
    }
    ...
    if (!g_file_equal (button_data->path, location))
    {
        ...
        else
        {
            ...
            else
            {
                 ...
                 */
                current_location = scatola_file_get_location (current_button_data->file);
            }

            scatola_path_bar_update_path (path_bar, location, FALSE);
            scatola_path_bar_set_path (path_bar, current_location);
            g_object_unref (location);
            g_object_unref (current_location);
            return;
        }
    }
    else if (scatola_file_is_gone (file))
    {
        gint idx, position;
...
static ButtonData *
make_directory_button (ScatolaPathBar  *path_bar,
                       ScatolaFile     *file,
                       gboolean          current_dir,
                       gboolean          base_dir,
                       gboolean          file_is_hidden)
{
    ...
    path = scatola_file_get_location (file);
    ...
    */
    ...
    {
        button_data->dir_name = scatola_file_get_display_name (file);
    }
    if (button_data->file == NULL)
    {
        button_data->file = scatola_file_ref (file);
        scatola_file_monitor_add (button_data->file, button_data,
    ...
    scatola_path_bar_update_button_state (button_data, current_dir);
    ...
    scatola_drag_slot_proxy_init (button_data->button,
...
static gboolean
scatola_path_bar_check_parent_path (ScatolaPathBar *path_bar,
                                 GFile *location,
                                 ButtonData **current_button_data)
{
    GList *list;
    ...
    if (current_path)
    {
        ...
        for (list = path_bar->button_list; list; list = list->next)
        {
            ...
            scatola_path_bar_update_button_state (BUTTON_DATA (list->data),
...
static gboolean
scatola_path_bar_update_path (ScatolaPathBar *path_bar,
                           GFile *file_path,
                           gboolean emit_signal)
{
    ScatolaFile *file, *parent_file;
    gboolean first_directory, last_directory;
    gboolean result;
    GList *new_buttons, *l, *fake_root;
    ButtonData *button_data, *current_button_data;

    g_return_val_if_fail (SCATOLA_IS_PATH_BAR (path_bar), FALSE);
    ...
    file = scatola_file_get (file_path);
    ...
    scatola_path_bar_clear_buttons (path_bar);
...
gboolean
scatola_path_bar_set_path (ScatolaPathBar *path_bar, GFile *file_path)
{
    ButtonData *button_data;
    ...
    if (scatola_path_bar_check_parent_path (path_bar, file_path, &button_data))
    {
        if (path_bar->current_path != NULL)
        {
            g_object_unref (path_bar->current_path);
    ...
    return scatola_path_bar_update_path (path_bar, file_path, TRUE);
}

GFile *
scatola_path_bar_get_path_for_button (ScatolaPathBar *path_bar,
                                   GtkWidget       *button)
{
    GList *list;

    g_return_val_if_fail (SCATOLA_IS_PATH_BAR (path_bar), NULL);
...
/**
 * _scatola_path_bar_up:
 * @path_bar: a #ScatolaPathBar
 *
 * If the selected button in the pathbar is not the furthest button "up" (in the
 * root direction), act as if the user clicked on the next button up.
 **/
void
scatola_path_bar_up (ScatolaPathBar *path_bar)
{
    GList *l;
 ...
 **/
void
scatola_path_bar_down (ScatolaPathBar *path_bar)
{
    GList *l;
...
GtkWidget *
scatola_path_bar_get_button_from_button_list_entry (gpointer entry)
{
