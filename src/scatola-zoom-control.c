...
#include <config.h>
#include "scatola-zoom-control.h"
...
struct ScatolaZoomControlDetails
{
    GtkWidget *zoom_in;
    GtkWidget *zoom_out;
...
static const char * const scatola_zoom_control_accessible_action_names[] =
{
    N_("Zoom In"),
    N_("Zoom Out"),
    N_("Zoom to Default"),
...
static const int scatola_zoom_control_accessible_action_signals[] =
{
    ZOOM_IN,
    ZOOM_OUT,
    ZOOM_TO_DEFAULT,
...
static const char * const scatola_zoom_control_accessible_action_descriptions[] =
{
...
static GtkMenu *create_zoom_menu (ScatolaZoomControl *zoom_control);

static GType scatola_zoom_control_accessible_get_type (void);

/* button assignments */
#define CONTEXTUAL_MENU_BUTTON 3

#define NUM_ACTIONS ((int)G_N_ELEMENTS (scatola_zoom_control_accessible_action_names))

#if GTK_CHECK_VERSION (3, 0, 0)
G_DEFINE_TYPE (ScatolaZoomControl, scatola_zoom_control, GTK_TYPE_BOX);
#else
G_DEFINE_TYPE (ScatolaZoomControl, scatola_zoom_control, GTK_TYPE_HBOX);
#endif

static void
scatola_zoom_control_finalize (GObject *object)
{
    g_list_free (SCATOLA_ZOOM_CONTROL (object)->details->preferred_zoom_levels);

    G_OBJECT_CLASS (scatola_zoom_control_parent_class)->finalize (object);
}

static void
zoom_button_clicked (GtkButton *button, ScatolaZoomControl *zoom_control)
{
    g_signal_emit (zoom_control, signals[ZOOM_TO_DEFAULT], 0);
}

static void
zoom_popup_menu_show (GdkEventButton *event, ScatolaZoomControl *zoom_control)
{
...
static void
zoom_popup_menu (GtkWidget *widget, ScatolaZoomControl *zoom_control)
{
    GtkMenu *menu;
...
static gboolean
scatola_zoom_control_button_press_event (GtkWidget *widget,
                                      GdkEventButton *event,
                                      ScatolaZoomControl *zoom_control)
{
    ...
    /* We don't change our state (to reflect the new zoom) here.
       ...
       which someone (e.g. scatola_window) picks up and handles by
       calling into is - scatola_zoom_control_set_zoom_level.
    */
...
zoom_out_clicked (GtkButton *button,
                  ScatolaZoomControl *zoom_control)
{
    if (scatola_zoom_control_can_zoom_out (zoom_control))
    {
...
zoom_in_clicked (GtkButton *button,
                 ScatolaZoomControl *zoom_control)
{
    if (scatola_zoom_control_can_zoom_in (zoom_control))
    {
...
static void
set_label_size (ScatolaZoomControl *zoom_control)
{
    const char *text;
    PangoLayout *layout;
    int width;
...
label_style_set_callback (GtkWidget *label,
                          ...
                          gpointer user_data)
{
    set_label_size (SCATOLA_ZOOM_CONTROL (user_data));
}

static void
scatola_zoom_control_init (ScatolaZoomControl *zoom_control)
{
    GtkWidget *image;
    int i;

    zoom_control->details = G_TYPE_INSTANCE_GET_PRIVATE (zoom_control, SCATOLA_TYPE_ZOOM_CONTROL, ScatolaZoomControlDetails);

    zoom_control->details->zoom_level = SCATOLA_ZOOM_LEVEL_STANDARD;
    zoom_control->details->min_zoom_level = SCATOLA_ZOOM_LEVEL_SMALLEST;
    zoom_control->details->max_zoom_level = SCATOLA_ZOOM_LEVEL_LARGEST;
    zoom_control->details->has_min_zoom_level = TRUE;
    zoom_control->details->has_max_zoom_level = TRUE;

    for (i = SCATOLA_ZOOM_LEVEL_LARGEST; i >= SCATOLA_ZOOM_LEVEL_SMALLEST; i--)
    {
    ...
    g_signal_connect (G_OBJECT (zoom_control->details->zoom_button),
                      "button-press-event",
                      G_CALLBACK (scatola_zoom_control_button_press_event),
                      zoom_control);
...
GtkWidget *
scatola_zoom_control_new (void)
{
    return gtk_widget_new (scatola_zoom_control_get_type (), NULL);
}

static void
scatola_zoom_control_redraw (ScatolaZoomControl *zoom_control)
{
    int percent;
    char *num_str;

    gtk_widget_set_sensitive (zoom_control->details->zoom_in,
                              scatola_zoom_control_can_zoom_in (zoom_control));
    gtk_widget_set_sensitive (zoom_control->details->zoom_out,
                              scatola_zoom_control_can_zoom_out (zoom_control));

    percent = floor ((100.0 * scatola_get_relative_icon_size_for_zoom_level (zoom_control->details->zoom_level)) + .2);
...
zoom_menu_callback (GtkMenuItem *item, gpointer callback_data)
{
    ...
    zoom_control = SCATOLA_ZOOM_CONTROL (callback_data);
    ...
    zoom_level = (ScatolaZoomLevel) GPOINTER_TO_INT (g_object_get_data (G_OBJECT (item), "zoom_level"));
...
create_zoom_menu_item (ScatolaZoomControl *zoom_control, GtkMenu *menu,
                       ScatolaZoomLevel zoom_level,
                       GtkRadioMenuItem *previous_radio_item)
{
    ...
    percent = floor ((100.0 * scatola_get_relative_icon_size_for_zoom_level (zoom_level)) + .5);
...
static GtkMenu *
create_zoom_menu (ScatolaZoomControl *zoom_control)
{
    GtkMenu *menu;
...
#if !GTK_CHECK_VERSION(3, 0, 0)
static AtkObject *
scatola_zoom_control_get_accessible (GtkWidget *widget)
{
    ...
    accessible = g_object_new
                 (scatola_zoom_control_accessible_get_type (), NULL);
...
static void
scatola_zoom_control_change_value (ScatolaZoomControl *zoom_control,
                                GtkScrollType scroll)
{
    ...
    case GTK_SCROLL_STEP_UP :
        ...
        break;
    default :
        g_warning ("Invalid scroll type %d for ScatolaZoomControl:change_value", scroll);
    }
}

void
scatola_zoom_control_set_zoom_level (ScatolaZoomControl *zoom_control,
                                  ScatolaZoomLevel zoom_level)
{
    zoom_control->details->zoom_level = zoom_level;
    scatola_zoom_control_redraw (zoom_control);
}

void
scatola_zoom_control_set_parameters (ScatolaZoomControl *zoom_control,
                                  ScatolaZoomLevel min_zoom_level,
                                  ScatolaZoomLevel max_zoom_level,
                                  gboolean has_min_zoom_level,
                                  gboolean has_max_zoom_level,
                                  GList *zoom_levels)
{
    g_return_if_fail (SCATOLA_IS_ZOOM_CONTROL (zoom_control));
...
ScatolaZoomLevel
scatola_zoom_control_get_zoom_level (ScatolaZoomControl *zoom_control)
{
    return zoom_control->details->zoom_level;
}

ScatolaZoomLevel
scatola_zoom_control_get_min_zoom_level (ScatolaZoomControl *zoom_control)
{
    return zoom_control->details->min_zoom_level;
}

ScatolaZoomLevel
scatola_zoom_control_get_max_zoom_level (ScatolaZoomControl *zoom_control)
{
    return zoom_control->details->max_zoom_level;
}

gboolean
scatola_zoom_control_has_min_zoom_level (ScatolaZoomControl *zoom_control)
{
    return zoom_control->details->has_min_zoom_level;
}

gboolean
scatola_zoom_control_has_max_zoom_level (ScatolaZoomControl *zoom_control)
{
    return zoom_control->details->has_max_zoom_level;
}

gboolean
scatola_zoom_control_can_zoom_in (ScatolaZoomControl *zoom_control)
{
    return !zoom_control->details->has_max_zoom_level ||
...
gboolean
scatola_zoom_control_can_zoom_out (ScatolaZoomControl *zoom_control)
{
    return !zoom_control->details->has_min_zoom_level ||
           (zoom_control->details->zoom_level
...
static gboolean
scatola_zoom_control_scroll_event (GtkWidget *widget, GdkEventScroll *event)
{
    ScatolaZoomControl *zoom_control;

    zoom_control = SCATOLA_ZOOM_CONTROL (widget);
     ...
     * someone (e.g. scatola_window) picks up and handles by calling into us -
     * scatola_zoom_control_set_zoom_level.
     */
    return TRUE;
}
...
static void
scatola_zoom_control_class_init (ScatolaZoomControlClass *class)
{
    GtkWidgetClass *widget_class;
    GtkBindingSet *binding_set;

    G_OBJECT_CLASS (class)->finalize = scatola_zoom_control_finalize;

    widget_class = GTK_WIDGET_CLASS (class);

#if GTK_CHECK_VERSION(3, 0, 0)
    gtk_widget_class_set_accessible_type (widget_class,
                                          scatola_zoom_control_accessible_get_type ());
#else
    widget_class->get_accessible = scatola_zoom_control_get_accessible;
#endif
    widget_class->scroll_event = scatola_zoom_control_scroll_event;

    class->change_value = scatola_zoom_control_change_value;

    signals[ZOOM_IN] =
        g_signal_new ("zoom_in",
                      G_TYPE_FROM_CLASS (class),
                      G_SIGNAL_RUN_LAST,
                      G_STRUCT_OFFSET (ScatolaZoomControlClass,
                                       zoom_in),
                      NULL, NULL,
    ...
    g_type_class_add_private (G_OBJECT_CLASS (class), sizeof (ScatolaZoomControlDetails));
}

static gboolean
scatola_zoom_control_accessible_do_action (AtkAction *accessible, int i)
{
    GtkWidget *widget;
    ...
    g_signal_emit (widget,
                   signals[scatola_zoom_control_accessible_action_signals [i]],
                   0);
...
static int
scatola_zoom_control_accessible_get_n_actions (AtkAction *accessible)
{

    return NUM_ACTIONS;
}

static const char* scatola_zoom_control_accessible_action_get_description(AtkAction* accessible, int i)
{
    g_assert(i >= 0 && i < NUM_ACTIONS);

    return _(scatola_zoom_control_accessible_action_descriptions[i]);
}

static const char* scatola_zoom_control_accessible_action_get_name(AtkAction* accessible, int i)
{
    g_assert (i >= 0 && i < NUM_ACTIONS);

    return _(scatola_zoom_control_accessible_action_names[i]);
}

static void scatola_zoom_control_accessible_action_interface_init(AtkActionIface* iface)
{
    iface->do_action = scatola_zoom_control_accessible_do_action;
    iface->get_n_actions = scatola_zoom_control_accessible_get_n_actions;
    iface->get_description = scatola_zoom_control_accessible_action_get_description;
    iface->get_name = scatola_zoom_control_accessible_action_get_name;
}

static void
scatola_zoom_control_accessible_get_current_value (AtkValue *accessible,
        GValue *value)
{
    ScatolaZoomControl *control;

    g_value_init (value, G_TYPE_INT);

    control = SCATOLA_ZOOM_CONTROL (gtk_accessible_get_widget (GTK_ACCESSIBLE (accessible)));
    if (!control)
    {
        g_value_set_int (value, SCATOLA_ZOOM_LEVEL_STANDARD);
        return;
    }
...
static void
scatola_zoom_control_accessible_get_maximum_value (AtkValue *accessible,
        GValue *value)
{
    ScatolaZoomControl *control;
...
static void
scatola_zoom_control_accessible_get_minimum_value (AtkValue *accessible,
        GValue *value)
{
    ScatolaZoomControl *control;
...
nearest_preferred (ScatolaZoomControl *zoom_control, ScatolaZoomLevel value)
{
    ScatolaZoomLevel last_value;
    ScatolaZoomLevel current_value;
    GList *l;
...
static gboolean
scatola_zoom_control_accessible_set_current_value (AtkValue *accessible,
        const GValue *value)
{
    ScatolaZoomControl *control;
    ScatolaZoomLevel zoom;
...
static void
scatola_zoom_control_accessible_value_interface_init (AtkValueIface *iface)
{
    iface->get_current_value = scatola_zoom_control_accessible_get_current_value;
    iface->get_maximum_value = scatola_zoom_control_accessible_get_maximum_value;
    iface->get_minimum_value = scatola_zoom_control_accessible_get_minimum_value;
    iface->set_current_value = scatola_zoom_control_accessible_set_current_value;
}

static const char* scatola_zoom_control_accessible_get_name(AtkObject* accessible)
{
    return _("Zoom");
}

static const char* scatola_zoom_control_accessible_get_description(AtkObject* accessible)
{
    return _("Set the zoom level of the current view");
}

static void
scatola_zoom_control_accessible_initialize (AtkObject *accessible,
        gpointer  data)
{
...
#if GTK_CHECK_VERSION(3, 0, 0)
typedef struct _ScatolaZoomControlAccessible ScatolaZoomControlAccessible;
typedef struct _ScatolaZoomControlAccessibleClass ScatolaZoomControlAccessibleClass;

struct _ScatolaZoomControlAccessible
{
    GtkContainerAccessible parent;
};

struct _ScatolaZoomControlAccessibleClass
{
    GtkContainerAccessibleClass parent_class;
};

G_DEFINE_TYPE_WITH_CODE (ScatolaZoomControlAccessible,
                         scatola_zoom_control_accessible,
                         GTK_TYPE_CONTAINER_ACCESSIBLE,
                         G_IMPLEMENT_INTERFACE (ATK_TYPE_ACTION,
                                                scatola_zoom_control_accessible_action_interface_init)
                         G_IMPLEMENT_INTERFACE (ATK_TYPE_VALUE,
                                                scatola_zoom_control_accessible_value_interface_init));
static void
scatola_zoom_control_accessible_class_init (ScatolaZoomControlAccessibleClass *klass)
{
    AtkObjectClass *atk_class = ATK_OBJECT_CLASS (klass);
    accessible_parent_class = g_type_class_peek_parent (klass);

    atk_class->get_name = scatola_zoom_control_accessible_get_name;
    atk_class->get_description = scatola_zoom_control_accessible_get_description;
    atk_class->initialize = scatola_zoom_control_accessible_initialize;
}

static void
scatola_zoom_control_accessible_init (ScatolaZoomControlAccessible *accessible)
{
}
#else
static void
scatola_zoom_control_accessible_class_init (AtkObjectClass *klass)
{
    accessible_parent_class = g_type_class_peek_parent (klass);

    klass->get_name = scatola_zoom_control_accessible_get_name;
    klass->get_description = scatola_zoom_control_accessible_get_description;
    klass->initialize = scatola_zoom_control_accessible_initialize;
}

static GType
scatola_zoom_control_accessible_get_type (void)
{
    static GType type = 0;
    ...
    if (!type)
    {
        ...
        {
            (GInterfaceInitFunc)scatola_zoom_control_accessible_action_interface_init,
        ...
        {
            (GInterfaceInitFunc)scatola_zoom_control_accessible_value_interface_init,
            (GInterfaceFinalizeFunc)NULL,
            NULL
        };

        type = eel_accessibility_create_derived_type
               ("ScatolaZoomControlAccessible",
#if GTK_CHECK_VERSION (3, 0, 0)
                GTK_TYPE_BOX,
#else
                GTK_TYPE_HBOX,
#endif
                scatola_zoom_control_accessible_class_init);
...
void
scatola_zoom_control_set_active_appearance (ScatolaZoomControl *zoom_control, gboolean is_active)
{
