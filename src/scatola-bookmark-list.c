...
/* scatola-bookmark-list.c - implementation of centralized list of bookmarks.
 */
...
static ScatolaBookmarkList *singleton = NULL;

/* forward declarations */

static void        scatola_bookmark_list_load_file     (ScatolaBookmarkList *bookmarks);
static void        scatola_bookmark_list_save_file     (ScatolaBookmarkList *bookmarks);

G_DEFINE_TYPE(ScatolaBookmarkList, scatola_bookmark_list, G_TYPE_OBJECT)

static ScatolaBookmark *
new_bookmark_from_uri (const char *uri, const char *label)
{
    ...
    if (uri)
    {
        ...
        icon = NULL;
        if (scatola_file_check_if_ready (file,
                                      SCATOLA_FILE_ATTRIBUTES_FOR_ICON))
        {
            icon = scatola_file_get_gicon (file, 0);
        }
        scatola_file_unref (file);

        if (icon == NULL)
        {
            icon = native ? g_themed_icon_new (SCATOLA_ICON_FOLDER) :
                   g_themed_icon_new (SCATOLA_ICON_FOLDER_REMOTE);
        }

        new_bookmark = scatola_bookmark_new (location, name, has_label, icon);
...
static GFile *
scatola_bookmark_list_get_file (void)
{
    char *filename;
    GFile *file;
...
static void
bookmark_in_list_changed_callback (ScatolaBookmark     *bookmark,
                                   ScatolaBookmarkList *bookmarks)
{
    g_assert (SCATOLA_IS_BOOKMARK (bookmark));
    g_assert (SCATOLA_IS_BOOKMARK_LIST (bookmarks));

    /* Save changes so we'll have the good icon next time. */
    scatola_bookmark_list_save_file (bookmarks);
}

static void
stop_monitoring_bookmark (ScatolaBookmarkList *bookmarks,
...
{
    g_assert (SCATOLA_IS_BOOKMARK (data));
    g_assert (SCATOLA_IS_BOOKMARK_LIST (user_data));

    stop_monitoring_bookmark (SCATOLA_BOOKMARK_LIST (user_data),
                              SCATOLA_BOOKMARK (data));
}

static void
clear (ScatolaBookmarkList *bookmarks)
{
...
{
    if (SCATOLA_BOOKMARK_LIST (object)->monitor != NULL)
    {
        g_file_monitor_cancel (SCATOLA_BOOKMARK_LIST (object)->monitor);
        SCATOLA_BOOKMARK_LIST (object)->monitor = NULL;
    }

    g_queue_free (SCATOLA_BOOKMARK_LIST (object)->pending_ops);

    clear (SCATOLA_BOOKMARK_LIST (object));

    G_OBJECT_CLASS (scatola_bookmark_list_parent_class)->finalize (object);
}
                ...
                GObjectConstructParam *construct_params)
{
    ...
    retval = G_OBJECT_CLASS (scatola_bookmark_list_parent_class)->constructor
             (type, n_construct_params, construct_params);

    singleton = SCATOLA_BOOKMARK_LIST (retval);
...
static void
scatola_bookmark_list_class_init (ScatolaBookmarkListClass *class)
{
    ...
    signals[CONTENTS_CHANGED] =
        g_signal_new ("contents_changed",
                      ...
                      G_SIGNAL_RUN_LAST,
                      G_STRUCT_OFFSET (ScatolaBookmarkListClass,
                                       contents_changed),
                      NULL, NULL,
                             ...
                             gpointer           user_data)
{
    ...
    {
        g_return_if_fail (SCATOLA_IS_BOOKMARK_LIST (SCATOLA_BOOKMARK_LIST (user_data)));
        scatola_bookmark_list_load_file (SCATOLA_BOOKMARK_LIST (user_data));
    }
}

static void
scatola_bookmark_list_init (ScatolaBookmarkList *bookmarks)
{
    GFile *file;

    bookmarks->pending_ops = g_queue_new ();

    scatola_bookmark_list_load_file (bookmarks);

    file = scatola_bookmark_list_get_file ();
...
static void
insert_bookmark_internal (ScatolaBookmarkList *bookmarks,
                          ScatolaBookmark     *bookmark,
                          int                   index)
{
...
/**
 * scatola_bookmark_list_append:
 *
 * Append a bookmark to a bookmark list.
 * @bookmarks: ScatolaBookmarkList to append to.
 * @bookmark: Bookmark to append a copy of.
 **/
void
scatola_bookmark_list_append (ScatolaBookmarkList *bookmarks,
                           ScatolaBookmark     *bookmark)
{
    g_return_if_fail (SCATOLA_IS_BOOKMARK_LIST (bookmarks));
    g_return_if_fail (SCATOLA_IS_BOOKMARK (bookmark));

    insert_bookmark_internal (bookmarks,
                              scatola_bookmark_copy (bookmark),
                              -1);
...
/**
 * scatola_bookmark_list_contains:
 *
 * Check whether a bookmark with matching name and url is already in the list.
 * @bookmarks: ScatolaBookmarkList to check contents of.
 * @bookmark: ScatolaBookmark to match against.
 *
 * Return value: TRUE if matching bookmark is in list, FALSE otherwise
 **/
gboolean
scatola_bookmark_list_contains (ScatolaBookmarkList *bookmarks,
                             ScatolaBookmark     *bookmark)
{
    g_return_val_if_fail (SCATOLA_IS_BOOKMARK_LIST (bookmarks), FALSE);
...
/**
 * scatola_bookmark_list_delete_item_at:
 *
 ...
 **/
void
scatola_bookmark_list_delete_item_at (ScatolaBookmarkList *bookmarks,
                                   guint                 index)
{
    GList *doomed;
    ...
    g_assert (SCATOLA_IS_BOOKMARK (doomed->data));
    stop_monitoring_bookmark (bookmarks, SCATOLA_BOOKMARK (doomed->data));
...
/**
 * scatola_bookmark_list_move_item:
 *
 ...
 **/
void
scatola_bookmark_list_move_item (ScatolaBookmarkList *bookmarks,
                              guint index,
                              guint destination)
{
    GList *bookmark_item;
...
/**
 * scatola_bookmark_list_delete_items_with_uri:
 *
 ...
 **/
void
scatola_bookmark_list_delete_items_with_uri (ScatolaBookmarkList *bookmarks,
        const char           *uri)
{
    GList *node, *next;
    ...
    for (node = bookmarks->list; node != NULL;  node = next)
    {
        ...
        bookmark_uri = scatola_bookmark_get_uri (SCATOLA_BOOKMARK (node->data));
        if (g_strcmp0 (bookmark_uri, uri) == 0)
        {
            bookmarks->list = g_list_remove_link (bookmarks->list, node);
            stop_monitoring_bookmark (bookmarks, SCATOLA_BOOKMARK (node->data));
...
/**
 * scatola_bookmark_list_get_window_geometry:
 *
 * Get a string representing the bookmark_list's window's geometry.
 * This is the value set earlier by scatola_bookmark_list_set_window_geometry.
 ...
 * passing to gde2_parse_geometry(), or NULL if
 * no window geometry has yet been saved for this bookmark list.
 **/
const char *
scatola_bookmark_list_get_window_geometry (ScatolaBookmarkList *bookmarks)
{
    return window_geometry;
}

/**
 * scatola_bookmark_list_insert_item:
 *
 ...
 **/
void
scatola_bookmark_list_insert_item (ScatolaBookmarkList *bookmarks,
                                ScatolaBookmark     *new_bookmark,
                                guint                 index)
{
    g_return_if_fail (SCATOLA_IS_BOOKMARK_LIST (bookmarks));
    ...
    insert_bookmark_internal (bookmarks,
                              scatola_bookmark_copy (new_bookmark),
                              index);
 ...
 **/
ScatolaBookmark *
scatola_bookmark_list_item_at (ScatolaBookmarkList *bookmarks, guint index)
{
    g_return_val_if_fail (SCATOLA_IS_BOOKMARK_LIST (bookmarks), NULL);
    g_return_val_if_fail (index < g_list_length (bookmarks->list), NULL);

    return SCATOLA_BOOKMARK (g_list_nth_data (bookmarks->list, index));
}

/**
 * scatola_bookmark_list_length:
 *
 ...
 **/
guint
scatola_bookmark_list_length (ScatolaBookmarkList *bookmarks)
{
    g_return_val_if_fail (SCATOLA_IS_BOOKMARK_LIST(bookmarks), 0);

    return g_list_length (bookmarks->list);
}

static void
load_file_finish (ScatolaBookmarkList *bookmarks,
                  GObject *source,
                  GAsyncResult *res)
{
    GError *error = NULL;
...
static void
load_file_async (ScatolaBookmarkList *self,
...
static void
save_file_finish (ScatolaBookmarkList *bookmarks,
                  GObject *source,
                  GAsyncResult *res)
{
    GError *error = NULL;
...
static void
save_file_async (ScatolaBookmarkList *bookmarks,
                 GAsyncReadyCallback callback)
{
    GFile *file;
    GList *l;
    ...
    for (l = bookmarks->list; l; l = l->next)
    {
        ...
        {
            char *label, *uri;
            label = scatola_bookmark_get_name (bookmark);
            uri = scatola_bookmark_get_uri (bookmark);
...
static void
process_next_op (ScatolaBookmarkList *bookmarks);
...
op_processed_cb (GObject *source,
                 ...
                 gpointer user_data)
{
    ScatolaBookmarkList *self = user_data;
    int op;
...
/**
 * scatola_bookmark_list_save_file:
 *
 * Save bookmarks to disk.
 * @bookmarks: the list of bookmarks to save.
 **/
static void
scatola_bookmark_list_save_file (ScatolaBookmarkList *bookmarks)
{
    g_signal_emit (bookmarks, signals[CONTENTS_CHANGED], 0);
 ...
 **/
ScatolaBookmarkList *
scatola_bookmark_list_new (void)
{
    ScatolaBookmarkList *list;

    list = SCATOLA_BOOKMARK_LIST (g_object_new (SCATOLA_TYPE_BOOKMARK_LIST, NULL));

    return list;
}

/**
 * scatola_bookmark_list_set_window_geometry:
 *
 ...
 **/
void
scatola_bookmark_list_set_window_geometry (ScatolaBookmarkList *bookmarks,
                                        const char           *geometry)
{
    g_return_if_fail (SCATOLA_IS_BOOKMARK_LIST (bookmarks));
