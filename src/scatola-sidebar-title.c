...
static void                scatola_sidebar_title_size_allocate     (GtkWidget             *widget,
        							 GtkAllocation         *allocation);
static void                update_icon                          (ScatolaSidebarTitle      *sidebar_title);
static GtkWidget *         sidebar_title_create_title_label     (void);
static GtkWidget *         sidebar_title_create_more_info_label (void);
static void		   update_all 				(ScatolaSidebarTitle      *sidebar_title);
static void		   update_more_info			(ScatolaSidebarTitle      *sidebar_title);
static void		   update_title_font			(ScatolaSidebarTitle      *sidebar_title);
#if GTK_CHECK_VERSION (3, 0, 0)
...
#endif
static guint		   get_best_icon_size 			(ScatolaSidebarTitle      *sidebar_title);
...
struct ScatolaSidebarTitleDetails
{
    ScatolaFile		*file;
...
#if GTK_CHECK_VERSION (3, 0, 0)
G_DEFINE_TYPE (ScatolaSidebarTitle, scatola_sidebar_title, GTK_TYPE_BOX)
#else
G_DEFINE_TYPE (ScatolaSidebarTitle, scatola_sidebar_title, GTK_TYPE_VBOX)
#endif
...
{
    ScatolaSidebarTitle *sidebar_title;
#else
static void
           ...
           GtkStyle  *previous_style)
{
    ...
    g_return_if_fail (SCATOLA_IS_SIDEBAR_TITLE (widget));

    sidebar_title = SCATOLA_SIDEBAR_TITLE (widget);
...
static void
scatola_sidebar_title_init (ScatolaSidebarTitle *sidebar_title)
{
    sidebar_title->details = G_TYPE_INSTANCE_GET_PRIVATE (sidebar_title,
    							  SCATOLA_TYPE_SIDEBAR_TITLE,
    							  ScatolaSidebarTitleDetails);
...
static void
release_file (ScatolaSidebarTitle *sidebar_title)
{
    ...
    {
        scatola_file_monitor_remove (sidebar_title->details->file, sidebar_title);
        scatola_file_unref (sidebar_title->details->file);
        sidebar_title->details->file = NULL;
    }
}

static void
scatola_sidebar_title_finalize (GObject *object)
{
    ScatolaSidebarTitle *sidebar_title;

    sidebar_title = SCATOLA_SIDEBAR_TITLE (object);
    ...
    G_OBJECT_CLASS (scatola_sidebar_title_parent_class)->finalize (object);
}

static void
scatola_sidebar_title_class_init (ScatolaSidebarTitleClass *klass)
{
    GtkWidgetClass *widget_class;

    G_OBJECT_CLASS (klass)->finalize = scatola_sidebar_title_finalize;

    widget_class = GTK_WIDGET_CLASS (klass);
    widget_class->size_allocate = scatola_sidebar_title_size_allocate;
#if GTK_CHECK_VERSION (3, 0, 0)
    ...
    g_type_class_add_private (klass, sizeof (ScatolaSidebarTitleDetails));
}

/* return a new index title object */
GtkWidget *
scatola_sidebar_title_new (void)
{
    return gtk_widget_new (scatola_sidebar_title_get_type (), NULL);
}

static void
#if GTK_CHECK_VERSION (3, 0, 0)
setup_gc_with_fg (ScatolaSidebarTitle *sidebar_title, int idx, GdkRGBA *color)
{
    sidebar_title->details->label_colors[idx] = *color;
#else
setup_gc_with_fg (ScatolaSidebarTitle *sidebar_title, int idx, guint32 color)
{
...
void
scatola_sidebar_title_select_text_color (ScatolaSidebarTitle *sidebar_title,
                                      EelBackground    *background)
{
    ...
    g_assert (SCATOLA_IS_SIDEBAR_TITLE (sidebar_title));
...
static char*
get_property_from_component (ScatolaSidebarTitle *sidebar_title, const char *property)
{
...
static guint
get_best_icon_size (ScatolaSidebarTitle *sidebar_title)
{
    gint width;
    GtkAllocation allocation;
    ...
    if (width < 0)
    {
        ...
        return scatola_icon_get_smaller_icon_size (0);
    }
    else
    {
        return scatola_icon_get_smaller_icon_size ((guint) width);
    }
}

/* set up the icon image */
static void
update_icon (ScatolaSidebarTitle *sidebar_title)
{
    GdkPixbuf *pixbuf;
    ...
    {
        info = scatola_icon_info_lookup_from_name (icon_name, SCATOLA_ICON_SIZE_LARGE);
        pixbuf = scatola_icon_info_get_pixbuf_at_size (info,  SCATOLA_ICON_SIZE_LARGE);
        g_object_unref (info);
    }
    else if (sidebar_title->details->file != NULL &&
             scatola_file_check_if_ready (sidebar_title->details->file,
                                       SCATOLA_FILE_ATTRIBUTES_FOR_ICON))
    {
        pixbuf = scatola_file_get_icon_pixbuf (sidebar_title->details->file,
                                            sidebar_title->details->best_icon_size,
                                            TRUE,
                                            SCATOLA_FILE_ICON_FLAGS_USE_THUMBNAILS |
                                            SCATOLA_FILE_ICON_FLAGS_USE_MOUNT_ICON_AS_EMBLEM);
    }
...
static void
update_title_font (ScatolaSidebarTitle *sidebar_title)
{
...
static void
update_title (ScatolaSidebarTitle *sidebar_title)
{
    GtkLabel *label;
    const char *text;
...
static void
update_more_info (ScatolaSidebarTitle *sidebar_title)
{
    ScatolaFile *file;
    GString *info_string;
    ...
    else
    {
        ...
        type_string = NULL;
        if (file != NULL && scatola_file_should_show_type (file))
        {
            type_string = scatola_file_get_string_attribute (file, "type");
        }
        ...
        if (type_string != NULL)
        {
            ...
            append_and_eat (info_string, ", ",
                            scatola_file_get_string_attribute (file, "size"));
        }
        else
        {
        ...
        if (sidebar_width > MINIMUM_INFO_WIDTH)
        {
            ...
            date_modified_str = scatola_file_fit_modified_date_as_string
...
static void
add_emblem (ScatolaSidebarTitle *sidebar_title, GdkPixbuf *pixbuf)
{
...
static void
update_emblems (ScatolaSidebarTitle *sidebar_title)
{
    GList *pixbufs, *p;
    GdkPixbuf *pixbuf;
    ...
    pixbufs = scatola_file_get_emblem_pixbufs (sidebar_title->details->file,
                                            scatola_icon_get_emblem_size_for_icon_size (SCATOLA_ICON_SIZE_STANDARD),
                                            FALSE,
                                            NULL);
...
char *
scatola_sidebar_title_get_text (ScatolaSidebarTitle *sidebar_title)
{
...
void
scatola_sidebar_title_set_text (ScatolaSidebarTitle *sidebar_title,
                             const char* new_text)
{
    g_free (sidebar_title->details->title_text);
...
static gboolean
item_count_ready (ScatolaSidebarTitle *sidebar_title)
{
...
static void
monitor_add (ScatolaSidebarTitle *sidebar_title)
{
    ...
    attributes = SCATOLA_FILE_ATTRIBUTES_FOR_ICON | SCATOLA_FILE_ATTRIBUTE_INFO;
    if (sidebar_title->details->monitoring_count)
    {
        attributes |= SCATOLA_FILE_ATTRIBUTE_DIRECTORY_ITEM_COUNT;
    }

    scatola_file_monitor_add (sidebar_title->details->file, sidebar_title, attributes);
}

static void
update_all (ScatolaSidebarTitle *sidebar_title)
{
...
void
scatola_sidebar_title_set_file (ScatolaSidebarTitle *sidebar_title,
                             ScatolaFile         *file,
                             const char           *initial_text)
{
    if (file != sidebar_title->details->file)
    {
        ...
        scatola_file_ref (sidebar_title->details->file);
...
static void
scatola_sidebar_title_size_allocate (GtkWidget *widget,
                                  GtkAllocation *allocation)
{
    ScatolaSidebarTitle *sidebar_title;
    guint16 old_width;
    ...
    GTK_WIDGET_CLASS (scatola_sidebar_title_parent_class)->size_allocate (widget, allocation);
...
gboolean
scatola_sidebar_title_hit_test_icon (ScatolaSidebarTitle *sidebar_title, int x, int y)
{
    GtkAllocation *allocation;
    gboolean icon_hit;

    g_return_val_if_fail (SCATOLA_IS_SIDEBAR_TITLE (sidebar_title), FALSE);
