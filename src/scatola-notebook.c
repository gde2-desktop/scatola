...
static void scatola_notebook_init		 (ScatolaNotebook *notebook);
static void scatola_notebook_class_init	 (ScatolaNotebookClass *klass);
static int  scatola_notebook_insert_page	 (GtkNotebook *notebook,
        GtkWidget *child,
        GtkWidget *tab_label,
        GtkWidget *menu_label,
        int position);
static void scatola_notebook_remove	 (GtkContainer *container,
...
G_DEFINE_TYPE (ScatolaNotebook, scatola_notebook, GTK_TYPE_NOTEBOOK);

static void
scatola_notebook_class_init (ScatolaNotebookClass *klass)
{
    GObjectClass *object_class = G_OBJECT_CLASS (klass);
    GtkContainerClass *container_class = GTK_CONTAINER_CLASS (klass);
    GtkNotebookClass *notebook_class = GTK_NOTEBOOK_CLASS (klass);

    container_class->remove = scatola_notebook_remove;

    notebook_class->insert_page = scatola_notebook_insert_page;

#if !GTK_CHECK_VERSION (3, 0, 0)
    gtk_rc_parse_string ("style \"scatola-tab-close-button-style\"\n"
                         "{\n"
                         ...
                         "ythickness = 0\n"
                         "}\n"
                         "widget \"*.scatola-tab-close-button\" style \"scatola-tab-close-button-style\"");
#endif
    ...
    signals[TAB_CLOSE_REQUEST] =
        g_signal_new ("tab-close-request",
                      ...
                      G_SIGNAL_RUN_LAST,
                      G_STRUCT_OFFSET (ScatolaNotebookClass, tab_close_request),
                      NULL, NULL,
                      g_cclosure_marshal_VOID__OBJECT,
                      G_TYPE_NONE,
                      1,
                      SCATOLA_TYPE_WINDOW_SLOT);
}

static gint
find_tab_num_at_pos (ScatolaNotebook *notebook, gint abs_x, gint abs_y)
{
...
static gboolean
button_press_cb (ScatolaNotebook *notebook,
...
static void
scatola_notebook_init (ScatolaNotebook *notebook)
{
#if GTK_CHECK_VERSION (3, 0, 0)
    GtkStyleContext *context;

    context = gtk_widget_get_style_context (GTK_WIDGET (notebook));
    gtk_style_context_add_class (context, "scatola-notebook");
#endif
...
void
scatola_notebook_sync_loading (ScatolaNotebook *notebook,
                            ScatolaWindowSlot *slot)
{
    GtkWidget *tab_label, *spinner, *icon;
    gboolean active;

    g_return_if_fail (SCATOLA_IS_NOTEBOOK (notebook));
...
void
scatola_notebook_sync_tab_label (ScatolaNotebook *notebook,
                              ScatolaWindowSlot *slot)
{
                         ...
                         ScatolaWindowSlot *slot)
{
    ...
    notebook = gtk_widget_get_ancestor (slot->content_box, SCATOLA_TYPE_NOTEBOOK);
    if (notebook != NULL)
    {
...
static GtkWidget *
build_tab_label (ScatolaNotebook *nb, ScatolaWindowSlot *slot)
{
    ...
    gtk_widget_set_name (close_button, "scatola-tab-close-button");
...
static int
scatola_notebook_insert_page (GtkNotebook *gnotebook,
                           GtkWidget *tab_widget,
                           GtkWidget *tab_label,
                           GtkWidget *menu_label,
                           int position)
{
    ...
    position = GTK_NOTEBOOK_CLASS (scatola_notebook_parent_class)->insert_page (gnotebook,
               tab_widget,
               tab_label,
               menu_label,
               position);
...
int
scatola_notebook_add_tab (ScatolaNotebook *notebook,
                       ScatolaWindowSlot *slot,
                       int position,
                       gboolean jump_to)
{
    ...
    g_return_val_if_fail (SCATOLA_IS_NOTEBOOK (notebook), -1);
    g_return_val_if_fail (SCATOLA_IS_WINDOW_SLOT (slot), -1);
...
static void
scatola_notebook_remove (GtkContainer *container,
                      GtkWidget *tab_widget)
{
    GtkNotebook *gnotebook = GTK_NOTEBOOK (container);
    GTK_CONTAINER_CLASS (scatola_notebook_parent_class)->remove (container, tab_widget);
...
void
scatola_notebook_reorder_current_child_relative (ScatolaNotebook *notebook,
        int offset)
{
    ...
    if (!scatola_notebook_can_reorder_current_child_relative (notebook, offset))
    {
        return;
    }
...
void
scatola_notebook_set_current_page_relative (ScatolaNotebook *notebook,
        int offset)
{
    GtkNotebook *gnotebook;
    int page;
    ...
    if (!scatola_notebook_can_set_current_page_relative (notebook, offset))
    {
        return;
    }
...
static gboolean
scatola_notebook_is_valid_relative_position (ScatolaNotebook *notebook,
        int offset)
{
    GtkNotebook *gnotebook;
    int page;
...
gboolean
scatola_notebook_can_reorder_current_child_relative (ScatolaNotebook *notebook,
        int offset)
{
    g_return_val_if_fail (SCATOLA_IS_NOTEBOOK (notebook), FALSE);

    return scatola_notebook_is_valid_relative_position (notebook, offset);
}

gboolean
scatola_notebook_can_set_current_page_relative (ScatolaNotebook *notebook,
        int offset)
{
    g_return_val_if_fail (SCATOLA_IS_NOTEBOOK (notebook), FALSE);
