...
#include "scatola-x-content-bar.h"
...
static void begin_location_change                     (ScatolaWindowSlot         *slot,
        GFile                      *location,
        GFile                      *previous_location,
        GList                      *new_selection,
        ScatolaLocationChangeType      type,
        guint                       distance,
        const char                 *scroll_pos,
        ScatolaWindowGoToCallback      callback,
        gpointer                    user_data);
static void free_location_change                      (ScatolaWindowSlot         *slot);
static void end_location_change                       (ScatolaWindowSlot         *slot);
static void cancel_location_change                    (ScatolaWindowSlot         *slot);
static void got_file_info_for_view_selection_callback (ScatolaFile               *file,
        gpointer                    callback_data);
static void create_content_view                       (ScatolaWindowSlot         *slot,
        const char                 *view_id);
static void display_view_selection_failure            (ScatolaWindow             *window,
        ScatolaFile               *file,
        GFile                      *location,
        GError                     *error);
static void load_new_location                         (ScatolaWindowSlot         *slot,
        GFile                      *location,
        GList                      *selection,
        gboolean                    tell_current_content_view,
        gboolean                    tell_new_content_view);
static void location_has_really_changed               (ScatolaWindowSlot         *slot);
static void update_for_new_location                   (ScatolaWindowSlot         *slot);

void
scatola_window_report_selection_changed (ScatolaWindowInfo *window)
{
 ...
 */
static void
set_displayed_location (ScatolaWindowSlot *slot, GFile *location)
{
    ...
    {
        bookmark_location = scatola_bookmark_get_location (slot->current_location_bookmark);
    ...
    if (recreate)
    {
        ...
        slot->current_location_bookmark = (location == NULL) ? NULL
                                          : scatola_bookmark_new (location, name, FALSE, NULL);
        g_free (name);
    }
}

static void
check_bookmark_location_matches (ScatolaBookmark *bookmark, GFile *location)
{
    GFile *bookmark_location;
 ...
 */
static void
check_last_bookmark_location_matches_slot (ScatolaWindowSlot *slot)
{
...
static void
handle_go_back (ScatolaNavigationWindowSlot *navigation_slot,
                GFile *location)
{
    ...
    slot = SCATOLA_WINDOW_SLOT (navigation_slot);
    ...
    check_bookmark_location_matches (SCATOLA_BOOKMARK (g_list_nth_data (navigation_slot->back_list,
    ...
    {
        bookmark = SCATOLA_BOOKMARK (navigation_slot->back_list->data);
...
static void
handle_go_forward (ScatolaNavigationWindowSlot *navigation_slot,
                   GFile *location)
{
    ScatolaWindowSlot *slot;
    guint i;
    ...
    check_bookmark_location_matches (SCATOLA_BOOKMARK (g_list_nth_data (navigation_slot->forward_list,
                                     slot->location_change_distance)),
                                     location);
    g_assert (slot->location != NULL);
    ...
    for (i = 0; i < slot->location_change_distance; ++i)
    {
        bookmark = SCATOLA_BOOKMARK (navigation_slot->forward_list->data);
        navigation_slot->forward_list =
            g_list_remove (navigation_slot->back_list, bookmark);
        navigation_slot->back_list =
            g_list_prepend (navigation_slot->forward_list, bookmark);
...
static void
handle_go_elsewhere (ScatolaWindowSlot *slot, GFile *location)
{
#if !NEW_UI_COMPLETE
    ScatolaNavigationWindowSlot *navigation_slot;

    if (SCATOLA_IS_NAVIGATION_WINDOW_SLOT (slot))
    {
        navigation_slot = SCATOLA_NAVIGATION_WINDOW_SLOT (slot);
...
void
scatola_window_update_up_button (ScatolaWindow *window)
{
    ScatolaWindowSlot *slot;
    gboolean allowed;
    GFile *parent;
    ...
    scatola_window_allow_up (window, allowed);
}

static void
viewed_file_changed_callback (ScatolaFile *file,
                              ScatolaWindowSlot *slot)
{
    ...
    if (!scatola_file_is_not_yet_confirmed (file))
    {
    ...
    slot->viewed_file_in_trash = is_in_trash = scatola_file_is_in_trash (file);

    /* Close window if the file it's viewing has been deleted or moved to trash. */
    if (scatola_file_is_gone (file) || (is_in_trash && !was_in_trash))
    {
        ...
        if (slot->viewed_file_seen)
        {
            ...
            if (SCATOLA_IS_NAVIGATION_WINDOW (window))
            {
                ...
                if (parent)
                {
                    go_to_file = scatola_find_existing_uri_in_hierarchy (parent);
                    g_object_unref (parent);
                }
                ...
                if (go_to_file != NULL)
                {
                    ...
                    {
                        /* multiview-TODO also update ScatolaWindowSlot
                         * [which as of writing doesn't save/store any path bar state]
                         */
                        scatola_path_bar_clear_buttons (SCATOLA_PATH_BAR (SCATOLA_NAVIGATION_WINDOW_PANE (slot->pane)->path_bar));
                    }

                    scatola_window_slot_go_to (slot, go_to_file, FALSE);
                    g_object_unref (go_to_file);
                }
                else
                {
                    scatola_window_slot_go_home (slot, FALSE);
                }
            }
            else
            {
    ...
    {
        new_location = scatola_file_get_location (file);
...
static void
update_history (ScatolaWindowSlot *slot,
                ScatolaLocationChangeType type,
                GFile *new_location)
{
    switch (type)
    {
    case SCATOLA_LOCATION_CHANGE_STANDARD:
    case SCATOLA_LOCATION_CHANGE_FALLBACK:
        scatola_window_slot_add_current_location_to_history_list (slot);
        handle_go_elsewhere (slot, new_location);
        return;
    case SCATOLA_LOCATION_CHANGE_RELOAD:
        /* for reload there is no work to do */
        return;
    case SCATOLA_LOCATION_CHANGE_BACK:
        scatola_window_slot_add_current_location_to_history_list (slot);
        handle_go_back (SCATOLA_NAVIGATION_WINDOW_SLOT (slot), new_location);
        return;
    case SCATOLA_LOCATION_CHANGE_FORWARD:
        scatola_window_slot_add_current_location_to_history_list (slot);
        handle_go_forward (SCATOLA_NAVIGATION_WINDOW_SLOT (slot), new_location);
        return;
    case SCATOLA_LOCATION_CHANGE_REDIRECT:
...
static void
cancel_viewed_file_changed_callback (ScatolaWindowSlot *slot)
{
    ScatolaFile *file;
    ...
    if (file != NULL)
    {
        g_signal_handlers_disconnect_by_func (G_OBJECT (file),
                                              ...
                                              slot);
        scatola_file_monitor_remove (file, &slot->viewed_file);
    }
}
...
void
scatola_window_slot_open_location_full (ScatolaWindowSlot *slot,
                                     GFile *location,
...
{
    ScatolaWindow *window;
    ScatolaWindow *target_window;
    ScatolaWindowPane *pane;
    ScatolaWindowSlot *target_slot;
    ScatolaWindowOpenFlags slot_flags;
    ...
    old_uri = scatola_window_slot_get_location_uri (slot);
    if (old_uri == NULL)
    {
    ...
    g_assert (!((flags & SCATOLA_WINDOW_OPEN_FLAG_NEW_WINDOW) != 0 &&
                (flags & SCATOLA_WINDOW_OPEN_FLAG_NEW_TAB) != 0));

    is_desktop = SCATOLA_IS_DESKTOP_WINDOW (window);
    is_navigation = SCATOLA_IS_NAVIGATION_WINDOW (window);
    target_same = is_desktop &&
    		!scatola_desktop_window_loaded (SCATOLA_DESKTOP_WINDOW (window));

    old_location = scatola_window_slot_get_location (slot);

    switch (mode)
    {
    case SCATOLA_WINDOW_OPEN_ACCORDING_TO_MODE :
        ...
        } else if (flags & SCATOLA_WINDOW_OPEN_FLAG_NEW_WINDOW) {
        ...
        break;
    case SCATOLA_WINDOW_OPEN_IN_NAVIGATION :
        target_navigation = TRUE;
        break;
    default :
    ...
    if (target_same) {
        ...
        target_window = scatola_application_create_navigation_window
            (window->application,
             NULL,
             gtk_window_get_screen (GTK_WINDOW (window)));
    } else {
        target_window = scatola_application_get_spatial_window
            (window->application,
             window,
             NULL,
             location,
    ...
    if (existing) {
        ...
        if (new_selection != NULL && slot->content_view != NULL) {
            scatola_view_set_selection (target_slot->content_view, new_selection);
        }
    ...
    if ((flags & SCATOLA_WINDOW_OPEN_FLAG_NEW_TAB) != 0 &&
            SCATOLA_IS_NAVIGATION_WINDOW (window))
    {
        ...
        {
            slot_flags = SCATOLA_WINDOW_OPEN_SLOT_APPEND;
        }

        target_slot = scatola_window_open_slot (window->details->active_pane, slot_flags);
    }

    if ((flags & SCATOLA_WINDOW_OPEN_FLAG_CLOSE_BEHIND) != 0)
    {
        if (SCATOLA_IS_SPATIAL_WINDOW (window) && !SCATOLA_IS_DESKTOP_WINDOW (window))
        {
    ...
    begin_location_change (target_slot, location, old_location, new_selection,
                           (is_desktop && target_same) ? SCATOLA_LOCATION_CHANGE_RELOAD : SCATOLA_LOCATION_CHANGE_STANDARD, 0, NULL, callback, user_data);
    ...
    for (l = target_window->details->panes; l != NULL; l = l->next)
    {
        ...
        if (slot->location == NULL && slot->pending_location == NULL) {
            begin_location_change (slot, location, old_location, new_selection,
                                   SCATOLA_LOCATION_CHANGE_STANDARD, 0, NULL, NULL, NULL);
        }
    }
...
void
scatola_window_slot_open_location (ScatolaWindowSlot *slot,
                                GFile *location,
                                gboolean close_behind)
{
    ...
    scatola_window_slot_open_location_full (slot, location,
                                         SCATOLA_WINDOW_OPEN_ACCORDING_TO_MODE,
                                         flags, NULL, NULL, NULL);
}

void
scatola_window_slot_open_location_with_selection (ScatolaWindowSlot *slot,
        GFile *location,
        GList *selection,
        gboolean close_behind)
{
...
void
scatola_window_slot_go_home (ScatolaWindowSlot *slot, gboolean new_tab)
{
    GFile *home;
    ScatolaWindowOpenFlags flags;

    g_return_if_fail (SCATOLA_IS_WINDOW_SLOT (slot));
    ...
    scatola_window_slot_open_location_full (slot, home,
                                         SCATOLA_WINDOW_OPEN_ACCORDING_TO_MODE,
                                         flags, NULL, NULL, NULL);
    g_object_unref (home);
}
...
#if 0
static char *
scatola_window_slot_get_view_label (ScatolaWindowSlot *slot)
{
    const ScatolaViewInfo *info;

    info = scatola_view_factory_lookup (scatola_window_slot_get_content_view_id (slot));
...
static char *
scatola_window_slot_get_view_error_label (ScatolaWindowSlot *slot)
{
    const ScatolaViewInfo *info;
...
static char *
scatola_window_slot_get_view_startup_error_label (ScatolaWindowSlot *slot)
{
    const ScatolaViewInfo *info;
...
static void
report_current_content_view_failure_to_user (ScatolaWindowSlot *slot)
{
    ScatolaWindow *window;
    char *message;
    ...
    message = scatola_window_slot_get_view_startup_error_label (slot);
...
static void
report_nascent_content_view_failure_to_user (ScatolaWindowSlot *slot,
        ScatolaView *view)
{
    ScatolaWindow *window;
    char *message;
     ...
     */
    message = scatola_window_slot_get_view_error_label (slot);
    eel_show_error_dialog (message,
...
const char *
scatola_window_slot_get_content_view_id (ScatolaWindowSlot *slot)
{
    if (slot->content_view == NULL)
    {
        return NULL;
    }
    return scatola_view_get_view_id (slot->content_view);
}

gboolean
scatola_window_slot_content_view_matches_iid (ScatolaWindowSlot *slot,
        const char *iid)
{
    if (slot->content_view == NULL)
    {
    ...
    return g_strcmp0 (scatola_view_get_view_id (slot->content_view), iid) == 0;
}

static gboolean
report_callback (ScatolaWindowSlot *slot,
                 GError *error)
{
 ...
 * @window: The ScatolaWindow whose location should be changed.
 ...
 */
static void
begin_location_change (ScatolaWindowSlot *slot,
                       GFile *location,
                       GFile *previous_location,
                       GList *new_selection,
                       ScatolaLocationChangeType type,
                       ...
                       gpointer user_data)
{
    ...
    g_assert (type == SCATOLA_LOCATION_CHANGE_BACK
              || type == SCATOLA_LOCATION_CHANGE_FORWARD
              || distance == 0);
    ...
    scatola_window_slot_set_allow_stop (slot, TRUE);
    scatola_window_slot_set_status (slot, " ");
     ...
     */
    if (type == SCATOLA_LOCATION_CHANGE_RELOAD)
    {
        force_reload = TRUE;
    }
    else if (!scatola_monitor_active ())
    {
        force_reload = TRUE;
    }
    else
    {
        force_reload = !scatola_directory_is_local (directory);
    }

    if (force_reload)
    {
        scatola_directory_force_reload (directory);
        file = scatola_directory_get_corresponding_file (directory);
        scatola_file_invalidate_all_attributes (file);
    ...
    {
        current_pos = scatola_view_get_first_visible_file (slot->content_view);
        scatola_bookmark_set_scroll_pos (slot->current_location_bookmark, current_pos);
        g_free (current_pos);
    }

    /* Get the info needed for view selection */

    slot->determine_view_file = scatola_file_get (location);
    ...
    scatola_file_call_when_ready (slot->determine_view_file,
...
static void
setup_new_spatial_window (ScatolaWindowSlot *slot, ScatolaFile *file)
{
    ScatolaWindow *window;
    ...
    if (SCATOLA_IS_SPATIAL_WINDOW (window) && !SCATOLA_IS_DESKTOP_WINDOW (window))
    {
        ...
        show_hidden_file_setting = scatola_file_get_metadata
                                   (file, SCATOLA_METADATA_KEY_WINDOW_SHOW_HIDDEN_FILES,
                                    NULL);
        if (show_hidden_file_setting != NULL)
        {
            if (strcmp (show_hidden_file_setting, "1") == 0)
            {
                window->details->show_hidden_files_mode = SCATOLA_WINDOW_SHOW_HIDDEN_FILES_ENABLE;
            }
            else
            {
                window->details->show_hidden_files_mode = SCATOLA_WINDOW_SHOW_HIDDEN_FILES_DISABLE;
            }

            /* Update the UI, since we initialize it to the default */
            action = gtk_action_group_get_action (window->details->main_action_group, SCATOLA_ACTION_SHOW_HIDDEN_FILES);
        ...
        {
            SCATOLA_WINDOW (window)->details->show_hidden_files_mode = SCATOLA_WINDOW_SHOW_HIDDEN_FILES_DEFAULT;
        }
        g_free (show_hidden_file_setting);

        /* load the saved window geometry */
        maximized = scatola_file_get_boolean_metadata
                    (file, SCATOLA_METADATA_KEY_WINDOW_MAXIMIZED, FALSE);
        if (maximized)
        {
        ...
        sticky = scatola_file_get_boolean_metadata
                 (file, SCATOLA_METADATA_KEY_WINDOW_STICKY, FALSE);
        if (sticky)
        {
        ...
        above = scatola_file_get_boolean_metadata
                (file, SCATOLA_METADATA_KEY_WINDOW_KEEP_ABOVE, FALSE);
        if (above)
        {
        ...
        geometry_string = scatola_file_get_metadata
                          (file, SCATOLA_METADATA_KEY_WINDOW_GEOMETRY, NULL);
        if (geometry_string != NULL)
        {
            eel_gtk_window_set_initial_geometry_from_string
            (GTK_WINDOW (window),
             geometry_string,
             SCATOLA_SPATIAL_WINDOW_MIN_WIDTH,
             SCATOLA_SPATIAL_WINDOW_MIN_HEIGHT,
             FALSE);
        }
        ...
        if (slot->pending_selection == NULL)
        {
            ...
            scroll_string = scatola_file_get_metadata
                            (file, SCATOLA_METADATA_KEY_WINDOW_SCROLL_POSITION,
                             NULL);
        }
        else
        {
                            ...
                            gpointer user_data)
{
    ...
    slot->determine_view_file = scatola_file_get (slot->pending_location);
    ...
    {
        scatola_file_invalidate_all_attributes (slot->determine_view_file);
        scatola_file_call_when_ready (slot->determine_view_file,
                                   SCATOLA_FILE_ATTRIBUTE_INFO,
                                   got_file_info_for_view_selection_callback,
                                   slot);
...
static void
got_file_info_for_view_selection_callback (ScatolaFile *file,
        gpointer callback_data)
{
    GError *error;
    char *view_id;
            ...
            (error->domain == G_IO_ERROR && error->code == G_IO_ERROR_NOT_SUPPORTED))
    {
        ...
        mimetype = scatola_file_get_mime_type (file);

        /* If fallback, don't use view from metadata */
        if (slot->location_change_type != SCATOLA_LOCATION_CHANGE_FALLBACK)
        {
            /* Look in metadata for view */
            view_id = scatola_file_get_metadata
                      (file, SCATOLA_METADATA_KEY_DEFAULT_VIEW, NULL);
            if (view_id != NULL &&
                    !scatola_view_factory_view_supports_uri (view_id,
                            location,
                            scatola_file_get_file_type (file),
                            mimetype))
            {
                g_free (view_id);
                view_id = NULL;
        ...
        if (view_id == NULL)
        {
            view_id = scatola_global_preferences_get_default_folder_viewer_preference_as_iid ();
    ...
    if (view_id != NULL)
    {
        if (!gtk_widget_get_visible (GTK_WIDGET (window)) && SCATOLA_IS_SPATIAL_WINDOW (window))
        {
    ...
    else
    {
        ...
        if (!gtk_widget_get_visible (GTK_WIDGET (window)))
        {
            ...
            if (scatola_application_get_n_windows () <= 1)
            {
                g_assert (scatola_application_get_n_windows () == 1);

                /* the user could have typed in a home directory that doesn't exist,
                   in which case going home would cause an infinite loop, so we
                   better test for that */

                if (!scatola_is_root_directory (location))
                {
                    if (!scatola_is_home_directory (location))
                    {
                        scatola_window_slot_go_home (slot, FALSE);
                    }
                    else
        ...
        else
        {
            ...
            else
            {
                ...
                viewed_file = scatola_file_get (slot->location);
                scatola_window_slot_set_viewed_file (slot, viewed_file);
                scatola_file_monitor_add (viewed_file, &slot->viewed_file, 0);
                g_signal_connect_object (viewed_file, "changed",
                                         G_CALLBACK (viewed_file_changed_callback), slot, 0);
                scatola_file_unref (viewed_file);
 ...
 */
static void
create_content_view (ScatolaWindowSlot *slot,
                     const char *view_id)
{
    ScatolaWindow *window;
    ScatolaView *view;
     ...
     * for the desktop window.
     */
    if (SCATOLA_IS_DESKTOP_WINDOW (window))
    {
        /* We force the desktop to use a desktop_icon_view. It's simpler
         * to fix it here than trying to make it pick the right view in
         * the first place.
         */
        view_id = SCATOLA_DESKTOP_ICON_VIEW_IID;
    }

    if (slot->content_view != NULL &&
            g_strcmp0 (scatola_view_get_view_id (slot->content_view),
                        view_id) == 0)
    {
    ...
    else
    {
        ...
        view = scatola_view_factory_create (view_id,
                                         SCATOLA_WINDOW_SLOT_INFO (slot));

        eel_accessibility_set_name (view, _("Content View"));
        eel_accessibility_set_description (view, _("View of the current folder"));

        slot->new_content_view = view;
        scatola_window_slot_connect_content_view (slot, slot->new_content_view);
    }
    ...
    {
        selection = scatola_view_get_selection (slot->content_view);
        load_new_location (slot,
                           slot->location,
                           selection,
                           FALSE,
...
static void
load_new_location (ScatolaWindowSlot *slot,
                   GFile *location,
                   GList *selection,
                   gboolean tell_current_content_view,
                   gboolean tell_new_content_view)
{
    ...
    if (slot->content_view != NULL && tell_current_content_view)
    {
        view = slot->content_view;
        uri = g_file_get_uri (location);
        scatola_view_load_location (slot->content_view, uri);
        g_free (uri);
    }
             ...
             slot->new_content_view != slot->content_view) )
    {
        ...
        uri = g_file_get_uri (location);
        scatola_view_load_location (slot->new_content_view, uri);
        g_free (uri);
    }
    if (view != NULL)
    {
        ...
        scatola_view_set_selection (view, selection_copy);
    }
 ...
 */
void
scatola_window_report_load_underway (ScatolaWindow *window,
                                  ScatolaView *view)
{
    ScatolaWindowSlot *slot;
...
static void
scatola_window_emit_location_change (ScatolaWindow *window,
                                  GFile *location)
{
    char *uri;
 ...
 */
void
scatola_window_report_location_change (ScatolaWindow *window)
{
    ScatolaWindowSlot *slot;
    GFile *location;
    ...
    if (location != NULL)
    {
        scatola_window_emit_location_change (window, location);
    }
}

/* This is called when we have decided we can actually change to the new view/location situation. */
static void
location_has_really_changed (ScatolaWindowSlot *slot)
{
    ScatolaWindow *window;
    GtkWidget *widget;
    GFile *location_copy;
    ...
    if (slot->new_content_view != NULL)
    {
        ...
        if (gtk_widget_get_parent (widget) == NULL)
        {
            ...
            scatola_window_slot_set_content_view_widget (slot, slot->new_content_view);
        }
    ...
    {
        if (slot == scatola_window_get_active_slot (window))
        {
            scatola_window_emit_location_change (window, location_copy);
        }
...
static void
slot_add_extension_extra_widgets (ScatolaWindowSlot *slot)
{
    GList *providers, *l;
    GtkWidget *widget;
    char *uri;

    providers = scatola_extensions_get_for_type (SCATOLA_TYPE_LOCATION_WIDGET_PROVIDER);

    uri = g_file_get_uri (slot->location);
    for (l = providers; l != NULL; l = l->next)
    {
        ScatolaLocationWidgetProvider *provider;

        provider = SCATOLA_LOCATION_WIDGET_PROVIDER (l->data);
        widget = scatola_location_widget_provider_get_widget (provider, uri, GTK_WIDGET (slot->pane->window));
        if (widget != NULL)
        {
            scatola_window_slot_add_extra_location_widget (slot, widget);
        }
    }
    g_free (uri);
...
static void
scatola_window_slot_show_x_content_bar (ScatolaWindowSlot *slot, GMount *mount, const char **x_content_types)
{
    unsigned int n;
    ...
    for (n = 0; x_content_types[n] != NULL; n++)
    {
        ...
        {
            GtkWidget *bar;
            bar = scatola_x_content_bar_new (mount, x_content_types[n]);
            gtk_widget_show (bar);
            scatola_window_slot_add_extra_location_widget (slot, bar);
...
static void
scatola_window_slot_show_trash_bar (ScatolaWindowSlot *slot,
                                 ScatolaWindow *window)
{
    GtkWidget *bar;

    bar = scatola_trash_bar_new (window);
    gtk_widget_show (bar);
...
found_content_type_cb (const char **x_content_types, FindMountData *data)
{
    ...
    {
        scatola_window_slot_show_x_content_bar (slot, data->mount, x_content_types);
    }
                ...
                gpointer user_data)
{
    ...
    if (mount != NULL)
    {
        ...
        scatola_autorun_get_x_content_types_for_mount_async (mount,
                (ScatolaAutorunGetContent)found_content_type_cb,
                data->cancellable,
                data);
        return;
    }
...
/* Handle the changes for the ScatolaWindow itself. */
static void
update_for_new_location (ScatolaWindowSlot *slot)
{
    ScatolaWindow *window;
    GFile *new_location;
    ScatolaFile *file;
    ...
    /* Create a ScatolaFile for this location, so we can catch it
     * if it goes away.
     */
    cancel_viewed_file_changed_callback (slot);
    file = scatola_file_get (slot->location);
    scatola_window_slot_set_viewed_file (slot, file);
    slot->viewed_file_seen = !scatola_file_is_not_yet_confirmed (file);
    slot->viewed_file_in_trash = scatola_file_is_in_trash (file);
    scatola_file_monitor_add (file, &slot->viewed_file, 0);
    ...
    if (slot == window->details->active_pane->active_slot)
    {
        ...
        scatola_window_update_up_button (window);
        ...
        /* Load menus from scatola extensions for this location */
        scatola_window_load_extension_menus (window);
    }

    if (location_really_changed)
    {
        scatola_window_slot_remove_extra_location_widgets (slot);

        directory = scatola_directory_get (slot->location);

        scatola_window_slot_update_query_editor (slot);

        if (scatola_directory_is_in_trash (directory))
        {
            scatola_window_slot_show_trash_bar (slot, window);
        }
    ...
    if (slot == slot->pane->active_slot)
    {
        ...
        {
            scatola_navigation_window_load_extension_toolbar_items (SCATOLA_NAVIGATION_WINDOW (window));
        }
    }
}
 ...
 * has been finished */
void
scatola_window_report_load_complete (ScatolaWindow *window,
                                  ScatolaView *view)
{
    ScatolaWindowSlot *slot;
    ...
    if (view == slot->content_view)
    {
        ...
        {
            scatola_view_scroll_to_file (slot->content_view,
...
static void
end_location_change (ScatolaWindowSlot *slot)
{
    ScatolaWindow *window;
    char *uri;
    ...
    scatola_window_slot_set_allow_stop (slot, FALSE);
...
static void
free_location_change (ScatolaWindowSlot *slot)
{
    ScatolaWindow *window;
    ...
    if (slot->new_content_view != NULL)
    {
        ...
        scatola_view_stop_loading (slot->new_content_view);
        window->details->temporarily_ignore_view_signals = FALSE;

        scatola_window_slot_disconnect_content_view (slot, slot->new_content_view);
        g_object_unref (slot->new_content_view);
        slot->new_content_view = NULL;
    }
}
...
static void
cancel_location_change (ScatolaWindowSlot *slot)
{
    GList *selection;
...
void
scatola_window_report_view_failed (ScatolaWindow *window,
                                ScatolaView *view)
{
    ScatolaWindowSlot *slot;
    ...
    if (view == slot->content_view)
    {
        scatola_window_slot_disconnect_content_view (slot, view);
        scatola_window_slot_set_content_view_widget (slot, NULL);
    ...
    if (fallback_load_location != NULL)
    {
        ...
        begin_location_change (slot, fallback_load_location, NULL, NULL,
                               SCATOLA_LOCATION_CHANGE_FALLBACK, 0, NULL, NULL, NULL);
...
static void
display_view_selection_failure (ScatolaWindow *window, ScatolaFile *file,
                                GFile *location, GError *error)
{
    ...
    if (error == NULL)
    {
        if (scatola_file_is_directory (file))
        {
            ...
            detail_message = g_strdup
                             (_("Scatola has no installed viewer capable of displaying the folder."));
        }
        else
        {
            error_message = g_strdup_printf
    ...
    else if (error->domain == G_IO_ERROR)
    {
        ...
        case G_IO_ERROR_NOT_SUPPORTED:
            ...
            {
                detail_message = g_strdup_printf (_("Scatola cannot handle \"%s\" locations."),
                                                  scheme_string);
            }
            else
            {
                detail_message = g_strdup (_("Scatola cannot handle this kind of location."));
            }
        ...
        case G_IO_ERROR_HOST_NOT_FOUND:
             ...
             * But this case is also hit for legitigde2 web addresses when
...
void
scatola_window_slot_stop_loading (ScatolaWindowSlot *slot)
{
    ScatolaWindow *window;
    ...
    scatola_view_stop_loading (slot->content_view);
...
void
scatola_window_slot_set_content_view (ScatolaWindowSlot *slot,
                                   const char *id)
{
    ScatolaWindow *window;
    ScatolaFile *file;
    ...
    if (scatola_window_slot_content_view_matches_iid (slot, id))
    {
        return;
    }
    ...
    (file, SCATOLA_METADATA_KEY_DEFAULT_VIEW, NULL, id);
    scatola_file_unref (file);

    scatola_window_slot_set_allow_stop (slot, TRUE);

    if (scatola_view_get_selection_count (slot->content_view) == 0)
    {
        /* If there is no selection, queue a scroll to the same icon that
         * is currently visible */
        slot->pending_scroll_to = scatola_view_get_first_visible_file (slot->content_view);
    }
    slot->location_change_type = SCATOLA_LOCATION_CHANGE_RELOAD;

    create_content_view (slot, id);
}

void
scatola_window_manage_views_close_slot (ScatolaWindowPane *pane,
                                     ScatolaWindowSlot *slot)
{
    if (slot->content_view != NULL)
    {
...
void
scatola_navigation_window_back_or_forward (ScatolaNavigationWindow *window,
                                        gboolean back, guint distance, gboolean new_tab)
{
    ...
    navigation_slot = (ScatolaNavigationWindowSlot *) slot;
    ...
    else
    {
        ...
        old_location = scatola_window_slot_get_location (slot);
        scroll_pos = scatola_bookmark_get_scroll_pos (bookmark);
        begin_location_change
        (slot,
         location, old_location, NULL,
         back ? SCATOLA_LOCATION_CHANGE_BACK : SCATOLA_LOCATION_CHANGE_FORWARD,
         distance,
         scroll_pos,
         NULL, NULL);
...
void
scatola_window_slot_reload (ScatolaWindowSlot *slot)
{
    GFile *location;
    char *current_pos;
    GList *selection;
    ...
    (slot, location, location, selection,
     SCATOLA_LOCATION_CHANGE_RELOAD, 0, current_pos,
     NULL, NULL);
    g_free (current_pos);
    g_object_unref (location);
    g_list_free_full (selection, g_object_unref);
...
void
scatola_window_reload (ScatolaWindow *window)
{
    g_assert (SCATOLA_IS_WINDOW (window));

    scatola_window_slot_reload (window->details->active_pane->active_slot);
}
