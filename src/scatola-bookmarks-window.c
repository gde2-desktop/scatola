...
/* scatola-bookmarks-window.c - implementation of bookmark-editing window.
 */
...
static ScatolaBookmarkList *bookmarks = NULL;
static GtkTreeView	    *bookmark_list_widget = NULL; /* awkward name to distinguish from ScatolaBookmarkList */
...
static ScatolaApplication  *application;
static gboolean              parent_is_browser_window;

/* forward declarations */
static guint    get_selected_row                            (void);
static gboolean get_selection_exists                        (void);
static void     name_or_uri_field_activate                  (ScatolaEntry        *entry);
static void     scatola_bookmarks_window_restore_geometry  (GtkWidget            *window);
static void     on_bookmark_list_changed                    (ScatolaBookmarkList *list,
        gpointer              user_data);
...
/* We store a pointer to the bookmark in a column so when an item is moved
   ...
   scatola_bookmark_list_insert_item() makes a copy of the bookmark, so we
...
static void
scatola_bookmarks_window_response_callback (GtkDialog *dialog,
        int response_id,
        gpointer callback_data)
{
    if (response_id == GTK_RESPONSE_HELP)
    {
        ...
        gtk_show_uri (gtk_window_get_screen (GTK_WINDOW (dialog)),
                      "help:gde2-user-guide/gosscatola-36",
...
bookmarks_set_empty (gboolean empty)
{
    ...
    else
    {
        ...
        if (scatola_bookmark_list_length (bookmarks) > 0 &&
 ...
 * @list: The ScatolaBookmarkList that this window will edit.
 *
 * Return value: A pointer to the new window.
 **/
GtkWindow *
create_bookmarks_window (ScatolaBookmarkList *list, ScatolaWindow *window_source)
{
    GtkWidget         *window;
    ...
    if (!gtk_builder_add_from_file (builder,
                                    UIDIR  "/scatola-bookmarks-window.ui",
                                    NULL))
    {
        return NULL;
    }
    ...
    if (SCATOLA_IS_NAVIGATION_WINDOW (window_source))
    {
    ...
    gtk_window_set_wmclass (GTK_WINDOW (window), "bookmarks", "Scatola");
    scatola_bookmarks_window_restore_geometry (window);
    ...
    g_signal_connect (window, "response",
                      G_CALLBACK (scatola_bookmarks_window_response_callback), NULL);
...
void
edit_bookmarks_dialog_set_signals (ScatolaWindow *window)
{
...
get_selected_bookmark (void)
{
    ...
    if (scatola_bookmark_list_length (bookmarks) < 1)
        return NULL;

    return scatola_bookmark_list_item_at(bookmarks, get_selected_row ());
}
...
static void
scatola_bookmarks_window_restore_geometry (GtkWidget *window)
{
    ...
    window_geometry = scatola_bookmark_list_get_window_geometry (bookmarks);
...
/**
 * scatola_bookmarks_window_save_geometry:
 *
 * Save window size & position to disk.
 * @window: The bookmarks window whose geometry should be saved.
 **/
void
scatola_bookmarks_window_save_geometry (GtkWindow *window)
{
    g_return_if_fail (GTK_IS_WINDOW (window));
    g_return_if_fail (SCATOLA_IS_BOOKMARK_LIST (bookmarks));
    ...
    if (gtk_widget_get_visible (GTK_WIDGET (window)))
    {
        ...
        scatola_bookmark_list_set_window_geometry (bookmarks, geometry_string);
        g_free (geometry_string);
    }
}

static void
on_bookmark_list_changed (ScatolaBookmarkList *bookmarks, gpointer data)
{
    g_return_if_fail (SCATOLA_IS_BOOKMARK_LIST (bookmarks));
...
open_selected_bookmark (gpointer user_data, GdkScreen *screen)
{
    ...
    location = scatola_bookmark_get_location (selected);
    if (location == NULL)
    {
        return;
    }

    if (SCATOLA_IS_NAVIGATION_WINDOW (user_data))
    {
        window = user_data;
    }
    else if (SCATOLA_IS_SPATIAL_WINDOW (user_data))
    {
        ...
        if (parent_is_browser_window || g_settings_get_boolean (scatola_preferences, SCATOLA_PREFERENCES_ALWAYS_USE_BROWSER)) {
...
on_row_changed (GtkListStore *store,
                ...
                gpointer user_data)
{
    ScatolaBookmark *bookmark = NULL, *bookmark_in_list;
    gint *indices, row;
    ...
    if (row < (gint) scatola_bookmark_list_length (bookmarks))
    {
        bookmark_in_list = scatola_bookmark_list_item_at (bookmarks,
                           row);
    ...
    if (insert_bookmark)
    {
        g_signal_handler_block (bookmarks,
                                bookmark_list_changed_signal_id);
        scatola_bookmark_list_insert_item (bookmarks, bookmark, row);
        ...
        bookmark = scatola_bookmark_list_item_at (bookmarks, row);
                ...
                gpointer user_data)
{
    ...
    scatola_bookmark_list_delete_item_at (bookmarks, row);
                      ...
                      gpointer user_data)
{
    ...
    if (selected)
    {
        name = scatola_bookmark_get_name (selected);
        location = scatola_bookmark_get_location (selected);
    ...
    scatola_entry_set_text (SCATOLA_ENTRY (name_field),
                         name ? name : "");
    ...
    scatola_entry_set_text (SCATOLA_ENTRY (uri_field),
...
update_bookmark_from_text (void)
{
    if (text_changed)
    {
        ScatolaBookmark *bookmark, *bookmark_in_list;
        char *name;
        GdkPixbuf *pixbuf;
        guint selected_row;
        GtkTreeIter iter;
        ...
        bookmark = scatola_bookmark_new (location, gtk_entry_get_text (GTK_ENTRY (name_field)),
        ...
        g_signal_handler_block (bookmarks,
                                bookmark_list_changed_signal_id);
        scatola_bookmark_list_delete_item_at (bookmarks, selected_row);
        scatola_bookmark_list_insert_item (bookmarks, bookmark, selected_row);
        g_signal_handler_unblock (bookmarks,
                                  bookmark_list_changed_signal_id);
        ...
        name = scatola_bookmark_get_name (bookmark_in_list);

        pixbuf = scatola_bookmark_get_pixbuf (bookmark_in_list, GTK_ICON_SIZE_MENU);
...
static void
name_or_uri_field_activate (ScatolaEntry *entry)
{
    g_assert (SCATOLA_IS_ENTRY (entry));

    update_bookmark_from_text ();
    scatola_entry_select_all_at_idle (entry);
}
...
restore_geometry (gpointer data)
{
    ...
    scatola_bookmarks_window_restore_geometry (GTK_WIDGET (data));
...
on_window_hide_event (GtkWidget *widget,
                      gpointer user_data)
{
    scatola_bookmarks_window_save_geometry (GTK_WINDOW (widget));
...
repopulate (void)
{
    ...
    for (index = 0; index < scatola_bookmark_list_length (bookmarks); ++index)
    {
        ...
        bookmark_name = scatola_bookmark_get_name (bookmark);
        bookmark_pixbuf = scatola_bookmark_get_pixbuf (bookmark, GTK_ICON_SIZE_MENU);
