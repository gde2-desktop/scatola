                                       ...
                                       GtkEntry *entry)
{
    ...
    file = scatola_file_get_by_uri (uris[0]);
    g_strfreev (uris);
                        ...
                        GList *files)
{
    ...
    info = SCATOLA_FILE_INFO (files->data);

    uri = scatola_file_info_get_uri (info);
...
fm_ditem_page_should_show (GList *files)
{
    ...
    if (!scatola_file_info_is_mime_type (info, "application/x-desktop"))
    {
        return FALSE;
    }
