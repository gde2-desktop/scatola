...
#include "libscatola-private/scatola-open-with-dialog.h"
...
#include <gio/gio.h>
#include <libscatola-private/scatola-recent.h>
#include <libscatola-extension/scatola-menu-provider.h>
#include <libscatola-private/scatola-clipboard.h>
#include <libscatola-private/scatola-clipboard-monitor.h>
#include <libscatola-private/scatola-debug-log.h>
#include <libscatola-private/scatola-desktop-icon-file.h>
#include <libscatola-private/scatola-desktop-directory.h>
#include <libscatola-private/scatola-extensions.h>
#include <libscatola-private/scatola-search-directory.h>
#include <libscatola-private/scatola-directory-background.h>
...
#include <libscatola-private/scatola-file-changes-queue.h>
...
#include <libscatola-private/scatola-file-private.h> /* for scatola_file_get_existing_by_uri */
#include <libscatola-private/scatola-global-preferences.h>
#include <libscatola-private/scatola-link.h>
...
#include <libscatola-private/scatola-autorun.h>
...
struct FMDirectoryViewDetails
{
	...
	ScatolaDirectory *model;
	ScatolaFile *directory_as_file;
	ScatolaFile *location_popup_directory_as_file;
...
static void     fm_directory_view_load_location                (ScatolaView         *scatola_view,
								const char           *location);
static void     fm_directory_view_stop_loading                 (ScatolaView         *scatola_view);
...
static void     fm_directory_view_drop_proxy_received_netscape_url (FMDirectoryView *view,
								    ...
								    const char *target_uri,
								    GdkDragAction action);
static void     clipboard_changed_callback                     (ScatolaClipboardMonitor *monitor,
								FMDirectoryView      *view);
...
static void     metadata_for_directory_as_file_ready_callback  (ScatolaFile         *file,
								gpointer              callback_data);
static void     metadata_for_files_in_directory_ready_callback (ScatolaDirectory    *directory,
								GList                *files,
								gpointer              callback_data);
static void     fm_directory_view_trash_state_changed_callback (ScatolaTrashMonitor *trash,
								gboolean              state,
								gpointer              callback_data);
...
static void undo_redo_menu_update_callback (ScatolaUndoStackManager* manager, gpointer arg1, gpointer data);
			           ...
			           FMDirectoryView *directory_view)
{
	...
	result->files = scatola_file_list_copy (files);
...
application_launch_parameters_free (ApplicationLaunchParameters *parameters)
{
	...
	scatola_file_list_free (parameters->files);
...
file_and_directory_list_to_files (GList *fad_list)
{
	...
	for (l = fad_list; l != NULL; l = l->next) {
		fad = l->data;
		res = g_list_prepend (res, scatola_file_ref (fad->file));
	}
...
static GList *
file_and_directory_list_from_files (ScatolaDirectory *directory, GList *files)
{
	GList *res, *l;
	FileAndDirectory *fad;
	...
	for (l = files; l != NULL; l = l->next) {
		...
		fad->directory = scatola_directory_ref (directory);
		fad->file = scatola_file_ref (l->data);
...
{
	scatola_directory_unref (fad->directory);
	scatola_file_unref (fad->file);
	g_free (fad);
}
...
script_launch_parameters_new (ScatolaFile *file,
			      FMDirectoryView *directory_view)
{
...
{
	g_object_unref (parameters->directory_view);
	scatola_file_unref (parameters->file);
	g_free (parameters);
}

static CreateTemplateParameters *
create_template_parameters_new (ScatolaFile *file,
				FMDirectoryView *directory_view)
{
...
ScatolaWindowInfo *
fm_directory_view_get_scatola_window (FMDirectoryView  *view)
{
...
fm_directory_view_get_scatola_window_slot (FMDirectoryView  *view)
{
...
get_view_directory (FMDirectoryView *view)
{
	...
	uri = scatola_directory_get_uri (view->details->model);
	if (eel_uri_is_desktop (uri)) {
		g_free (uri);
		uri = scatola_get_desktop_directory_uri ();
				  ...
				  gboolean confirm_multiple)
{
	...
	scatola_mime_activate_files (fm_directory_view_get_containing_window (view),
				      view->details->slot,
				      files,
				      path,
				      mode,
				 ...
				 ScatolaWindowOpenFlags flags)
{
	...
	path = get_view_directory (view);
	scatola_mime_activate_file (fm_directory_view_get_containing_window (view),
				     view->details->slot,
				     file,
				     path,
				     mode,
		      ...
		      gpointer callback_data)
{
	...
	fm_directory_view_activate_files (view,
					  ...
					  0,
					  TRUE);
	scatola_file_list_free (selection);
}
				   ...
				   gpointer callback_data)
{
	...
	fm_directory_view_activate_files (view,
					  selection,
					  SCATOLA_WINDOW_OPEN_ACCORDING_TO_MODE,
					  SCATOLA_WINDOW_OPEN_FLAG_CLOSE_BEHIND,
					  TRUE);
	scatola_file_list_free (selection);
}
	       ...
	       ScatolaWindowOpenFlags flags)
{
	...
	scatola_window_slot_info_open_location (directory_view->details->slot,
						 location, mode, flags, NULL);
	g_object_unref (location);
}

static void
application_selected_cb (ScatolaOpenWithDialog *dialog,
			 GAppInfo *app,
			 gpointer user_data)
{
	...
	files.prev = NULL;
	files.data = file;
	scatola_launch_application (app, &files, parent_window);
}
		...
		ScatolaFile *file)
{
	...
	dialog = scatola_open_with_dialog_new (uri, mime_type, NULL);
...
open_with_other_program (FMDirectoryView *view)
{
	...
	if (selection_contains_one_item_in_menu_callback (view, selection)) {
		choose_program (view, SCATOLA_FILE (selection->data));
	}
...
delete_selected_files (FMDirectoryView *view)
{
	...
	for (node = selection; node != NULL; node = node->next) {
		locations = g_list_prepend (locations,
					    scatola_file_get_location ((ScatolaFile *) node->data));
	}
	locations = g_list_reverse (locations);

	scatola_file_operations_delete (locations, fm_directory_view_get_containing_window (view), NULL, NULL);
				    ...
				    gpointer callback_data)
{
	...
	selection = fm_directory_view_get_selection_for_file_transfer (view);
	scatola_restore_files_from_trash (selection,
					   fm_directory_view_get_containing_window (view));
...
pattern_select_response_cb (GtkWidget *dialog, int response, gpointer user_data)
{
	...
	case GTK_RESPONSE_OK :
		...
		selection = scatola_directory_match_pattern (directory,
	...
	case GTK_RESPONSE_HELP :
		...
		gtk_show_uri (gtk_window_get_screen (GTK_WINDOW (dialog)),
			      "help:gde2-user-guide/scatola-select-pattern",
...
static void
hidden_files_mode_changed (ScatolaWindow *window,
			   gpointer callback_data)
{
	FMDirectoryView *directory_view;
...
action_save_search_callback (GtkAction *action,
			     gpointer callback_data)
{
	ScatolaSearchDirectory *search;
	FMDirectoryView	*directory_view;

        directory_view = FM_DIRECTORY_VIEW (callback_data);

	if (directory_view->details->model &&
	    SCATOLA_IS_SEARCH_DIRECTORY (directory_view->details->model)) {
		search = SCATOLA_SEARCH_DIRECTORY (directory_view->details->model);
		scatola_search_directory_save_search (search);
				...
				gpointer callback_data)
{
	...
	if (directory_view->details->model &&
		...
		if (gtk_dialog_run (GTK_DIALOG (dialog)) == GTK_RESPONSE_OK) {
			...
			if (g_str_has_suffix (entry_text, SCATOLA_SAVED_SEARCH_EXTENSION)) {
				filename_utf8 = g_strdup (entry_text);
			} else {
				filename_utf8 = g_strconcat (entry_text, SCATOLA_SAVED_SEARCH_EXTENSION, NULL);
			}
			...
			scatola_search_directory_save_to_file (search, uri);
			location = g_file_new_for_uri (uri);
			scatola_file_changes_queue_file_added (location);
			g_object_unref (location);
			scatola_file_changes_consume_changes (TRUE);
			g_free (uri);
		}
			     ...
			     gpointer callback_data)
{
	...
	scatola_file_operations_empty_trash (GTK_WIDGET (callback_data));
}
			      ...
			      gpointer callback_data)
{
	...
	scatola_launch_application_from_command (gtk_widget_get_screen (GTK_WIDGET (view)),
						  "gde2-desktop-item-edit",
						  "gde2-desktop-item-edit",
						  FALSE,
			    ...
			    gpointer callback_data)
{
	...
	if (g_list_length (selection) == 0) {
		if (view->details->directory_as_file != NULL) {
			files = g_list_append (NULL, scatola_file_ref (view->details->directory_as_file));
				     ...
				     gpointer   callback_data)
{
	...
	view = FM_DIRECTORY_VIEW (callback_data);
	g_assert (SCATOLA_IS_FILE (view->details->location_popup_directory_as_file));

	files = g_list_append (NULL, scatola_file_ref (view->details->location_popup_directory_as_file));
...
all_files_in_trash (GList *files)
{
	...
	for (node = files; node != NULL; node = node->next) {
		if (!scatola_file_is_in_trash (SCATOLA_FILE (node->data))) {
			return FALSE;
		}
	}
...
we_are_in_vfolder_desktop_dir (FMDirectoryView *view)
{
	...
	file = scatola_directory_get_corresponding_file (view->details->model);
	mime_type = scatola_file_get_mime_type (file);
	scatola_file_unref (file);
...
sort_directories_first_changed_callback (gpointer callback_data)
{
	...
	preference_value =
		g_settings_get_boolean (scatola_preferences, SCATOLA_PREFERENCES_SORT_DIRECTORIES_FIRST);
...
static void set_up_scripts_directory_global(void)
{
	...
	const char* override = g_getenv ("GDE222_USER_DIR"); //TODO: remove?

	if (override)
	{
		scripts_directory_path = g_build_filename(override, "scatola", "scripts", NULL);
	}
	else
	{
		scripts_directory_path = g_build_filename(g_get_user_config_dir(), "scatola", "scripts", NULL);
	}
	...
	if (g_mkdir_with_parents(scripts_directory_path, 0755) == 0)
	{
		...
		{
			char* nautilus_syslink = g_build_filename(g_get_user_config_dir(), "scatola", "scripts", "nautilus", NULL);
...
static void
scripts_added_or_changed_callback (ScatolaDirectory *directory,
				   GList *files,
				   gpointer callback_data)
{
	FMDirectoryView *view;
...
static void
templates_added_or_changed_callback (ScatolaDirectory *directory,
				     GList *files,
				     gpointer callback_data)
{
	FMDirectoryView *view;
				 ...
				 GCallback changed_callback)
{
	...
	if (g_list_find (*directory_list, directory) == NULL) {
		scatola_directory_ref (directory);
		...
		scatola_directory_file_monitor_add (directory, directory_list,
						     FALSE, attributes,
						     (ScatolaDirectoryCallback)changed_callback, view);
				      ...
				      GCallback changed_callback)
{
	*directory_list = g_list_remove	(*directory_list, directory);
	...
	scatola_directory_file_monitor_remove (directory, directory_list);
...
static void
slot_active (ScatolaWindowSlot *slot,
	     FMDirectoryView *view)
{
...
static void
slot_inactive (ScatolaWindowSlot *slot,
	       FMDirectoryView *view)
{
	g_assert (view->details->active ||
...
static void
fm_directory_view_grab_focus (ScatolaView *view)
{
...
static void
view_iface_update_menus (ScatolaView *view)
{
	fm_directory_view_update_menus (FM_DIRECTORY_VIEW (view));
}

static GtkWidget *
fm_directory_view_get_widget (ScatolaView *view)
{
	return GTK_WIDGET (view);
}

static int
fm_directory_view_get_selection_count (ScatolaView *view)
{
...
static GList *
fm_directory_view_get_selection_locations (ScatolaView *view)
{
	GList *files;
	GList *locations;
	GFile *location;
	...
	locations = NULL;
	for (l = files; l != NULL; l = l->next) {
		location = scatola_file_get_location (SCATOLA_FILE (l->data));
...
file_list_from_location_list (const GList *uri_list)
{
	...
	for (node = uri_list; node != NULL; node = node->next) {
		file_list = g_list_prepend
			(file_list,
			 scatola_file_get (node->data));
	}
	return g_list_reverse (file_list);
}

static void
fm_directory_view_set_selection_locations (ScatolaView *scatola_view,
					   GList *selection_locations)
{
	GList *selection;
	FMDirectoryView *view;

	view = FM_DIRECTORY_VIEW (scatola_view);
...
void
fm_directory_view_init_view_iface (ScatolaViewIface *iface)
{
...
{
	ScatolaDirectory *scripts_directory;
	ScatolaDirectory *templates_directory;
	char *templates_uri;
	...
	scripts_directory = scatola_directory_get_by_uri (scripts_directory_uri);
	add_directory_to_scripts_directory_list (view, scripts_directory);
	scatola_directory_unref (scripts_directory);

	if (scatola_should_use_templates_directory ()) {
		templates_uri = scatola_get_templates_directory_uri ();
		templates_directory = scatola_directory_get_by_uri (templates_uri);
		g_free (templates_uri);
		add_directory_to_templates_directory_list (view, templates_directory);
		scatola_directory_unref (templates_directory);
	}
	...
	g_signal_connect_swapped (scatola_preferences,
							  ...
							  view);
	g_signal_connect_swapped (scatola_preferences,
							  "changed::" SCATOLA_PREFERENCES_SORT_DIRECTORIES_FIRST, 
							  G_CALLBACK(sort_directories_first_changed_callback),
							  view);
	g_signal_connect_swapped (gde2_lockdown_preferences,
							  "changed::" SCATOLA_PREFERENCES_LOCKDOWN_COMMAND_LINE,
	...
	ScatolaUndoStackManager* manager = scatola_undostack_manager_instance ();

	g_signal_connect_object (G_OBJECT(manager), "request-menu-update",
		   G_CALLBACK(undo_redo_menu_update_callback), view, 0);

	scatola_undostack_manager_request_menu_update (scatola_undostack_manager_instance());
}
...
real_unmerge_menus (FMDirectoryView *view)
{
	...
	ui_manager = scatola_window_info_get_ui_manager (view->details->window);

	scatola_ui_unmerge_ui (ui_manager,
...
#endif
{
	...
	if (view->details->model) {
		scatola_directory_unref (view->details->model);
		view->details->model = NULL;
	}

	if (view->details->directory_as_file) {
		scatola_file_unref (view->details->directory_as_file);
...
fm_directory_view_finalize (GObject *object)
{
	...
	g_signal_handlers_disconnect_by_func (gde2_lockdown_preferences,
        				      schedule_update_menus, view);
...
fm_directory_view_display_selection_info (FMDirectoryView *view)
{
	...
	for (p = selection; p != NULL; p = p->next) {
		...
		if (scatola_file_is_directory (file)) {
			folder_count++;
			if (scatola_file_get_directory_item_count (file, &file_item_count, NULL)) {
			...
			non_folder_count++;
			if (!scatola_file_can_get_size (file)) {
				non_folder_size_known = TRUE;
				non_folder_size += scatola_file_get_size (file);
			}
		}

		if (first_item_name == NULL) {
			first_item_name = scatola_file_get_display_name (file);
		}
	}
	...
	free_space_str = scatola_file_get_volume_free_space (view->details->directory_as_file);
	...
	scatola_window_slot_info_set_status (view->details->slot,
					      status_string);
...
{
	scatola_window_info_report_selection_changed (view->details->window);
...
static void
fm_directory_view_load_location (ScatolaView *scatola_view,
				 const char *location)
{
	ScatolaDirectory *directory;
	FMDirectoryView *directory_view;

	directory_view = FM_DIRECTORY_VIEW (scatola_view);
	...
	directory = scatola_directory_get_by_uri (location);
	load_directory (directory_view, directory);
	scatola_directory_unref (directory);
}

static void
fm_directory_view_stop_loading (ScatolaView *scatola_view)
{
	fm_directory_view_stop (FM_DIRECTORY_VIEW (scatola_view));
}
	      ...
	      gboolean all_files_seen)
{
	 ...
	 * is no ScatolaWindowInfo any more.
	 */
	if (view->details->window != NULL) {
		if (all_files_seen) {
			scatola_window_info_report_load_complete (view->details->window, SCATOLA_VIEW (view));
		}
...
debuting_files_data_free (DebutingFilesData *data)
{
	...
	scatola_file_list_free (data->added_files);
	g_free (data);
}
...
debuting_files_add_file_callback (FMDirectoryView *view,
				  ScatolaFile *new_file,
				  ScatolaDirectory *directory,
				  DebutingFilesData *data)
{
	GFile *location;

	location = scatola_file_get_location (new_file);

	if (g_hash_table_remove (data->debuting_files, location)) {
		scatola_file_ref (new_file);
...
/* This needs to be called prior to scatola_file_operations_copy_move.
...
copy_move_done_partition_func (gpointer data, gpointer callback_data)
{
	...
	location = scatola_file_get_location (SCATOLA_FILE (data));
...
static gboolean
real_file_still_belongs (FMDirectoryView *view, ScatolaFile *file, ScatolaDirectory *directory)
{
	...
	return scatola_directory_contains_file (directory, file);
}

static gboolean
still_should_show_file (FMDirectoryView *view, ScatolaFile *file, ScatolaDirectory *directory)
{
...
static gboolean
ready_to_load (ScatolaFile *file)
{
	return scatola_file_check_if_ready (file,
					     SCATOLA_FILE_ATTRIBUTES_FOR_ICON);
}
...
display_pending_files (FMDirectoryView *view)
{
	...
	if (view->details->model != NULL
	    && scatola_directory_are_all_files_seen (view->details->model)
		     ...
		     GList **pending_list)
{
	...
	*pending_list = g_list_concat (file_and_directory_list_from_files (directory, files),
	...
	if (! view->details->loading || scatola_directory_are_all_files_seen (directory)) {
...
static void
files_added_callback (ScatolaDirectory *directory,
		      GList *files,
		      gpointer callback_data)
{
	FMDirectoryView *view;
	...
	scatola_debug_log_with_file_list (FALSE, SCATOLA_DEBUG_LOG_DOMAIN_ASYNC, files,
...
static void
files_changed_callback (ScatolaDirectory *directory,
			GList *files,
			gpointer callback_data)
{
	FMDirectoryView *view;
...
static void
done_loading_callback (ScatolaDirectory *directory,
		       gpointer callback_data)
{
	FMDirectoryView *view;
...
static void
load_error_callback (ScatolaDirectory *directory,
		     GError *error,
		     gpointer callback_data)
{
	FMDirectoryView *view;
...
real_load_error (FMDirectoryView *view, GError *error)
{
	 ...
	 * point of view, not from the ScatolaDirectory point of view).
				    ...
				    ScatolaDirectory*directory)
{
	...
	scatola_directory_file_monitor_add (directory,
					     &view->details->model,
				       ...
				       ScatolaDirectory*directory)
{
	...
	scatola_directory_file_monitor_remove (directory, &view->details->model);
...
{
	g_return_val_if_fail (FM_IS_DIRECTORY_VIEW (view), SCATOLA_ZOOM_LEVEL_STANDARD);
 ...
 **/
static gboolean
fm_directory_view_can_rename_file (FMDirectoryView *view, ScatolaFile *file)
{
	return EEL_CALL_METHOD_WITH_RETURN_VALUE
		(FM_DIRECTORY_VIEW_CLASS, view,
 ...
 *
 * Get a list of ScatolaFile pointers that represents the
 ...
 *
 * Return value: GList of ScatolaFile pointers representing the selection.
 *
 **/
GList *
...
fm_directory_view_get_ui_manager (FMDirectoryView  *view)
{
	...
	return scatola_window_info_get_ui_manager (view->details->window);
}
 ...
 *
 * Return value: ScatolaDirectory for this view.
 *
 **/
ScatolaDirectory *
...
prepend_uri_one (gpointer data, gpointer callback_data)
{
	...
	g_assert (SCATOLA_IS_FILE (data));
	g_assert (callback_data != NULL);

	result = (GList **) callback_data;
	file = (ScatolaFile *) data;
	*result = g_list_prepend (*result, scatola_file_get_uri (file));
}
					  ...
					  GArray *relative_item_points)
{
	...
	scatola_file_operations_copy_move (uris, relative_item_points, dir_uri, GDK_ACTION_LINK,
				       ...
				       GArray *relative_item_points)
{
	...
	offset_drop_points (relative_item_points,
        ...
        copy_move_done_data = pre_copy_move (view);
	scatola_file_operations_copy_move (uris, relative_item_points, NULL, GDK_ACTION_COPY,
		GTK_WIDGET (view), copy_move_done_callback, copy_move_done_data);
	g_list_free_full (uris, g_free);
}
 ...
 *	 SCATOLA_DESKTOP_LINK_TRASH, SCATOLA_DESKTOP_LINK_HOME, SCATOLA_DESKTOP_LINK_MOUNT
 */
...
special_link_in_selection (FMDirectoryView *view)
{
	...
	for (node = selection; node != NULL; node = node->next) {
		file = SCATOLA_FILE (node->data);

		saw_link = SCATOLA_IS_DESKTOP_ICON_FILE (file);
...
desktop_or_home_dir_in_selection (FMDirectoryView *view)
{
	...
	for (node = selection; node != NULL; node = node->next) {
		...
		saw_desktop_or_home_dir =
			...
			|| scatola_file_is_desktop_directory (file);
		       ...
		       FMDirectoryView *view)
{
	...
	scatola_file_operations_trash_or_delete (locations,
						  parent_window,
						  (ScatolaDeleteCallback) trash_or_delete_done_cb,
						  view);
	g_list_free_full (locations, g_object_unref);
}

static gboolean
can_rename_file (FMDirectoryView *view, ScatolaFile *file)
{
	return scatola_file_can_rename (file);
}
...
delayed_rename_file_hack_removed (RenameData *data)
{
	...
	scatola_file_unref (data->new_file);
	g_free (data);
}


static void
rename_file (FMDirectoryView *view, ScatolaFile *new_file)
{
	RenameData *data;
	...
	*/
	if (FM_IS_LIST_VIEW (view)) {
		...
		data->new_file = scatola_file_ref (new_file);
		if (view->details->delayed_rename_file_id != 0) {
			g_source_remove (view->details->delayed_rename_file_id);
		}
...
static void
reveal_newly_added_folder (FMDirectoryView *view, ScatolaFile *new_file,
			   ScatolaDirectory *directory, GFile *target_location)
{
	GFile *location;
...
static void
track_newly_added_locations (FMDirectoryView *view, ScatolaFile *new_file,
			     ScatolaDirectory *directory, gpointer user_data)
{
	NewFolderData *data;

	data = user_data;

	g_hash_table_insert (data->added_locations, scatola_file_get_location (new_file), NULL);
}
...
new_folder_done (GFile *new_folder, gpointer user_data)
{
	...
	file = scatola_file_get (new_folder);
	scatola_file_set_metadata
		(file, SCATOLA_METADATA_KEY_SCREEN,
		 NULL,
		 screen_string);
...
fm_directory_view_new_folder (FMDirectoryView *directory_view)
{
	...
	scatola_file_operations_new_folder (GTK_WIDGET (directory_view),
					     pos, parent_uri,
						  ...
						  GdkPoint *pos)
{
	...
	scatola_file_operations_new_file (GTK_WIDGET (directory_view),
			    ...
			    ScatolaFile *source)
{
	...
	g_return_if_fail (scatola_file_is_local (source));

	pos = context_menu_to_file_operation_position (directory_view);

	data = setup_new_folder_data (directory_view);

	source_uri = scatola_file_get_uri (source);

	scatola_file_operations_new_file_from_template (GTK_WIDGET (directory_view),
							 pos,
							 parent_uri != NULL ? parent_uri : container_uri,
							 NULL,
							 source_uri,
				   ...
				   const gboolean submenu)
{
	...
	gtk_ui_manager_add_ui (scatola_window_info_get_ui_manager (view->details->window),
			       view->details->open_with_merge_id,
			       menu_placeholder,
			       action_name,
			       action_name,
	...
	menuitem = gtk_ui_manager_get_widget (
			scatola_window_info_get_ui_manager (view->details->window),
			path);
				...
				const char *popup_placeholder)
{
	...
	uri = scatola_file_get_parent_uri (file);
...
static void
add_x_content_apps (FMDirectoryView *view, ScatolaFile *file, GList **applications)
{
	GMount *mount;
	...
	x_content_types = scatola_autorun_get_cached_x_content_types_for_mount (mount);
	if (x_content_types != NULL) {
		...
		scatola_autorun_get_x_content_types_for_mount_async (mount,
...
reset_open_with_menu (FMDirectoryView *view, GList *selection)
{
	...
	scatola_ui_prepare_merge_ui (ui_manager,
				      "OpenWithGroup",
				      &view->details->open_with_merge_id,
				      &view->details->open_with_action_group);
	...
	default_app = NULL;
	if (filter_default) {
		default_app = scatola_mime_get_default_application_for_files (selection);
	}

	applications = NULL;
	if (other_applications_visible) {
		applications = scatola_mime_get_applications_for_files (selection);
	}

	if (g_list_length (selection) == 1) {
		add_x_content_apps (view, SCATOLA_FILE (selection->data), &applications);
	}
			      ...
			      GList *selection)
{
	...
	for (l = providers; l != NULL; l = l->next) {
		...
		file_items = scatola_menu_provider_get_file_items (provider,
								    window,
								    selection);
...
extension_action_callback_data_free (ExtensionActionCallbackData *data)
{
	...
	scatola_file_list_free (data->selection);
...
search_in_menu_items (GList* items, const char *item_name)
{
	...
	for (list = items; list != NULL; list = list->next) {
		...
		if (menu != NULL) {
			...
			submenus = scatola_menu_get_items (menu);
			ret = search_in_menu_items (submenus, item_name);
			scatola_menu_item_list_free (submenus);
			g_object_unref (menu);
			if (ret) {
			    return TRUE;
			}
			   ...
			   gpointer callback_data)
{
	...
	if (is_valid) {
		scatola_menu_item_activate (data->item);
	}
}
...
get_menu_icon (const char *icon_name)
{
	...
	size = scatola_get_icon_size_for_stock_size (GTK_ICON_SIZE_MENU);

	if (g_path_is_absolute (icon_name)) {
		info = scatola_icon_info_lookup_from_path (icon_name, size);
	} else {
		info = scatola_icon_info_lookup_from_name (icon_name, size);
	}
	pixbuf = scatola_icon_info_get_pixbuf_nodefault_at_size (info, size);
	g_object_unref (info);
...
static GdkPixbuf *
get_menu_icon_for_file (ScatolaFile *file)
{
	ScatolaIconInfo *info;
	GdkPixbuf *pixbuf;
	int size;
	...
	info = scatola_file_get_icon (file, size, 0);
	pixbuf = scatola_icon_info_get_pixbuf_nodefault_at_size (info, size);
	g_object_unref (info);
				...
				GList *files)
{
	...
	data->view = view;
	data->selection = scatola_file_list_copy (files);
	data->action = action;
			  ...
			  const char *subdirectory)
{
	...
	for (l = menu_items; l; l = l->next) {
		...
		if (menu != NULL) {
			...
			children = scatola_menu_get_items (menu);
			...
			scatola_menu_item_list_free (children);
			g_free (subdir);
		}
	}
}
				   ...
				   ScatolaDirectory *model)
{
	...
	model_location = scatola_directory_get_location (model);

	for (node = selection, i = 0; node != NULL; node = node->next, i++) {
		file = SCATOLA_FILE (node->data);

		if (!scatola_file_is_local (file)) {
			parameters[i] = NULL;
			g_strfreev (parameters);
			return NULL;
		}

		file_location = scatola_file_get_location (SCATOLA_FILE (node->data));
...
get_file_paths_or_uris_as_newline_delimited_string (GList *selection, gboolean get_paths)
{
	...
	for (node = selection; node != NULL; node = node->next) {
		uri = NULL;
		if (SCATOLA_IS_DESKTOP_ICON_FILE (node->data)) {
			link = scatola_desktop_icon_file_get_link (SCATOLA_DESKTOP_ICON_FILE (node->data));
			if (link != NULL) {
			...
			uri = scatola_file_get_uri (SCATOLA_FILE (node->data));
		}
		if (uri == NULL) {
			continue;
		}
				       ...
				       char **file_paths, char **uris, char **uri)
{
	 ...
	 * scatola_directory_is_local returns FALSE for nfs.
	 */
	directory_uri = scatola_directory_get_uri (view->details->model);
...
{
	ScatolaWindowSlotInfo *slot;
	ScatolaView *next_view;

	slot = scatola_window_info_get_extra_slot (fm_directory_view_get_scatola_window (view));
	if (slot != NULL) {
		next_view = scatola_window_slot_info_get_current_view (slot);
 ...
 * to take advantage of the current Scatola state.
 */
static void set_script_environment_variables(FMDirectoryView* view, GList* selected_files)
{
	...
	g_setenv("SCATOLA_SCRIPT_SELECTED_FILE_PATHS", file_paths, TRUE);
	g_setenv("NAUTILUS_SCRIPT_SELECTED_FILE_PATHS", file_paths, TRUE); // compatibilidad GNOME

	g_free(file_paths);

	g_setenv("SCATOLA_SCRIPT_SELECTED_URIS", uris, TRUE);
	g_setenv("NAUTILUS_SCRIPT_SELECTED_URIS", uris, TRUE); // compatibilidad GNOME

	g_free(uris);

	g_setenv("SCATOLA_SCRIPT_CURRENT_URI", uri, TRUE);
	...
	g_setenv("SCATOLA_SCRIPT_WINDOW_GEOMETRY", geometry_string, TRUE);
	...
	if (next_view)
	{
		...
		scatola_file_list_free(next_pane_selected_files);
	}
	else
	{
		file_paths = g_strdup("");
	...
	g_setenv("SCATOLA_SCRIPT_NEXT_PANE_SELECTED_FILE_PATHS", file_paths, TRUE);
	g_setenv("NAUTILUS_SCRIPT_NEXT_PANE_SELECTED_FILE_PATHS", file_paths, TRUE); // compatibilidad GNOME
	g_free(file_paths);

	g_setenv("SCATOLA_SCRIPT_NEXT_PANE_SELECTED_URIS", uris, TRUE);
	g_setenv("NAUTILUS_SCRIPT_NEXT_PANE_SELECTED_URIS", uris, TRUE); // compatibilidad GNOME
	g_free(uris);

	g_setenv("SCATOLA_SCRIPT_NEXT_PANE_CURRENT_URI", uri, TRUE);
...
{
	g_unsetenv("SCATOLA_SCRIPT_SELECTED_FILE_PATHS");
	g_unsetenv("NAUTILUS_SCRIPT_SELECTED_FILE_PATHS");

	g_unsetenv("SCATOLA_SCRIPT_SELECTED_URIS");
	g_unsetenv("NAUTILUS_SCRIPT_SELECTED_URIS");

	g_unsetenv("SCATOLA_SCRIPT_CURRENT_URI");
	g_unsetenv("NAUTILUS_SCRIPT_CURRENT_URI");

	g_unsetenv("SCATOLA_SCRIPT_WINDOW_GEOMETRY");
	g_unsetenv("NAUTILUS_SCRIPT_WINDOW_GEOMETRY");

	g_unsetenv("SCATOLA_SCRIPT_NEXT_PANE_SELECTED_FILE_PATHS");
	g_unsetenv("NAUTILUS_SCRIPT_NEXT_PANE_SELECTED_FILE_PATHS");

	g_unsetenv("SCATOLA_SCRIPT_NEXT_PANE_SELECTED_URIS");
	g_unsetenv("NAUTILUS_SCRIPT_NEXT_PANE_SELECTED_URIS");

	g_unsetenv("SCATOLA_SCRIPT_NEXT_PANE_CURRENT_URI");
...
run_script_callback (GtkAction *action, gpointer callback_data)
{
	...
	file_uri = scatola_file_get_uri (launch_parameters->file);
	...
	name = scatola_file_get_name (launch_parameters->file);
	...
	scatola_launch_application_from_command_array (screen, name, quoted_path, FALSE,
							(const char * const *) parameters);
	g_free (name);
	g_strfreev (parameters);

	scatola_file_list_free (selected_files);
			     ...
			     const char *popup_bg_path)
{
	...
	ui_manager = scatola_window_info_get_ui_manager (directory_view->details->window);
...
static gboolean
update_directory_in_scripts_menu (FMDirectoryView *view, ScatolaDirectory *directory)
{
	char *menu_path, *popup_path, *popup_bg_path;
	GList *file_list, *filtered, *node;
	gboolean any_scripts;
	ScatolaFile *file;
	ScatolaDirectory *dir;
	char *uri;
	char *escaped_path;

	uri = scatola_directory_get_uri (directory);
	...
	file_list = scatola_directory_get_file_list (directory);
	filtered = scatola_file_list_filter_hidden (file_list, FALSE);
	scatola_file_list_free (file_list);

	file_list = scatola_file_list_sort_by_display_name (filtered);

	any_scripts = FALSE;
	for (node = file_list; node != NULL; node = node->next) {
		file = node->data;

		if (scatola_file_is_launchable (file)) {
			add_script_to_scripts_menus (view, file, menu_path, popup_path, popup_bg_path);
			any_scripts = TRUE;
		} else if (scatola_file_is_directory (file)) {
			uri = scatola_file_get_uri (file);
			if (directory_belongs_in_scripts_menu (uri)) {
				dir = scatola_directory_get_by_uri (uri);
				add_directory_to_scripts_directory_list (view, dir);
				scatola_directory_unref (dir);
...
update_scripts_menu (FMDirectoryView *view)
{
	...
	any_scripts = FALSE;
	sorted_copy = scatola_directory_list_sort_by_uri
		(scatola_directory_list_copy (view->details->scripts_directory_list));
	...
	scatola_directory_list_free (sorted_copy);
				 ...
				 const char *popup_bg_path)
{
	...
	tmp = scatola_file_get_display_name (file);
				    ...
				    ScatolaDirectory *directory)
{
	...
	for (node = file_list; num < TEMPLATE_LIMIT && node != NULL; node = node->next, num++) {
		...
		if (scatola_file_is_directory (file)) {
			...
			g_free (uri);
		} else if (scatola_file_can_read (file)) {
...
update_templates_menu (FMDirectoryView *view)
{
	...
	if (scatola_should_use_templates_directory ()) {
		templates_directory_uri = scatola_get_templates_directory_uri ();
	} else {
	...
	any_templates = FALSE;
	sorted_copy = scatola_directory_list_sort_by_uri
		(scatola_directory_list_copy (view->details->templates_directory_list));
	for (node = sorted_copy; node != NULL; node = node->next) {
		directory = node->data;
				     ...
				     gpointer callback_data)
{
	...
	open_location (view, scripts_directory_uri, SCATOLA_WINDOW_OPEN_ACCORDING_TO_MODE, 0);
	...
	eel_show_info_dialog_with_details
		(_("All executable files in this folder will appear in the "
		 ...
		 _("All executable files in this folder will appear in the "
		   ...
		   "set by Scatola, which the scripts may use:\n\n"
		   "SCATOLA_SCRIPT_SELECTED_FILE_PATHS: newline-delimited paths for selected files (only if local)\n\n"
		   "SCATOLA_SCRIPT_SELECTED_URIS: newline-delimited URIs for selected files\n\n"
		   "SCATOLA_SCRIPT_CURRENT_URI: URI for current location\n\n"
		   "SCATOLA_SCRIPT_WINDOW_GEOMETRY: position and size of current window\n\n"
		   "SCATOLA_SCRIPT_NEXT_PANE_SELECTED_FILE_PATHS: newline-delimited paths for selected files in the inactive pane of a split-view window (only if local)\n\n"
		   "SCATOLA_SCRIPT_NEXT_PANE_SELECTED_URIS: newline-delimited URIs for selected files in the inactive pane of a split-view window\n\n"
		   "SCATOLA_SCRIPT_NEXT_PANE_CURRENT_URI: URI for current location in the inactive pane of a split-view window"),
		 fm_directory_view_get_containing_window (view));
}
...
create_popup_menu (FMDirectoryView *view, const char *popup_path)
{
	...
	menu = gtk_ui_manager_get_widget (scatola_window_info_get_ui_manager (view->details->window),
					  popup_path);
		   ...
		   gboolean         cut)
{
	...
	gtk_clipboard_set_with_data (scatola_clipboard_get (GTK_WIDGET (view)),
				     targets, n_targets,
				     scatola_get_clipboard_callback, scatola_clear_clipboard_callback,
				     NULL);
        gtk_target_table_free (targets, n_targets);

	scatola_clipboard_monitor_set_clipboard_info (scatola_clipboard_monitor_get (), &info);

	count = g_list_length (clipboard_contents);
	if (count == 1) {
		name = scatola_file_get_display_name (clipboard_contents->data);
		if (cut) {
				 ...
				 char *target_uri)
{
	...
	for (l = selection; l != NULL; l = l->next) {
		uris = g_list_prepend (uris,
				       scatola_file_get_uri ((ScatolaFile *) l->data));
	}
	uris = g_list_reverse (uris);
				  ...
				  int copy_action)
{
	...
	dest_location = scatola_window_slot_info_get_current_location (slot);
...
action_copy_to_home_callback (GtkAction *action, gpointer callback_data)
{
	...
	dest_location = scatola_get_home_directory_uri ();
...
action_copy_to_desktop_callback (GtkAction *action, gpointer callback_data)
{
	...
	dest_location = scatola_get_desktop_directory_uri ();
	move_copy_selection_to_location (view, GDK_ACTION_COPY, dest_location);
	g_free (dest_location);
}
		      ...
		      char *destination_uri)
{
	...
	cut = FALSE;
	item_uris = scatola_clipboard_get_uri_list_from_selection_data (selection_data, &cut,
									 copied_files_atom);
	...
	if (item_uris == NULL|| destination_uri == NULL) {
		...
		if (cut) {
			gtk_clipboard_clear (scatola_clipboard_get (GTK_WIDGET (view)));
		}
					...
					gpointer          callback_data)
{
	...
	if (view->details->window != NULL) {
		directory_uri = scatola_file_get_activation_uri (data->target);
	...
	g_object_unref (view);
	scatola_file_unref (data->target);
	g_free (data);
}
			     ...
			     gpointer callback_data)
{
	...
	g_object_ref (view);
	gtk_clipboard_request_contents (scatola_clipboard_get (GTK_WIDGET (view)),
					copied_files_atom,
	    ...
	    ScatolaFile *target)
{
	...
	g_assert (FM_IS_DIRECTORY_VIEW (view));
	g_assert (SCATOLA_IS_FILE (target));

	data = g_new (PasteIntoData, 1);

	data->view = g_object_ref (view);
	data->target = scatola_file_ref (target);
				  ...
				  gpointer callback_data)
{
	...
	selection = fm_directory_view_get_selection (view);
	if (selection != NULL) {
		paste_into (view, SCATOLA_FILE (selection->data));
		scatola_file_list_free (selection);
	}
...
real_action_undo (FMDirectoryView *view)
{
	...
	scatola_undostack_manager_undo (manager, GTK_WIDGET (view), finish_undoredo_callback);
}
...
real_action_redo (FMDirectoryView *view)
{
	...
	scatola_undostack_manager_redo (manager, GTK_WIDGET (view), finish_undoredo_callback);
}
...
static void
file_mount_callback (ScatolaFile  *file,
...
static void
file_unmount_callback (ScatolaFile  *file,
		       GFile         *result_location,
		       GError        *error,
		       gpointer       callback_data)
{
...
static void
file_eject_callback (ScatolaFile  *file,
		     GFile         *result_location,
		     GError        *error,
		     gpointer       callback_data)
{
...
static void
file_stop_callback (ScatolaFile  *file,
		    GFile         *result_location,
		    GError        *error,
		    gpointer       callback_data)
{
				...
				gpointer data)
{
	...
	for (l = selection; l != NULL; l = l->next) {
		file = SCATOLA_FILE (l->data);
		if (scatola_file_can_unmount (file)) {
			GMountOperation *mount_op;
			mount_op = gtk_mount_operation_new (fm_directory_view_get_containing_window (view));
			fm_directory_view_set_initiated_unmount (view, TRUE);
			scatola_file_unmount (file, mount_op, NULL,
			      ...
			      gpointer data)
{
	...
	for (l = selection; l != NULL; l = l->next) {
		...
		if (scatola_file_can_eject (file)) {
			GMountOperation *mount_op;
			mount_op = gtk_mount_operation_new (fm_directory_view_get_containing_window (view));
			fm_directory_view_set_initiated_unmount (view, TRUE);
			scatola_file_eject (file, mount_op, NULL,
...
static void
file_start_callback (ScatolaFile  *file,
		     GFile         *result_location,
		     GError        *error,
		     gpointer       callback_data)
{
			      ...
			      gpointer   data)
{
	...
	for (l = selection; l != NULL; l = l->next) {
		...
		if (scatola_file_can_start (file) || scatola_file_can_start_degraded (file)) {
			mount_op = gtk_mount_operation_new (fm_directory_view_get_containing_window (view));
			scatola_file_start (file, mount_op, NULL,
			     ...
			     gpointer   data)
{
	...
	for (l = selection; l != NULL; l = l->next) {
		...
		if (scatola_file_can_stop (file)) {
			GMountOperation *mount_op;
			mount_op = gtk_mount_operation_new (fm_directory_view_get_containing_window (view));
			scatola_file_stop (file, mount_op, NULL,
			      ...
			      gpointer   data)
{
	...
	for (l = selection; l != NULL; l = l->next) {
		...
		if (scatola_file_can_poll_for_media (file) && !scatola_file_is_media_check_automatic (file)) {
			scatola_file_poll_for_media (file);
		}
	}
	scatola_file_list_free (selection);
}
				   ...
				   gpointer data)
{
	...
	mount_op = gtk_mount_operation_new (fm_directory_view_get_containing_window (view));
	g_mount_operation_set_password_save (mount_op, G_PASSWORD_SAVE_FOR_SESSION);
	scatola_file_mount (file, mount_op, NULL, file_mount_callback, NULL);
	g_object_unref (mount_op);
}
				     ...
				     gpointer data)
{
	...
	mount_op = gtk_mount_operation_new (fm_directory_view_get_containing_window (view));
	fm_directory_view_set_initiated_unmount (view, TRUE);
	scatola_file_unmount (file, mount_op, NULL, file_unmount_callback, g_object_ref (view));
	g_object_unref (mount_op);
}
				   ...
				   gpointer data)
{
	...
	mount_op = gtk_mount_operation_new (fm_directory_view_get_containing_window (view));
	fm_directory_view_set_initiated_unmount (view, TRUE);
	scatola_file_eject (file, mount_op, NULL, file_eject_callback, g_object_ref (view));
	g_object_unref (mount_op);
}
				   ...
				   gpointer   data)
{
	...
	mount_op = gtk_mount_operation_new (fm_directory_view_get_containing_window (view));
	scatola_file_start (file, mount_op, NULL, file_start_callback, NULL);
	g_object_unref (mount_op);
}
				     ...
				     gpointer data)
{
	...
	case GTK_RESPONSE_OK:
		...
		gde2_vfs_connect_to_server (uri, (char *)name, icon);
		gtk_widget_destroy (GTK_WIDGET (dialog));
		break;
	case GTK_RESPONSE_NONE:
	case GTK_RESPONSE_DELETE_EVENT:
					...
					gpointer data)
{
	...
	uri = scatola_file_get_activation_uri (file);
	icon = scatola_file_get_icon (file, SCATOLA_ICON_SIZE_STANDARD, 0);
	icon_name = scatola_icon_info_get_used_name (icon);
	name = scatola_file_get_display_name (file);
				 ...
				 gpointer   callback_data)
{
	...
	scatola_file_operations_delete (files, fm_directory_view_get_containing_window (view),
					 NULL, NULL);
					     ...
					     gpointer callback_data)
{
	...
	l.next = NULL;
	l.data = file;
	scatola_restore_files_from_trash (&l,
					   fm_directory_view_get_containing_window (view));
}
...
fm_directory_view_init_show_hidden_files (FMDirectoryView *view)
{
	...
	mode = scatola_window_info_get_hidden_files_mode (view->details->window);

	if (mode == SCATOLA_WINDOW_SHOW_HIDDEN_FILES_DEFAULT) {
		show_hidden_default_setting = g_settings_get_boolean (scatola_preferences, SCATOLA_PREFERENCES_SHOW_HIDDEN_FILES);
...
static const GtkActionEntry directory_view_entries[] = {
  ...
  /* name, stock id */         { FM_ACTION_LOCATION_DELETE, SCATOLA_ICON_DELETE,
...
real_merge_menus (FMDirectoryView *view)
{
	...
	tooltip = g_strdup_printf(_("Run or manage scripts from %s"), "~/.config/scatola/scripts");
	...
	ui = scatola_ui_string_get ("scatola-directory-view-ui.xml");
...
static gboolean
can_paste_into_file (ScatolaFile *file)
{
	if (scatola_file_is_directory (file) &&
	    scatola_file_can_write (file)) {
		return TRUE;
	}
	if (scatola_file_has_activation_uri (file)) {
		GFile *location;
		ScatolaFile *activation_file;
		gboolean res;

		location = scatola_file_get_activation_location (file);
		activation_file = scatola_file_get (location);
		g_object_unref (location);

		/* The target location might not have data for it read yet,
		   and we can't want to do sync I/O, so treat the unknown
		   case as can-write */
		res = (scatola_file_get_file_type (activation_file) == G_FILE_TYPE_UNKNOWN) ||
			(scatola_file_get_file_type (activation_file) == G_FILE_TYPE_DIRECTORY &&
			 scatola_file_can_write (activation_file));

		scatola_file_unref (activation_file);
			    ...
			    gpointer          user_data)
{
	...
	gtk_action_set_sensitive (action,
	                          can_paste && count == 1 &&
	                          can_paste_into_file (SCATOLA_FILE (selection->data)));
...
showing_trash_directory (FMDirectoryView *view)
{
	...
	file = fm_directory_view_get_directory_as_file (view);
	if (file != NULL) {
		return scatola_file_is_in_trash (file);
	}
	return FALSE;
}
...
{
	return (showing_trash_directory (view) || scatola_window_info_get_window_type (view->details->window) == SCATOLA_WINDOW_NAVIGATION);
}
...
file_list_all_are_folders (GList *file_list)
{
	...
	for (l = file_list; l != NULL; l = l->next) {
		...
		if (scatola_file_is_scatola_link (file) &&
		    !SCATOLA_IS_DESKTOP_ICON_FILE (file)) {
			if (scatola_file_is_launcher (file)) {
				return FALSE;
			}

			activation_uri = scatola_file_get_activation_uri (file);
			...
			linked_file = scatola_file_get_existing_by_uri (activation_uri);
			...
			is_dir =
				(linked_file != NULL &&
				 scatola_file_is_directory (linked_file)) ||
				(activation_uri != NULL &&
				 activation_uri[strlen (activation_uri) - 1] == '/');

			scatola_file_unref (linked_file);
			g_free (activation_uri);
...
static void
file_should_show_foreach (ScatolaFile        *file,
			  gboolean            *show_mount,
			  ...
			  GDriveStartStopType *start_stop_type)
{
	...
	*show_poll = FALSE;
	...
	if (scatola_file_can_unmount (file) && !*show_eject && !*show_stop) {
		*show_unmount = TRUE;
	}
	...
	*start_stop_type = scatola_file_get_start_stop_type (file);
...
static void
file_should_show_self (ScatolaFile        *file,
		       gboolean            *show_mount,
		       gboolean            *show_unmount,
		       gboolean            *show_eject,
		       gboolean            *show_format,
...
files_are_all_directories (GList *files)
{
	...
	for (l = files; l != NULL; l = l->next) {
		file = SCATOLA_FILE (l->data);
		all_directories &= scatola_file_is_directory (file);
	}
...
files_is_none_directory (GList *files)
{
	...
	for (l = files; l != NULL; l = l->next) {
		file = SCATOLA_FILE (l->data);
		no_directory &= !scatola_file_is_directory (file);
	}
				  ...
				  gboolean is_self)
{
	...
	if (files != NULL) {
		if (g_list_length (files) == 1) {
			original_file = scatola_file_get_trash_original_file (files->data);
		} else {
			original_dirs_hash = scatola_trashed_files_get_original_directories (files, NULL);
			if (original_dirs_hash != NULL) {
				original_dirs = g_hash_table_get_keys (original_dirs_hash);
				if (g_list_length (original_dirs) == 1) {
					original_dir = scatola_file_ref (SCATOLA_FILE (original_dirs->data));
				}
			}
		}
	}
	...
	if (original_file != NULL || original_dirs != NULL) {
		...
		if (original_file != NULL) {
			original_location = scatola_file_get_location (original_file);
		} else if (original_dir != NULL) {
			original_location = scatola_file_get_location (original_dir);
		}
	...
	scatola_file_unref (original_file);
	scatola_file_unref (original_dir);
...
real_update_location_menu_volumes (FMDirectoryView *view)
{
	...
	file = SCATOLA_FILE (view->details->location_popup_directory_as_file);
	file_should_show_foreach (file,
				  &show_mount,
				  &show_unmount,
				  &show_eject,
			...
			gint selection_count)
{
	...
	selection_is_read_only = selection_count == 1 &&
		(!scatola_file_can_write (SCATOLA_FILE (selection->data)) &&
		 !scatola_file_has_activation_uri (SCATOLA_FILE (selection->data)));
	...
	gtk_clipboard_request_targets (scatola_clipboard_get (GTK_WIDGET (view)),
...
real_update_location_menu (FMDirectoryView *view)
{
	...
	if (scatola_window_info_get_window_type (view->details->window) == SCATOLA_WINDOW_NAVIGATION) {
	...
	file = view->details->location_popup_directory_as_file;
	g_assert (SCATOLA_IS_FILE (file));
	g_assert (scatola_file_check_if_ready (file, SCATOLA_FILE_ATTRIBUTE_INFO |
						      SCATOLA_FILE_ATTRIBUTE_MOUNT |
						      SCATOLA_FILE_ATTRIBUTE_FILESYSTEM_INFO));

	is_special_link = SCATOLA_IS_DESKTOP_ICON_FILE (file);
	is_desktop_or_home_dir = scatola_file_is_home (file)
		|| scatola_file_is_desktop_directory (file);

	can_delete_file =
		scatola_file_can_delete (file) &&
		!is_special_link &&
	...
	if (file != NULL &&
		...
		show_separate_delete_command = g_settings_get_boolean (scatola_preferences, SCATOLA_PREFERENCES_ENABLE_DELETE);
	}
	...
	g_object_set (action,
		      ...
		      "icon-name", (file != NULL &&
				   scatola_file_is_in_trash (file)) ?
					SCATOLA_ICON_DELETE : SCATOLA_ICON_TRASH_FULL,
		      NULL);
	gtk_action_set_sensitive (action, can_delete_file);
...
static void
clipboard_changed_callback (ScatolaClipboardMonitor *monitor, FMDirectoryView *view)
{
	GList *selection;
	gint selection_count;
...
can_delete_all (GList *files)
{
	...
	for (l = files; l != NULL; l = l->next) {
		file = l->data;
		if (!scatola_file_can_delete (file)) {
			return FALSE;
		}
	}
	return TRUE;
...
real_update_menus (FMDirectoryView *view)
{
	...
	for (l = selection; l != NULL; l = l->next) {
		...
		if (!scatola_mime_file_opens_in_external_app (file)) {
			show_app = FALSE;
		}
	...
	if (can_open && show_app) {
		app = scatola_mime_get_default_application_for_files (selection);
	}
	...
	show_open_alternate = file_list_all_are_folders (selection) &&
				selection_count > 0 &&
				!(scatola_window_info_get_window_type (view->details->window) == SCATOLA_WINDOW_DESKTOP &&
					g_settings_get_boolean (scatola_preferences, SCATOLA_PREFERENCES_ALWAYS_USE_BROWSER));
	show_open_folder_window = FALSE;
	if (scatola_window_info_get_window_type (view->details->window) == SCATOLA_WINDOW_NAVIGATION) {
		if (g_settings_get_boolean (scatola_preferences, SCATOLA_PREFERENCES_ALWAYS_USE_BROWSER)) {
	...
	g_object_set (action,
		      ...
		      NULL);
	gtk_action_set_sensitive (action, !scatola_trash_monitor_is_empty ());
	...
	if (view->details->model &&
	    SCATOLA_IS_SEARCH_DIRECTORY (view->details->model)) {
		ScatolaSearchDirectory *search;

		search = SCATOLA_SEARCH_DIRECTORY (view->details->model);
		if (scatola_search_directory_is_saved_search (search)) {
			show_save_search = TRUE;
			save_search_sensitive = scatola_search_directory_is_modified (search);
		} else {
	...
	disable_command_line = g_settings_get_boolean (gde2_lockdown_preferences, SCATOLA_PREFERENCES_LOCKDOWN_COMMAND_LINE);
	action = gtk_action_group_get_action (view->details->dir_action_group,
...
static void
location_popup_file_attributes_ready (ScatolaFile *file,
				      gpointer      data)
{
	FMDirectoryView *view;
...
unschedule_pop_up_location_context_menu (FMDirectoryView *view)
{
	if (view->details->location_popup_directory_as_file != NULL) {
		g_assert (SCATOLA_IS_FILE (view->details->location_popup_directory_as_file));
		scatola_file_cancel_call_when_ready (view->details->location_popup_directory_as_file,
						      location_popup_file_attributes_ready,
						      view);
		scatola_file_unref (view->details->location_popup_directory_as_file);
				       ...
				       ScatolaFile    *file)
{
	...
	if (file == view->details->location_popup_directory_as_file) {
		if (scatola_file_check_if_ready (file, SCATOLA_FILE_ATTRIBUTE_INFO |
							SCATOLA_FILE_ATTRIBUTE_MOUNT |
							SCATOLA_FILE_ATTRIBUTE_FILESYSTEM_INFO)) {
			real_pop_up_location_context_menu (view);
		}
		...
		view->details->location_popup_directory_as_file = scatola_file_ref (file);
		scatola_file_call_when_ready (view->details->location_popup_directory_as_file,
					       SCATOLA_FILE_ATTRIBUTE_INFO |
					       SCATOLA_FILE_ATTRIBUTE_MOUNT |
					       SCATOLA_FILE_ATTRIBUTE_FILESYSTEM_INFO,
					       location_popup_file_attributes_ready,
						...
						const char      *location)
{
	...
	if (location != NULL) {
		file = scatola_file_get_by_uri (location);
	} else {
		file = scatola_file_ref (view->details->directory_as_file);
	}
					    ...
					    GdkDragAction action)
{
	...
	if (action == GDK_ACTION_ASK) {
		action = scatola_drag_drop_action_ask
			(GTK_WIDGET (view),
	...
	scatola_clipboard_clear_if_colliding_uris (GTK_WIDGET (view),
						    source_uri_list,
...
fm_directory_view_notify_selection_changed (FMDirectoryView *view)
{
	...
	if (scatola_debug_log_is_domain_enabled (SCATOLA_DEBUG_LOG_DOMAIN_USER)) {
		selection = fm_directory_view_get_selection (view);

		window = fm_directory_view_get_containing_window (view);
		scatola_debug_log_with_file_list (FALSE, SCATOLA_DEBUG_LOG_DOMAIN_USER, selection,
...
static void
file_changed_callback (ScatolaFile *file, gpointer callback_data)
{
		...
		ScatolaDirectory *directory)
{
	...
	view->details->directory_as_file =
		scatola_directory_get_corresponding_file (directory);
	scatola_file_unref (old_file);
	...
	attributes =
		SCATOLA_FILE_ATTRIBUTE_INFO |
		SCATOLA_FILE_ATTRIBUTE_FILESYSTEM_INFO;
	scatola_file_monitor_add (view->details->directory_as_file,
				   &view->details->directory_as_file,
				   attributes);
...
finish_loading (FMDirectoryView *view)
{
	...
	scatola_window_info_report_load_underway (view->details->window,
						   SCATOLA_VIEW (view));
	...
	scatola_window_info_view_visible  (view->details->window, SCATOLA_VIEW (view));

	if (scatola_directory_are_all_files_seen (view->details->model)) {
		/* Unschedule a pending update and schedule a new one with the minimal
		 * update interval. This gives the view a short chance at gathering the
		 * (cached) deep counts.
		 */
	...
	scatola_directory_file_monitor_add (view->details->model,
					     &view->details->model,
					     view->details->show_hidden_files,
					     attributes,
					     files_added_callback, view);
...
static void
metadata_for_directory_as_file_ready_callback (ScatolaFile *file,
			      		       gpointer callback_data)
{
	FMDirectoryView *view;
...
static void
metadata_for_files_in_directory_ready_callback (ScatolaDirectory *directory,
				   		GList *files,
			           		gpointer callback_data)
{
	FMDirectoryView *view;
...
real_get_emblem_names_to_exclude (FMDirectoryView *view)
{
	...
	i = 0;
	excludes[i++] = g_strdup (SCATOLA_FILE_EMBLEM_NAME_TRASH);

	if (!scatola_file_can_write (view->details->directory_as_file)) {
		excludes[i++] = g_strdup (SCATOLA_FILE_EMBLEM_NAME_CANT_WRITE);
	}
...
disconnect_model_handlers (FMDirectoryView *view)
{
	...
	scatola_file_cancel_call_when_ready (view->details->directory_as_file,
					      metadata_for_directory_as_file_ready_callback,
					      view);
	scatola_directory_cancel_callback (view->details->model,
					    metadata_for_files_in_directory_ready_callback,
					    view);
	scatola_directory_file_monitor_remove (view->details->model,
						&view->details->model);
	scatola_file_monitor_remove (view->details->directory_as_file,
				      &view->details->directory_as_file);
}
...
fm_directory_view_reset_to_defaults (FMDirectoryView *view)
{
	...
	mode = scatola_window_info_get_hidden_files_mode (view->details->window);
	if (mode != SCATOLA_WINDOW_SHOW_HIDDEN_FILES_DEFAULT) {
		scatola_window_info_set_hidden_files_mode (view->details->window,
 ...
 * should be a list of ScatolaFiles
 *
 **/
void
...
static void
fm_directory_view_select_file (FMDirectoryView *view, ScatolaFile *file)
{
	GList file_list;
...
fm_directory_view_stop (FMDirectoryView *view)
{
	...
	if (view->details->model != NULL) {
		scatola_directory_file_monitor_remove (view->details->model, view);
	}
...
fm_directory_view_is_editable (FMDirectoryView *view)
{
	...
	if (directory != NULL) {
		return scatola_directory_is_editable (directory);
	}
...
{
	if (view->details->window != NULL) {
		scatola_window_info_set_initiated_unmount(view->details->window,
							   initiated_unmount);
	}
}
...
real_is_read_only (FMDirectoryView *view)
{
	...
	file = fm_directory_view_get_directory_as_file (view);
	if (file != NULL) {
		return !scatola_file_can_write (file);
	}
	return FALSE;
}
 ...
 */
gboolean
fm_directory_view_should_show_file (FMDirectoryView *view, ScatolaFile *file)
{
...
fm_directory_view_get_uri (FMDirectoryView *view)
{
	...
	return scatola_directory_get_uri (view->details->model);
}
...
fm_directory_view_get_backing_uri (FMDirectoryView *view)
{
	...
	if (SCATOLA_IS_DESKTOP_DIRECTORY (directory)) {
		directory = scatola_desktop_directory_get_real_directory (SCATOLA_DESKTOP_DIRECTORY (directory));
	} else {
		scatola_directory_ref (directory);
	}
				   ...
				   FMDirectoryView *view)
{
	...
	target_file = scatola_file_get_existing_by_uri (target_uri);
	/* special-case "command:" here instead of starting a move/copy */
	if (target_file != NULL && scatola_file_is_launcher (target_file)) {
		scatola_file_unref (target_file);
		scatola_launch_desktop_file (
				gtk_widget_get_screen (GTK_WIDGET (view)),
				target_uri, item_uris,
				fm_directory_view_get_containing_window (view));
		return;
	} else if (copy_action == GDK_ACTION_COPY &&
		   scatola_is_engrampa_installed () &&
		   target_file != NULL &&
		   scatola_file_is_archive (target_file)) {
		...
		gde2_gdk_spawn_command_line_on_screen(screen, command, NULL);
		g_free (command);
	...
	scatola_file_operations_copy_move
...
gboolean
fm_directory_view_can_accept_item (ScatolaFile *target_item,
				   const char *item_uri,
				   FMDirectoryView *view)
{
	g_return_val_if_fail (SCATOLA_IS_FILE (target_item), FALSE);
	g_return_val_if_fail (item_uri != NULL, FALSE);
	g_return_val_if_fail (FM_IS_DIRECTORY_VIEW (view), FALSE);

	return scatola_drag_can_accept_item (target_item, item_uri);
}

static void
fm_directory_view_trash_state_changed_callback (ScatolaTrashMonitor *trash_monitor,
					    ...
					    int               y)
{
	...
	if (action == GDK_ACTION_LINK) {
		...
		if (!eel_str_is_empty (link_name)) {
			...
			scatola_link_local_create (target_uri != NULL ? target_uri : container_uri,
						    link_name,
						    link_display_name,
						    "gde2-fs-bookmark",
						    url,
				...
				GParamSpec      *pspec)
{
  ...
  case PROP_WINDOW_SLOT:
	  ...
	  slot = SCATOLA_WINDOW_SLOT_INFO (g_value_get_object (value));
          window = scatola_window_slot_info_get_window (slot);
			      ...
			      GtkWidget *old_parent)
{
	...
	if (parent != NULL) {
		...
		if (view->details->slot ==
		    scatola_window_info_get_active_slot (view->details->window)) {
			view->details->active = TRUE;
...
fm_directory_view_class_init (FMDirectoryViewClass *klass)
{
	...
	signals[ADD_FILE] =
		g_signal_new ("add_file",
		              ...
		              G_TYPE_NONE, 2, SCATOLA_TYPE_FILE, SCATOLA_TYPE_DIRECTORY);
	...
	copied_files_atom = gdk_atom_intern ("x-special/gde2-copied-files", FALSE);
...
static void
undo_redo_menu_update_callback (ScatolaUndoStackManager* manager, gpointer arg, gpointer data)
{
	FMDirectoryView *view;
	view = FM_DIRECTORY_VIEW (data);

	ScatolaUndoStackMenuData* menudata = (ScatolaUndoStackMenuData*) arg;
