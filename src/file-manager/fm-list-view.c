...
#include <libscatola-private/scatola-tree-view-drag-dest.h>
#include <libscatola-private/scatola-view-factory.h>
#include <libscatola-private/scatola-clipboard.h>
#include <libscatola-private/scatola-cell-renderer-text-ellipsized.h>
...
struct FMListViewDetails
{
    ...
    ScatolaTreeViewDragDest *drag_dest;
...
static ScatolaFileSortType         default_sort_order_auto_value;
static gboolean			default_sort_reversed_auto_value;
static ScatolaZoomLevel        default_zoom_level_auto_value;
...
static void   fm_list_view_scroll_to_file                  (FMListView        *view,
        ScatolaFile      *file);
static void   fm_list_view_iface_init                      (ScatolaViewIface *iface);
static void   fm_list_view_rename_callback                 (ScatolaFile      *file,
...
get_default_sort_order (ScatolaFile *file, gboolean *reversed)
{
    ...
    retval = scatola_file_get_default_sort_attribute (file, reversed);
                           ...
                           GdkEventButton *event)
{
                                       ...
                                       &path, NULL, NULL, NULL))
    {
        ...
        if ((click_policy_auto_value == SCATOLA_CLICK_POLICY_SINGLE)
...
button_press_callback (GtkWidget *widget, GdkEventButton *event, gpointer callback_data)
{
    ...
    if (click_policy_auto_value == SCATOLA_CLICK_POLICY_SINGLE && click_count >= 2)
    {
        return TRUE;
    }
...
static void
subdirectory_done_loading_callback (ScatolaDirectory *directory, FMListView *view)
{
...
row_expanded_callback (GtkTreeView *treeview, GtkTreeIter *iter, GtkTreePath *path, gpointer callback_data)
{
    ...
    if (fm_list_model_load_subdirectory (view->details->model, path, &directory))
    {
        ...
        if (scatola_directory_are_all_files_seen (directory))
        {
...
unload_file_timeout (gpointer data)
{
    ...
    {
        scatola_directory_unref (unload_data->directory);
    }
    scatola_file_unref (unload_data->file);
    g_free (unload_data);
    return FALSE;
}
                              ...
                              FMListView *view)
{
    ...
    scatola_file_set_metadata (file, SCATOLA_METADATA_KEY_LIST_VIEW_SORT_COLUMN,
            ...
            sort_criterion_changes_due_to_user (view->details->tree_view))
    {
        ...
        if (sort_attr == default_sort_attr)
        {
            ...
            reversed = g_settings_get_boolean (scatola_preferences, SCATOLA_PREFERENCES_DEFAULT_SORT_IN_REVERSE_ORDER);
        }
        else
        {
            reversed = scatola_file_is_date_sort_attribute_q (sort_attr);
        }
    ...
    scatola_file_set_metadata (file, SCATOLA_METADATA_KEY_LIST_VIEW_SORT_REVERSED,
                      ...
                      FMListView          *view)
{
    ...
    {
        view->details->renaming_file = scatola_file_ref (file);
...
static char *
get_root_uri_callback (ScatolaTreeViewDragDest *dest,
                       gpointer user_data)
{
    FMListView *view;
...
static ScatolaFile *
get_file_for_path_callback (ScatolaTreeViewDragDest *dest,
                            GtkTreePath *path,
                            gpointer user_data)
{
    FMListView *view;
...
static void
list_view_handle_netscape_url (ScatolaTreeViewDragDest *dest, const char *encoded_url,
...
static void
list_view_handle_uri_list (ScatolaTreeViewDragDest *dest, const char *item_uris,
...
static void
list_view_handle_text (ScatolaTreeViewDragDest *dest, const char *text,
                       const char *target_uri,
                       GdkDragAction action, int x, int y, FMListView *view)
{
...
static void
list_view_handle_raw (ScatolaTreeViewDragDest *dest, const char *raw_data,
...
static void
move_copy_items_callback (ScatolaTreeViewDragDest *dest,
                        ...
                        char **visible_columns)
{
    ...
    all_columns = scatola_get_columns_for_file (file);
    all_columns = scatola_sort_columns (all_columns, column_order);
    ...
    scatola_column_list_free (all_columns);
...
focus_in_event_callback (GtkWidget *widget, GdkEventFocus *event, gpointer user_data)
{
    ...
    slot_info = fm_directory_view_get_scatola_window_slot (FM_DIRECTORY_VIEW (list_view));
...
create_and_set_up_tree_view (FMListView *view)
{
    ...
    view->details->drag_dest =
        scatola_tree_view_drag_dest_new (view->details->tree_view);
    ...
    scatola_columns = scatola_get_all_columns ();

    for (l = scatola_columns; l != NULL; l = l->next)
    {
        ScatolaColumn *scatola_column;
        int column_num, font_size;
        char *name;
        char *label;
        float xalign;

        scatola_column = SCATOLA_COLUMN (l->data);

        g_object_get (scatola_column,
                      "name", &name,
                      "label", &label,
                      "xalign", &xalign, NULL);
    ...
    scatola_column_list_free (scatola_columns);
...
static void
fm_list_view_add_file (FMDirectoryView *view, ScatolaFile *file, ScatolaDirectory *directory)
{
    FMListModel *model;
...
get_visible_columns (FMListView *list_view)
{
    ...
    visible_columns = scatola_file_get_metadata_list
                      (file,
...
get_column_order (FMListView *list_view)
{
    ...
    column_order = scatola_file_get_metadata_list
                   (file,
...
set_sort_order_from_metadata_and_preferences (FMListView *list_view)
{
    ...
    file = fm_directory_view_get_directory_as_file (FM_DIRECTORY_VIEW (list_view));
    sort_attribute = scatola_file_get_metadata (file,
    ...
    sort_reversed = scatola_file_get_boolean_metadata (file,
...
{
    ScatolaZoomLevel default_zoom_level;

    default_zoom_level = default_zoom_level_auto_value;

    if (default_zoom_level <  SCATOLA_ZOOM_LEVEL_SMALLEST
            || SCATOLA_ZOOM_LEVEL_LARGEST < default_zoom_level)
    {
        default_zoom_level = SCATOLA_ZOOM_LEVEL_SMALL;
    }
...
static void
fm_list_view_rename_callback (ScatolaFile *file,
                              GFile *result_location,
                              GError *error,
                              gpointer callback_data)
{
    ...
    if (view->details->renaming_file)
    {
        ...
        if (error != NULL)
        {
             ...
             */
            scatola_file_unref (view->details->renaming_file);
...
static void
fm_list_view_file_changed (FMDirectoryView *view, ScatolaFile *file, ScatolaDirectory *directory)
{
    FMListView *listview;
    GtkTreeIter iter;
            ...
            listview->details->rename_done)
    {
        ...
        scatola_file_unref (listview->details->renaming_file);
...
static void
fm_list_view_remove_file (FMDirectoryView *view, ScatolaFile *file, ScatolaDirectory *directory)
{
    GtkTreePath *path;
    GtkTreePath *file_path;
    GtkTreeIter iter;
...
static void
column_chooser_changed_callback (ScatolaColumnChooser *chooser,
                                 FMListView *view)
{
    ScatolaFile *file;
    char **visible_columns;
    ...
    scatola_column_chooser_get_settings (chooser,
                                      &visible_columns,
                                      &column_order);
    ...
    scatola_file_set_metadata_list (file,
                                 SCATOLA_METADATA_KEY_LIST_VIEW_VISIBLE_COLUMNS,
                                 list);
    g_list_free (list);
...
static void
column_chooser_set_from_arrays (ScatolaColumnChooser *chooser,
                                FMListView *view,
                                char **visible_columns,
                                char **column_order)
{
    ...
    scatola_column_chooser_set_settings (chooser,
                                      visible_columns,
                                      column_order);
...
static void
column_chooser_set_from_settings (ScatolaColumnChooser *chooser,
                                  FMListView *view)
{
    char **visible_columns;
    char **column_order;
...
static void
column_chooser_use_default_callback (ScatolaColumnChooser *chooser,
                                     FMListView *view)
{
    ScatolaFile *file;
    ...
    scatola_file_set_metadata_list (file, SCATOLA_METADATA_KEY_LIST_VIEW_COLUMN_ORDER, NULL);
    scatola_file_set_metadata_list (file, SCATOLA_METADATA_KEY_LIST_VIEW_VISIBLE_COLUMNS, NULL);

    /* set view values ourselves, as new metadata could not have been
     * updated yet.
     */
    default_columns = scatola_file_is_in_trash (file) ?
                      g_strdupv ((gchar **) default_trash_visible_columns) :
                      g_strdupv (default_visible_columns_auto_value);

    default_order = scatola_file_is_in_trash (file) ?
                    g_strdupv ((gchar **) default_trash_columns_order) :
                    g_strdupv (default_column_order_auto_value);
...
create_column_editor (FMListView *view)
{
...
#if GTK_CHECK_VERSION (3, 0, 0)
    column_chooser = scatola_column_chooser_new (file);
    ...
    (SCATOLA_COLUMN_CHOOSER (column_chooser), view);
...
fm_list_view_merge_menus (FMDirectoryView *view)
{
    ...
    ui = scatola_ui_string_get ("scatola-list-view-ui.xml");
...
fm_list_view_reset_to_defaults (FMDirectoryView *view)
{
    ...
    scatola_file_set_metadata (file, SCATOLA_METADATA_KEY_LIST_VIEW_SORT_COLUMN, NULL, NULL);
    scatola_file_set_metadata (file, SCATOLA_METADATA_KEY_LIST_VIEW_SORT_REVERSED, NULL, NULL);
    scatola_file_set_metadata (file, SCATOLA_METADATA_KEY_LIST_VIEW_ZOOM_LEVEL, NULL, NULL);
    scatola_file_set_metadata_list (file, SCATOLA_METADATA_KEY_LIST_VIEW_COLUMN_ORDER, NULL);
    scatola_file_set_metadata_list (file, SCATOLA_METADATA_KEY_LIST_VIEW_VISIBLE_COLUMNS, NULL);
                              ...
                              ScatolaZoomLevel new_level)
{
    ...
    {
        first_time = FALSE;
        medium = SCATOLA_ZOOM_LEVEL_SMALLER;
        pango_scale[medium] = PANGO_SCALE_MEDIUM;
        for (i = medium; i > SCATOLA_ZOOM_LEVEL_SMALLEST; i--)
        {
            pango_scale[i - 1] = (1 / 1.2) * pango_scale[i];
        }
        for (i = medium; i < SCATOLA_ZOOM_LEVEL_LARGEST; i++)
        {
                             ...
                             gboolean always_emit)
{
    ...
    icon_size = scatola_get_icon_size_for_zoom_level (new_level);
...
fm_list_view_get_zoom_level (FMDirectoryView *view)
{
    ...
    g_return_val_if_fail (FM_IS_LIST_VIEW (view), SCATOLA_ZOOM_LEVEL_STANDARD);
...
fm_list_view_can_zoom_in (FMDirectoryView *view)
{
    ...
    return FM_LIST_VIEW (view)->details->zoom_level	< SCATOLA_ZOOM_LEVEL_LARGEST;
}
...
fm_list_view_can_zoom_out (FMDirectoryView *view)
{
    ...
    return FM_LIST_VIEW (view)->details->zoom_level > SCATOLA_ZOOM_LEVEL_SMALLEST;
}
...
fm_list_view_click_policy_changed (FMDirectoryView *directory_view)
{
    ...
    if (click_policy_auto_value == SCATOLA_CLICK_POLICY_DOUBLE)
    {
        if (view->details->hover_path != NULL)
        {
    ...
    else if (click_policy_auto_value == SCATOLA_CLICK_POLICY_SINGLE)
    {
        if (hand_cursor == NULL)
        {
...
static int
fm_list_view_compare_files (FMDirectoryView *view, ScatolaFile *file1, ScatolaFile *file2)
{
    FMListView *list_view;
...
fm_list_view_finalize (GObject *object)
{
    ...
    g_signal_handlers_disconnect_by_func (scatola_preferences,
                                          ...
                                          list_view);
    g_signal_handlers_disconnect_by_func (scatola_list_view_preferences,
...
static char *
fm_list_view_get_first_visible_file (ScatolaView *view)
{
    ScatolaFile *file;
    GtkTreePath *path;
    GtkTreeIter iter;
...
static void
list_view_scroll_to_file (ScatolaView *view,
                          const char *uri)
{
    ScatolaFile *file;
...
static void
list_view_notify_clipboard_info (ScatolaClipboardMonitor *monitor,
...
fm_list_view_class_init (FMListViewClass *class)
{
    ...
    eel_g_settings_add_auto_enum (scatola_list_view_preferences,
                                  SCATOLA_PREFERENCES_LIST_VIEW_DEFAULT_ZOOM_LEVEL,
                                  (int *) &default_zoom_level_auto_value);
    eel_g_settings_add_auto_strv (scatola_list_view_preferences,
...
static const char *
fm_list_view_get_id (ScatolaView *view)
{
...
static void
fm_list_view_iface_init (ScatolaViewIface *iface)
{
...
fm_list_view_init (FMListView *list_view)
{
    ...
    g_signal_connect_swapped (scatola_preferences,
                              ...
                              G_CALLBACK (default_sort_order_changed_callback),
                              list_view);
    g_signal_connect_swapped (scatola_list_view_preferences,
                              "changed::" SCATOLA_PREFERENCES_LIST_VIEW_DEFAULT_ZOOM_LEVEL,
                              G_CALLBACK (default_zoom_level_changed_callback),
                              list_view);
    g_signal_connect_swapped (scatola_list_view_preferences,
                              "changed::" SCATOLA_PREFERENCES_LIST_VIEW_DEFAULT_VISIBLE_COLUMNS,
                              G_CALLBACK (default_visible_columns_changed_callback),
                              list_view);
    g_signal_connect_swapped (scatola_list_view_preferences,
                              "changed::" SCATOLA_PREFERENCES_LIST_VIEW_DEFAULT_COLUMN_ORDER,
    ...
    list_view->details->zoom_level = SCATOLA_ZOOM_LEVEL_SMALLEST - 1;
...
static ScatolaView *
fm_list_view_create (ScatolaWindowSlotInfo *slot)
{
    FMListView *view;
...
static ScatolaViewInfo fm_list_view =
{
    FM_LIST_VIEW_ID,
...
fm_list_view_register (void)
{
    ...
    scatola_view_factory_register (&fm_list_view);
}
