...
#include "libscatola-private/scatola-mime-application-chooser.h"
...
#include <libscatola-private/scatola-emblem-utils.h>
...
struct FMPropertiesWindowDetails {
	...
	ScatolaFile *group_change_file;
	char         *group_change_group;
	unsigned int  group_change_timeout;
	ScatolaFile *owner_change_file;
...
static const GtkTargetEntry target_table[] = {
	...
	{ "x-special/gde2-reset-background", 0, TARGET_RESET_BACKGROUND }
};
...
static void file_changed_callback                 (ScatolaFile       *file,
						   gpointer            user_data);
...
static void properties_window_update              (FMPropertiesWindow *window,
						   GList              *files);
static void is_directory_ready_callback           (ScatolaFile       *file,
						   gpointer            data);
...
static gboolean name_field_focus_out              (ScatolaEntry *name_field,
						   GdkEventFocus *event,
						   gpointer callback_data);
static void name_field_activate                   (ScatolaEntry *name_field,
						   gpointer callback_data);
#if GTK_CHECK_VERSION (3, 0, 0)
...
is_multi_file_window (FMPropertiesWindow *window)
{
	...
	for (l = window->details->original_files; l != NULL; l = l->next) {
		if (!scatola_file_is_gone (SCATOLA_FILE (l->data))) {
			count++;
			if (count > 1) {
				return TRUE;
			}
...
get_original_file (FMPropertiesWindow *window)
{
	...
	return SCATOLA_FILE (window->details->original_files->data);
}

static ScatolaFile *
get_target_file_for_original_file (ScatolaFile *file)
{
	...
	if (SCATOLA_IS_DESKTOP_ICON_FILE (file)) {
		...
		if (link != NULL) {
			...
			if (location) {
				target_file = scatola_file_get (location);
		...
		uri_to_display = scatola_file_get_activation_uri (file);
		if (uri_to_display != NULL) {
			target_file = scatola_file_get_by_uri (uri_to_display);
...
{
	return SCATOLA_FILE (window->details->target_files->data);
}
				 ...
				 GdkPixbuf **icon_pixbuf)
{
	...
	for (l = window->details->original_files; l != NULL; l = l->next) {
		...
		if (!icon) {
			icon = scatola_file_get_icon (file, SCATOLA_ICON_SIZE_STANDARD, SCATOLA_FILE_ICON_FLAGS_USE_THUMBNAILS | SCATOLA_FILE_ICON_FLAGS_IGNORE_VISITING);
		} else {
			new_icon = scatola_file_get_icon (file, SCATOLA_ICON_SIZE_STANDARD, SCATOLA_FILE_ICON_FLAGS_USE_THUMBNAILS | SCATOLA_FILE_ICON_FLAGS_IGNORE_VISITING);
	...
	if (!icon) {
		icon = scatola_icon_info_lookup_from_name ("text-x-generic", SCATOLA_ICON_SIZE_STANDARD);
	}

	if (icon_name != NULL) {
		*icon_name = g_strdup (scatola_icon_info_get_used_name (icon));
	}

	if (icon_pixbuf != NULL) {
		*icon_pixbuf = scatola_icon_info_get_pixbuf_at_size (icon, SCATOLA_ICON_SIZE_STANDARD);
	}
...
#endif
{
	 ...
	 */
	use_label = is_multi_file_window (window) || !scatola_file_can_rename (get_original_file (window));
	new_widget = !window->details->name_field || (use_label ? SCATOLA_IS_ENTRY (window->details->name_field) : GTK_IS_LABEL (window->details->name_field));
	...
	if (new_widget) {
		...
		if (use_label) {
			...
			window->details->name_field = scatola_entry_new ();
...
update_name_field (FMPropertiesWindow *window)
{
	...
	if (is_multi_file_window (window)) {
		...
		if (file == NULL || scatola_file_is_gone (file)) {
			...
			current_name = scatola_file_get_display_name (file);
		}
...
static void
name_field_restore_original_name (ScatolaEntry *name_field)
{
	...
	scatola_entry_select_all (name_field);

	g_free (displayed_name);
}

static void
rename_callback (ScatolaFile *file, GFile *res_loc, GError *error, gpointer callback_data)
{
	FMPropertiesWindow *window;
	...
	if (error != NULL) {
		...
		if (window->details->name_field != NULL) {
			name_field_restore_original_name (SCATOLA_ENTRY (window->details->name_field));
		}
	}
...
static void
name_field_done_editing (ScatolaEntry *name_field, FMPropertiesWindow *window)
{
	ScatolaFile *file;
	char *new_name;
	const char *original_name;

	g_return_if_fail (SCATOLA_IS_ENTRY (name_field));
	...
	if (strlen (new_name) == 0) {
		name_field_restore_original_name (SCATOLA_ENTRY (name_field));
	} else {
		...
		if (strcmp (new_name, original_name) != 0) {
			...
			g_object_ref (window);
			scatola_file_rename (file, new_name,
...
static gboolean
name_field_focus_out (ScatolaEntry *name_field,
		      GdkEventFocus *event,
		      gpointer callback_data)
{
...
static void
name_field_activate (ScatolaEntry *name_field, gpointer callback_data)
{
	g_assert (SCATOLA_IS_ENTRY (name_field));
	g_assert (FM_IS_PROPERTIES_WINDOW (callback_data));

	/* Accept changes. */
	name_field_done_editing (name_field, FM_PROPERTIES_WINDOW (callback_data));

	scatola_entry_select_all_at_idle (name_field);
}

static gboolean
file_has_keyword (ScatolaFile *file, const char *keyword)
{
	GList *keywords, *word;
		       ...
		       FMPropertiesWindow *window)
{
	...
	name = g_object_get_data (G_OBJECT (button), "scatola_emblem_name");
		    ...
		    ScatolaFile *file)
{
	...
	original_file = SCATOLA_FILE (original_link->data);
	target_file = SCATOLA_FILE (target_link->data);
	...
	scatola_file_monitor_remove (original_file, &window->details->original_files);
	scatola_file_monitor_remove (target_file, &window->details->target_files);
...
get_mime_list (FMPropertiesWindow *window)
{
	...
	ret = NULL;
	for (l = window->details->target_files; l != NULL; l = l->next) {
		ret = g_list_append (ret, scatola_file_get_mime_type (SCATOLA_FILE (l->data)));
	}
			  ...
			  GList *files)
{
	...
	for (tmp = files; tmp != NULL; tmp = tmp->next) {
		changed_file = SCATOLA_FILE (tmp->data);

		if (changed_file && scatola_file_is_gone (changed_file)) {
...
update_files_callback (gpointer data)
{
	...
	if (window->details->original_files == NULL) {
		...
		scatola_file_list_free (window->details->changed_files);
...
file_list_attributes_identical (GList *file_list, const char *attribute_name)
{
	...
	for (l = file_list; l != NULL; l = l->next) {
		...
		if (first_attr == NULL) {
			first_attr = scatola_file_get_string_attribute_with_default (file, attribute_name);
		} else {
			char *attr;
			attr = scatola_file_get_string_attribute_with_default (file, attribute_name);
				...
				const char *inconsistent_value)
{
	if (file_list_attributes_identical (file_list, attribute_name)) {
		...
		for (l = file_list; l != NULL; l = l->next) {
			...
			file = SCATOLA_FILE (l->data);
			if (!scatola_file_is_gone (file)) {
				return scatola_file_get_string_attribute_with_default
					(file,
					 attribute_name);
			}
		}
...
file_list_all_directories (GList *file_list)
{
	GList *l;
	for (l = file_list; l != NULL; l = l->next) {
		if (!scatola_file_is_directory (SCATOLA_FILE (l->data))) {
			return FALSE;
		}
	}
	return TRUE;
...
static void
group_change_callback (ScatolaFile *file,
		       GFile *res_loc,
		       GError *error,
		       FMPropertiesWindow *window)
{
...
cancel_group_change_callback (FMPropertiesWindow *window)
{
	...
	scatola_file_cancel (file, (ScatolaFileOperationCallback) group_change_callback, window);
...
schedule_group_change_timeout (FMPropertiesWindow *window)
{
	...
	scatola_file_set_group
		(file,  group,
		 (ScatolaFileOperationCallback) group_change_callback, window);
		       ...
		       const char         *group)
{
	...
	window->details->group_change_file = scatola_file_ref (file);
...
static void
changed_group_callback (GtkComboBox *combo_box, ScatolaFile *file)
{
	FMPropertiesWindow *window;
	char *group;
	char *cur_group;
	...
	cur_group = scatola_file_get_group_name (file);
...
static void
synch_groups_combo_box (GtkComboBox *combo_box, ScatolaFile *file)
{
	GList *groups;
	GList *node;
	GtkTreeModel *model;
	...
	groups = scatola_file_get_settable_group_names (file);
	...
	current_group_name = scatola_file_get_group_name (file);
                        ...
                        ScatolaFile *file)
{
	...
	g_signal_connect_data (combo_box, "changed",
			       ...
			       scatola_file_ref (file),
			       (GClosureNotify)scatola_file_unref, 0);

	return combo_box;
}

static void
owner_change_callback (ScatolaFile *file,
...
cancel_owner_change_callback (FMPropertiesWindow *window)
{
	...
	scatola_file_cancel (file, (ScatolaFileOperationCallback) owner_change_callback, window);
...
schedule_owner_change_timeout (FMPropertiesWindow *window)
{
	...
	scatola_file_set_owner
		(file,  owner,
		 (ScatolaFileOperationCallback) owner_change_callback, window);
		       ...
		       const char         *owner)
{
	...
	window->details->owner_change_file = scatola_file_ref (file);
...
static void
changed_owner_callback (GtkComboBox *combo_box, ScatolaFile* file)
{
	FMPropertiesWindow *window;
	char *owner_text;
	char **name_array;
	...
	g_free (owner_text);
	cur_owner = scatola_file_get_owner_name (file);
...
static void
synch_user_menu (GtkComboBox *combo_box, ScatolaFile *file)
{
	GList *users;
	GList *node;
	GtkTreeModel *model;
	...
	owner_name = scatola_file_get_string_attribute (file, "owner");
...
static gboolean
file_has_prefix (ScatolaFile *file,
		 GList *prefix_candidates)
{
	...
	for (p = prefix_candidates; p != NULL; p = p->next) {
		...
		candidate_location = scatola_file_get_location (SCATOLA_FILE (p->data));
...
{
	ScatolaRequestStatus file_status, status;
	char *text, *temp;
	...
	status = SCATOLA_REQUEST_DONE;
	file_status = SCATOLA_REQUEST_NOT_STARTED;
	...
	for (l = window->details->target_files; l; l = l->next) {
		...
		if (scatola_file_is_directory (file)) {
			file_status = scatola_file_get_deep_counts (file,
					 &directory_count,
					 &file_count,
					 &file_unreadable,
					 &file_size,
			...
			if (file_status != SCATOLA_REQUEST_DONE) {
				status = file_status;
			}
		} else {
			++total_count;
			total_size += scatola_file_get_size (file);
		}
	}
...
#endif
{
	...
	for (l = window->details->target_files; l; l = l->next) {
		file = SCATOLA_FILE (l->data);
		scatola_file_recompute_deep_counts (file);
...
static gboolean
is_merged_trash_directory (ScatolaFile *file)
{
	char *file_uri;
	gboolean result;
...
static gboolean
is_computer_directory (ScatolaFile *file)
{
	char *file_uri;
	gboolean result;
...
static gboolean
is_network_directory (ScatolaFile *file)
{
	char *file_uri;
	gboolean result;
...
static gboolean
is_burn_directory (ScatolaFile *file)
{
	char *file_uri;
	gboolean result;
...
{
	if (!is_multi_file_window (window)
	    && scatola_file_is_symbolic_link (get_target_file (window))) {
		return TRUE;
	}
...
create_basic_page (FMPropertiesWindow *window)
{
	...
	if (SCATOLA_IS_ENTRY (window->details->name_field)) {
		scatola_entry_select_all (SCATOLA_ENTRY (window->details->name_field));
	...
	if (is_multi_file_window (window) ||
	    scatola_file_is_directory (get_target_file (window))) {
...
files_has_changable_permissions_directory (FMPropertiesWindow *window)
{
	...
	for (l = window->details->target_files; l != NULL; l = l->next) {
		...
		if (scatola_file_is_directory (file) &&
		    ...
		    scatola_file_can_set_permissions (file)) {
			return TRUE;
		}
...
/*stop GTK 3.22 builds from ballooning the properties dialog to full screen height */
	...
	icons = scatola_emblem_list_available ();
	...
	while (l != NULL) {
		...
		if (!scatola_emblem_should_show_in_list (emblem_name)) {
			continue;
		}

		info = scatola_icon_info_lookup_from_name (emblem_name, SCATOLA_ICON_SIZE_SMALL);
		pixbuf = scatola_icon_info_get_pixbuf_nodefault_at_size (info, SCATOLA_ICON_SIZE_SMALL);

		if (pixbuf == NULL) {
			continue;
		}

		label = g_strdup (scatola_icon_info_get_display_name (info));
		g_object_unref (info);

		if (label == NULL) {
			label = scatola_emblem_get_keyword_from_icon_name (emblem_name);
		}
		...
		g_object_set_data_full (G_OBJECT (button), "scatola_emblem_name",
					scatola_emblem_get_keyword_from_icon_name (emblem_name), g_free);
...
static void
permission_change_callback (ScatolaFile *file,
			    GFile *res_loc,
			    GError *error,
			    gpointer callback_data)
{
		    ...
		    gboolean use_original)
{
	...
	for (l = window->details->target_files; l != NULL; l = l->next) {
		...
		if (!apply_to_both_folder_and_dir &&
		    ((scatola_file_is_directory (file) && !is_folder) ||
		     (!scatola_file_is_directory (file) && is_folder))) {
			continue;
		}

		permissions = scatola_file_get_permissions (file);
		if (use_original) {
			gpointer ptr;
			  ...
			  GtkToggleButton *button)
{
	...
	for (l = window->details->target_files; l != NULL; l = l->next) {
		...
		file_permissions = scatola_file_get_permissions (file);
...
create_simple_permissions (FMPropertiesWindow *window, GtkGrid *page_grid)
{
	...
	if (!is_multi_file_window (window) && scatola_file_can_set_owner (get_target_file (window))) {
	...
	if (!is_multi_file_window (window) && scatola_file_can_set_group (get_target_file (window))) {
			 ...
			 FMPropertiesWindow *window)
{
	...
	for (l = window->details->target_files; l != NULL; l = l->next) {
		...
		if (scatola_file_is_directory (file) &&
			...
			start_long_operation (window);
			g_object_ref (window);
			scatola_file_set_permissions_recursive (uri,
								 file_permission,
...
create_permissions_page (FMPropertiesWindow *window)
{
	...
	if (all_can_get_permissions (file_list) && all_can_get_permissions (window->details->target_files)) {
		...
		if (g_settings_get_boolean (scatola_preferences, SCATOLA_PREFERENCES_SHOW_ADVANCED_PERMISSIONS)) {
	...
	if (all_can_get_permissions (file_list) && all_can_get_permissions (window->details->target_files)) {
		...
		if (!is_multi_file_window (window)) {
			file_name = scatola_file_get_display_name (get_target_file (window));
...
append_extension_pages (FMPropertiesWindow *window)
{
	...
	providers = scatola_extensions_get_for_type (SCATOLA_TYPE_PROPERTY_PAGE_PROVIDER);
	 ...
	 */
	module_providers = scatola_module_get_extensions_for_type (SCATOLA_TYPE_PROPERTY_PAGE_PROVIDER);
	for (p = module_providers; p != NULL; p = p->next) {
		const gchar *type_name = G_OBJECT_TYPE_NAME (G_OBJECT (p->data));
		if (g_strcmp0 (type_name, "ScatolaNotesViewerProvider") == 0 ||
		    g_strcmp0 (type_name, "ScatolaImagePropertiesPageProvider") == 0) {
			providers = g_list_prepend (providers, p->data);
		}
	}

	for (p = providers; p != NULL; p = p->next) {
		ScatolaPropertyPageProvider *provider;
		GList *pages;
		GList *l;

		provider = SCATOLA_PROPERTY_PAGE_PROVIDER (p->data);

		pages = scatola_property_page_provider_get_pages
		...
		for (l = pages; l != NULL; l = l->next) {
			...
			page = SCATOLA_PROPERTY_PAGE (l->data);
...
get_pending_key (GList *file_list)
{
	...
	uris = NULL;
	for (l = file_list; l != NULL; l = l->next) {
		uris = g_list_prepend (uris, scatola_file_get_uri (SCATOLA_FILE (l->data)));
	}
		  ...
		  GtkWidget *parent_widget)
{
	...
	data->original_files = scatola_file_list_copy (original_files);
	data->target_files = scatola_file_list_copy (target_files);
...
{
	scatola_file_list_free (data->original_files);
	scatola_file_list_free (data->target_files);
...
static void
file_changed_callback (ScatolaFile *file, gpointer user_data)
{
...
static gboolean
is_a_special_file (ScatolaFile *file)
{
	if (file == NULL ||
...
create_open_with_page (FMPropertiesWindow *window)
{
	...
	mime_type = scatola_file_get_mime_type (get_target_file (window));

	if (!is_multi_file_window (window)) {
		uri = scatola_file_get_uri (get_target_file (window));
		if (uri == NULL) {
			return;
		}
		vbox = scatola_mime_application_chooser_new (uri, mime_type);
		...
		vbox = scatola_mime_application_chooser_new_for_multiple_files (uris, mime_type);
	}
...
create_properties_window (StartupData *startup_data)
{
	...
	window->details->original_files = scatola_file_list_copy (startup_data->original_files);

	window->details->target_files = scatola_file_list_copy (startup_data->target_files);

	gtk_window_set_wmclass (GTK_WINDOW (window), "file_properties", "Scatola");
	...
	for (l = window->details->target_files; l != NULL; l = l->next) {
		...
		attributes = 0;
		if (scatola_file_is_directory (file)) {
			attributes |= SCATOLA_FILE_ATTRIBUTE_DEEP_COUNTS;
		}

		attributes |= SCATOLA_FILE_ATTRIBUTE_INFO;
		scatola_file_monitor_add (file, &window->details->target_files, attributes);
	}

	for (l = window->details->target_files; l != NULL; l = l->next) {
		g_signal_connect_object (SCATOLA_FILE (l->data),
					 "changed",
					 G_CALLBACK (file_changed_callback),
					 G_OBJECT (window),
					 0);
...
get_target_file_list (GList *original_files)
{
	...
	for (l = original_files; l != NULL; l = l->next) {
		...
		target = get_target_file_for_original_file (SCATOLA_FILE (l->data));
...
static void
is_directory_ready_callback (ScatolaFile *file,
			     gpointer data)
{
			      ...
			      GtkWidget *parent_widget)
{
	...
	scatola_file_list_free (target_files);
	g_free(pending_key);
	       ...
	       int        response)
{
	...
	case GTK_RESPONSE_HELP:
		gtk_show_uri (gtk_window_get_screen (GTK_WINDOW (dialog)),
			      "help:gde2-user-guide/gosscatola-51",
...
#endif
{
	...
	for (l = window->details->original_files; l != NULL; l = l->next) {
		scatola_file_monitor_remove (SCATOLA_FILE (l->data), &window->details->original_files);
	}
	scatola_file_list_free (window->details->original_files);
	window->details->original_files = NULL;

	for (l = window->details->target_files; l != NULL; l = l->next) {
		scatola_file_monitor_remove (SCATOLA_FILE (l->data), &window->details->target_files);
	}
	scatola_file_list_free (window->details->target_files);
...
set_icon (const char* icon_uri, FMPropertiesWindow *properties_window)
{
	...
	if (icon_path != NULL) {
		...
		for (l = properties_window->details->original_files; l != NULL; l = l->next) {
			...
			if (scatola_file_is_mime_type (file, "application/x-desktop")) {
				if (scatola_link_local_set_icon (file_uri, icon_path)) {
				...
				scatola_file_set_metadata (file, SCATOLA_METADATA_KEY_CUSTOM_ICON, NULL, real_icon_uri);
				scatola_file_set_metadata (file, SCATOLA_METADATA_KEY_ICON_SCALE, NULL, NULL);
			 ...
			 FMPropertiesWindow *window)
{
	...
	if (pixbuf != NULL) {
		...
		if (gdk_pixbuf_get_width (pixbuf) > PREVIEW_IMAGE_WIDTH) {
			...
			scaled_pixbuf = gde2_desktop_thumbnail_scale_down_pixbuf
				(pixbuf,
				 PREVIEW_IMAGE_WIDTH,
			      ...
			      FMPropertiesWindow *window)
{
	...
	if (g_list_length (window->details->original_files) == 1) {
		file = SCATOLA_FILE (window->details->original_files->data);
	...
	for (l = window->details->original_files; l != NULL; l = l->next) {
		file = SCATOLA_FILE (l->data);
		image_path = scatola_file_get_metadata (file, SCATOLA_METADATA_KEY_CUSTOM_ICON, NULL);
