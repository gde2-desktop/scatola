...
#include <libscatola-private/scatola-icon-dnd.h>
...
#include "scatola-audio-mime-types.h"
...
typedef struct
{
    const ScatolaFileSortType sort_type;
...
struct FMIconViewDetails
{
    ...
    ScatolaFile *audio_preview_file;
...
static const SortCriterion *get_sort_criterion_by_sort_type           (ScatolaFileSortType  sort_type);
...
static ScatolaFileSortType get_default_sort_order                    (ScatolaFile         *file,
        gboolean             *reversed);
...
static void fm_icon_view_iface_init (ScatolaViewIface *iface);

G_DEFINE_TYPE_WITH_CODE (FMIconView, fm_icon_view, FM_TYPE_DIRECTORY_VIEW,
                         G_IMPLEMENT_INTERFACE (SCATOLA_TYPE_VIEW,
...
#endif
{
    ...
    {
        g_signal_handler_disconnect (scatola_clipboard_monitor_get (),
    ...
    {
        scatola_file_list_free (icon_view->details->icons_not_positioned);
...
fm_icon_view_finalize (GObject *object)
{
    ...
    g_signal_handlers_disconnect_by_func (scatola_icon_view_preferences,
                                          ...
                                          icon_view);
    g_signal_handlers_disconnect_by_func (scatola_compact_view_preferences,
                                          default_zoom_level_changed_callback,
                                          icon_view);
    g_signal_handlers_disconnect_by_func (scatola_compact_view_preferences,
...
static ScatolaIconContainer *
get_icon_container (FMIconView *icon_view)
{
    return SCATOLA_ICON_CONTAINER (gtk_bin_get_child (GTK_BIN (icon_view)));
}

static gboolean
get_stored_icon_position_callback (ScatolaIconContainer *container,
                                   ScatolaFile *file,
                                   ...
                                   FMIconView *icon_view)
{
    ...
    position_string = scatola_file_get_metadata
                      (file, SCATOLA_METADATA_KEY_ICON_POSITION, "");
    position_good = sscanf
                    (position_string, " %d , %d %c",
                     &position->x, &position->y, &c) == 2;
    g_free (position_string);

    /* If it is the desktop directory, maybe the gde2-libs metadata has information about it */
    ...
    if (fm_icon_view_supports_scaling (icon_view))
    {
        ...
        scale_string = scatola_file_get_metadata
                       (file, SCATOLA_METADATA_KEY_ICON_SCALE, "1");
                         ...
                         gboolean clear)
{
    ...
    if (clear)
    {
        scatola_file_set_metadata (file,
                                SCATOLA_METADATA_KEY_ICON_VIEW_SORT_BY, NULL, NULL);
        scatola_file_set_metadata (file,
                                SCATOLA_METADATA_KEY_ICON_VIEW_SORT_REVERSED, NULL, NULL);
...
fm_icon_view_real_clean_up (FMIconView *icon_view)
{
    ...
    scatola_icon_container_sort (icon_container);
    scatola_icon_container_freeze_icon_positions (icon_container);
...
set_tighter_layout (FMIconView *icon_view, gboolean new_value)
{
    fm_icon_view_set_directory_tighter_layout (icon_view,
            ...
            new_value);
    scatola_icon_container_set_tighter_layout (get_icon_container (icon_view),
                                            new_value);
}
...
{
    return scatola_icon_container_is_auto_layout
...
{
    return scatola_icon_container_is_tighter_layout
           (get_icon_container (icon_view));
}
                            ...
                            FMIconView *view)
{
     ...
     */
    if (sort_type == SCATOLA_FILE_SORT_NONE)
    {
...
static void
list_covers (ScatolaIconData *data, gpointer callback_data)
{
    GSList **file_list;
...
static void
unref_cover (ScatolaIconData *data, gpointer callback_data)
{
    scatola_file_unref (SCATOLA_FILE (data));
}
...
fm_icon_view_clear (FMDirectoryView *view)
{
    ...
    file_list = NULL;
    scatola_icon_container_for_each (icon_container, list_covers, &file_list);
    scatola_icon_container_clear (icon_container);
...
static gboolean
should_show_file_on_screen (FMDirectoryView *view, ScatolaFile *file)
{
    FMIconView *icon_view;
...
static void
fm_icon_view_remove_file (FMDirectoryView *view, ScatolaFile *file, ScatolaDirectory *directory)
{
    FMIconView *icon_view;
    ...
    if (directory != fm_directory_view_get_model (view))
    {
        ...
        file_uri = scatola_file_get_uri (file);
        dir_uri = scatola_directory_get_uri (directory);
        model_uri = scatola_directory_get_uri (fm_directory_view_get_model (view));
    ...
    if (scatola_icon_container_remove (get_icon_container (icon_view),
                                    SCATOLA_ICON_CONTAINER_ICON_DATA (file)))
    {
...
static void
fm_icon_view_add_file (FMDirectoryView *view, ScatolaFile *file, ScatolaDirectory *directory)
{
    FMIconView *icon_view;
    ScatolaIconContainer *icon_container;
    ...
    if (fm_directory_view_get_loading (view) && scatola_icon_container_is_empty (icon_container))
    {
        scatola_icon_container_reset_scroll_region (icon_container);
    }

    if (scatola_icon_container_add (icon_container,
                                 SCATOLA_ICON_CONTAINER_ICON_DATA (file)))
    {
...
{
    scatola_icon_container_layout_now (get_icon_container (FM_ICON_VIEW (view)));
}

static void
fm_icon_view_file_changed (FMDirectoryView *view, ScatolaFile *file, ScatolaDirectory *directory)
{
    FMIconView *icon_view;
...
update_layout_menus (FMIconView *view)
{
    ...
    if (fm_icon_view_supports_auto_layout (view))
    {
        ...
        if (file != NULL && scatola_file_is_in_trash (file))
        {
    ...
    gtk_toggle_action_set_active (GTK_TOGGLE_ACTION (action),
                                  scatola_icon_container_is_keep_aligned (get_icon_container (view)));
    gtk_action_set_sensitive (action, !is_auto_layout);
}
...
static ScatolaFileSortType default_sort_order = SCATOLA_FILE_SORT_BY_DISPLAY_NAME;

static ScatolaFileSortType
get_default_sort_order (ScatolaFile *file, gboolean *reversed)
{
    ...
    if (auto_storaged_added == FALSE)
    {
        ...
        eel_g_settings_add_auto_enum (scatola_preferences,
                                      SCATOLA_PREFERENCES_DEFAULT_SORT_ORDER,
                                      (int *) &default_sort_order);
        eel_g_settings_add_auto_boolean (scatola_preferences,
                                         SCATOLA_PREFERENCES_DEFAULT_SORT_IN_REVERSE_ORDER,
                                         &default_sort_in_reverse_order);
    }

    retval = scatola_file_get_default_sort_type (file, reversed);

    if (retval == SCATOLA_FILE_SORT_NONE)
    {
        ...
        retval = CLAMP (default_sort_order, SCATOLA_FILE_SORT_BY_DISPLAY_NAME,
                        SCATOLA_FILE_SORT_BY_EMBLEMS);
    }
        ...
        ScatolaFile *file)
{
    ...
    return scatola_file_get_metadata
           (file, SCATOLA_METADATA_KEY_ICON_VIEW_SORT_BY,
        ...
        ScatolaFile *file)
{
    ...
    return scatola_file_get_boolean_metadata
           (file,
        ...
        gboolean sort_reversed)
{
    ...
    get_default_sort_order (file, &reversed);
    scatola_file_set_boolean_metadata
    (file,
     SCATOLA_METADATA_KEY_ICON_VIEW_SORT_REVERSED,
        ...
        gboolean keep_aligned)
{
    ...
    scatola_file_set_boolean_metadata
    (file, SCATOLA_METADATA_KEY_ICON_VIEW_KEEP_ALIGNED,
     get_default_directory_keep_aligned (),
     keep_aligned);
}
        ...
        ScatolaFile *file)
{
    ...
    return scatola_file_get_boolean_metadata
           (file, SCATOLA_METADATA_KEY_ICON_VIEW_AUTO_LAYOUT, TRUE);
}
        ...
        gboolean auto_layout)
{
    ...
    scatola_file_set_boolean_metadata
    (file, SCATOLA_METADATA_KEY_ICON_VIEW_AUTO_LAYOUT,
     TRUE,
     auto_layout);
}
...
get_default_directory_tighter_layout (void)
{
    ...
    {
        auto_storaged_added = TRUE;
        eel_g_settings_add_auto_boolean (scatola_icon_view_preferences,
                                         SCATOLA_PREFERENCES_ICON_VIEW_DEFAULT_USE_TIGHTER_LAYOUT,
        ...
        gboolean tighter_layout)
{
    ...
    scatola_file_set_boolean_metadata
    (file, SCATOLA_METADATA_KEY_ICON_VIEW_TIGHTER_LAYOUT,
     get_default_directory_tighter_layout (),
     tighter_layout);
}
...
static const SortCriterion *
get_sort_criterion_by_sort_type (ScatolaFileSortType sort_type)
{
    guint i;
...
static ScatolaZoomLevel default_zoom_level = SCATOLA_ZOOM_LEVEL_STANDARD;
static ScatolaZoomLevel default_compact_zoom_level = SCATOLA_ZOOM_LEVEL_STANDARD;
#define DEFAULT_ZOOM_LEVEL(icon_view) icon_view->details->compact ? default_compact_zoom_level : default_zoom_level

static ScatolaZoomLevel
get_default_zoom_level (FMIconView *icon_view)
{
    ...
    if (!auto_storage_added)
    {
        ...
        eel_g_settings_add_auto_enum (scatola_icon_view_preferences,
                                      SCATOLA_PREFERENCES_ICON_VIEW_DEFAULT_ZOOM_LEVEL,
                                      (int *) &default_zoom_level);
        eel_g_settings_add_auto_enum (scatola_compact_view_preferences,
                                      SCATOLA_PREFERENCES_COMPACT_VIEW_DEFAULT_ZOOM_LEVEL,
                                      (int *) &default_compact_zoom_level);
    }

    return CLAMP (DEFAULT_ZOOM_LEVEL(icon_view), SCATOLA_ZOOM_LEVEL_SMALLEST, SCATOLA_ZOOM_LEVEL_LARGEST);
}
...
set_labels_beside_icons (FMIconView *icon_view)
{
    ...
    if (fm_icon_view_supports_labels_beside_icons (icon_view))
    {
        labels_beside = fm_icon_view_is_compact (icon_view) ||
                        g_settings_get_boolean (scatola_icon_view_preferences, SCATOLA_PREFERENCES_ICON_VIEW_LABELS_BESIDE_ICONS);

        if (labels_beside)
        {
            scatola_icon_container_set_label_position
            (get_icon_container (icon_view),
...
set_columns_same_width (FMIconView *icon_view)
{
    ...
    {
        all_columns_same_width = g_settings_get_boolean (scatola_compact_view_preferences, SCATOLA_PREFERENCES_COMPACT_VIEW_ALL_COLUMNS_SAME_WIDTH);
        scatola_icon_container_set_all_columns_same_width (get_icon_container (icon_view), all_columns_same_width);
    }
}
...
fm_icon_view_begin_loading (FMDirectoryView *view)
{
    ...
    scatola_icon_container_begin_loading (SCATOLA_ICON_CONTAINER (icon_container));

    scatola_icon_container_set_allow_moves (SCATOLA_ICON_CONTAINER (icon_container),
    ...
    if (FM_IS_DESKTOP_ICON_VIEW (view))
    {
        scatola_connect_desktop_background_to_settings (SCATOLA_ICON_CONTAINER (icon_container));
    }
    else
    {
        GdkDragAction default_action;

        if (scatola_window_info_get_window_type (fm_directory_view_get_scatola_window (view)) == SCATOLA_WINDOW_NAVIGATION)
        {
            default_action = SCATOLA_DND_ACTION_SET_AS_GLOBAL_BACKGROUND;
        }
        else
        {
            default_action = SCATOLA_DND_ACTION_SET_AS_FOLDER_BACKGROUND;
        }

        scatola_connect_background_to_file_metadata (icon_container, file, default_action);
    }
    ...
    if (fm_directory_view_supports_zooming (FM_DIRECTORY_VIEW (icon_view)))
    {
        ...
        {
            level = scatola_file_get_integer_metadata
                    (file,
    ...
    scatola_icon_container_set_keep_aligned
    (get_icon_container (icon_view),
     fm_icon_view_get_directory_keep_aligned (icon_view, file));
    scatola_icon_container_set_tighter_layout
    (get_icon_container (icon_view),
     ...
     */
    scatola_icon_container_set_auto_layout
    (get_icon_container (icon_view),
...
static void
icon_view_notify_clipboard_info (ScatolaClipboardMonitor *monitor,
                          ...
                          gboolean all_files_seen)
{
    FMIconView *icon_view;
    GtkWidget *icon_container;
    ScatolaClipboardMonitor *monitor;
    ScatolaClipboardInfo *info;

    icon_view = FM_ICON_VIEW (view);

    icon_container = GTK_WIDGET (get_icon_container (icon_view));
    scatola_icon_container_end_loading (SCATOLA_ICON_CONTAINER (icon_container), all_files_seen);

    monitor = scatola_clipboard_monitor_get ();
    info = scatola_clipboard_monitor_get_clipboard_info (monitor);
...
{
    g_return_val_if_fail (FM_IS_ICON_VIEW (view), SCATOLA_ZOOM_LEVEL_STANDARD);

    return scatola_icon_container_get_zoom_level (get_icon_container (FM_ICON_VIEW (view)));
}
                             ...
                             gboolean always_emit)
{
    ...
    g_return_if_fail (FM_IS_ICON_VIEW (view));
    g_return_if_fail (new_level >= SCATOLA_ZOOM_LEVEL_SMALLEST &&
                      new_level <= SCATOLA_ZOOM_LEVEL_LARGEST);

    icon_container = get_icon_container (view);
    if (scatola_icon_container_get_zoom_level (icon_container) == new_level)
    {
        if (always_emit)
        {
    ...
    {
        scatola_file_set_integer_metadata
    ...
    scatola_icon_container_set_zoom_level (icon_container, new_level);
...
fm_icon_view_bump_zoom_level (FMDirectoryView *view, int zoom_increment)
{
    ...
    if (new_level >= SCATOLA_ZOOM_LEVEL_SMALLEST &&
            new_level <= SCATOLA_ZOOM_LEVEL_LARGEST)
    {
...
fm_icon_view_is_empty (FMDirectoryView *view)
{
    ...
    return scatola_icon_container_is_empty
...
fm_icon_view_get_selection (FMDirectoryView *view)
{
    ...
    list = scatola_icon_container_get_selection
           (get_icon_container (FM_ICON_VIEW (view)));
...
static void
count_item (ScatolaIconData *icon_data,
            gpointer callback_data)
{
    guint *count;
...
fm_icon_view_get_item_count (FMDirectoryView *view)
{
    ...
    scatola_icon_container_for_each
    (get_icon_container (FM_ICON_VIEW (view)),
     count_item, &count);
                                 ...
                                 ScatolaFileSortType  sort_type)
{
    ...
    scatola_icon_container_sort (get_icon_container (icon_view));
                              ...
                              gpointer user_data)
{
    ...
    scatola_icon_container_set_keep_aligned (get_icon_container (icon_view),
                                          keep_aligned);
}
...
static void
layout_changed_callback (ScatolaIconContainer *container,
                         FMIconView *icon_view)
{
    ScatolaFile *file;
...
static gboolean
fm_icon_view_can_rename_file (FMDirectoryView *view, ScatolaFile *file)
{
    if (!(fm_icon_view_get_zoom_level (view) > SCATOLA_ZOOM_LEVEL_SMALLEST))
    {
        return FALSE;
    }
...
fm_icon_view_merge_menus (FMDirectoryView *view)
{
    ...
    ui = scatola_ui_string_get ("scatola-icon-view-ui.xml");
...
fm_icon_view_update_menus (FMDirectoryView *view)
{
    ...
    gtk_action_set_sensitive (action,
                              selection_count == 1
                              && icon_container != NULL
                              && !scatola_icon_container_has_stretch_handles (icon_container));
    ...
    gtk_action_set_sensitive (action,
                              icon_container != NULL
                              && scatola_icon_container_is_stretched (icon_container));
...
fm_icon_view_select_all (FMDirectoryView *view)
{
    ...
    icon_container = get_icon_container (FM_ICON_VIEW (view));
    scatola_icon_container_select_all (icon_container);
}
...
fm_icon_view_get_selected_icon_locations (FMDirectoryView *view)
{
    ...
    return scatola_icon_container_get_selected_icon_locations
           (get_icon_container (FM_ICON_VIEW (view)));
}
        ...
        GdkPoint *position)
{
    ...
    scatola_icon_container_widget_to_file_operation_position
    (get_icon_container (FM_ICON_VIEW (view)), position);
}

static void
icon_container_activate_callback (ScatolaIconContainer *container,
                                  GList *file_list,
                                  FMIconView *icon_view)
{
    g_assert (FM_IS_ICON_VIEW (icon_view));
    ...
    fm_directory_view_activate_files (FM_DIRECTORY_VIEW (icon_view),
                                      file_list,
                                      SCATOLA_WINDOW_OPEN_ACCORDING_TO_MODE, 0,
                                      TRUE);
}

static void
icon_container_activate_alternate_callback (ScatolaIconContainer *container,
        GList *file_list,
        FMIconView *icon_view)
{
    GdkEvent *event;
    ...
    window_info = fm_directory_view_get_scatola_window (FM_DIRECTORY_VIEW (icon_view));

    if (scatola_window_info_get_window_type (window_info) == SCATOLA_WINDOW_NAVIGATION)
    {
...
static void
band_select_started_callback (ScatolaIconContainer *container,
                              FMIconView *icon_view)
{
    g_assert (FM_IS_ICON_VIEW (icon_view));
    g_assert (container == get_icon_container (icon_view));
...
static void
band_select_ended_callback (ScatolaIconContainer *container,
                            FMIconView *icon_view)
{
    g_assert (FM_IS_ICON_VIEW (icon_view));
    g_assert (container == get_icon_container (icon_view));
...
play_file (gpointer callback_data)
{
    ...
    gfile = scatola_file_get_location (file);
...
static void
preview_audio (FMIconView *icon_view, ScatolaFile *file, gboolean start_flag)
{
...
static gboolean
sound_preview_type_supported (ScatolaFile *file)
{
    char *mime_type;
    guint i;
...
static gboolean
should_preview_sound (ScatolaFile *file)
{
    GFile *location;
    ...
    if (preview_sound_auto_value == SCATOLA_SPEED_TRADEOFF_NEVER)
    {
        return FALSE;
    }

    if (preview_sound_auto_value == SCATOLA_SPEED_TRADEOFF_ALWAYS)
    {
...
static int
icon_container_preview_callback (ScatolaIconContainer *container,
                                 ScatolaFile *file,
                                 gboolean start_flag,
                                 FMIconView *icon_view)
{
     ...
     */
    if (fm_icon_view_get_zoom_level (FM_DIRECTORY_VIEW (icon_view)) <= SCATOLA_ZOOM_LEVEL_SMALLER)
    {
        if (start_flag)
        {
            file_name = scatola_file_get_display_name (file);
            message = g_strdup_printf (_("pointing at \"%s\""), file_name);
            g_free (file_name);
            scatola_window_slot_info_set_status
            (fm_directory_view_get_scatola_window_slot (FM_DIRECTORY_VIEW (icon_view)),
             message);
            g_free (message);
        }
        else
...
static void
renaming_icon_callback (ScatolaIconContainer *container,
                        GtkWidget *widget,
                        gpointer callback_data)
{
    ...
    scatola_clipboard_set_up_editable
    (GTK_EDITABLE (widget),
...
fm_icon_view_compare_files (FMIconView   *icon_view,
                            ...
                            ScatolaFile *b)
{
    return scatola_file_compare_for_sort
                             ...
                             GdkScreen *previous_screen)
{
    ...
    if (FM_ICON_VIEW (view)->details->filter_by_screen)
    {
        ...
        files = scatola_directory_get_file_list (directory);
...
static void
selection_changed_callback (ScatolaIconContainer *container,
                            FMIconView *icon_view)
{
    g_assert (FM_IS_ICON_VIEW (icon_view));
    g_assert (container == get_icon_container (icon_view));
...
static void
icon_container_context_click_selection_callback (ScatolaIconContainer *container,
...
static void
icon_container_context_click_background_callback (ScatolaIconContainer *container,
        GdkEventButton *event,
        FMIconView *icon_view)
{
    g_assert (SCATOLA_IS_ICON_CONTAINER (container));
...
static void
icon_position_changed_callback (ScatolaIconContainer *container,
                                ScatolaFile *file,
                                ...
                                FMIconView *icon_view)
{
    ...
    scatola_file_set_metadata
    (file, SCATOLA_METADATA_KEY_ICON_SCALE,
     "1.0", scale_string);
}

/* Attempt to change the filename to the new text.  Notify user if operation fails. */
static void
fm_icon_view_icon_text_changed_callback (ScatolaIconContainer *container,
        ScatolaFile *file,
        char *new_name,
        FMIconView *icon_view)
{
...
static char *
get_icon_uri_callback (ScatolaIconContainer *container,
                       ScatolaFile *file,
                       FMIconView *icon_view)
{
    g_assert (SCATOLA_IS_ICON_CONTAINER (container));
...
static char *
get_icon_drop_target_uri_callback (ScatolaIconContainer *container,
                                   ScatolaFile *file,
                                   FMIconView *icon_view)
{
    ...
    return scatola_file_get_drop_target_uri (file);
}
...
fm_icon_view_text_attribute_names_changed (FMDirectoryView *directory_view)
{
    ...
    scatola_icon_container_request_update_all (get_icon_container (FM_ICON_VIEW (directory_view)));
}
...
default_sort_order_changed_callback (gpointer callback_data)
{
    ...
    icon_container = get_icon_container (icon_view);
    g_return_if_fail (SCATOLA_IS_ICON_CONTAINER (icon_container));

    scatola_icon_container_request_update_all (icon_container);
}
...
default_zoom_level_changed_callback (gpointer callback_data)
{
    ...
    if (fm_directory_view_supports_zooming (FM_DIRECTORY_VIEW (icon_view)))
    {
        ...
        if (fm_icon_view_is_compact (icon_view))
        {
            level = scatola_file_get_integer_metadata (file,
                                                    SCATOLA_METADATA_KEY_COMPACT_VIEW_ZOOM_LEVEL,
                                                    get_default_zoom_level (icon_view));
        }
        else
...
fm_icon_view_sort_directories_first_changed (FMDirectoryView *directory_view)
{
    ...
    if (fm_icon_view_using_auto_layout (icon_view))
    {
        scatola_icon_container_sort
        (get_icon_container (icon_view));
    }
}

static gboolean
icon_view_can_accept_item (ScatolaIconContainer *container,
                           ScatolaFile *target_item,
                           const char *item_uri,
                           FMDirectoryView *view)
{
...
static char *
icon_view_get_container_uri (ScatolaIconContainer *container,
                             FMDirectoryView *view)
{
...
static void
icon_view_move_copy_items (ScatolaIconContainer *container,
...
fm_icon_view_update_click_mode (FMIconView *icon_view)
{
    ...
    click_mode = g_settings_get_enum (scatola_preferences, SCATOLA_PREFERENCES_CLICK_POLICY);

    scatola_icon_container_set_single_click_mode (icon_container,
            click_mode == SCATOLA_CLICK_POLICY_SINGLE);
}

static gboolean
get_stored_layout_timestamp (ScatolaIconContainer *container,
                             ...
                             FMIconView *view)
{
    ...
    if (icon_data == NULL)
    {
        ...
        *timestamp = scatola_file_get_time_metadata (file,
                     SCATOLA_METADATA_KEY_ICON_VIEW_LAYOUT_TIMESTAMP);
        scatola_file_unref (file);
    }
    else
    {
        *timestamp = scatola_file_get_time_metadata (SCATOLA_FILE (icon_data),
...
static gboolean
store_layout_timestamp (ScatolaIconContainer *container,
                        ScatolaIconData *icon_data,
                        ...
                        FMIconView *view)
{
    ...
    if (icon_data == NULL)
    {
        ...
        file = scatola_directory_get_corresponding_file (directory);
        scatola_file_set_time_metadata (file,
                                     SCATOLA_METADATA_KEY_ICON_VIEW_LAYOUT_TIMESTAMP,
                                     (time_t) *timestamp);
        scatola_file_unref (file);
    }
    ...
    {
        scatola_file_set_time_metadata (SCATOLA_FILE (icon_data),
                                     SCATOLA_METADATA_KEY_ICON_POSITION_TIMESTAMP,
                                     (time_t) *timestamp);
    }
...
focus_in_event_callback (GtkWidget *widget, GdkEventFocus *event, gpointer user_data)
{
    ...
    slot_info = fm_directory_view_get_scatola_window_slot (FM_DIRECTORY_VIEW (icon_view));
    scatola_window_slot_info_make_hosting_pane_active (slot_info);
...
static void
icon_view_handle_netscape_url (ScatolaIconContainer *container, const char *encoded_url,
...
static void
icon_view_handle_uri_list (ScatolaIconContainer *container, const char *item_uris,
                           const char *target_uri,
                           GdkDragAction action, int x, int y, FMIconView *view)
{
...
static void
icon_view_handle_text (ScatolaIconContainer *container, const char *text,
                       const char *target_uri,
                       GdkDragAction action, int x, int y, FMIconView *view)
{
...
static void
icon_view_handle_raw (ScatolaIconContainer *container, const char *raw_data,
...
static char *
icon_view_get_first_visible_file (ScatolaView *view)
{
    ScatolaFile *file;
    FMIconView *icon_view;
    ...
    file = SCATOLA_FILE (scatola_icon_container_get_first_visible_icon (get_icon_container (icon_view)));
...
static void
icon_view_scroll_to_file (ScatolaView *view,
                          const char *uri)
{
    ScatolaFile *file;
    FMIconView *icon_view;
    ...
    if (uri != NULL)
    {
        ...
        file = scatola_file_get_existing_by_uri (uri);
        if (file != NULL)
        {
            scatola_icon_container_scroll_to_icon (get_icon_container (icon_view),
                                                SCATOLA_ICON_CONTAINER_ICON_DATA (file));
            scatola_file_unref (file);
        }
    }
                           ...
                           GParamSpec      *pspec)
{
    ...
    case PROP_COMPACT:
        ...
        if (icon_view->details->compact)
        {
            scatola_icon_container_set_layout_mode (get_icon_container (icon_view),
                                                 gtk_widget_get_direction (GTK_WIDGET(icon_view)) == GTK_TEXT_DIR_RTL ?
                                                 SCATOLA_ICON_LAYOUT_T_B_R_L :
                                                 SCATOLA_ICON_LAYOUT_T_B_L_R);
            scatola_icon_container_set_forced_icon_size (get_icon_container (icon_view),
                    SCATOLA_ICON_SIZE_SMALLEST);
        }
        break;
...
static const char *
fm_icon_view_get_id (ScatolaView *view)
{
    if (FM_IS_DESKTOP_ICON_VIEW (view))
    {
...
static void
fm_icon_view_iface_init (ScatolaViewIface *iface)
{
...
fm_icon_view_init (FMIconView *icon_view)
{
    ...
    scatola_icon_container_set_layout_mode (icon_container,
    ...
    {
        eel_g_settings_add_auto_enum (scatola_preferences,
                                      SCATOLA_PREFERENCES_PREVIEW_SOUND,
    ...
    g_signal_connect_swapped (scatola_preferences,
                              "changed::" SCATOLA_PREFERENCES_DEFAULT_SORT_ORDER,
                              G_CALLBACK (default_sort_order_changed_callback),
                              icon_view);
    g_signal_connect_swapped (scatola_preferences,
                              "changed::" SCATOLA_PREFERENCES_DEFAULT_SORT_IN_REVERSE_ORDER,
                              G_CALLBACK (default_sort_in_reverse_order_changed_callback),
                              icon_view);
    g_signal_connect_swapped (scatola_icon_view_preferences,
                              "changed::" SCATOLA_PREFERENCES_ICON_VIEW_DEFAULT_USE_TIGHTER_LAYOUT,
                              G_CALLBACK (default_use_tighter_layout_changed_callback),
                              icon_view);
    g_signal_connect_swapped (scatola_icon_view_preferences,
                              "changed::" SCATOLA_PREFERENCES_ICON_VIEW_DEFAULT_ZOOM_LEVEL,
                              G_CALLBACK (default_zoom_level_changed_callback),
                              icon_view);
    g_signal_connect_swapped (scatola_icon_view_preferences,
                              "changed::" SCATOLA_PREFERENCES_ICON_VIEW_LABELS_BESIDE_ICONS,
                              G_CALLBACK (labels_beside_icons_changed_callback),
                              icon_view);

    g_signal_connect_swapped (scatola_compact_view_preferences,
                              "changed::" SCATOLA_PREFERENCES_COMPACT_VIEW_DEFAULT_ZOOM_LEVEL,
                              G_CALLBACK (default_zoom_level_changed_callback),
                              icon_view);
    g_signal_connect_swapped (scatola_compact_view_preferences,
                              "changed::" SCATOLA_PREFERENCES_COMPACT_VIEW_ALL_COLUMNS_SAME_WIDTH,
    ...
    icon_view->details->clipboard_handler_id =
        g_signal_connect (scatola_clipboard_monitor_get (),
                          "clipboard_info",
                          G_CALLBACK (icon_view_notify_clipboard_info), icon_view);
}

static ScatolaView *
fm_icon_view_create (ScatolaWindowSlotInfo *slot)
{
    FMIconView *view;
...
#endif
    return SCATOLA_VIEW (view);
}

static ScatolaView *
fm_compact_view_create (ScatolaWindowSlotInfo *slot)
{
    FMIconView *view;
                           ...
                           const char *mime_type)
{
    ...
    if (strcmp (mime_type, SCATOLA_SAVED_SEARCH_MIMETYPE) == 0)
    {
        return TRUE;
    }
...
static ScatolaViewInfo fm_icon_view =
{
    FM_ICON_VIEW_ID,
...
static ScatolaViewInfo fm_compact_view =
{
    FM_COMPACT_VIEW_ID,
    /* translators: this is used in the view selection dropdown
     * of navigation windows and in the preferences dialog */
...
fm_icon_view_register (void)
{
    ...
    scatola_view_factory_register (&fm_icon_view);
}
...
fm_compact_view_register (void)
{
    ...
    scatola_view_factory_register (&fm_compact_view);
}
