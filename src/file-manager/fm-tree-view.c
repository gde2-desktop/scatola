...
#include <libscatola-private/scatola-file-private.h>
...
struct FMTreeViewDetails
{
    ...
    ScatolaWindowOpenFlags activation_flags;
...
static void  fm_tree_view_iface_init        (ScatolaSidebarIface         *iface);
...
static void
notify_clipboard_info (ScatolaClipboardMonitor *monitor,
...
static gboolean
show_iter_for_file (FMTreeView *view, ScatolaFile *file, GtkTreeIter *iter)
{
    GtkTreeModel *model;
...
show_selection_idle_callback (gpointer callback_data)
{
    ...
    file = scatola_file_get_by_uri (view->details->selection_location);
    if (file == NULL)
    {
        return FALSE;
    }
    ...
    {
        old_file = file;
        file = scatola_file_get_parent (file);
...
row_loaded_callback (GtkTreeModel     *tree_model,
                     ...
                     FMTreeView *view)
{
    ScatolaFile *file, *tmp_file, *selection_file;
    ...
    selection_file = scatola_file_get_by_uri (view->details->selection_location);
    while (selection_file != NULL)
    {
        if (file == selection_file)
        {
            scatola_file_unref (file);
            scatola_file_unref (selection_file);

            schedule_show_selection (view);
            return;
        }
        tmp_file = scatola_file_get_parent (selection_file);
        scatola_file_unref (selection_file);
...
static void
got_activation_uri_callback (ScatolaFile *file, gpointer callback_data)
{
    char *uri, *file_uri;
    FMTreeView *view;
    GdkScreen *screen;
    ...
    open_in_same_slot =
        (view->details->activation_flags &
         (SCATOLA_WINDOW_OPEN_FLAG_NEW_WINDOW |
          SCATOLA_WINDOW_OPEN_FLAG_NEW_TAB)) == 0;

    slot = scatola_window_info_get_active_slot (view->details->window);
    ...
    if (scatola_file_is_launcher (file))
    {
        ...
        scatola_launch_desktop_file (screen, file_uri, NULL, NULL);
        g_free (file_uri);
    }
    else if (uri != NULL
             ...
             && !scatola_file_is_directory (file))
    {
        ...
        else
        {
            scatola_debug_log (FALSE, SCATOLA_DEBUG_LOG_DOMAIN_USER,
                            ...
                            view->details->window, file_uri);
            scatola_launch_application_from_command (screen, NULL, file_uri, FALSE, NULL);
            g_free (file_uri);
        }
    ...
    g_free (uri);
    scatola_file_unref (view->details->activation_file);
...
selection_changed_timer_callback(FMTreeView *view)
{
    ...
    attributes = SCATOLA_FILE_ATTRIBUTE_INFO | SCATOLA_FILE_ATTRIBUTE_LINK_INFO;
    scatola_file_call_when_ready (view->details->activation_file, attributes,
                               got_activation_uri_callback, view);
...
compare_rows (GtkTreeModel *model, GtkTreeIter *a, GtkTreeIter *b, gpointer callback_data)
{
    ...
    {
        result = scatola_file_compare_for_sort (file_a, file_b,
                                             SCATOLA_FILE_SORT_BY_DISPLAY_NAME,
                                             FALSE, FALSE);
    }

    scatola_file_unref (file_a);
    scatola_file_unref (file_b);
...
static gboolean
is_parent_writable (ScatolaFile *file)
{
    ScatolaFile *parent;
    gboolean result;
    ...
    result = scatola_file_can_write (parent);
                         ...
                         FMTreeView *view)
{
    ...
    if (event->button == 3)
    {
        ...
        gtk_widget_set_sensitive (view->details->popup_open_in_new_window,
                                  scatola_file_is_directory (view->details->popup_file));
        gtk_widget_set_sensitive (view->details->popup_create_folder,
                                  scatola_file_is_directory (view->details->popup_file) &&
                                  scatola_file_can_write (view->details->popup_file));
        gtk_widget_set_sensitive (view->details->popup_paste, FALSE);
        if (scatola_file_is_directory (view->details->popup_file) &&
                scatola_file_can_write (view->details->popup_file))
        {
            gtk_clipboard_request_contents (scatola_clipboard_get (GTK_WIDGET (view->details->tree_widget)),
                                            copied_files_atom,
                                            clipboard_contents_received_callback, g_object_ref (view));
        }
        can_move_file_to_trash = scatola_file_can_trash (view->details->popup_file);
        gtk_widget_set_sensitive (view->details->popup_trash, can_move_file_to_trash);

        if (g_settings_get_boolean (scatola_preferences, SCATOLA_PREFERENCES_ENABLE_DELETE))
        {
            parent_file_is_writable = is_parent_writable (view->details->popup_file);
            file_is_home_or_desktop = scatola_file_is_home (view->details->popup_file)
                                      || scatola_file_is_desktop_directory (view->details->popup_file);
            file_is_special_link = SCATOLA_IS_DESKTOP_ICON_FILE (view->details->popup_file);
                            ...
                            ScatolaWindowOpenFlags flags)
{
    ...
    view->details->activation_file = scatola_file_ref (file);
...
fm_tree_view_open_in_new_tab_cb (GtkWidget *menu_item,
                                 FMTreeView *view)
{
    fm_tree_view_activate_file (view, view->details->popup_file, SCATOLA_WINDOW_OPEN_FLAG_NEW_TAB);
}
...
fm_tree_view_open_in_new_window_cb (GtkWidget *menu_item,
                                    FMTreeView *view)
{
    /* fm_tree_view_activate_file (view, view->details->popup_file, SCATOLA_WINDOW_OPEN_FLAG_NEW_WINDOW); */

    scatola_mime_activate_file  (fm_tree_view_get_containing_window (view),
...
new_folder_done (GFile *new_folder, gpointer data)
{
     ...
     */
    list = g_list_prepend (NULL, scatola_file_get (new_folder));
                               ...
                               FMTreeView *view)
{
    ...
    parent_uri = scatola_file_get_uri (view->details->popup_file);
    scatola_file_operations_new_folder (GTK_WIDGET (view->details->tree_widget),
                                     NULL,
                                     parent_uri,
                   ...
                   gboolean cut)
{
    ...
    gtk_clipboard_set_with_data (scatola_clipboard_get (GTK_WIDGET (view->details->tree_widget)),
                                 targets, n_targets,
                                 scatola_get_clipboard_callback, scatola_clear_clipboard_callback,
                                 NULL);
    gtk_target_table_free (targets, n_targets);

    scatola_clipboard_monitor_set_clipboard_info (scatola_clipboard_monitor_get (),
            &info);
    g_list_free (info.files);

    name = scatola_file_get_display_name (view->details->popup_file);
    if (cut)
    {
    ...
    scatola_window_info_push_status (view->details->window,
                                  status_string);
                                        ...
                                        gpointer          data)
{
    ...
    directory_uri = scatola_file_get_uri (view->details->popup_file);
                       ...
                       FMTreeView *view)
{
    ...
    if (!scatola_file_can_trash (view->details->popup_file))
    {
        return;
    }

    list = g_list_prepend (NULL,
                           scatola_file_get_location (view->details->popup_file));

    scatola_file_operations_trash_or_delete (list,
                        ...
                        FMTreeView *view)
{
    ...
    scatola_file_operations_delete (location_list, fm_tree_view_get_containing_window (view), NULL, NULL);
                            ...
                            FMTreeView *view)
{
    ...
    list = g_list_prepend (NULL, scatola_file_ref (view->details->popup_file));
...
fm_tree_view_unmount_cb (GtkWidget *menu_item,
                         FMTreeView *view)
{
    ScatolaFile *file = view->details->popup_file;
    GMount *mount;
    ...
    if (mount != NULL)
    {
        scatola_file_operations_unmount_mount (fm_tree_view_get_containing_window (view),
                                            mount, FALSE, TRUE);
    }
}
...
free_popup_file_in_idle_cb (gpointer data)
{
    ...
    if (view->details->popup_file != NULL)
    {
        scatola_file_unref (view->details->popup_file);
...
create_popup_menu (FMTreeView *view)
{
    ...
    menu_image = gtk_image_new_from_icon_name (SCATOLA_ICON_TRASH_FULL,
                 GTK_ICON_SIZE_MENU);
    gtk_widget_show (menu_image);
    ...
    menu_image = gtk_image_new_from_icon_name (SCATOLA_ICON_DELETE,
                 GTK_ICON_SIZE_MENU);
    gtk_widget_show (menu_image);
...
create_tree (FMTreeView *view)
{
    ...
    home_uri = scatola_get_home_directory_uri ();
    ...
    g_object_unref (icon);
    icon = g_themed_icon_new (SCATOLA_ICON_TRASH);
    ...
    view->details->drag_dest =
        scatola_tree_view_drag_dest_new (view->details->tree_widget);
...
update_filtering_from_preferences (FMTreeView *view)
{
    ...
    if (mode == SCATOLA_WINDOW_SHOW_HIDDEN_FILES_DEFAULT)
    {
        ...
        (view->details->child_model,
         g_settings_get_boolean (scatola_preferences, SCATOLA_PREFERENCES_SHOW_HIDDEN_FILES));
    }
    else
    {
        fm_tree_model_set_show_hidden_files
        (view->details->child_model,
         mode == SCATOLA_WINDOW_SHOW_HIDDEN_FILES_ENABLE);
    }
    fm_tree_model_set_show_only_directories
    (view->details->child_model,
     g_settings_get_boolean (scatola_tree_sidebar_preferences, SCATOLA_PREFERENCES_TREE_SHOW_ONLY_DIRECTORIES));
}
...
fm_tree_view_init (FMTreeView *view)
{
    ...
    g_signal_connect_swapped (scatola_preferences,
                              ...
                              view);
    g_signal_connect_swapped (scatola_tree_sidebar_preferences,
                              "changed::" SCATOLA_PREFERENCES_TREE_SHOW_ONLY_DIRECTORIES,
...
fm_tree_view_dispose (GObject *object)
{
    ...
    g_signal_handlers_disconnect_by_func (scatola_tree_sidebar_preferences,
                                          G_CALLBACK(filtering_changed_callback),
                                          view);
...
static const char *
fm_tree_view_get_sidebar_id (ScatolaSidebar *sidebar)
{
    return TREE_SIDEBAR_ID;
}

static char *
fm_tree_view_get_tab_label (ScatolaSidebar *sidebar)
{
    return g_strdup (_("Tree"));
}

static char *
fm_tree_view_get_tab_tooltip (ScatolaSidebar *sidebar)
{
    return g_strdup (_("Show Tree"));
}

static GdkPixbuf *
fm_tree_view_get_tab_icon (ScatolaSidebar *sidebar)
{
    return NULL;
}

static void
fm_tree_view_is_visible_changed (ScatolaSidebar *sidebar,
                                 gboolean         is_visible)
{
    /* Do nothing */
}
...
static void
hidden_files_mode_changed_callback (ScatolaWindowInfo *window,
                                    FMTreeView *view)
{
    update_filtering_from_preferences (view);
}
...
static void
fm_tree_view_iface_init (ScatolaSidebarIface *iface)
{
...
fm_tree_view_create (ScatolaSidebarProvider *provider,
                     ScatolaWindowInfo *window)
{
    FMTreeView *sidebar;
...
{
    scatola_module_add_type (fm_tree_view_provider_get_type ());
}
