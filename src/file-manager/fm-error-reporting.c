...
#define NEW_NAME_TAG "Scatola: new name"
#define MAXIMUM_DISPLAYED_FILE_NAME_LENGTH	50

static void finish_rename (ScatolaFile *file, gboolean stop_timer, GError *error);

void
fm_report_error_loading_directory (ScatolaFile *file,
                                   GError *error,
...
void
fm_report_error_renaming_file (ScatolaFile *file,
                               const char *new_name,
                               GError *error,
                               GtkWindow *parent_window)
{
     ...
     */
    original_name = scatola_file_get_display_name (file);
...
void
fm_report_error_setting_group (ScatolaFile *file,
                               GError *error,
                               GtkWindow *parent_window)
{
    char *file_name;
...
void
fm_report_error_setting_owner (ScatolaFile *file,
                               GError *error,
                               GtkWindow *parent_window)
{
    char *file_name;
...
void
fm_report_error_setting_permissions (ScatolaFile *file,
                                     GError *error,
                                     GtkWindow *parent_window)
{
    char *file_name;
...
static void
rename_callback (ScatolaFile *file, GFile *result_location,
...
cancel_rename_callback (gpointer callback_data)
{
    ...
    finish_rename (SCATOLA_FILE (callback_data), FALSE, error);
    g_error_free (error);
}

static void
finish_rename (ScatolaFile *file, gboolean stop_timer, GError *error)
{
    FMRenameData *data;
    ...
    scatola_file_cancel (file, rename_callback, NULL);
    if (stop_timer)
    {
...
void
fm_rename_file (ScatolaFile *file,
                const char *new_name,
                ScatolaFileOperationCallback callback,
                gpointer callback_data)
{
