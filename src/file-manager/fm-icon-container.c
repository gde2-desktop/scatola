...
#include <libscatola-private/scatola-thumbnails.h>
...
G_DEFINE_TYPE (FMIconContainer, fm_icon_container, SCATOLA_TYPE_ICON_CONTAINER);

static GQuark attribute_none_q;

static FMIconView *
get_icon_view (ScatolaIconContainer *container)
{
...
fm_icon_container_get_icon_images (ScatolaIconContainer *container,
                                   ...
                                   gboolean              *has_window_open)
{
    ...
    if (embedded_text)
    {
        *embedded_text = scatola_file_peek_top_left_text (file, need_large_embeddded_text, embedded_text_needs_loading);
        use_embedding = *embedded_text != NULL;
    }

    if (emblem_pixbufs != NULL)
    {
        emblem_size = scatola_icon_get_emblem_size_for_icon_size (size);
        ...
        if (emblem_size > 0)
        {
            ...
            *emblem_pixbufs = scatola_file_get_emblem_pixbufs (file,
                              emblem_size,
                              FALSE,
                              emblems_to_ignore);
    ...
    *has_window_open = scatola_file_has_open_window (file);

    flags = SCATOLA_FILE_ICON_FLAGS_USE_MOUNT_ICON_AS_EMBLEM;
    if (!fm_icon_view_is_compact (icon_view) ||
            scatola_icon_container_get_zoom_level (container) > SCATOLA_ZOOM_LEVEL_STANDARD)
    {
        flags |= SCATOLA_FILE_ICON_FLAGS_USE_THUMBNAILS;
        if (fm_icon_view_is_compact (icon_view))
        {
            flags |= SCATOLA_FILE_ICON_FLAGS_FORCE_THUMBNAIL_SIZE;
        }
    }
    ...
    return scatola_file_get_icon (file, size, flags);
}

static char *
fm_icon_container_get_icon_description (ScatolaIconContainer *container,
                                        ScatolaIconData      *data)
{
    ScatolaFile *file;
    char *mime_type;
...
static void
fm_icon_container_start_monitor_top_left (ScatolaIconContainer *container,
        ScatolaIconData      *data,
        ...
        gboolean               large_text)
{
    ...
    attributes = SCATOLA_FILE_ATTRIBUTE_TOP_LEFT_TEXT;
    if (large_text)
    {
        attributes |= SCATOLA_FILE_ATTRIBUTE_LARGE_TOP_LEFT_TEXT;
    }
    scatola_file_monitor_add (file, client, attributes);
}

static void
fm_icon_container_stop_monitor_top_left (ScatolaIconContainer *container,
        ScatolaIconData      *data,
        gconstpointer          client)
{
    ScatolaFile *file;
    ...
    scatola_file_monitor_remove (file, client);
}

static void
fm_icon_container_prioritize_thumbnailing (ScatolaIconContainer *container,
        ScatolaIconData      *data)
{
    ScatolaFile *file;
    char *uri;
    ...
    if (scatola_file_is_thumbnailing (file))
    {
        uri = scatola_file_get_uri (file);
        scatola_thumbnail_prioritize (uri);
        g_free (uri);
    }
}
...
fm_icon_container_get_icon_text_attributes_from_preferences (void)
{
    ...
    if (attributes == NULL)
    {
        eel_g_settings_add_auto_strv_as_quarks (scatola_icon_view_preferences,
     ...
     *
     * 2) Value stored in Gde2Conf are tampered with.  Its possible physically do
     *    this by pulling the rug underneath Gde2Conf and manually editing its
     *    config files.  Its also possible to use a third party Gde2Conf key
     *    editor and store garbage for the keys in question.
     *
     * Thankfully, the Scatola preferences machinery deals with both of
     * these cases.
     *
 ...
 **/
static GQuark *
fm_icon_container_get_icon_text_attribute_names (ScatolaIconContainer *container,
        int *len)
{
    GQuark *attributes;
    int piece_count;
    ...
    {
        0,	/* SCATOLA_ZOOM_LEVEL_SMALLEST */
        0,	/* SCATOLA_ZOOM_LEVEL_SMALLER */
        0,	/* SCATOLA_ZOOM_LEVEL_SMALL */
        1,	/* SCATOLA_ZOOM_LEVEL_STANDARD */
        2,	/* SCATOLA_ZOOM_LEVEL_LARGE */
        2,	/* SCATOLA_ZOOM_LEVEL_LARGER */
        3	/* SCATOLA_ZOOM_LEVEL_LARGEST */
    };

    piece_count = pieces_by_level[scatola_icon_container_get_zoom_level (container)];
 ...
 */
static void
fm_icon_container_get_icon_text (ScatolaIconContainer *container,
                                 ScatolaIconData      *data,
                                 char                 **editable_text,
                                 ...
                                 gboolean               include_invisible)
{
    ...
    if (scatola_icon_container_get_zoom_level (container) == SCATOLA_ZOOM_LEVEL_SMALLEST &&
            !include_invisible)
    {
        *editable_text = NULL;
    }
    ...
    {
        /* Strip the suffix for scatola object xml files. */
        *editable_text = scatola_file_get_display_name (file);
    }
    ...
    if (scatola_file_is_scatola_link (file))
    {
        ...
        *additional_text = NULL;
        ...
        {
            actual_uri = scatola_file_get_uri (file);
            description = scatola_link_local_get_additional_text (actual_uri);
            if (description)
    ...
    for (i = 0; i < num_attributes; ++i)
    {
        ...
        text_array[j++] =
            scatola_file_get_string_attribute_with_default_q (file, attributes[i]);
    }
    text_array[j] = NULL;
...
static SortCategory
get_sort_category (ScatolaFile *file)
{
    ...
    if (SCATOLA_IS_DESKTOP_ICON_FILE (file))
    {
        ...
        if (link != NULL)
        {
            switch (scatola_desktop_link_get_link_type (link))
            {
...
static int
fm_desktop_icon_container_icons_compare (ScatolaIconContainer *container,
        ScatolaIconData      *data_a,
        ScatolaIconData      *data_b)
{
    ScatolaFile *file_a;
    ScatolaFile *file_b;
    ...
    file_a = (ScatolaFile *) data_a;
    file_b = (ScatolaFile *) data_b;
    ...
    if (category_a == category_b)
    {
        return scatola_file_compare_for_sort
               (file_a, file_b, SCATOLA_FILE_SORT_BY_DISPLAY_NAME,
...
static int
fm_icon_container_compare_icons (ScatolaIconContainer *container,
...
static int
fm_icon_container_compare_icons_by_name (ScatolaIconContainer *container,
        ScatolaIconData      *icon_a,
        ScatolaIconData      *icon_b)
{
    return scatola_file_compare_for_sort
...
static void
fm_icon_container_freeze_updates (ScatolaIconContainer *container)
{
    FMIconView *icon_view;
    icon_view = get_icon_view (container);
    g_return_if_fail (icon_view != NULL);
...
static void
fm_icon_container_unfreeze_updates (ScatolaIconContainer *container)
{
    FMIconView *icon_view;
    icon_view = get_icon_view (container);
    g_return_if_fail (icon_view != NULL);
...
{
    ScatolaIconContainerClass *ic_class;
...
fm_icon_container_construct (FMIconContainer *icon_container, FMIconView *view)
{
    ...
    return SCATOLA_ICON_CONTAINER (icon_container);
}
