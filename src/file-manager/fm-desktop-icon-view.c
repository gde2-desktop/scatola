...
#include <libscatola-private/scatola-directory-notify.h>
...
get_icon_container (FMDesktopIconView *icon_view)
{
    ...
    g_return_val_if_fail (SCATOLA_IS_ICON_CONTAINER (gtk_bin_get_child (GTK_BIN (icon_view))), NULL);

    return SCATOLA_ICON_CONTAINER (gtk_bin_get_child (GTK_BIN (icon_view)));
}

static void
icon_container_set_workarea (ScatolaIconContainer *icon_container,
                             GdkScreen             *screen,
                             long                  *workareas,
                             int                    n_items)
{
    ...
    scatola_icon_container_set_margins (icon_container,
...
static void
fm_desktop_icon_view_handle_middle_click (ScatolaIconContainer *icon_container,
...
realized_callback (GtkWidget *widget, FMDesktopIconView *desktop_icon_view)
{
     ...
     * Gde2ComponentPlug.) This was leading to problems where initial
...
get_default_zoom_level (void)
{
    ...
    return CLAMP (default_zoom_level, SCATOLA_ZOOM_LEVEL_SMALLEST, SCATOLA_ZOOM_LEVEL_LARGEST);
}
...
default_zoom_level_changed (gpointer user_data)
{
    ...
    scatola_icon_container_set_zoom_level (get_icon_container (desktop_icon_view),
                                        new_level);
}
...
do_desktop_rescan (gpointer data)
{
    ...
    scatola_directory_force_reload (
...
static void
done_loading (ScatolaDirectory *model,
	      FMDesktopIconView *desktop_icon_view)
{
    struct stat buf;
...
/* This function is used because the ScatolaDirectory model does not
...
fm_desktop_icon_view_update_icon_container_fonts (FMDesktopIconView *icon_view)
{
    ...
    font = g_settings_get_string (scatola_desktop_preferences, SCATOLA_PREFERENCES_DESKTOP_FONT);

    scatola_icon_container_set_font (icon_container, font);
...
fm_desktop_icon_view_init (FMDesktopIconView *desktop_icon_view)
{
    ...
    scatola_icon_container_set_use_drop_shadows (icon_container, TRUE);
    ...
    scatola_icon_container_set_is_fixed_size (icon_container, TRUE);
    scatola_icon_container_set_is_desktop (icon_container, TRUE);
    scatola_icon_container_set_store_layout_timestamps (icon_container, TRUE);
    ...
    g_signal_connect_swapped (scatola_desktop_preferences,
                              "changed::" SCATOLA_PREFERENCES_DESKTOP_FONT,
...
action_new_launcher_callback (GtkAction *action, gpointer data)
{
    ...
    scatola_launch_application_from_command (gtk_widget_get_screen (GTK_WIDGET (data)),
        ...
        gpointer data)
{
    ...
    scatola_file_operations_empty_trash (GTK_WIDGET (data));
}
...
trash_link_is_selection (FMDirectoryView *view)
{
    ...
    if (eel_g_list_exactly_one_item (selection) &&
            SCATOLA_IS_DESKTOP_ICON_FILE (selection->data))
    {
        link = scatola_desktop_icon_file_get_link (SCATOLA_DESKTOP_ICON_FILE (selection->data));
        /* link may be NULL if the link was recently removed (unmounted) */
        if (link != NULL &&
                scatola_desktop_link_get_link_type (link) == SCATOLA_DESKTOP_LINK_TRASH)
        {
            result = TRUE;
        }
        if (link)
...
real_merge_menus (FMDirectoryView *view)
{
    ...
    ui = scatola_ui_string_get ("scatola-desktop-icon-view-ui.xml");
...
static ScatolaView *
fm_desktop_icon_view_create (ScatolaWindowSlotInfo *slot)
{
    FMIconView *view;
...
static ScatolaViewInfo fm_desktop_icon_view =
{
...
fm_desktop_icon_view_register (void)
{
    ...
    scatola_view_factory_register (&fm_desktop_icon_view);
}
