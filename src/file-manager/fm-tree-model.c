...
typedef gboolean (* FilePredicate) (ScatolaFile *);
...
static TreeNode *
tree_node_new (ScatolaFile *file, FMTreeModelRoot *root)
{
    TreeNode *node;

    node = g_new0 (TreeNode, 1);
    node->file = scatola_file_ref (file);
    node->root = root;
    return node;
}
...
tree_node_destroy (FMTreeModel *model, TreeNode *node)
{
    ...
    scatola_directory_unref (node->directory);
                        ...
                        ScatolaFileIconFlags flags)
{
    ...
    i = 0;
    emblems_to_ignore[i++] = SCATOLA_FILE_EMBLEM_NAME_TRASH;

    if (node->parent && node->parent->file) {
        if (!scatola_file_can_write (node->parent->file)) {
            emblems_to_ignore[i++] = SCATOLA_FILE_EMBLEM_NAME_CANT_WRITE;
        }
    }
    ...
    emblem = NULL;
    emblem_icons = scatola_file_get_emblem_icons (node->file,
                		       emblems_to_ignore);

    /* pick only the first emblem we can render for the tree view */
    for (l = emblem_icons; l != NULL; l = l->next) {
        emblem_icon = l->data;
        if (scatola_icon_theme_can_render (G_THEMED_ICON (emblem_icon))) {
    ...
    info = scatola_icon_info_lookup (gicon, size);
    retval = scatola_icon_info_get_pixbuf_nodefault_at_size (info, size);
    model = node->root->model;

    g_object_unref (gicon);

    highlight = (g_list_find_custom (model->details->highlighted_files,
                                     file, (GCompareFunc) scatola_file_compare_location) != NULL);
...
{
    return tree_node_update_pixbuf (node, &node->open_pixbuf, SCATOLA_FILE_ICON_FLAGS_FOR_OPEN_FOLDER);
}
...
tree_node_update_display_name (TreeNode *node)
{
    ...
    display_name = scatola_file_get_display_name (node->file);
...
tree_node_get_open_pixbuf (TreeNode *node)
{
    ...
    {
        node->open_pixbuf = tree_node_get_pixbuf (node, SCATOLA_FILE_ICON_FLAGS_FOR_OPEN_FOLDER);
    }
...
tree_node_get_display_name (TreeNode *node)
{
    if (node->display_name == NULL)
    {
        node->display_name = scatola_file_get_display_name (node->file);
    }
...
static TreeNode *
get_node_from_file (FMTreeModelRoot *root, ScatolaFile *file)
{
    return g_hash_table_lookup (root->file_to_node_map, file);
}

static TreeNode *
get_parent_node_from_file (FMTreeModelRoot *root, ScatolaFile *file)
{
...
static TreeNode *
create_node_for_file (FMTreeModelRoot *root, ScatolaFile *file)
{
    TreeNode *node;
...
get_node_uri (GtkTreeIter *iter)
{
    ...
    if (node != NULL)
    {
        return scatola_file_get_uri (node->file);
    }

    parent = iter->user_data2;
    parent_uri = scatola_file_get_uri (parent->file);
...
abandon_node_ref_count (FMTreeModel *model, TreeNode *node)
{
    if (node->parent != NULL)
    {
        ...
        if (node->ref_count != 0)
        {
            ...
            uri = scatola_file_get_uri (node->file);
...
stop_monitoring_directory (FMTreeModel *model, TreeNode *node)
{
    ...
    scatola_directory_file_monitor_remove (node->directory, model);
}
...
update_node_without_reporting (FMTreeModel *model, TreeNode *node)
{
    ...
    if (node->directory == NULL &&
            (scatola_file_is_directory (node->file) || node->parent == NULL))
    {
        node->directory = scatola_directory_get_for_file (node->file);
    }
...
static gboolean
should_show_file (FMTreeModel *model, ScatolaFile *file)
{
    gboolean should;
    TreeNode *node;

    should = scatola_file_should_show (file,
    ...
    if (should && scatola_file_is_gone (file))
    {
        should = FALSE;
    }
...
update_node (FMTreeModel *model, TreeNode *node)
{
    ...
    if (node->parent != NULL && node->parent->directory != NULL
            && !scatola_directory_contains_file (node->parent->directory, node->file))
    {
                        ...
                        gpointer callback_data)
{
    ...
    {
        process_file_change (root, SCATOLA_FILE (node->data));
    }
}
                       ...
                       FMTreeModelRoot *root)
{
    ...
    if (node == NULL)
    {
         ...
         * broken due to scatola_directory_remove_file()
...
start_monitoring_directory (FMTreeModel *model, TreeNode *node)
{
    ...
    set_done_loading (model, node, scatola_directory_are_all_files_seen (directory));

    attributes = get_tree_monitor_attributes ();
    scatola_directory_file_monitor_add (directory, model,
                                     model->details->show_hidden_files,
...
iter_is_valid (FMTreeModel *model, const GtkTreeIter *iter)
{
    ...
    if (node == NULL)
    {
        if (parent != NULL)
        {
            if (!SCATOLA_IS_FILE (parent->file))
            {
                return FALSE;
            }
            if (!tree_node_has_dummy_child (parent))
...
#if 0
    g_warning ("Node '%s' %s",
               node && node->file ? scatola_file_get_uri (node->file) : "no name",
...
fm_tree_model_add_root_uri (FMTreeModel *model, const char *root_uri, const char *display_name, GIcon *icon, GMount *mount)
{
    ...
    file = scatola_file_get_by_uri (root_uri);
...
GMount *
fm_tree_model_get_mount_for_root_node_file (FMTreeModel *model, ScatolaFile *file)
{
    TreeNode *node;
...
fm_tree_model_remove_root_uri (FMTreeModel *model, const char *uri)
{
    ...
    if (node)
    {
        ...
        scatola_file_monitor_remove (node->file, model);
        path = get_node_path (model, node);
                                     ...
                                     gboolean show_hidden_files)
{
    ...
    {
        destroy_by_function (model, scatola_file_is_hidden_file);
    }
    schedule_monitoring_update (model);
}

static gboolean
file_is_not_directory (ScatolaFile *file)
{
...
fm_tree_model_iter_get_file (FMTreeModel *model, GtkTreeIter *iter)
{
    ...
    node = iter->user_data;
    return node == NULL ? NULL : scatola_file_ref (node->file);
}
...
static void
do_update_node (ScatolaFile *file,
                FMTreeModel *model)
{
                                       ...
                                       GList *files)
{
    ...
    if (model->details->highlighted_files != NULL)
    {
        ...
        scatola_file_list_free (old_files);
    }
...
fm_tree_model_finalize (GObject *object)
{
    ...
    if (model->details->highlighted_files != NULL)
    {
        scatola_file_list_free (model->details->highlighted_files);
    }
