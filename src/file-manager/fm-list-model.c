...
{
    scatola_file_unref (file_entry->file);
    ...
    {
        scatola_directory_unref (file_entry->subdirectory);
    }
...
fm_list_model_get_column_type (GtkTreeModel *tree_model, int index)
{
    ...
    case FM_LIST_MODEL_FILE_COLUMN:
        return SCATOLA_TYPE_FILE;
    case FM_LIST_MODEL_SUBDIRECTORY_COLUMN:
        return SCATOLA_TYPE_DIRECTORY;
...
fm_list_model_get_value (GtkTreeModel *tree_model, GtkTreeIter *iter, int column, GValue *value)
{
    ...
    {
    case FM_LIST_MODEL_FILE_COLUMN:
        g_value_init (value, SCATOLA_TYPE_FILE);

        g_value_set_object (value, file);
        break;
    case FM_LIST_MODEL_SUBDIRECTORY_COLUMN:
        g_value_init (value, SCATOLA_TYPE_DIRECTORY);
    ...
    case FM_LIST_MODEL_LARGEST_ICON_COLUMN:
        ...
        if (file != NULL)
        {
            ...
            icon_size = scatola_get_icon_size_for_zoom_level (zoom_level);

            flags = SCATOLA_FILE_ICON_FLAGS_USE_THUMBNAILS |
                    SCATOLA_FILE_ICON_FLAGS_FORCE_THUMBNAIL_SIZE |
            ...
            if (parent_file) {
            	if (!scatola_file_can_write (parent_file)) {
            ...
            emblem = NULL;
            emblem_icons = scatola_file_get_emblem_icons (file,
            					       emblems_to_ignore);
            ...
            icon_info = scatola_icon_info_lookup (gicon, icon_size);
            icon = scatola_icon_info_get_pixbuf_at_size (icon_info, icon_size);
            ...
            if (model->details->highlight_files != NULL &&
                    g_list_find_custom (model->details->highlight_files,
                                        file, (GCompareFunc) scatola_file_compare_location))
            {
#if GTK_CHECK_VERSION(3,0,0)
    ...
    case FM_LIST_MODEL_FILE_NAME_IS_EDITABLE_COLUMN:
        ...
        g_value_set_boolean (value, file != NULL && scatola_file_can_rename (file));
        break;
    default:
        if (column >= FM_LIST_MODEL_NUM_COLUMNS || column < FM_LIST_MODEL_NUM_COLUMNS + model->details->columns->len)
        {
            ...
            GQuark attribute;
            scatola_column = model->details->columns->pdata[column - FM_LIST_MODEL_NUM_COLUMNS];
            ...
            if (file != NULL)
            {
                str = scatola_file_get_string_attribute_with_default_q (file,
                        attribute);
...
lookup_file (FMListModel *model, ScatolaFile *file,
...
GList *
fm_list_model_get_all_iters_for_file (FMListModel *model, ScatolaFile *file)
{
    struct GetIters data;
...
gboolean
fm_list_model_get_tree_iter_from_file (FMListModel *model, ScatolaFile *file,
                                       ScatolaDirectory *directory,
                                       GtkTreeIter *iter)
{
    GSequenceIter *ptr;
                                       ...
                                       gpointer      user_data)
{
    ...
    {
        result = scatola_file_compare_for_sort_by_attribute_q (file_entry1->file, file_entry2->file,
                            ...
                            ScatolaFile *file2)
{
    ...
    result = scatola_file_compare_for_sort_by_attribute_q (file1, file2,
             model->details->sort_attribute,
             model->details->sort_directories_first,
             (model->details->order == GTK_SORT_DESCENDING));
...
static void
each_path_get_data_binder (ScatolaDragEachSelectedItemDataGet data_get,
                           gpointer context,
                           gpointer data)
{
                                   ...
                                   GtkSelectionData *selection_data)
{
    ...
    if (gtk_target_list_find (drag_target_list,
                              ...
                              &target_info))
    {
        scatola_drag_drag_data_get (NULL,
                                 NULL,
                                 selection_data,
                                 target_info,
                                 GDK_CURRENT_TIME,
...
gboolean
fm_list_model_add_file (FMListModel *model, ScatolaFile *file,
                        ScatolaDirectory *directory)
{
    GtkTreeIter iter;
    GtkTreePath *path;
    ...
    file_entry->file = scatola_file_ref (file);
...
void
fm_list_model_file_changed (FMListModel *model, ScatolaFile *file,
                            ScatolaDirectory *directory)
{
...
void
fm_list_model_remove_file (FMListModel *model, ScatolaFile *file,
                           ScatolaDirectory *directory)
{
    GtkTreeIter iter;
...
gboolean
fm_list_model_load_subdirectory (FMListModel *model, GtkTreePath *path, ScatolaDirectory **directory)
{
    GtkTreeIter iter;
    FileEntry *file_entry;
    ScatolaDirectory *subdirectory;
    ...
    subdirectory = scatola_directory_get_for_file (file_entry->file);

    if (g_hash_table_lookup (model->details->directory_reverse_map,
                             subdirectory) != NULL)
    {
        scatola_directory_unref (subdirectory);
    ...
    /* Return a ref too */
    scatola_directory_ref (subdirectory);
        ...
        GQuark attribute)
{
    ...
    for (i = 0; i < model->details->columns->len; i++)
    {
        ...
        column =
            SCATOLA_COLUMN (model->details->columns->pdata[i]);
        ...
        int sort_column_id)
{
    ...
    column = SCATOLA_COLUMN (model->details->columns->pdata[index]);
...
fm_list_model_get_zoom_level_from_column_id (int column)
{
    ...
    {
    case FM_LIST_MODEL_SMALLEST_ICON_COLUMN:
        return SCATOLA_ZOOM_LEVEL_SMALLEST;
    case FM_LIST_MODEL_SMALLER_ICON_COLUMN:
        return SCATOLA_ZOOM_LEVEL_SMALLER;
    case FM_LIST_MODEL_SMALL_ICON_COLUMN:
        return SCATOLA_ZOOM_LEVEL_SMALL;
    case FM_LIST_MODEL_STANDARD_ICON_COLUMN:
        return SCATOLA_ZOOM_LEVEL_STANDARD;
    case FM_LIST_MODEL_LARGE_ICON_COLUMN:
        return SCATOLA_ZOOM_LEVEL_LARGE;
    case FM_LIST_MODEL_LARGER_ICON_COLUMN:
        return SCATOLA_ZOOM_LEVEL_LARGER;
    case FM_LIST_MODEL_LARGEST_ICON_COLUMN:
        return SCATOLA_ZOOM_LEVEL_LARGEST;
    }

    g_return_val_if_reached (SCATOLA_ZOOM_LEVEL_STANDARD);
}

int
fm_list_model_get_column_id_from_zoom_level (ScatolaZoomLevel zoom_level)
{
    switch (zoom_level)
    {
...
fm_list_model_get_drag_target_list ()
{
    ...
    gtk_target_list_add_text_targets (target_list, SCATOLA_ICON_DND_TEXT);
...
fm_list_model_finalize (GObject *object)
{
    ...
    if (model->details->highlight_files != NULL)
    {
        scatola_file_list_free (model->details->highlight_files);
...
void
fm_list_model_subdirectory_done_loading (FMListModel *model, ScatolaDirectory *directory)
{
    GtkTreeIter iter;
    GtkTreePath *path;
     ...
     */
    if (!scatola_directory_is_not_empty (directory) &&
                                       ...
                                       GList *files)
{
    ...
    if (files != NULL)
    {
        model->details->highlight_files = scatola_file_list_copy (files);
        g_list_foreach (model->details->highlight_files,
                        refresh_row, model);
