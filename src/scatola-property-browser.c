...
#include <config.h>
#include <math.h>
#include "scatola-property-browser.h"
...
#include <gio/gio.h>
#include <libscatola-private/scatola-customization-data.h>
...
typedef enum
{
    SCATOLA_PROPERTY_NONE,
    SCATOLA_PROPERTY_PATTERN,
    SCATOLA_PROPERTY_COLOR,
    SCATOLA_PROPERTY_EMBLEM
} ScatolaPropertyType;

struct ScatolaPropertyBrowserDetails
{
    GtkWidget *container;
    ...
    ScatolaPropertyType category_type;
...
static void     scatola_property_browser_update_contents       (ScatolaPropertyBrowser       *property_browser);
static void     scatola_property_browser_set_category          (ScatolaPropertyBrowser       *property_browser,
        const char                    *new_category);
static void     scatola_property_browser_set_dragged_file      (ScatolaPropertyBrowser       *property_browser,
        const char                    *dragged_file_name);
static void     scatola_property_browser_set_drag_type         (ScatolaPropertyBrowser       *property_browser,
        const char                    *new_drag_type);
static void     add_new_button_callback                         (GtkWidget                     *widget,
        ScatolaPropertyBrowser       *property_browser);
static void     cancel_remove_mode                              (ScatolaPropertyBrowser       *property_browser);
...
static void     remove_button_callback                          (GtkWidget                     *widget,
        ScatolaPropertyBrowser       *property_browser);
static gboolean scatola_property_browser_delete_event_callback (GtkWidget                     *widget,
        GdkEvent                      *event,
        gpointer                       user_data);
static void     scatola_property_browser_hide_callback         (GtkWidget                     *widget,
        gpointer                       user_data);
static void     scatola_property_browser_drag_end              (GtkWidget                     *widget,
        GdkDragContext                *context);
static void     scatola_property_browser_drag_begin            (GtkWidget                     *widget,
        GdkDragContext                *context);
static void     scatola_property_browser_drag_data_get         (GtkWidget                     *widget,
        GdkDragContext                *context,
...
static GdkPixbuf * make_drag_image                              (ScatolaPropertyBrowser       *property_browser,
        const char                    *file_name);
static GdkPixbuf * make_color_drag_image                        (ScatolaPropertyBrowser       *property_browser,
...
G_DEFINE_TYPE (ScatolaPropertyBrowser, scatola_property_browser, GTK_TYPE_WINDOW)


/* Destroy the three dialogs for adding patterns/colors/emblems if any of them
   exist. */
static void
scatola_property_browser_destroy_dialogs (ScatolaPropertyBrowser *property_browser)
{
...
static void
scatola_property_browser_dispose (GObject *object)
{
    ScatolaPropertyBrowser *property_browser;

    property_browser = SCATOLA_PROPERTY_BROWSER (object);

    scatola_property_browser_destroy_dialogs (property_browser);
    ...
    G_OBJECT_CLASS (scatola_property_browser_parent_class)->dispose (object);
}

/* initializing the class object by installing the operations we override */
static void
scatola_property_browser_class_init (ScatolaPropertyBrowserClass *klass)
{
    GtkWidgetClass *widget_class = GTK_WIDGET_CLASS (klass);

    G_OBJECT_CLASS (klass)->dispose = scatola_property_browser_dispose;
    widget_class->drag_begin = scatola_property_browser_drag_begin;
    widget_class->drag_data_get  = scatola_property_browser_drag_data_get;
    widget_class->drag_end  = scatola_property_browser_drag_end;

    g_type_class_add_private (klass, sizeof (ScatolaPropertyBrowserDetails));
}

/* initialize the instance's fields, create the necessary subviews, etc. */

static void
scatola_property_browser_init (ScatolaPropertyBrowser *property_browser)
{
    ...
    property_browser->details = G_TYPE_INSTANCE_GET_PRIVATE (property_browser,
    							     SCATOLA_TYPE_PROPERTY_BROWSER,
    							     ScatolaPropertyBrowserDetails);

    property_browser->details->category = g_strdup ("patterns");
    property_browser->details->category_type = SCATOLA_PROPERTY_PATTERN;

    /* load the chit frame */
    temp_str = scatola_pixmap_file ("chit_frame.png");
    if (temp_str != NULL)
    {
    ...
    gtk_window_set_wmclass (GTK_WINDOW (widget), "property_browser", "Scatola");
    ...
    gtk_style_context_add_class (context, "scatola-property-browser");
#endif
    ...
    g_signal_connect (property_browser, "delete_event",
                      G_CALLBACK (scatola_property_browser_delete_event_callback), NULL);
    g_signal_connect (property_browser, "hide",
                      G_CALLBACK (scatola_property_browser_hide_callback), NULL);
    ...
    scatola_property_browser_set_path(property_browser, BROWSER_CATEGORIES_FILE_NAME);
}

/* create a new instance */
ScatolaPropertyBrowser *
scatola_property_browser_new (GdkScreen *screen)
{
    ScatolaPropertyBrowser *browser;

    browser = SCATOLA_PROPERTY_BROWSER
              (gtk_widget_new (scatola_property_browser_get_type (), NULL));
...
void
scatola_property_browser_show (GdkScreen *screen)
{
    static GtkWindow *browser = NULL;

    if (browser == NULL)
    {
        browser = GTK_WINDOW (scatola_property_browser_new (screen));
...
static gboolean
scatola_property_browser_delete_event_callback (GtkWidget *widget,
        GdkEvent  *event,
        gpointer   user_data)
{
...
static void
scatola_property_browser_hide_callback (GtkWidget *widget,
                                     gpointer   user_data)
{
    ScatolaPropertyBrowser *property_browser;

    property_browser = SCATOLA_PROPERTY_BROWSER (widget);
...
static void
scatola_property_browser_set_dragged_file (ScatolaPropertyBrowser *property_browser,
                                        const char *dragged_file_name)
{
    g_free (property_browser->details->dragged_file);
...
static void
scatola_property_browser_set_drag_type (ScatolaPropertyBrowser *property_browser,
                                     const char *new_drag_type)
{
    g_free (property_browser->details->drag_type);
...
static void
scatola_property_browser_drag_begin (GtkWidget *widget,
                                  GdkDragContext *context)
{
    ScatolaPropertyBrowser *property_browser;
    GtkWidget *child;
...
static void
scatola_property_browser_drag_data_get (GtkWidget *widget,
                                     GdkDragContext *context,
                                     GtkSelectionData *selection_data,
                                     guint info,
                                     guint32 time)
{
    ...
    gboolean is_reset;
    ScatolaPropertyBrowser *property_browser = SCATOLA_PROPERTY_BROWSER(widget);
    GdkAtom target;
...
static void
scatola_property_browser_drag_end (GtkWidget *widget, GdkDragContext *context)
{
    ScatolaPropertyBrowser *property_browser = SCATOLA_PROPERTY_BROWSER(widget);
...
static GdkPixbuf *
make_drag_image (ScatolaPropertyBrowser *property_browser, const char* file_name)
{
    ...
    if (property_browser->details->category_type == SCATOLA_PROPERTY_EMBLEM)
    {
        if (strcmp (file_name, "erase") == 0)
        {
            pixbuf = NULL;

            image_file_name = scatola_pixmap_file (ERASE_OBJECT_NAME);
        ...
        {
            icon_name = scatola_emblem_get_icon_name_from_keyword (file_name);
            info = scatola_icon_info_lookup_from_name (icon_name, SCATOLA_ICON_SIZE_STANDARD);
            pixbuf = scatola_icon_info_get_pixbuf_at_size (info, SCATOLA_ICON_SIZE_STANDARD);
            g_object_unref (info);
            g_free (icon_name);
        }
        return pixbuf;
    ...
    {
        pixbuf = scatola_customization_make_pattern_chit (orig_pixbuf, property_browser->details->property_chit, TRUE, is_reset);
    }
    else
    {
...
static GdkPixbuf*
make_color_drag_image (ScatolaPropertyBrowser *property_browser, const char *color_spec, gboolean trim_edges)
{
    ...
    {
        ret = scatola_customization_make_pattern_chit (color_square,
...
category_toggled_callback (GtkWidget *widget, char *category_name)
{
    ...
    property_browser = SCATOLA_PROPERTY_BROWSER (g_object_get_data (G_OBJECT (widget), "user_data"));
    ...
    scatola_property_browser_set_category (property_browser, category_name);
}

static xmlDocPtr
read_browser_xml (ScatolaPropertyBrowser *property_browser)
{
    char *path;
    xmlDocPtr document;

    path = scatola_get_data_file_path (property_browser->details->path);
    if (path == NULL)
    {
        return NULL;
    }
...
static void
write_browser_xml (ScatolaPropertyBrowser *property_browser,
                   xmlDocPtr document)
{
...
static void
remove_color (ScatolaPropertyBrowser *property_browser, const char* color_name)
{
...
static void
remove_pattern(ScatolaPropertyBrowser *property_browser, const char* pattern_name)
{
    char *pattern_path;
    char *user_directory;
...
static void
remove_emblem (ScatolaPropertyBrowser *property_browser, const char* emblem_name)
{
    /* delete the emblem from the emblem directory */
    if (scatola_emblem_remove_emblem (emblem_name) == FALSE)
    {
...
static void
scatola_property_browser_remove_element (ScatolaPropertyBrowser *property_browser, EelLabeledImage *child)
{
...
static GtkWidget*
scatola_emblem_dialog_new (ScatolaPropertyBrowser *property_browser)
{
    GtkWidget *widget;
    GtkWidget *button;
    GtkWidget *dialog;
    ...
    property_browser->details->image_path = g_build_filename (SCATOLA_PIXMAPDIR, "emblems.png", NULL);
...
static GtkWidget*
scatola_color_selection_dialog_new (ScatolaPropertyBrowser *property_browser)
{
    GtkWidget *widget;
    GtkWidget *dialog;
#if GTK_CHECK_VERSION (3, 0, 0)
...
add_pattern_to_browser (GtkDialog *dialog, gint response_id, gpointer *data)
{
    ...
    ScatolaPropertyBrowser *property_browser = SCATOLA_PROPERTY_BROWSER (data);
    ...
    scatola_property_browser_update_contents (property_browser);
...
static void
add_new_pattern (ScatolaPropertyBrowser *property_browser)
{
    GtkWidget *dialog;
    GtkFileFilter *filter;
    GtkWidget *preview;
...
static void
add_color_to_file (ScatolaPropertyBrowser *property_browser, const char *color_spec, const char *color_name)
{
...
show_color_selection_window (GtkWidget *widget, gpointer *data)
{
    ...
    property_browser->details->colors_dialog = scatola_color_selection_dialog_new (property_browser);
...
static void
add_new_color (ScatolaPropertyBrowser *property_browser)
{
    if (property_browser->details->colors_dialog)
    {
...
static void
emblem_dialog_clicked (GtkWidget *dialog, int which_button, ScatolaPropertyBrowser *property_browser)
{
    ...
    if (which_button == GTK_RESPONSE_OK)
    {
        ...
        pixbuf = scatola_emblem_load_pixbuf_for_emblem (emblem_file);
        g_object_unref (emblem_file);
        ...
        scatola_emblem_install_custom_emblem (pixbuf,
                                           stripped_keyword,
                                           stripped_keyword,
                                           GTK_WINDOW (property_browser));
        if (pixbuf != NULL)
...
static void
add_new_emblem (ScatolaPropertyBrowser *property_browser)
{
    if (property_browser->details->emblems_dialog)
    {
    ...
    {
        property_browser->details->emblems_dialog = scatola_emblem_dialog_new (property_browser);
...
/* cancelremove mode */
static void
cancel_remove_mode (ScatolaPropertyBrowser *property_browser)
{
...
static void
add_new_button_callback(GtkWidget *widget, ScatolaPropertyBrowser *property_browser)
{
...
{
    cancel_remove_mode (SCATOLA_PROPERTY_BROWSER (property_browser));
...
help_button_callback (GtkWidget *widget, GtkWidget *property_browser)
{
    ...
    gtk_show_uri (gtk_widget_get_screen (property_browser),
                  "help:gde2-user-guide/gosscatola-50",
...
static void
remove_button_callback(GtkWidget *widget, ScatolaPropertyBrowser *property_browser)
{
    if (property_browser->details->remove_mode)
    {
        return;
                          ...
                          gpointer callback_data)
{
    ...
    g_return_if_fail (SCATOLA_IS_PROPERTY_BROWSER (callback_data));
    g_return_if_fail (g_object_get_data (G_OBJECT (child), "property-name") != NULL);

    element_name = g_object_get_data (G_OBJECT (child), "property-name");
    property_browser = SCATOLA_PROPERTY_BROWSER (callback_data);

    /* handle remove mode by removing the element */
    if (property_browser->details->remove_mode)
    {
        scatola_property_browser_remove_element (property_browser, EEL_LABELED_IMAGE (child));
        property_browser->details->remove_mode = FALSE;
        scatola_property_browser_update_contents (property_browser);
        gtk_widget_show (property_browser->details->help_label);
        return;
    ...
    {
        drag_types[0].target = "x-special/gde2-reset-background";
    }

    target_list = gtk_target_list_new (drag_types, G_N_ELEMENTS (drag_types));
    scatola_property_browser_set_dragged_file(property_browser, element_name);
...
static void
make_properties_from_directories (ScatolaPropertyBrowser *property_browser)
{
    ScatolaCustomizationData *customization_data;
    char *object_name;
    char *object_label;
    ...
    g_return_if_fail (SCATOLA_IS_PROPERTY_BROWSER (property_browser));
    ...
    if (property_browser->details->category_type == SCATOLA_PROPERTY_EMBLEM)
    {
        ...
        while (l != NULL)
        {
            ...
            if (!scatola_emblem_should_show_in_list (icon_name))
            {
                continue;
            }

            object_name = scatola_emblem_get_keyword_from_icon_name (icon_name);
            if (scatola_emblem_can_remove_emblem (object_name))
            {
            ...
            info = scatola_icon_info_lookup_from_name (icon_name, SCATOLA_ICON_SIZE_STANDARD);
            object_pixbuf = scatola_icon_info_get_pixbuf_at_size (info, SCATOLA_ICON_SIZE_STANDARD);
            object_label = g_strdup (scatola_icon_info_get_display_name (info));
            g_object_unref (info);
    ...
    {
        customization_data = scatola_customization_data_new (property_browser->details->category,
        ...
        while (scatola_customization_data_get_next_element_for_display (customization_data,
                &object_name,
                &object_pixbuf,
                &object_label))
        {
            ...
            if (property_browser->details->category_type == SCATOLA_PROPERTY_PATTERN
        ...
        property_browser->details->has_local = scatola_customization_data_private_data_was_displayed (customization_data);
        scatola_customization_data_destroy (customization_data);
    }
    ...
    if (property_browser->details->category_type == SCATOLA_PROPERTY_EMBLEM)
    {
        ...
        path = scatola_pixmap_file (ERASE_OBJECT_NAME);
        if (path != NULL)
        {
...
static void
add_reset_property (ScatolaPropertyBrowser *property_browser)
{
    ...
    reset_image_file_name = g_strdup_printf ("%s/%s/%s", SCATOLA_DATADIR, "patterns", RESET_IMAGE_NAME);
    ...
    {
        reset_chit = scatola_customization_make_pattern_chit (reset_pixbuf, property_browser->details->property_chit, FALSE, TRUE);
    }
...
static void
make_properties_from_xml_node (ScatolaPropertyBrowser *property_browser,
                               xmlNodePtr node)
{
...
static void
make_category(ScatolaPropertyBrowser *property_browser, const char* path, const char* mode, xmlNodePtr node, const char *description)
{
                                      ...
                                      const char *image)
{
    ...
    file_name = scatola_pixmap_file (image);
    if (file_name != NULL)
    {
...
static void
make_category_link (ScatolaPropertyBrowser *property_browser,
                    const char *name,
...
void
scatola_property_browser_update_contents (ScatolaPropertyBrowser *property_browser)
{
    xmlNodePtr cur_node;
    xmlDocPtr document;
    GtkWidget *viewport;
        ...
        if (strcmp (cur_node->name, "category") == 0)
        {
                    ...
                    && strcmp (property_browser->details->category, name) == 0)
            {
                ...
                make_category (property_browser,
                               ...
                               cur_node,
                               description);
                scatola_property_browser_set_drag_type (property_browser, type);
...
static void
scatola_property_browser_set_category (ScatolaPropertyBrowser *property_browser,
                                    const char *new_category)
{
    ...
    {
        property_browser->details->category_type = SCATOLA_PROPERTY_COLOR;
    }
    else if (g_strcmp0 (new_category, "emblems") == 0)
    {
        property_browser->details->category_type = SCATOLA_PROPERTY_EMBLEM;
    }
    else
    {
        property_browser->details->category_type = SCATOLA_PROPERTY_NONE;
    }
...
void
scatola_property_browser_set_path (ScatolaPropertyBrowser *property_browser,
                                const char *new_path)
{
...
{
    g_signal_emit_by_name (scatola_signaller_get_current (), "emblems_changed");
}
