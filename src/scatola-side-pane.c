...
struct _ScatolaSidePaneDetails
{
    GtkWidget *notebook;
    GtkWidget *menu;
...
static void scatola_side_pane_class_init (ScatolaSidePaneClass *klass);
static void scatola_side_pane_init       (GObject *object);
static void scatola_side_pane_dispose    (GObject *object);
static void scatola_side_pane_finalize   (GObject *object);
...
#if GTK_CHECK_VERSION (3, 0, 0)
EEL_CLASS_BOILERPLATE (ScatolaSidePane, scatola_side_pane, GTK_TYPE_BOX)
#else
EEL_CLASS_BOILERPLATE (ScatolaSidePane, scatola_side_pane, GTK_TYPE_VBOX)
#endif

static SidePanel *
panel_for_widget (ScatolaSidePane *side_pane, GtkWidget *widget)
{
    GList *l;
    SidePanel *panel;
...
switch_page_callback (GtkWidget *notebook,
                      ...
                      gpointer user_data)
{
    ScatolaSidePane *side_pane;
    SidePanel *panel;

    side_pane = SCATOLA_SIDE_PANE (user_data);
...
static void
select_panel (ScatolaSidePane *side_pane, SidePanel *panel)
{
    int page_num;
...
static void
scatola_side_pane_class_init (ScatolaSidePaneClass *klass)
{
    ...
    gobject_class->finalize = scatola_side_pane_finalize;
    gobject_class->dispose = scatola_side_pane_dispose;

    signals[CLOSE_REQUESTED] = g_signal_new
                               ("close_requested",
                                G_TYPE_FROM_CLASS (klass),
                                G_SIGNAL_RUN_LAST,
                                G_STRUCT_OFFSET (ScatolaSidePaneClass,
                                        close_requested),
                                NULL, NULL,
    ...
    g_type_class_add_private (gobject_class, sizeof (ScatolaSidePaneDetails));
}
                      ...
                      GtkMenu *menu)
{
    ...
    side_pane = SCATOLA_SIDE_PANE (widget);

    side_pane->details->menu = NULL;
}

static void
scatola_side_pane_init (GObject *object)
{
    ScatolaSidePane *side_pane;
    GtkWidget *hbox;
    ...
    side_pane = SCATOLA_SIDE_PANE (object);

    side_pane->details = G_TYPE_INSTANCE_GET_PRIVATE (object, SCATOLA_TYPE_SIDE_PANE, ScatolaSidePaneDetails);

#if GTK_CHECK_VERSION(3, 0, 0)
    GtkStyleContext *context;

    context = gtk_widget_get_style_context (GTK_WIDGET (object));
    gtk_style_context_add_class (context, "scatola-side-pane");
#endif
...
static void
scatola_side_pane_dispose (GObject *object)
{
    ScatolaSidePane *side_pane;
...
static void
scatola_side_pane_finalize (GObject *object)
{
    ScatolaSidePane *side_pane;
    GList *l;
...
ScatolaSidePane *
scatola_side_pane_new (void)
{
    return SCATOLA_SIDE_PANE (gtk_widget_new (scatola_side_pane_get_type (), NULL));
}

void
scatola_side_pane_add_panel (ScatolaSidePane *side_pane,
                          GtkWidget *widget,
                          const char *title,
                          const char *tooltip)
{
    ...
    g_return_if_fail (SCATOLA_IS_SIDE_PANE (side_pane));
...
void
scatola_side_pane_remove_panel (ScatolaSidePane *side_pane,
                             GtkWidget *widget)
{
    SidePanel *panel;
    int page_num;
...
void
scatola_side_pane_show_panel (ScatolaSidePane *side_pane,
                           GtkWidget        *widget)
{
    SidePanel *panel;
    int page_num;
                           ...
                           gpointer user_data)
{
    ...
    scatola_side_pane_show_panel (side_pane, page);
}

static GtkWidget *
create_shortcut (ScatolaSidePane *side_pane,
                 SidePanel *panel,
                 GdkPixbuf *pixbuf)
{
    GtkWidget *button;
...
void
scatola_side_pane_set_panel_image (ScatolaSidePane *side_pane,
                                GtkWidget *widget,
                                GdkPixbuf *pixbuf)
{
    SidePanel *panel;
...
GtkWidget *
scatola_side_pane_get_current_panel (ScatolaSidePane *side_pane)
{
    int index;
...
GtkWidget *
scatola_side_pane_get_title (ScatolaSidePane *side_pane)
{
