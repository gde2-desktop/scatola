...
#define SCATOLA_MENU_PATH_EXTRA_VIEWER_PLACEHOLDER	"/MenuBar/View/View Choices/Extra Viewer"
#define SCATOLA_MENU_PATH_SHORT_LIST_PLACEHOLDER  	"/MenuBar/View/View Choices/Short List"
...
static void cancel_view_as_callback         (ScatolaWindowSlot      *slot);
static void scatola_window_info_iface_init (ScatolaWindowInfoIface *iface);
static void action_view_as_callback         (GtkAction               *action,
        ActivateViewData        *data);

static GList *history_list;

G_DEFINE_TYPE_WITH_CODE (ScatolaWindow, scatola_window, GTK_TYPE_WINDOW,
                         G_IMPLEMENT_INTERFACE (SCATOLA_TYPE_WINDOW_INFO,
                                 scatola_window_info_iface_init));
...
{
#ifdef HAVE_X11_XF86KEYSYM_H
    { XF86XK_AddFavorite,	SCATOLA_ACTION_ADD_BOOKMARK },
    { XF86XK_Favorites,	SCATOLA_ACTION_EDIT_BOOKMARKS },
    { XF86XK_Go,		SCATOLA_ACTION_GO_TO_LOCATION },
    /* TODO?{ XF86XK_History,	SCATOLA_ACTION_HISTORY }, */
    { XF86XK_HomePage,      SCATOLA_ACTION_GO_HOME },
    { XF86XK_OpenURL,	SCATOLA_ACTION_GO_TO_LOCATION },
    { XF86XK_Refresh,	SCATOLA_ACTION_RELOAD },
    { XF86XK_Reload,	SCATOLA_ACTION_RELOAD },
    { XF86XK_Search,	SCATOLA_ACTION_SEARCH },
    { XF86XK_Start,		SCATOLA_ACTION_GO_HOME },
    { XF86XK_Stop,		SCATOLA_ACTION_STOP },
    { XF86XK_ZoomIn,	SCATOLA_ACTION_ZOOM_IN },
    { XF86XK_ZoomOut,	SCATOLA_ACTION_ZOOM_OUT }
#endif
};

static void
scatola_window_init (ScatolaWindow *window)
{
    ...
    if (error != NULL) {
            g_warning ("Can't parse ScatolaWindow's CSS custom description: %s\n", error->message);
            g_error_free (error);
    } else {
...
#endif
    window->details = G_TYPE_INSTANCE_GET_PRIVATE (window, SCATOLA_TYPE_WINDOW, ScatolaWindowDetails);

    window->details->panes = NULL;
    window->details->active_pane = NULL;

    window->details->show_hidden_files_mode = SCATOLA_WINDOW_SHOW_HIDDEN_FILES_DEFAULT;
...
/* set margin to zero to reduce size of statusbar */
    ...
    scatola_window_initialize_menus (window);
    ...
    g_signal_connect_object (scatola_signaller_get_current (), "popup_menu_changed",
                             G_CALLBACK (scatola_window_load_extension_menus), window, G_CONNECT_SWAPPED);

/* Keep the main event loop alive as long as the window exists */
#if GTK_CHECK_VERSION(3, 0, 0)
    /* FIXME: port to GtkApplication with GTK3 */
    //gtk_quit_add_destroy (1, GTK_WIDGET (window));
    scatola_main_event_loop_register (GTK_WIDGET (window));
#else
    gtk_quit_add_destroy (1, GTK_OBJECT (window));
    scatola_main_event_loop_register (GTK_OBJECT (window));
#endif
}

/* Unconditionally synchronize the GtkUIManager of WINDOW. */
static void
scatola_window_ui_update (ScatolaWindow *window)
{
...
static void
scatola_window_push_status (ScatolaWindow *window,
                         const char *text)
{
...
void
scatola_window_sync_status (ScatolaWindow *window)
{
    ScatolaWindowSlot *slot;

    slot = window->details->active_pane->active_slot;
    scatola_window_push_status (window, slot->status_text);
}

void
scatola_window_go_to (ScatolaWindow *window, GFile *location)
{
    g_return_if_fail (SCATOLA_IS_WINDOW (window));

    scatola_window_slot_go_to (window->details->active_pane->active_slot, location, FALSE);
}

void
scatola_window_go_to_full (ScatolaWindow *window,
                        GFile                 *location,
                        ...
                        gpointer               user_data)
{
    ...
    scatola_window_slot_go_to_full (window->details->active_pane->active_slot, location, FALSE, callback, user_data);
}

void
scatola_window_go_to_with_selection (ScatolaWindow *window, GFile *location, GList *new_selection)
{
    g_return_if_fail (SCATOLA_IS_WINDOW (window));

    scatola_window_slot_go_to_with_selection (window->details->active_pane->active_slot, location, new_selection);
}

static gboolean
scatola_window_go_up_signal (ScatolaWindow *window, gboolean close_behind)
{
    scatola_window_go_up (window, close_behind, FALSE);
    return TRUE;
}

void
scatola_window_new_tab (ScatolaWindow *window)
{
    ScatolaWindowSlot *current_slot;
    ScatolaWindowSlot *new_slot;
    ...
    location = scatola_window_slot_get_location (current_slot);

    if (location != NULL) {
    	flags = 0;

    	new_slot_position = g_settings_get_enum (scatola_preferences, SCATOLA_PREFERENCES_NEW_TAB_POSITION);
    	if (new_slot_position == SCATOLA_NEW_TAB_POSITION_END) {
    		flags = SCATOLA_WINDOW_OPEN_SLOT_APPEND;
    	}

    	scheme = g_file_get_uri_scheme (location);
    	if (!strcmp (scheme, "x-scatola-search")) {
    		g_object_unref (location);
    		location = g_file_new_for_path (g_get_home_dir ());
    	}
    	g_free (scheme);

    	new_slot = scatola_window_open_slot (current_slot->pane, flags);
    	scatola_window_set_active_slot (window, new_slot);
    	scatola_window_slot_go_to (new_slot, location, FALSE);
    	g_object_unref (location);
    }
}

void
scatola_window_go_up (ScatolaWindow *window, gboolean close_behind, gboolean new_tab)
{
    ScatolaWindowSlot *slot;
    GFile *parent;
    GList *selection;
    ...
    if (close_behind)
    {
        flags |= SCATOLA_WINDOW_OPEN_FLAG_CLOSE_BEHIND;
    }
    if (new_tab)
    {
    ...
    scatola_window_slot_open_location_full (slot, parent,
...
static void
real_set_allow_up (ScatolaWindow *window,
                   gboolean        allow)
{
    GtkAction *action;
    ...
    action = gtk_action_group_get_action (window->details->main_action_group,
                                          SCATOLA_ACTION_UP_ACCEL);
    gtk_action_set_sensitive (action, allow);
}

void
scatola_window_allow_up (ScatolaWindow *window, gboolean allow)
{
    g_return_if_fail (SCATOLA_IS_WINDOW (window));

    EEL_CALL_METHOD (SCATOLA_WINDOW_CLASS, window,
                     set_allow_up, (window, allow));
}

static void
update_cursor (ScatolaWindow *window)
{
    ScatolaWindowSlot *slot;
    GdkCursor *cursor;
...
void
scatola_window_sync_allow_stop (ScatolaWindow *window,
                             ScatolaWindowSlot *slot)
{
    GtkAction *action;
    gboolean allow_stop;
...
void
scatola_window_allow_reload (ScatolaWindow *window, gboolean allow)
{
    GtkAction *action;
    ...
    action = gtk_action_group_get_action (window->details->main_action_group,
                                          SCATOLA_ACTION_RELOAD);
    gtk_action_set_sensitive (action, allow);
}

void
scatola_window_go_home (ScatolaWindow *window)
{
    g_return_if_fail (SCATOLA_IS_WINDOW (window));

    scatola_window_slot_go_home (window->details->active_pane->active_slot, FALSE);
}

void
scatola_window_prompt_for_location (ScatolaWindow *window,
                                 const char     *initial)
{
    g_return_if_fail (SCATOLA_IS_WINDOW (window));
...
static char *
scatola_window_get_location_uri (ScatolaWindow *window)
{
    ScatolaWindowSlot *slot;
...
void
scatola_window_zoom_in (ScatolaWindow *window)
{
    g_assert (window != NULL);

    scatola_window_pane_zoom_in (window->details->active_pane);
}

void
scatola_window_zoom_to_level (ScatolaWindow *window,
                           ScatolaZoomLevel level)
{
    g_assert (window != NULL);

    scatola_window_pane_zoom_to_level (window->details->active_pane, level);
}

void
scatola_window_zoom_out (ScatolaWindow *window)
{
    g_assert (window != NULL);

    scatola_window_pane_zoom_out (window->details->active_pane);
}

void
scatola_window_zoom_to_default (ScatolaWindow *window)
{
    g_assert (window != NULL);

    scatola_window_pane_zoom_to_default (window->details->active_pane);
}
...
/* This must be called when construction of ScatolaWindow is finished,
 * since it depends on the type of the argument, which isn't decided at
 * construction time.
 */
static void
scatola_window_set_initial_window_geometry (ScatolaWindow *window)
{
    GdkScreen *screen;
...
static void
scatola_window_constructed (GObject *self)
{
    ScatolaWindow *window;

    window = SCATOLA_WINDOW (self);

    scatola_window_initialize_bookmarks_menu (window);
    scatola_window_set_initial_window_geometry (window);
}

static void
scatola_window_set_property (GObject *object,
                          guint arg_id,
                          const GValue *value,
                          GParamSpec *pspec)
{
    ...
    window = SCATOLA_WINDOW (object);

    switch (arg_id)
    {
    case ARG_APP:
        window->application = SCATOLA_APPLICATION (g_value_get_object (value));
        break;
    }
}

static void
scatola_window_get_property (GObject *object,
                          guint arg_id,
                          GValue *value,
                          GParamSpec *pspec)
{
    ...
    {
    case ARG_APP:
        g_value_set_object (value, SCATOLA_WINDOW (object)->application);
        break;
    }
}

static void
free_stored_viewers (ScatolaWindow *window)
{
...
#if GTK_CHECK_VERSION (3, 0, 0)
static void
scatola_window_destroy (GtkWidget *object)
#else
static void
scatola_window_destroy (GtkObject *object)
#endif
{
    ScatolaWindow *window;
    GList *panes_copy;
    ...
    g_list_free_full (panes_copy, (GDestroyNotify) scatola_window_close_pane);
...
#if GTK_CHECK_VERSION (3, 0, 0)
    GTK_WIDGET_CLASS (scatola_window_parent_class)->destroy (object);
#else
    GTK_OBJECT_CLASS (scatola_window_parent_class)->destroy (object);
#endif
}

static void
scatola_window_finalize (GObject *object)
{
    ScatolaWindow *window;

    window = SCATOLA_WINDOW (object);

    scatola_window_finalize_menus (window);
    ...
    /* scatola_window_close() should have run */
    g_assert (window->details->panes == NULL);

    g_object_unref (window->details->ui_manager);

    G_OBJECT_CLASS (scatola_window_parent_class)->finalize (object);
}

static GObject *
scatola_window_constructor (GType type,
                         ...
                         GObjectConstructParam *construct_params)
{
    ...
    object = (* G_OBJECT_CLASS (scatola_window_parent_class)->constructor) (type,
             n_construct_properties,
             construct_params);

    window = SCATOLA_WINDOW (object);

    slot = scatola_window_open_slot (window->details->active_pane, 0);
    scatola_window_set_active_slot (window, slot);

    return object;
}

void
scatola_window_show_window (ScatolaWindow    *window)
{
    ScatolaWindowSlot *slot;
    ...
    for (walk = window->details->panes; walk; walk = walk->next)
    {
        ...
        for (l = pane->slots; l != NULL; l = l->next)
        {
            ...
            scatola_window_slot_update_title (slot);
    ...
    if (slot->viewed_file)
    {
        if (SCATOLA_IS_SPATIAL_WINDOW (window))
        {
            scatola_file_set_has_open_window (slot->viewed_file, TRUE);
        }
    }
}

static void
scatola_window_view_visible (ScatolaWindow *window,
                          ScatolaView *view)
{
    ScatolaWindowSlot *slot;
    ScatolaWindowPane *pane;
    ...
    slot = scatola_window_get_slot_for_view (window, view);

    /* Ensure we got the right active state for newly added panes */
    scatola_window_slot_is_in_active_pane (slot, slot->pane->is_active);
    ...
    scatola_window_pane_grab_focus (window->details->active_pane);

    /* All slots and panes visible, show window */
    scatola_window_show_window (window);
}

void
scatola_window_close (ScatolaWindow *window)
{
    g_return_if_fail (SCATOLA_IS_WINDOW (window));
...
ScatolaWindowSlot *
scatola_window_open_slot (ScatolaWindowPane *pane,
                       ScatolaWindowOpenSlotFlags flags)
{
    ScatolaWindowSlot *slot;

    g_assert (SCATOLA_IS_WINDOW_PANE (pane));
    g_assert (SCATOLA_IS_WINDOW (pane->window));

    slot = EEL_CALL_METHOD_WITH_RETURN_VALUE (SCATOLA_WINDOW_CLASS, pane->window,
...
void
scatola_window_close_pane (ScatolaWindowPane *pane)
{
    ScatolaWindow *window;
    ...
    {
        ScatolaWindowSlot *slot = pane->slots->data;
     ...
     * for setting a new active pane with scatola_window_pane_switch_to()
...
real_close_slot (ScatolaWindowPane *pane,
                 ScatolaWindowSlot *slot)
{
    scatola_window_manage_views_close_slot (pane, slot);
    cancel_view_as_callback (slot);
}

void
scatola_window_close_slot (ScatolaWindowSlot *slot)
{
    ScatolaWindowPane *pane;
    ...
    EEL_CALL_METHOD (SCATOLA_WINDOW_CLASS, slot->pane->window,
...
ScatolaWindowPane*
scatola_window_get_active_pane (ScatolaWindow *window)
{
    g_assert (SCATOLA_IS_WINDOW (window));
...
static void
real_set_active_pane (ScatolaWindow *window, ScatolaWindowPane *new_pane)
{
    ...
    {
        scatola_window_pane_set_active (new_pane->window->details->active_pane, FALSE);
    }
    scatola_window_pane_set_active (new_pane, TRUE);
 ...
 * the window. */
void
scatola_window_set_active_pane (ScatolaWindow *window,
                             ScatolaWindowPane *new_pane)
{
    g_assert (SCATOLA_IS_WINDOW_PANE (new_pane));
    if (new_pane->active_slot)
    {
        scatola_window_set_active_slot (window, new_pane->active_slot);
    }
...
void
scatola_window_set_active_slot (ScatolaWindow *window, ScatolaWindowSlot *new_slot)
{
    ScatolaWindowSlot *old_slot;

    g_assert (SCATOLA_IS_WINDOW (window));

    if (new_slot)
    {
        g_assert (SCATOLA_IS_WINDOW_SLOT (new_slot));
        g_assert (SCATOLA_IS_WINDOW_PANE (new_slot->pane));
    ...
    if (old_slot != NULL)
    {
        ...
        {
            scatola_window_slot_disconnect_content_view (old_slot, old_slot->content_view);
        }
    ...
    if (new_slot)
    {
        ...
        scatola_window_report_location_change (window);
        /* TODO decide whether "selection-changed" should be emitted */

        if (new_slot->content_view != NULL)
        {
            /* inform window */
            scatola_window_slot_connect_content_view (new_slot, new_slot->content_view);
        }
...
void
scatola_window_slot_close (ScatolaWindowSlot *slot)
{
    scatola_window_pane_slot_close (slot->pane, slot);
}

static void
scatola_window_size_request (GtkWidget		*widget,
                          GtkRequisition	*requisition)
{
    ...
    g_assert (SCATOLA_IS_WINDOW (widget));
    g_assert (requisition != NULL);
#if !GTK_CHECK_VERSION(3,0,0)
    GTK_WIDGET_CLASS (scatola_window_parent_class)->size_request (widget, requisition);
#endif
...
static void
scatola_window_realize (GtkWidget *widget)
{
    GTK_WIDGET_CLASS (scatola_window_parent_class)->realize (widget);
    update_cursor (SCATOLA_WINDOW (widget));
}

static gboolean
scatola_window_key_press_event (GtkWidget *widget,
                             GdkEventKey *event)
{
    ScatolaWindow *window;
    int i;
    ...
    return GTK_WIDGET_CLASS (scatola_window_parent_class)->key_press_event (widget, event);
}
...
add_view_as_menu_item (ScatolaWindow *window,
                       ...
                       guint merge_id)
{
    ...
    info = scatola_view_factory_lookup (identifier);
    ...
    if (index >= 1 && index <= 9)
    {
        ...
        g_snprintf (accel_path, sizeof (accel_path), "<Scatola-Window>/%s", action_name);
 ...
 */
static void
update_extra_viewer_in_view_as_menus (ScatolaWindow *window,
                                      const char *id)
{
    ...
    if (id != NULL)
    {
        ...
        window->details->extra_viewer_radio_action =
            add_view_as_menu_item (window,
                                   SCATOLA_MENU_PATH_EXTRA_VIEWER_PLACEHOLDER,
...
static void
remove_extra_viewer_in_view_as_menus (ScatolaWindow *window)
{
    update_extra_viewer_in_view_as_menus (window, NULL);
}

static void
replace_extra_viewer_in_view_as_menus (ScatolaWindow *window)
{
    ScatolaWindowSlot *slot;
    const char *id;
...
/**
 * scatola_window_synch_view_as_menus:
 *
 ...
 *
 * @window: The ScatolaWindow whose "View as" option menu should be synched.
 */
static void
scatola_window_synch_view_as_menus (ScatolaWindow *window)
{
    ScatolaWindowSlot *slot;
    int index;
    char action_name[32];
...
static void
refresh_stored_viewers (ScatolaWindow *window)
{
    ScatolaWindowSlot *slot;
    GList *viewers;
    char *uri, *mimetype;
    ...
    uri = scatola_file_get_uri (slot->viewed_file);
    mimetype = scatola_file_get_mime_type (slot->viewed_file);
    viewers = scatola_view_factory_get_views_for_uri (uri,
              scatola_file_get_file_type (slot->viewed_file),
              mimetype);
    g_free (uri);
    g_free (mimetype);
...
static void
load_view_as_menu (ScatolaWindow *window)
{
    GList *node;
    int index;
    guint merge_id;
            ...
            node = node->next, ++index)
    {
        ...
        add_view_as_menu_item (window,
                               SCATOLA_MENU_PATH_SHORT_LIST_PLACEHOLDER,
                               node->data,
                               index,
                               merge_id);
    }
    ...
    scatola_window_synch_view_as_menus (window);
...
static void
load_view_as_menus_callback (ScatolaFile *file,
...
static void
cancel_view_as_callback (ScatolaWindowSlot *slot)
{
    scatola_file_cancel_call_when_ready (slot->viewed_file,
                                      load_view_as_menus_callback,
                                      slot);
}

void
scatola_window_load_view_as_menus (ScatolaWindow *window)
{
    ScatolaWindowSlot *slot;
    ...
    cancel_view_as_callback (slot);
    scatola_file_call_when_ready (slot->viewed_file,
                               attributes,
                               load_view_as_menus_callback,
                               slot);
}
...
void
scatola_window_display_error (ScatolaWindow *window, const char *error_msg)
{
    GtkWidget *dialog;
...
real_get_title (ScatolaWindow *window)
{
    ...
    return scatola_window_slot_get_title (window->details->active_pane->active_slot);
}
...
void
scatola_window_sync_title (ScatolaWindow *window,
                        ScatolaWindowSlot *slot)
{
    EEL_CALL_METHOD (SCATOLA_WINDOW_CLASS, window,
...
void
scatola_window_sync_zoom_widgets (ScatolaWindow *window)
{
    ScatolaWindowSlot *slot;
    ScatolaView *view;
    GtkAction *action;
    ...
    if (view != NULL)
    {
        supports_zooming = scatola_view_supports_zooming (view);
        zoom_level = scatola_view_get_zoom_level (view);
        can_zoom = supports_zooming &&
                   zoom_level >= SCATOLA_ZOOM_LEVEL_SMALLEST &&
                   zoom_level <= SCATOLA_ZOOM_LEVEL_LARGEST;
        can_zoom_in = can_zoom && scatola_view_can_zoom_in (view);
        can_zoom_out = can_zoom && scatola_view_can_zoom_out (view);
    }
    else
    {
        zoom_level = SCATOLA_ZOOM_LEVEL_STANDARD;
...
static void
zoom_level_changed_callback (ScatolaView *view,
                             ScatolaWindow *window)
{
    g_assert (SCATOLA_IS_WINDOW (window));
...
*/
void
scatola_window_connect_content_view (ScatolaWindow *window,
                                  ScatolaView *view)
{
    ScatolaWindowSlot *slot;

    g_assert (SCATOLA_IS_WINDOW (window));
    g_assert (SCATOLA_IS_VIEW (view));
    ...
    scatola_view_grab_focus (view);
}

void
scatola_window_disconnect_content_view (ScatolaWindow *window,
                                     ScatolaView *view)
{
    ScatolaWindowSlot *slot;
 ...
 */
static void
scatola_window_show (GtkWidget *widget)
{
    ScatolaWindow *window;

    window = SCATOLA_WINDOW (widget);

    GTK_WIDGET_CLASS (scatola_window_parent_class)->show (widget);

    scatola_window_ui_update (window);
}

GtkUIManager *
scatola_window_get_ui_manager (ScatolaWindow *window)
{
    g_return_val_if_fail (SCATOLA_IS_WINDOW (window), NULL);

    return window->details->ui_manager;
}

ScatolaWindowPane *
scatola_window_get_next_pane (ScatolaWindow *window)
{
...
void
scatola_window_slot_set_viewed_file (ScatolaWindowSlot *slot,
                                  ScatolaFile *file)
{
    ScatolaWindow *window;
    ScatolaFileAttributes attributes;
    ...
    if (slot->viewed_file != NULL)
    {
        ...
        if (SCATOLA_IS_SPATIAL_WINDOW (window))
        {
            scatola_file_set_has_open_window (slot->viewed_file,
                                           FALSE);
        }
        scatola_file_monitor_remove (slot->viewed_file,
                                  slot);
    }
    ...
    if (file != NULL)
    {
        ...
        scatola_file_monitor_add (file, slot, attributes);
    }

    scatola_file_unref (slot->viewed_file);
    slot->viewed_file = file;
}

void
scatola_send_history_list_changed (void)
{
    g_signal_emit_by_name (scatola_signaller_get_current (),
 ...
 */
static void
remove_from_history_list (ScatolaBookmark *bookmark)
{
    GList *node;
...
gboolean
scatola_add_bookmark_to_history_list (ScatolaBookmark *bookmark)
{
    /* Note that the history is shared amongst all windows so
     * this is not a ScatolaNavigationWindow function. Perhaps it belongs
     * in its own file.
     */
    int i;
    GList *l, *next;
    ...
    /*	g_warning ("Add to history list '%s' '%s'",
    		   scatola_bookmark_get_name (bookmark),
    		   scatola_bookmark_get_uri (bookmark)); */

    if (!history_list ||
            scatola_bookmark_compare_uris (history_list->data, bookmark))
    {
...
void
scatola_remove_from_history_list_no_notify (GFile *location)
{
    ScatolaBookmark *bookmark;

    bookmark = scatola_bookmark_new (location, "", FALSE, NULL);
    remove_from_history_list (bookmark);
    g_object_unref (bookmark);
}

gboolean
scatola_add_to_history_list_no_notify (GFile *location,
                                    const char *name,
                                    ...
                                    GIcon *icon)
{
    ...
    bookmark = scatola_bookmark_new (location, name, has_custom_name, icon);
    ret = scatola_add_bookmark_to_history_list (bookmark);
    g_object_unref (bookmark);
...
ScatolaWindowSlot *
scatola_window_get_slot_for_view (ScatolaWindow *window,
                               ScatolaView *view)
{
    ScatolaWindowSlot *slot;
    GList *l, *walk;
    ...
    for (walk = window->details->panes; walk; walk = walk->next)
    {
        ScatolaWindowPane *pane = walk->data;
...
scatola_forget_history (void)
{
     ...
     */
    for (window_node = scatola_application_get_window_list ();
            window_node != NULL;
            window_node = window_node->next)
    {

        if (SCATOLA_IS_NAVIGATION_WINDOW (window_node->data))
        {
            ScatolaNavigationWindow *window;

            window = SCATOLA_NAVIGATION_WINDOW (window_node->data);

            for (walk = SCATOLA_WINDOW (window_node->data)->details->panes; walk; walk = walk->next)
            {
                ScatolaWindowPane *pane = walk->data;
                for (l = pane->slots; l != NULL; l = l->next)
                {
                    ...
                    scatola_navigation_window_slot_clear_back_list (navigation_slot);
                    scatola_navigation_window_slot_clear_forward_list (navigation_slot);
                }
            }
            ...
            window_node = window_node->next)
    {
        ...
        window = SCATOLA_WINDOW (window_node->data);
        for (walk = window->details->panes; walk; walk = walk->next)
        {
            ScatolaWindowPane *pane = walk->data;
            for (l = pane->slots; l != NULL; l = l->next)
            {
                ...
                scatola_window_slot_add_current_location_to_history_list (slot);
            }
        }
    }
}
...
GList *
scatola_get_history_list (void)
{
    return history_list;
}

static GList *
scatola_window_get_history (ScatolaWindow *window)
{
    return eel_g_object_list_copy (history_list);
}


static ScatolaWindowType
scatola_window_get_window_type (ScatolaWindow *window)
{
    g_assert (SCATOLA_IS_WINDOW (window));

    return SCATOLA_WINDOW_GET_CLASS (window)->window_type;
}

static int
scatola_window_get_selection_count (ScatolaWindow *window)
{
    ScatolaWindowSlot *slot;
...
static GList *
scatola_window_get_selection (ScatolaWindow *window)
{
    ScatolaWindowSlot *slot;
    ...
    if (slot->content_view != NULL)
    {
        return scatola_view_get_selection (slot->content_view);
    }
    return NULL;
}

static ScatolaWindowShowHiddenFilesMode
scatola_window_get_hidden_files_mode (ScatolaWindowInfo *window)
{
    return window->details->show_hidden_files_mode;
}

static void
scatola_window_set_hidden_files_mode (ScatolaWindowInfo *window,
...
static gboolean
scatola_window_get_initiated_unmount (ScatolaWindowInfo *window)
{
    return window->details->initiated_unmount;
}

static void
scatola_window_set_initiated_unmount (ScatolaWindowInfo *window,
...
static char *
scatola_window_get_cached_title (ScatolaWindow *window)
{
    ScatolaWindowSlot *slot;
...
ScatolaWindowSlot *
scatola_window_get_active_slot (ScatolaWindow *window)
{
    g_assert (SCATOLA_IS_WINDOW (window));
...
ScatolaWindowSlot *
scatola_window_get_extra_slot (ScatolaWindow *window)
{
    ScatolaWindowPane *extra_pane;
    GList *node;
...
GList *
scatola_window_get_slots (ScatolaWindow *window)
{
    GList *walk,*list;
...
static void
scatola_window_info_iface_init (ScatolaWindowInfoIface *iface)
{
    iface->report_load_underway = scatola_window_report_load_underway;
    iface->report_load_complete = scatola_window_report_load_complete;
    iface->report_selection_changed = scatola_window_report_selection_changed;
    iface->report_view_failed = scatola_window_report_view_failed;
    iface->view_visible = scatola_window_view_visible;
    iface->close_window = scatola_window_close;
    iface->push_status = scatola_window_push_status;
    iface->get_window_type = scatola_window_get_window_type;
    iface->get_title = scatola_window_get_cached_title;
    iface->get_history = scatola_window_get_history;
    iface->get_current_location = scatola_window_get_location_uri;
    iface->get_ui_manager = scatola_window_get_ui_manager;
    iface->get_selection_count = scatola_window_get_selection_count;
    iface->get_selection = scatola_window_get_selection;
    iface->get_hidden_files_mode = scatola_window_get_hidden_files_mode;
    iface->set_hidden_files_mode = scatola_window_set_hidden_files_mode;
    iface->get_active_slot = scatola_window_get_active_slot;
    iface->get_extra_slot = scatola_window_get_extra_slot;
    iface->get_initiated_unmount = scatola_window_get_initiated_unmount;
    iface->set_initiated_unmount = scatola_window_set_initiated_unmount;
}

static void
scatola_window_class_init (ScatolaWindowClass *class)
{
    GtkBindingSet *binding_set;

    G_OBJECT_CLASS (class)->constructor = scatola_window_constructor;
    G_OBJECT_CLASS (class)->constructed = scatola_window_constructed;
    G_OBJECT_CLASS (class)->get_property = scatola_window_get_property;
    G_OBJECT_CLASS (class)->set_property = scatola_window_set_property;
    G_OBJECT_CLASS (class)->finalize = scatola_window_finalize;

#if !GTK_CHECK_VERSION (3, 0, 0)
    GTK_OBJECT_CLASS (class)->destroy = scatola_window_destroy;
#else
    GTK_WIDGET_CLASS (class)->destroy = scatola_window_destroy;
#endif

    GTK_WIDGET_CLASS (class)->show = scatola_window_show;
#if !GTK_CHECK_VERSION (3,0,0)
    GTK_WIDGET_CLASS (class)->size_request = scatola_window_size_request;
#endif
    GTK_WIDGET_CLASS (class)->realize = scatola_window_realize;
    GTK_WIDGET_CLASS (class)->key_press_event = scatola_window_key_press_event;
    ...
    g_object_class_install_property (G_OBJECT_CLASS (class),
                                     ...
                                     g_param_spec_object ("app",
                                             "Application",
                                             "The ScatolaApplication associated with this window.",
                                             SCATOLA_TYPE_APPLICATION,
    ...
    signals[GO_UP] =
        g_signal_new ("go_up",
                      ...
                      G_STRUCT_OFFSET (ScatolaWindowClass, go_up),
                      g_signal_accumulator_true_handled, NULL,
                      scatola_src_marshal_BOOLEAN__BOOLEAN,
                      G_TYPE_BOOLEAN, 1, G_TYPE_BOOLEAN);
    signals[RELOAD] =
        g_signal_new ("reload",
                      G_TYPE_FROM_CLASS (class),
                      G_SIGNAL_RUN_LAST | G_SIGNAL_ACTION,
                      G_STRUCT_OFFSET (ScatolaWindowClass, reload),
                      NULL, NULL,
    ...
    signals[PROMPT_FOR_LOCATION] =
        g_signal_new ("prompt-for-location",
                      G_TYPE_FROM_CLASS (class),
                      G_SIGNAL_RUN_LAST | G_SIGNAL_ACTION,
                      G_STRUCT_OFFSET (ScatolaWindowClass, prompt_for_location),
                      NULL, NULL,
    ...
    signals[ZOOM_CHANGED] =
        g_signal_new ("zoom-changed",
                      ...
                      0,
                      NULL, NULL,
                      scatola_src_marshal_VOID__INT_BOOLEAN_BOOLEAN_BOOLEAN_BOOLEAN,
                      G_TYPE_NONE, 5,
    ...
    class->reload = scatola_window_reload;
    class->go_up = scatola_window_go_up_signal;

#if !GTK_CHECK_VERSION (3,0,0)
    /* Allow to set the colors of the extra view widgets */
    gtk_rc_parse_string ("\n"
                         "   style \"scatola-extra-view-widgets-style-internal\"\n"
                         "   {\n"
                         "      bg[NORMAL] = \"" EXTRA_VIEW_WIDGETS_BACKGROUND "\"\n"
                         "   }\n"
                         "\n"
                         "    widget \"*.scatola-extra-view-widget\" style:rc \"scatola-extra-view-widgets-style-internal\" \n"
                         "\n");
#endif

    g_type_class_add_private (G_OBJECT_CLASS (class), sizeof (ScatolaWindowDetails));
}
