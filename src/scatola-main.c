...
/* scatola-main.c: Implementation of the routines that drive program lifecycle and main window creation/destruction. */
...
#include <libscatola-private/scatola-lib-self-check-functions.h>
...
void scatola_main_event_loop_quit (gboolean explicit)
{
    if (explicit)
    {
        ...
        {
            /* If this is reached, scatola must run in "daemon" mode
        ...
        /* TODO: With the old session we needed to set restart
           style to GDE2_RESTART_IF_RUNNING here, but i don't think we need
           that now since gde2-session doesn't restart apps except on startup. */
    }
...
static void dump_debug_log (void)
{
    ...
    filename = g_build_filename (g_get_home_dir (), "scatola-debug-log.txt", NULL);
    scatola_debug_log_dump (filename, NULL); /* NULL GError */
    g_free (filename);
}
...
static gboolean debug_log_io_cb (GIOChannel *io, GIOCondition condition, gpointer data)
{
    ...
    scatola_debug_log (TRUE, SCATOLA_DEBUG_LOG_DOMAIN_USER,
                 ...
                 gpointer       user_data)
{
    ...
    scatola_debug_log (is_milestone, SCATOLA_DEBUG_LOG_DOMAIN_GLOG, "%s", message);
...
setup_debug_log (void)
{
    ...
    config_filename = g_build_filename (g_get_home_dir (), "scatola-debug-log.conf", NULL);
    scatola_debug_log_load_configuration (config_filename, NULL); /* NULL GError */
...
static gboolean
running_in_gde2 (void)
{
    return (g_strcmp0 (g_getenv ("XDG_CURRENT_DESKTOP"), "GDE2") == 0)
        || (g_strcmp0 (g_getenv ("XDG_SESSION_DESKTOP"), "GDE2") == 0)
        || (g_strcmp0 (g_getenv ("DESKTOP_SESSION"), "GDE2") == 0);
}
...
main (int argc, char *argv[])
{
    ...
    {
#ifndef SCATOLA_OMIT_SELF_CHECK
        {
    ...
    /* Scatola uses lots and lots of small and medium size allocations,
     ...
     * be mmaped. Unfortunately this triggers quickly for scatola when
    ...
    /* This will be done by gtk+ later, but for now, force it to GDE2 */
    g_desktop_app_info_set_desktop_env ("GDE2");
#endif

    if (g_getenv ("SCATOLA_DEBUG") != NULL)
    {
    ...
    g_set_prgname ("scatola");

#if GTK_CHECK_VERSION(3, 0, 0)
    gdk_set_allowed_backends ("x11");
#endif

    if (g_file_test (DATADIR "/applications/scatola.desktop", G_FILE_TEST_EXISTS))
    {
        egg_set_desktop_file (DATADIR "/applications/scatola.desktop");
    }
    ...
    if (version)
    {
        g_print ("GDE2 scatola " PACKAGE_VERSION "\n");
        return 0;
    }
    ...
    /* If in autostart mode (aka started by gde2-session), we need to ensure
     * scatola starts with the correct options.
     */
    if (autostart_mode)
    {
    ...
    else if (running_as_root () || !running_in_gde2 ())
    {
    ...
    if (perform_self_check && remaining != NULL)
    {
        ...
        fprintf (stderr, _("scatola: %s cannot be used with URIs.\n"),
                 "--check");
        return EXIT_FAILURE;
    }
    if (perform_self_check && kill_shell)
    {
        fprintf (stderr, _("scatola: --check cannot be used with other options.\n"));
        return EXIT_FAILURE;
    }
    ...
    {
        fprintf (stderr, _("scatola: --geometry cannot be used with more than one URI.\n"));
        return EXIT_FAILURE;
    }
     ...
     * on other desktops or when running scatola as root. Otherwise, we read the value
     * from the configuration.
     */
    if (running_in_gde2 () && !running_as_root())
    {
        exit_with_last_window = g_settings_get_boolean (scatola_preferences, SCATOLA_PREFERENCES_EXIT_WITH_LAST_WINDOW);
    }
    ...
    {
		#ifndef SCATOLA_OMIT_SELF_CHECK
			/* Run the checks (each twice) for scatola and libscatola-private. */

			scatola_run_self_checks ();
			scatola_run_lib_self_checks ();
    ...
    else
    {
        ...
        if (remaining != NULL)
        {
                    ...
                    if (uri)
                    {
                        ...
                        else
                        {
                            ...
                            if (fileinfo)
                            {
                                ...
                                if (appinfo)
                                {
                                    if (g_strcmp0 (g_app_info_get_executable (appinfo), "scatola") != 0)
                                    {
                                        g_app_info_launch_default_for_uri (uri, NULL, NULL);
                                    }
                                    else
                                    {
                                        fprintf (stderr, _("scatola: set erroneously as default application for '%s' content type.\n"),
            ...
            if (uris_array->len == 0)
            {
                /* Scatola is being used only to open files (not directories), so closing */
        ...
        /* Run the scatola application. */
        application = scatola_application_new ();
