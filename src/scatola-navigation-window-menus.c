...
/* scatola-window-menus.h - implementation of scatola window menu operations,
...
static void                  schedule_refresh_go_menu                      (ScatolaNavigationWindow   *window);

static void
action_close_all_windows_callback (GtkAction *action,
                                   gpointer user_data)
{
    scatola_application_close_all_navigation_windows ();
}
...
action_back_callback (GtkAction *action,
                      gpointer user_data)
{
    scatola_navigation_window_back_or_forward (SCATOLA_NAVIGATION_WINDOW (user_data),
...
forget_history_if_yes (GtkDialog *dialog, int response, gpointer callback_data)
{
    ...
    {
        scatola_forget_history ();
    }
    gtk_widget_destroy (GTK_WIDGET (dialog));
}

static void
forget_history_if_confirmed (ScatolaWindow *window)
{
    GtkDialog *dialog;
...
action_clear_history_callback (GtkAction *action,
                               gpointer user_data)
{
    forget_history_if_confirmed (SCATOLA_WINDOW (user_data));
}
...
action_split_view_switch_next_pane_callback(GtkAction *action,
        gpointer user_data)
{
    scatola_window_pane_switch_to (scatola_window_get_next_pane (SCATOLA_WINDOW (user_data)));
}
        ...
        gpointer user_data)
{
    ...
    location = scatola_window_slot_get_location (next_pane->active_slot);
    if (location)
    {
...
static void
pane_show_hide_location_bar (ScatolaNavigationWindowPane *pane, gboolean is_active)
{
    if (scatola_navigation_window_pane_location_bar_showing (pane) != is_active)
    {
        if (is_active)
        {
                                        ...
                                        gpointer user_data)
{
    ...
    pane_show_hide_location_bar (SCATOLA_NAVIGATION_WINDOW_PANE (window->details->active_pane), is_active);

    for (walk = window->details->panes; walk; walk = walk->next)
    {
        pane_show_hide_location_bar (SCATOLA_NAVIGATION_WINDOW_PANE (walk->data), is_active);
    }
}
                            ...
                            gpointer user_data)
{
    ...
    is_active = gtk_toggle_action_get_active (GTK_TOGGLE_ACTION (action));
    if (is_active != scatola_navigation_window_split_view_showing (window))
    {
        ScatolaWindow *scatola_window;

        if (is_active)
        {
            scatola_navigation_window_split_view_on (window);
        }
        else
        {
            scatola_navigation_window_split_view_off (window);
        }
        scatola_window = SCATOLA_WINDOW (window);
        if (scatola_window->details->active_pane && scatola_window->details->active_pane->active_slot)
        {
            scatola_view_update_menus (scatola_window->details->active_pane->active_slot->content_view);
        }
    }
}

void
scatola_navigation_window_update_show_hide_menu_items (ScatolaNavigationWindow *window)
{
    GtkAction *action;
    ...
    action = gtk_action_group_get_action (window->details->navigation_action_group,
                                          SCATOLA_ACTION_SHOW_HIDE_TOOLBAR);
    gtk_toggle_action_set_active (GTK_TOGGLE_ACTION (action),
                                  scatola_navigation_window_toolbar_showing (window));

    action = gtk_action_group_get_action (window->details->navigation_action_group,
                                          SCATOLA_ACTION_SHOW_HIDE_SIDEBAR);
    gtk_toggle_action_set_active (GTK_TOGGLE_ACTION (action),
                                  scatola_navigation_window_sidebar_showing (window));

    action = gtk_action_group_get_action (window->details->navigation_action_group,
                                          SCATOLA_ACTION_SHOW_HIDE_LOCATION_BAR);
    gtk_toggle_action_set_active (GTK_TOGGLE_ACTION (action),
                                  scatola_navigation_window_pane_location_bar_showing (SCATOLA_NAVIGATION_WINDOW_PANE (SCATOLA_WINDOW (window)->details->active_pane)));

    action = gtk_action_group_get_action (window->details->navigation_action_group,
                                          SCATOLA_ACTION_SHOW_HIDE_STATUSBAR);
    gtk_toggle_action_set_active (GTK_TOGGLE_ACTION (action),
                                  scatola_navigation_window_status_bar_showing (window));

    action = gtk_action_group_get_action (window->details->navigation_action_group,
                                          SCATOLA_ACTION_SHOW_HIDE_EXTRA_PANE);
    gtk_toggle_action_set_active (GTK_TOGGLE_ACTION (action),
                                  scatola_navigation_window_split_view_showing (window));
}

void
scatola_navigation_window_update_spatial_menu_item (ScatolaNavigationWindow *window)
{
    GtkAction *action;
...
action_add_bookmark_callback (GtkAction *action,
                              gpointer user_data)
{
    scatola_window_add_bookmark_for_current_location (SCATOLA_WINDOW (user_data));
}
...
void
scatola_navigation_window_remove_go_menu_callback (ScatolaNavigationWindow *window)
{
...
void
scatola_navigation_window_remove_go_menu_items (ScatolaNavigationWindow *window)
{
...
static void
show_bogus_history_window (ScatolaWindow *window,
 ...
 * @window: The ScatolaWindow whose Go menu will be refreshed.
 **/
static void
refresh_go_menu (ScatolaNavigationWindow *window)
{
    GtkUIManager *ui_manager;
    GList *node;
    GtkWidget *menuitem;
    ...
    scatola_navigation_window_remove_go_menu_items (window);
    ...
    for (node = scatola_get_history_list (), index = 0;
            node != NULL && index < 10;
            node = node->next, index++)
    {
        scatola_menus_append_bookmark_to_menu
        (SCATOLA_WINDOW (window),
         SCATOLA_BOOKMARK (node->data),
...
{
    g_assert (SCATOLA_IS_NAVIGATION_WINDOW (data));

    refresh_go_menu (SCATOLA_NAVIGATION_WINDOW (data));
...
static void
schedule_refresh_go_menu (ScatolaNavigationWindow *window)
{
    g_assert (SCATOLA_IS_NAVIGATION_WINDOW (window));
...
/**
 * scatola_navigation_window_initialize_go_menu
 *
 * Wire up signals so we'll be notified when history list changes.
 */
static void
scatola_navigation_window_initialize_go_menu (ScatolaNavigationWindow *window)
{
    /* Recreate bookmarks part of menu if history list changes
     */
    g_signal_connect_object (scatola_signaller_get_current (), "history_list_changed",
                             G_CALLBACK (schedule_refresh_go_menu), window, G_CONNECT_SWAPPED);
}

void
scatola_navigation_window_update_split_view_actions_sensitivity (ScatolaNavigationWindow *window)
{
    ScatolaWindow *win;
    ...
    {
        active_pane_location = scatola_window_slot_get_location (win->details->active_pane->active_slot);
    }
    else
    {
        active_pane_location = NULL;
    }
    next_pane = scatola_window_get_next_pane (win);
    if (next_pane && next_pane->active_slot)
    {
        next_pane_location = scatola_window_slot_get_location (next_pane->active_slot);
...
action_new_window_callback (GtkAction *action,
                            gpointer user_data)
{
    ScatolaWindow *current_window;
    ScatolaWindow *new_window;

    current_window = SCATOLA_WINDOW (user_data);
    new_window = scatola_application_create_navigation_window (
                     current_window->application,
                     NULL,
                     gtk_window_get_screen (GTK_WINDOW (current_window)));
    scatola_window_go_home (new_window);
}
...
action_folder_window_callback (GtkAction *action,
                               gpointer user_data)
{
    ScatolaWindow *current_window, *window;
    ScatolaWindowSlot *slot;
    GFile *current_location;

    current_window = SCATOLA_WINDOW (user_data);
    slot = current_window->details->active_pane->active_slot;
    current_location = scatola_window_slot_get_location (slot);
    window = scatola_application_get_spatial_window
            (current_window->application,
             current_window,
             NULL,
             current_location,
    ...
    scatola_window_go_to (window, current_location);
                                  ...
                                  gpointer user_data)
{
    ...
    else
    {
        ...
        else
        {
            ...
            query = scatola_query_editor_get_query (slot->query_editor);
            if (query != NULL)
            {
        ...
        scatola_navigation_window_hide_search (window);
    }
}
                               ...
                               gpointer user_data)
{
    ...
    pane = SCATOLA_NAVIGATION_WINDOW_PANE (SCATOLA_WINDOW (user_data)->details->active_pane);
    scatola_notebook_set_current_page_relative (SCATOLA_NOTEBOOK (pane->notebook), -1);
}
...
action_tab_change_action_activate_callback (GtkAction *action, gpointer user_data)
{
    ...
    if (window && window->details->active_pane)
    {
        ...
        notebook = GTK_NOTEBOOK (SCATOLA_NAVIGATION_WINDOW_PANE (window->details->active_pane)->notebook);
        if (notebook)
        {
            int num;
...
static const GtkActionEntry navigation_entries[] =
{
    ...
    /* name, stock id, label */  { "New Window", "window-new", N_("New _Window"),
        "<control>N", N_("Open another Scatola window for the displayed location"),
...
static const GtkToggleActionEntry navigation_toggle_entries[] =
{
    ...
    /* name, stock id */     {
        SCATOLA_ACTION_SHOW_HIDE_EXTRA_PANE, NULL,
...
void
scatola_navigation_window_initialize_actions (ScatolaNavigationWindow *window)
{
    GtkActionGroup *action_group;
    GtkUIManager *ui_manager;
    GtkAction *action;
    ...
    action = g_object_new (SCATOLA_TYPE_NAVIGATION_ACTION,
                           "name", "Back",
                           "label", _("_Back"),
                           ...
                           "window", window,
                           "direction", SCATOLA_NAVIGATION_DIRECTION_BACK,
                           "is_important", TRUE,
                           NULL);
    ...
    action = g_object_new (SCATOLA_TYPE_NAVIGATION_ACTION,
                           ...
                           "window", window,
                           "direction", SCATOLA_NAVIGATION_DIRECTION_FORWARD,
                           "is_important", TRUE,
                           NULL);
    g_signal_connect (action, "activate",
    ...
    action = g_object_new (SCATOLA_TYPE_ZOOM_ACTION,
                           "name", "Zoom",
                           "label", _("_Zoom"),
                           "window", window,
                           "is_important", FALSE,
    ...
    action = g_object_new (SCATOLA_TYPE_VIEW_AS_ACTION,
                           "name", "ViewAs",
                           "label", _("_View As"),
                           "window", window,
                           "is_important", FALSE,
    ...
    action = gtk_action_group_get_action (action_group, SCATOLA_ACTION_SEARCH);
    ...
    g_signal_connect (window, "loading_uri",
                      G_CALLBACK (scatola_navigation_window_update_split_view_actions_sensitivity),
                      NULL);
...
/**
 * scatola_window_initialize_menus
 *
 * Create and install the set of menus for this window.
 * @window: A recently-created ScatolaWindow.
 */
void
scatola_navigation_window_initialize_menus (ScatolaNavigationWindow *window)
{
    GtkUIManager *ui_manager;
    const char *ui;
    ...
    ui = scatola_ui_string_get ("scatola-navigation-window-ui.xml");
    ...
    scatola_navigation_window_initialize_go_menu (window);
}
