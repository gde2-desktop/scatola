...
/* scatola-icon-dnd.c - Drag & drop handling for the icon container widget.
...
static const GtkTargetEntry drop_types [] =
{
    ...
    { SCATOLA_ICON_DND_NETSCAPE_URL_TYPE, 0, SCATOLA_ICON_DND_NETSCAPE_URL },
    { SCATOLA_ICON_DND_URI_LIST_TYPE, 0, SCATOLA_ICON_DND_URI_LIST },
    { SCATOLA_ICON_DND_COLOR_TYPE, 0, SCATOLA_ICON_DND_COLOR },
    { SCATOLA_ICON_DND_BGIMAGE_TYPE, 0, SCATOLA_ICON_DND_BGIMAGE },
    { SCATOLA_ICON_DND_KEYWORD_TYPE, 0, SCATOLA_ICON_DND_KEYWORD },
    { SCATOLA_ICON_DND_RESET_BACKGROUND_TYPE,  0, SCATOLA_ICON_DND_RESET_BACKGROUND },
    { SCATOLA_ICON_DND_XDNDDIRECTSAVE_TYPE, 0, SCATOLA_ICON_DND_XDNDDIRECTSAVE }, /* XDS Protocol Type */
    { SCATOLA_ICON_DND_RAW_TYPE, 0, SCATOLA_ICON_DND_RAW },
    /* Must be last: */
    { SCATOLA_ICON_DND_ROOTWINDOW_DROP_TYPE,  0, SCATOLA_ICON_DND_ROOTWINDOW_DROP }
};
...
static char * scatola_icon_container_find_drop_target (ScatolaIconContainer *container,
        GdkDragContext *context,
        int x, int y, gboolean *icon_hit,
        gboolean rewrite_desktop);

static EelCanvasItem *
create_selection_shadow (ScatolaIconContainer *container,
                         GList *list)
{
...
typedef struct
{
    ...
    ScatolaDragEachSelectedItemDataGet iteratee;
...
static gboolean
icon_get_data_binder (ScatolaIcon *icon, gpointer data)
{
    ...
    g_assert (SCATOLA_IS_ICON_CONTAINER (context->iterator_context));

    container = SCATOLA_ICON_CONTAINER (context->iterator_context);
    ...
    uri = scatola_icon_container_get_icon_uri (container, icon);
    if (uri == NULL)
    {
...
/* Iterate over each selected icon in a ScatolaIconContainer,
 * calling each_function on each.
 */
static void
scatola_icon_container_each_selected_icon (ScatolaIconContainer *container,
                                        gboolean (*each_function) (ScatolaIcon *, gpointer), gpointer data)
{
    GList *p;
    ScatolaIcon *icon;
...
/* Adaptor function used with scatola_icon_container_each_selected_icon
 * to help iterate over all selected items, passing uris, x, y, w and h
 * values to the iteratee
 */
static void
each_icon_get_data_binder (ScatolaDragEachSelectedItemDataGet iteratee,
                           gpointer iterator_context, gpointer data)
{
    IconGetDataBinderContext context;
    ScatolaIconContainer *container;

    g_assert (SCATOLA_IS_ICON_CONTAINER (iterator_context));
    container = SCATOLA_ICON_CONTAINER (iterator_context);

    context.iterator_context = iterator_context;
    context.iteratee = iteratee;
    context.iteratee_data = data;
    scatola_icon_container_each_selected_icon (container, icon_get_data_binder, &context);
}
                        ...
                        gpointer data)
{
    ...
    g_assert (SCATOLA_IS_ICON_CONTAINER (widget));
    g_return_if_fail (context != NULL);

    /* Call common function from scatola-drag that set's up
...
static void
scatola_icon_container_position_shadow (ScatolaIconContainer *container,
                                     int x, int y)
{
...
static void
scatola_icon_container_dropped_icon_feedback (GtkWidget *widget,
        GtkSelectionData *data,
        int x, int y)
{
    ScatolaIconContainer *container;
    ScatolaIconDndInfo *dnd_info;

    container = SCATOLA_ICON_CONTAINER (widget);
    dnd_info = container->details->dnd_info;

    /* Delete old selection list. */
    scatola_drag_destroy_selection_list (dnd_info->drag_info.selection_list);
    ...
    dnd_info->drag_info.selection_list = scatola_drag_build_selection_list (data);
    dnd_info->shadow = create_selection_shadow (container, dnd_info->drag_info.selection_list);
    scatola_icon_container_position_shadow (container, x, y);
}
...
get_direct_save_filename (GdkDragContext *context)
{
    ...
    if (!gdk_property_get (gdk_drag_context_get_source_window (context), gdk_atom_intern (SCATOLA_ICON_DND_XDNDDIRECTSAVE_TYPE, FALSE),
...
static void
set_direct_save_uri (GtkWidget *widget, GdkDragContext *context, ScatolaDragInfo *drag_info, int x, int y)
{
    GFile *base, *child;
    ...
    drag_info->data_type = SCATOLA_ICON_DND_XDNDDIRECTSAVE;

    uri = NULL;

    filename = get_direct_save_filename (context);
    drop_target = scatola_icon_container_find_drop_target (SCATOLA_ICON_CONTAINER (widget),
    ...
    if (filename != NULL && drop_target != NULL)
    {
        ...
        gdk_property_change (gdk_drag_context_get_source_window (context),
                             gdk_atom_intern (SCATOLA_ICON_DND_XDNDDIRECTSAVE_TYPE, FALSE),
...
get_data_on_first_target_we_support (GtkWidget *widget, GdkDragContext *context, guint32 time, int x, int y)
{
    ...
    if (drop_types_list == NULL)
    {
        ...
        gtk_target_list_add_text_targets (drop_types_list, SCATOLA_ICON_DND_TEXT);
    }
    if (drop_types_list_root == NULL)
    {
        drop_types_list_root = gtk_target_list_new (drop_types,
                               G_N_ELEMENTS (drop_types));
        gtk_target_list_add_text_targets (drop_types_list_root, SCATOLA_ICON_DND_TEXT);
    }

    if (scatola_icon_container_get_is_desktop (SCATOLA_ICON_CONTAINER (widget)))
    {
    ...
    if (target != GDK_NONE)
    {
        ...
        drag_info = &(SCATOLA_ICON_CONTAINER (widget)->details->dnd_info->drag_info);

        found = gtk_target_list_find (list, target, &info);
        g_assert (found);

        /* Don't get_data for destructive ops */
        if ((info == SCATOLA_ICON_DND_ROOTWINDOW_DROP ||
                info == SCATOLA_ICON_DND_XDNDDIRECTSAVE) &&
        ...
        {
            if (info == SCATOLA_ICON_DND_XDNDDIRECTSAVE)
            {
...
static void
scatola_icon_container_ensure_drag_data (ScatolaIconContainer *container,
                                      GdkDragContext *context,
                                      guint32 time)
{
    ScatolaIconDndInfo *dnd_info;
...
static ScatolaIcon *
scatola_icon_container_item_at (ScatolaIconContainer *container,
                             int x, int y)
{
    GList *p;
    int size;
    ...
    for (p = container->details->icons; p != NULL; p = p->next)
    {
        ...
        eel_canvas_w2c (EEL_CANVAS (container),
                        ...
                        &canvas_point.x1,
                        &canvas_point.y1);
        if (scatola_icon_canvas_item_hit_test_rectangle (icon->item, canvas_point))
        {
            return icon;
        }
    }
...
static char *
get_container_uri (ScatolaIconContainer *container)
{
    char *uri;
...
static gboolean
scatola_icon_container_selection_items_local (ScatolaIconContainer *container,
        GList *items)
{
    ...
    {
        result = scatola_drag_items_on_desktop (items);
    }
    else
    {
        result = scatola_drag_items_local (container_uri_string, items);
    }
...
get_background_drag_action (ScatolaIconContainer *container,
                            GdkDragAction action)
{
    /* FIXME: This function is very FMDirectoryView specific, and
     * should be moved out of scatola-icon-dnd.c */
    GdkDragAction valid_actions;

    if (action == GDK_ACTION_ASK)
    {
        valid_actions = SCATOLA_DND_ACTION_SET_AS_FOLDER_BACKGROUND;
        if (!eel_background_is_desktop (eel_get_widget_background (GTK_WIDGET (container))))
        {
            valid_actions |= SCATOLA_DND_ACTION_SET_AS_GLOBAL_BACKGROUND;
        }

        action = scatola_drag_drop_background_ask
...
static void
receive_dropped_color (ScatolaIconContainer *container,
                       int x, int y,
                       GdkDragAction action,
                       GtkSelectionData *data)
{
...
static void
receive_dropped_tile_image (ScatolaIconContainer *container, GdkDragAction action, GtkSelectionData *data)
{
    g_assert (data != NULL);
...
static void
receive_dropped_keyword (ScatolaIconContainer *container, const char *keyword, int x, int y)
{
    char *uri;
    double world_x, world_y;

    ScatolaIcon *drop_target_icon;
    ScatolaFile *file;
    ...
    drop_target_icon = scatola_icon_container_item_at (container, world_x, world_y);
     ...
     */
    uri = scatola_icon_container_get_icon_uri (container, drop_target_icon);
    ...
    scatola_drag_file_receive_dropped_keyword (file, keyword);

    scatola_file_unref (file);
    scatola_icon_container_update_icon (container, drop_target_icon);
}

/* handle dropped url */
static void
receive_dropped_netscape_url (ScatolaIconContainer *container, const char *encoded_url, GdkDragContext *context, int x, int y)
{
    char *drop_target;
    ...
    drop_target = scatola_icon_container_find_drop_target (container, context, x, y, NULL, TRUE);
...
static void
receive_dropped_uri_list (ScatolaIconContainer *container, const char *uri_list, GdkDragContext *context, int x, int y)
{
    char *drop_target;
...
/* handle dropped text */
static void
receive_dropped_text (ScatolaIconContainer *container, const char *text, GdkDragContext *context, int x, int y)
{
    char *drop_target;
...
static void
receive_dropped_raw (ScatolaIconContainer *container, const char *raw_data, int length, const char *direct_save_uri, GdkDragContext *context, int x, int y)
{
    char *drop_target;
...
auto_scroll_timeout_callback (gpointer data)
{
    ...
    g_assert (SCATOLA_IS_ICON_CONTAINER (data));
    ...
    scatola_drag_autoscroll_calculate_delta (widget, &x_scroll_delta, &y_scroll_delta);
    ...
    if (!scatola_icon_container_scroll (container, (int)x_scroll_delta, (int)y_scroll_delta))
    {
...
static void
set_up_auto_scroll_if_needed (ScatolaIconContainer *container)
{
    scatola_drag_autoscroll_start (&container->details->dnd_info->drag_info,
                                GTK_WIDGET (container),
...
static void
stop_auto_scroll (ScatolaIconContainer *container)
{
    scatola_drag_autoscroll_stop (&container->details->dnd_info->drag_info);
}

static void
handle_local_move (ScatolaIconContainer *container,
                   double world_x, double world_y)
{
    GList *moved_icons, *p;
    ScatolaDragSelectionItem *item;
    ...
    for (p = container->details->dnd_info->drag_info.selection_list; p != NULL; p = p->next)
    {
        ...
        icon = scatola_icon_container_get_icon_by_uri
               (container, item->uri);
        ...
        if (icon == NULL)
        {
            ...
            file = scatola_file_get_by_uri (item->uri);
            ...
            scatola_file_set_time_metadata (file,
                                         SCATOLA_METADATA_KEY_ICON_POSITION_TIMESTAMP, now);

            scatola_icon_container_add (container, SCATOLA_ICON_CONTAINER_ICON_DATA (file));
        ...
        if (item->got_icon_position)
        {
            scatola_icon_container_move_icon
            (container, icon,
...
static void
handle_nonlocal_move (ScatolaIconContainer *container,
                      GdkDragAction action,
                      int x, int y,
                      ...
                      gboolean icon_hit)
{
    ...
    for (p = container->details->dnd_info->drag_info.selection_list; p != NULL; p = p->next)
    {
        ...
        source_uris = g_list_prepend (source_uris, ((ScatolaDragSelectionItem *)p->data)->uri);
    }
    ...
    if (!icon_hit)
    {
        ...
        {
            item_x = ((ScatolaDragSelectionItem *)p->data)->icon_x;
            if (is_rtl)
                item_x = -item_x - ((ScatolaDragSelectionItem *)p->data)->icon_width;
            g_array_index (source_item_locations, GdkPoint, index).x = item_x;
            g_array_index (source_item_locations, GdkPoint, index).y =
                ((ScatolaDragSelectionItem *)p->data)->icon_y;
        }
    }
    ...
    {
        target_uri = scatola_get_desktop_directory_uri ();
...
static char *
scatola_icon_container_find_drop_target (ScatolaIconContainer *container,
                                      GdkDragContext *context,
                                      int x, int y,
                                      gboolean *icon_hit,
                                      gboolean rewrite_desktop)
{
    ...
    if (drop_target_icon != NULL)
    {
        icon_uri = scatola_icon_container_get_icon_uri (container, drop_target_icon);
        if (icon_uri != NULL)
        {
            file = scatola_file_get_by_uri (icon_uri);

            if (!scatola_drag_can_accept_info (file,
    ...
    if (drop_target_icon == NULL)
    {
                ...
                eel_uri_is_desktop (container_uri))
        {
            ...
            container_uri = scatola_get_desktop_directory_uri ();
        }
    ...
    return scatola_icon_container_get_icon_drop_target_uri (container, drop_target_icon);
}
...
selection_is_image_file (GList *selection_list)
{
    ...
    ScatolaDragSelectionItem *selected_item;
    gboolean result;
    GFile *location;
    GFileInfo *info;
...
static void
scatola_icon_container_receive_dropped_icons (ScatolaIconContainer *container,
        GdkDragContext *context,
        int x, int y)
{
    char *drop_target;
    ...
    if (real_action == GDK_ACTION_ASK)
    {
        ...
        if (scatola_drag_selection_includes_special_link (container->details->dnd_info->drag_info.selection_list))
        {
        ...
        else
        {
            ...
            {
                action |= SCATOLA_DND_ACTION_SET_AS_BACKGROUND;
            }
        }
        real_action = scatola_drag_drop_action_ask (GTK_WIDGET (container), action);
    }

    if (real_action == (GdkDragAction) SCATOLA_DND_ACTION_SET_AS_BACKGROUND)
    {
    ...
    if (real_action > 0)
    {
        ...
        drop_target = scatola_icon_container_find_drop_target (container,
        ...
        if (!icon_hit && real_action == GDK_ACTION_MOVE)
        {
             ...
             */
            local_move_only = scatola_icon_container_selection_items_local
    ...
    g_free (drop_target);
    scatola_drag_destroy_selection_list (container->details->dnd_info->drag_info.selection_list);
    container->details->dnd_info->drag_info.selection_list = NULL;
}

static void
scatola_icon_container_get_drop_action (ScatolaIconContainer *container,
                                     GdkDragContext *context,
                                     int x, int y,
                                     int *action)
{
    ...
    icon = scatola_icon_container_item_at (container, world_x, world_y);
    ...
    *action = 0;
    ...
    case SCATOLA_ICON_DND_GDE2_ICON_LIST:
        ...
        scatola_drag_default_drop_action_for_icons (context, drop_target,
                container->details->dnd_info->drag_info.selection_list,
                action);
        g_free (drop_target);
        break;
    case SCATOLA_ICON_DND_URI_LIST:
        drop_target = scatola_icon_container_find_drop_target (container,
                      context, x, y, &icon_hit, FALSE);
        *action = scatola_drag_default_drop_action_for_uri_list (context, drop_target);
    ...
    case SCATOLA_ICON_DND_NETSCAPE_URL:
        *action = scatola_drag_default_drop_action_for_netscape_url (context);
        break;
...
static void
set_drop_target (ScatolaIconContainer *container,
                 ScatolaIcon *icon)
{
    ...
    scatola_icon_container_update_icon (container, old_icon);
    scatola_icon_container_update_icon (container, icon);
}

static void
scatola_icon_dnd_update_drop_target (ScatolaIconContainer *container,
                                  GdkDragContext *context,
                                  int x, int y)
{
    ScatolaIcon *icon;
    ...
    if (icon != NULL && (container->details->dnd_info->drag_info.data_type != SCATOLA_ICON_DND_KEYWORD))
    {
        uri = scatola_icon_container_get_icon_uri (container, icon);
        file = scatola_file_get_by_uri (uri);
        g_free (uri);
...
static void
scatola_icon_container_free_drag_data (ScatolaIconContainer *container)
{
    ScatolaIconDndInfo *dnd_info;
                     ...
                     gpointer data)
{
    ...
    dnd_info = SCATOLA_ICON_CONTAINER (widget)->details->dnd_info;
    ...
    set_drop_target (SCATOLA_ICON_CONTAINER (widget), NULL);
    stop_auto_scroll (SCATOLA_ICON_CONTAINER (widget));
    scatola_icon_container_free_drag_data(SCATOLA_ICON_CONTAINER (widget));
}
...
#if GTK_CHECK_VERSION(3,0,0)
    surface = scatola_icon_canvas_item_get_drag_surface (container->details->drag_icon->item);
#else
    pixmap = scatola_icon_canvas_item_get_image (container->details->drag_icon->item, &mask, colormap);
#endif
...
void
scatola_icon_dnd_begin_drag (ScatolaIconContainer *container,
                      ...
                      guint32 time)
{
    ...
    scatola_icon_container_ensure_drag_data (SCATOLA_ICON_CONTAINER (widget), context, time);
    scatola_icon_container_position_shadow (SCATOLA_ICON_CONTAINER (widget), x, y);
    scatola_icon_dnd_update_drop_target (SCATOLA_ICON_CONTAINER (widget), context, x, y);
    set_up_auto_scroll_if_needed (SCATOLA_ICON_CONTAINER (widget));
    /* Find out what the drop actions are based on our drag selection and
     * the drop target.
     */
    action = 0;
    scatola_icon_container_get_drop_action (SCATOLA_ICON_CONTAINER (widget), context, x, y,
                                         &action);
    if (action != 0)
    {
...
void
scatola_icon_dnd_end_drag (ScatolaIconContainer *container)
{
    ScatolaIconDndInfo *dnd_info;
                             ...
                             gpointer user_data)
{
    ...
    {
    case SCATOLA_ICON_DND_GDE2_ICON_LIST:
        scatola_icon_container_dropped_icon_feedback (widget, data, x, y);
        break;
    case SCATOLA_ICON_DND_COLOR:
    case SCATOLA_ICON_DND_BGIMAGE:
    case SCATOLA_ICON_DND_KEYWORD:
    ...
    if (drag_info->drop_occured)
    {
        ...
        {
        case SCATOLA_ICON_DND_GDE2_ICON_LIST:
            scatola_icon_container_receive_dropped_icons
            (SCATOLA_ICON_CONTAINER (widget),
             context, x, y);
            break;
        case SCATOLA_ICON_DND_COLOR:
            receive_dropped_color (SCATOLA_ICON_CONTAINER (widget),
                                   x, y,
                                   gdk_drag_context_get_selected_action (context),
                                   data);
            success = TRUE;
        ...
        case SCATOLA_ICON_DND_XDNDDIRECTSAVE:
        {
                    ...
                    selection_data[0] == 'F')
            {
                gtk_drag_get_data (widget, context,
                                   gdk_atom_intern (SCATOLA_ICON_DND_RAW_TYPE,
                                                    FALSE),
                                   time);
                return;
            }
                     ...
                     drag_info->direct_save_uri != NULL)
            {
                ...
                p.x = x;
                p.y = y;
                scatola_file_changes_queue_schedule_position_set (
                    location,
                    p,
...
void
scatola_icon_dnd_init (ScatolaIconContainer *container)
{
    ...
    container->details->dnd_info = g_new0 (ScatolaIconDndInfo, 1);
    scatola_drag_init (&container->details->dnd_info->drag_info,
...
void
scatola_icon_dnd_fini (ScatolaIconContainer *container)
{
    g_return_if_fail (SCATOLA_IS_ICON_CONTAINER (container));
    ...
    if (container->details->dnd_info != NULL)
    {
        ...
        scatola_drag_finalize (&container->details->dnd_info->drag_info);
