...
/* scatola-mime-actions.c - uri-specific versions of mime action functions
...
#include <config.h>
#include "scatola-mime-actions.h"
...
typedef struct
{
    ...
    ScatolaFileListHandle *files_handle;
...
{
    scatola_file_unref (location->file);
    g_free (location->uri);
    g_free (location);
}
...
get_file_list_for_launch_locations (GList *locations)
{
    ...
    for (l = locations; l != NULL; l = l->next)
    {
        ...
        files = g_list_prepend (files,
                                scatola_file_ref (location->file));
    }
...
launch_location_from_file (ScatolaFile *file)
{
    LaunchLocation *location;
    location = g_new (LaunchLocation, 1);
    location->file = scatola_file_ref (file);
    location->uri = scatola_file_get_uri (file);
                                 ...
                                 const char *uri)
{
    scatola_file_unref (location->file);
    g_free (location->uri);
    location->file = scatola_file_get_by_uri (uri);
...
static GList*
filter_scatola_handler (GList *apps)
{
    GList *l, *next;
    GAppInfo *application;
    const char *id;
    ...
    while (l != NULL)
    {
        ...
        if (id != NULL &&
                strcmp (id,
                        "scatola-folder-handler.desktop") == 0)
        {
...
static gboolean
scatola_mime_actions_check_if_required_attributes_ready (ScatolaFile *file)
{
    ScatolaFileAttributes attributes;
    gboolean ready;

    attributes = scatola_mime_actions_get_required_file_attributes ();
    ready = scatola_file_check_if_ready (file, attributes);

    return ready;
}

ScatolaFileAttributes
scatola_mime_actions_get_required_file_attributes (void)
{
...
static gboolean
file_has_local_path (ScatolaFile *file)
{
    GFile *location;
    char *path;
    gboolean res;
...
GAppInfo *
scatola_mime_get_default_application_for_file (ScatolaFile *file)
{
    GAppInfo *app;
    char *mime_type;
    char *uri_scheme;

    if (!scatola_mime_actions_check_if_required_attributes_ready (file))
    {
        return NULL;
    }
    ...
    if (app == NULL)
    {
        uri_scheme = scatola_file_get_uri_scheme (file);
        if (uri_scheme != NULL)
        {
...
static int
file_compare_by_mime_type (ScatolaFile *file_a,
                           ScatolaFile *file_b)
{
    char *mime_type_a, *mime_type_b;
    int ret;

    mime_type_a = scatola_file_get_mime_type (file_a);
    mime_type_b = scatola_file_get_mime_type (file_b);
...
static int
file_compare_by_parent_uri (ScatolaFile *file_a,
                            ScatolaFile *file_b)
{
    char *parent_uri_a, *parent_uri_b;
    int ret;

    parent_uri_a = scatola_file_get_parent_uri (file_a);
    parent_uri_b = scatola_file_get_parent_uri (file_b);
...
GList *
scatola_mime_get_applications_for_file (ScatolaFile *file)
{
    char *mime_type;
    char *uri_scheme;
    GList *result;
    ...
    return filter_scatola_handler (result);
}

gboolean
scatola_mime_has_any_applications_for_file (ScatolaFile *file)
{
    GList *apps;
    char *mime_type;
    gboolean result;
    ...
    apps = filter_scatola_handler (apps);
...
GAppInfo *
scatola_mime_get_default_application_for_files (GList *files)
{
    ...
    for (l = sorted_files; l != NULL; l = l->next)
    {
        ...
        one_app = scatola_mime_get_default_application_for_file (file);
...
GList *
scatola_mime_get_applications_for_files (GList *files)
{
    GList *l, *sorted_files;
    ScatolaFile *file;
    GList *one_ret, *ret;
    ...
    for (l = sorted_files; l != NULL; l = l->next)
    {
        ...
        one_ret = scatola_mime_get_applications_for_file (file);
...
static void
report_broken_symbolic_link (GtkWindow *parent_window, ScatolaFile *file)
{
    char *target_path;
    char *display_name;
    char *prompt;
    ...
    target_path = scatola_file_get_symbolic_link_target_path (file);
    if (target_path == NULL)
    {
     ...
     * merged Gde2Component items not to be un-merged. Maybe we need to unmerge
     * explicitly when disconnecting views instead of relying on the
     * unmerge in Destroy. But since Gde2ComponentUIHandler is probably going
...
get_executable_text_file_action (GtkWindow *parent_window, ScatolaFile *file)
{
    GtkDialog *dialog;
    char *file_name;
    char *prompt;
    ...
    g_assert (scatola_file_contains_text (file));

    preferences_value = g_settings_get_enum (scatola_preferences,
                            SCATOLA_PREFERENCES_EXECUTABLE_TEXT_ACTIVATION);
    switch (preferences_value)
    {
    case SCATOLA_EXECUTABLE_TEXT_LAUNCH:
        return ACTIVATION_ACTION_LAUNCH;
    case SCATOLA_EXECUTABLE_TEXT_DISPLAY:
        return ACTIVATION_ACTION_OPEN_IN_APPLICATION;
    case SCATOLA_EXECUTABLE_TEXT_ASK:
        break;
    default:
        /* Complain non-fatally, since preference data can't be trusted */
        g_warning ("Unknown value %d for SCATOLA_PREFERENCES_EXECUTABLE_TEXT_ACTIVATION",
                   preferences_value);
...
gboolean
scatola_mime_file_opens_in_view (ScatolaFile *file)
{
    return (scatola_file_is_directory (file));
}

static ActivationAction
get_activation_action (ScatolaFile *file)
{
    ...
    {
        activation_uri = scatola_file_get_uri (file);
    }
    ...
    if (scatola_file_is_launchable (file))
    {
        ...
        else if (scatola_file_contains_text (file))
        {
    ...
    {
        if (scatola_mime_file_opens_in_view (file))
        {
...
gboolean
scatola_mime_file_opens_in_external_app (ScatolaFile *file)
{
 ...
 *
 * Construct a list of ApplicationLaunchParameters from a list of ScatolaFiles,
                            ...
                            GList **unhandled_uris)
{
    ...
    *unhandled_uris = NULL;
    ...
    for (l = uris; l != NULL; l = l->next)
    {
        ...
        app = scatola_mime_get_default_application_for_file (file);
        if (app != NULL)
        {
            app_uris = NULL;
...
static gboolean
file_was_cancelled (ScatolaFile *file)
{
    GError *error;
...
static gboolean
file_was_not_mounted (ScatolaFile *file)
{
    GError *error;
...
activation_parameters_free (ActivateParameters *parameters)
{
    ...
    scatola_file_list_free (parameters->mountables);
    scatola_file_list_free (parameters->start_mountables);
    scatola_file_list_free (parameters->not_mounted);
...
cancel_activate_callback (gpointer callback_data)
{
    ...
    {
        scatola_file_list_cancel_call_when_ready (parameters->files_handle);
...
activate_parameters_install_free (ActivateParametersInstall *parameters_install)
{
    ...
    scatola_file_unref (parameters_install->file);
    scatola_file_list_free (parameters_install->files);
...
static char *
get_application_no_mime_type_handler_message (ScatolaFile *file, char *uri)
{
...
choose_program (GtkDialog *message_dialog, int response, gpointer callback_data)
{
    ...
    g_object_set_data_full (G_OBJECT (dialog),
                            "mime-action:file",
                            scatola_file_ref (file),
                            (GDestroyNotify)scatola_file_unref);
...
show_unhandled_type_error (ActivateParametersInstall *parameters)
{
    ...
    char *mime_type = scatola_file_get_mime_type (parameters->file);
    ...
    g_object_set_data_full (G_OBJECT (dialog),
                            "mime-action:file",
                            scatola_file_ref (parameters->file),
                            (GDestroyNotify)scatola_file_unref);
        ...
        gpointer      user_data)
{
    ...
    scatola_mime_activate_files (parameters_install->parent_window,
                                    ...
                                    ActivateParametersInstall *parameters_install)
{
    ...
    {
        mime_type = scatola_file_get_mime_type (parameters_install->file);
...
application_unhandled_uri (ActivateParameters *parameters, char *uri)
{
    ...
    show_install_mime = g_settings_get_boolean (scatola_preferences, SCATOLA_PREFERENCES_INSTALL_MIME_ACTIVATION);
#else
...
activate_parameters_desktop_free (ActivateParametersDesktop *parameters_desktop)
{
    ...
    scatola_file_unref (parameters_desktop->file);
                                      ...
                                      ActivateParametersDesktop *parameters)
{
    ...
    case RESPONSE_RUN:
        ...
        uri = scatola_file_get_uri (parameters->file);
        scatola_debug_log (FALSE, SCATOLA_DEBUG_LOG_DOMAIN_USER,
                        "directory view activate_callback launch_desktop_file window=%p: %s",
                        parameters->parent_window, uri);
        scatola_launch_desktop_file (screen, uri, NULL,
                                  parameters->parent_window);
        g_free (uri);
        break;
    case RESPONSE_MARK_TRUSTED:
        file = scatola_file_get_location (parameters->file);
        scatola_file_mark_desktop_file_trusted (file,
                                             parameters->parent_window,
                                             TRUE,
                                             NULL, NULL);
        g_object_unref (file);
                       ...
                       ScatolaFile *file)
{
    ...
    if (!scatola_file_is_trusted_link (file))
    {
        ...
        parameters_desktop->file = scatola_file_ref (file);
...
activate_files (ActivateParameters *parameters)
{
    ...
    for (l = launch_files; l != NULL; l = l->next)
    {
        ...
        name = scatola_file_get_name (file);

        scatola_debug_log (FALSE, SCATOLA_DEBUG_LOG_DOMAIN_USER,
                        "directory view activate_callback launch_file window=%p: %s",
                        parameters->parent_window, quoted_path);

        scatola_launch_application_from_command (screen, name, quoted_path, FALSE, NULL);
        g_free (name);
        g_free (quoted_path);
        g_free (executable_path);
        g_free (uri);
    ...
    for (l = launch_in_terminal_files; l != NULL; l = l->next)
    {
        ...
        scatola_launch_application_from_command (screen, name, quoted_path, TRUE, NULL);
        g_free (name);
        g_free (quoted_path);
        g_free (executable_path);
        g_free (uri);
    ...
    if (count > 1)
    {
        if ((parameters->flags & SCATOLA_WINDOW_OPEN_FLAG_NEW_WINDOW) == 0)
        {
                                       ...
                                       (flags & SCATOLA_WINDOW_OPEN_FLAG_NEW_TAB) != 0)))
    {
        ...
        if ((flags & SCATOLA_WINDOW_OPEN_FLAG_NEW_TAB) != 0 &&
                g_settings_get_enum (scatola_preferences, SCATOLA_PREFERENCES_NEW_TAB_POSITION) ==
        ...
        for (l = open_in_view_files; l != NULL; l = l->next)
        {
            ...
            scatola_window_slot_info_open_location (parameters->slot_info,
    ...
    for (l = open_in_app_parameters; l != NULL; l = l->next)
    {
        ...
        scatola_launch_application_by_uri (one_parameters->application,
                                        one_parameters->uris,
                                        parameters->parent_window);
    ...
    if (parameters->slot_info != NULL)
    {
        window_info = scatola_window_slot_info_get_window (parameters->slot_info);
    }

    if (open_in_app_parameters != NULL ||
            unhandled_open_in_app_uris != NULL)
    {
        if ((parameters->flags & SCATOLA_WINDOW_OPEN_FLAG_CLOSE_BEHIND) != 0 &&
                window_info != NULL &&
                scatola_window_info_get_window_type (window_info) == SCATOLA_WINDOW_SPATIAL)
        {
            scatola_window_info_close (window_info);
        }
    }
...
activation_mount_not_mounted (ActivateParameters *parameters)
{
    ...
    for (l = parameters->locations; l != NULL; l = next)
    {
        loc = l->data;
        next = l->next;
        scatola_file_invalidate_all_attributes (loc->file);
    }

    files = get_file_list_for_launch_locations (parameters->locations);
    scatola_file_list_call_when_ready
    (files,
     scatola_mime_actions_get_required_file_attributes () | SCATOLA_FILE_ATTRIBUTE_LINK_INFO,
        ...
        gpointer callback_data)
{
    ...
    for (l = parameters->locations; l != NULL; l = next)
    {
        ...
        if (scatola_file_get_file_type (file) == G_FILE_TYPE_MOUNTABLE &&
                !scatola_file_has_activation_uri (file))
        {
    ...
    for (l = parameters->locations; l != NULL; l = l->next)
    {
         ...
         */
        uri = scatola_file_get_activation_uri (location->file);
        if (uri != NULL)
        {
...
static void
activation_mountable_mounted (ScatolaFile  *file,
                              ...
                              gpointer       callback_data)
{
    ...
    if (error == NULL)
    {
        ...
        target_file = scatola_file_get (result_location);
...
static void
activation_mountable_started (ScatolaFile  *file,
                              GFile         *gfile_of_file,
                              GError        *error,
                              gpointer       callback_data)
{
 ...
 * @files: A GList of ScatolaFiles to activate.
 *
 **/
void
scatola_mime_activate_files (GtkWindow *parent_window,
                          ScatolaWindowSlotInfo *slot_info,
                          GList *files,
                          ...
                          gboolean user_confirmation)
{
    ...
    scatola_debug_log_with_file_list (FALSE, SCATOLA_DEBUG_LOG_DOMAIN_USER, files,
                                   "scatola_mime_activate_files window=%p",
                                   parent_window);
    ...
    if (file_count == 1)
    {
        file_name = scatola_file_get_display_name (files->data);
    ...
    for (l = parameters->locations; l != NULL; l = next)
    {
        ...
        if (scatola_file_can_start (file))
        {
 ...
 * @view: FMDirectoryView in question.
 * @file: A ScatolaFile representing the file in this view to activate.
 * @use_new_window: Should this item be opened in a new window?
 *
 **/

void
scatola_mime_activate_file (GtkWindow *parent_window,
                         ScatolaWindowSlotInfo *slot_info,
                         ScatolaFile *file,
                         const char *launch_directory,
                         ScatolaWindowOpenMode mode,
                         ScatolaWindowOpenFlags flags)
{
    ...
    scatola_mime_activate_files (parent_window, slot_info, files, launch_directory, mode, flags, FALSE);
    g_list_free (files);
}
