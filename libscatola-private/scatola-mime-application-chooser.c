...
/*
   scatola-mime-application-chooser.c: an mime-application chooser
...
#include <config.h>
#include "scatola-mime-application-chooser.h"
...
struct _ScatolaMimeApplicationChooserDetails
{
    char *uri;
...
G_DEFINE_TYPE (ScatolaMimeApplicationChooser, scatola_mime_application_chooser, GTK_TYPE_BOX);
#else
G_DEFINE_TYPE (ScatolaMimeApplicationChooser, scatola_mime_application_chooser, GTK_TYPE_VBOX);
#endif

static void refresh_model             (ScatolaMimeApplicationChooser *chooser);
static void refresh_model_soon        (ScatolaMimeApplicationChooser *chooser);
static void mime_type_data_changed_cb (GObject                        *signaller,
                                       gpointer                        user_data);

static void
scatola_mime_application_chooser_finalize (GObject *object)
{
    ScatolaMimeApplicationChooser *chooser;

    chooser = SCATOLA_MIME_APPLICATION_CHOOSER (object);
    ...
    g_signal_handlers_disconnect_by_func (scatola_signaller_get_current (),
    ...
    G_OBJECT_CLASS (scatola_mime_application_chooser_parent_class)->finalize (object);
}

static void
scatola_mime_application_chooser_class_init (ScatolaMimeApplicationChooserClass *class)
{
    GObjectClass *gobject_class;

    gobject_class = G_OBJECT_CLASS (class);
    gobject_class->finalize = scatola_mime_application_chooser_finalize;
}
                    ...
                    gpointer user_data)
{
    ...
    chooser = SCATOLA_MIME_APPLICATION_CHOOSER (user_data);
...
static GAppInfo *
get_selected_application (ScatolaMimeApplicationChooser *chooser)
{
    GtkTreeIter iter;
...
static GtkWidget *
create_tree_view (ScatolaMimeApplicationChooser *chooser)
{
    GtkWidget *treeview;
    GtkListStore *store;
                ...
                gpointer user_data)
{
    ...
    {
        dialog = scatola_add_application_dialog_new_for_multiple_files (chooser->details->extension,
                 chooser->details->orig_mime_type);
    }
    else
    {
        dialog = scatola_add_application_dialog_new (chooser->details->uri,
                 chooser->details->orig_mime_type);
    }
...
static void
scatola_mime_application_chooser_init (ScatolaMimeApplicationChooser *chooser)
{
    GtkWidget *box;
    GtkWidget *scrolled;
    GtkWidget *button;

    chooser->details = g_new0 (ScatolaMimeApplicationChooserDetails, 1);
...
{
    ScatolaMimeApplicationChooser *chooser = data;
...
static void
refresh_model_soon (ScatolaMimeApplicationChooser *chooser)
{
...
static void
refresh_model (ScatolaMimeApplicationChooser *chooser)
{
    GList *applications;
...
static void
set_extension_and_description (ScatolaMimeApplicationChooser *chooser,
...
static gboolean
set_uri_and_type (ScatolaMimeApplicationChooser *chooser,
                  const char *uri,
                  const char *mime_type)
{
    char *label;
    ...
    set_extension_and_description (SCATOLA_MIME_APPLICATION_CHOOSER (chooser),
                                   extension, mime_type);
    g_free (extension);
...
static char *
get_extension_from_file (ScatolaFile *nfile)
{
    char *name;
    char *extension;

    name = scatola_file_get_name (nfile);
    extension = get_extension (name);
...
static gboolean
set_uri_and_type_for_multiple_files (ScatolaMimeApplicationChooser *chooser,
                                     GList *uris,
                                     const char *mime_type)
{
    char *label;
    ...
    chooser->details->orig_mime_type = g_strdup (mime_type);
    same_extension = TRUE;
    first_extension = get_extension_from_file (SCATOLA_FILE (uris->data));
    iter = uris->next;
    ...
    while (iter != NULL)
    {
        ...
        extension_current = get_extension_from_file (SCATOLA_FILE (iter->data));
...
GtkWidget *
scatola_mime_application_chooser_new (const char *uri,
                                   const char *mime_type)
{
    GtkWidget *chooser;

    chooser = gtk_widget_new (SCATOLA_TYPE_MIME_APPLICATION_CHOOSER, NULL);

    set_uri_and_type (SCATOLA_MIME_APPLICATION_CHOOSER (chooser), uri, mime_type);

    return chooser;
}

GtkWidget *
scatola_mime_application_chooser_new_for_multiple_files (GList *uris,
        const char *mime_type)
{
    GtkWidget *chooser;
    ...
    set_uri_and_type_for_multiple_files (SCATOLA_MIME_APPLICATION_CHOOSER (chooser),
                                         uris, mime_type);
