...
/* scatola-customization-data.c - functions to collect and load customization
   names and images */

#include <config.h>
#include "scatola-customization-data.h"
...
struct ScatolaCustomizationData
{
...
static char *            get_file_path_for_mode              (const ScatolaCustomizationData *data,
        const char *file_name);
static char*             format_name_for_display             (ScatolaCustomizationData *data, const char *name);
static void		 load_name_map_hash_table	     (ScatolaCustomizationData *data);
...
ScatolaCustomizationData*
scatola_customization_data_new (const char *customization_name,
                             gboolean show_public_customizations,
                             int maximum_icon_height,
                             int maximum_icon_width)
{
    ScatolaCustomizationData *data;
    char *public_directory_path, *private_directory_path;
    char *temp_str;
    gboolean public_result, private_result;

    data = g_new0 (ScatolaCustomizationData, 1);
...
gboolean
scatola_customization_data_get_next_element_for_display (ScatolaCustomizationData *data,
        char **emblem_name,
        ...
        char **label_out)
{
    ...
    if (data->current_file_list == NULL)
    {
        if (data->reading_mode == READ_PUBLIC_CUSTOMIZATIONS)
        {
            ...
            data->reading_mode = READ_PRIVATE_CUSTOMIZATIONS;
            data->current_file_list = data->private_file_list;
            return scatola_customization_data_get_next_element_for_display (data,
                    emblem_name,
                    pixbuf_out,
                    label_out);
        }
    ...
    *emblem_name = g_strdup (g_file_info_get_name (current_file_info));
    ...
    if (strcmp (data->customization_name, "patterns") == 0 &&
            data->pattern_frame != NULL)
    {
        pixbuf = scatola_customization_make_pattern_chit (orig_pixbuf, data->pattern_frame, FALSE, is_reset_image);
    }
    else
    {
...
gboolean
scatola_customization_data_private_data_was_displayed (ScatolaCustomizationData *data)
{
    return data->private_data_was_displayed;
}

void
scatola_customization_data_destroy (ScatolaCustomizationData *data)
{
...
{
    return g_build_filename (SCATOLA_DATADIR,
                             customization_name,
                             NULL);
}
...
static char *
get_file_path_for_mode (const ScatolaCustomizationData *data,
                        const char *file_name)
{
...
GdkPixbuf*
scatola_customization_make_pattern_chit (GdkPixbuf *pattern_tile, GdkPixbuf *frame, gboolean dragging, gboolean is_reset)
{
...
static char*
format_name_for_display (ScatolaCustomizationData *data, const char* name)
{
 ...
 */
static void
load_name_map_hash_table (ScatolaCustomizationData *data)
{
    char *xml_path;
    ...
    xml_path = g_build_filename (SCATOLA_DATADIR, "browser.xml", NULL);
    if (xml_path)
    {
