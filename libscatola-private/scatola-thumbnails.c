...
/* The list of ScatolaThumbnailInfo structs containing information about the
...
static ScatolaThumbnailInfo *currently_thumbnailing = NULL;

static Gde2DesktopThumbnailFactory *thumbnail_factory = NULL;
...
static void
free_thumbnail_info (ScatolaThumbnailInfo *info)
{
...
static Gde2DesktopThumbnailFactory *
get_thumbnail_factory (void)
{
    ...
    {
        thumbnail_factory = gde2_desktop_thumbnail_factory_new (GDE2_DESKTOP_THUMBNAIL_SIZE_NORMAL);
    }
...
void
scatola_thumbnail_remove_from_queue (const char *file_uri)
{
    GList *node;
...
void
scatola_thumbnail_prioritize (const char *file_uri)
{
    GList *node;
...
/* This is a one-shot idle callback called from the main loop to call
   ...
   We do this in an idle callback as I don't think scatola_file_changed() is
   thread-safe. */
static gboolean
thumbnail_thread_notify_file_changed (gpointer image_uri)
{
    ...
    file = scatola_file_get_by_uri ((char *) image_uri);
#ifdef DEBUG_THUMBNAILS
    ...
    if (file != NULL)
    {
        scatola_file_set_is_thumbnailing (file, FALSE);
...
gboolean
scatola_can_thumbnail_internally (ScatolaFile *file)
{
    char *mime_type;
    gboolean res;
...
gboolean
scatola_thumbnail_is_mimetype_limited_by_size (const char *mime_type)
{
    return pixbuf_can_load_type (mime_type);
}

gboolean
scatola_can_thumbnail (ScatolaFile *file)
{
    ...
    mime_type = scatola_file_get_mime_type (file);
    mtime = scatola_file_get_mtime (file);

    factory = get_thumbnail_factory ();
    res = gde2_desktop_thumbnail_factory_can_thumbnail (factory,
            uri,
            mime_type,
            mtime);
    g_free (mime_type);
...
void
scatola_create_thumbnail (ScatolaFile *file)
{
    time_t file_mtime = 0;
    ScatolaThumbnailInfo *info;
    ScatolaThumbnailInfo *existing_info;
    GList *existing, *node;

    scatola_file_set_is_thumbnailing (file, TRUE);

    info = g_new0 (ScatolaThumbnailInfo, 1);
    info->image_uri = scatola_file_get_uri (file);
    info->mime_type = scatola_file_get_mime_type (file);

    /* Hopefully the ScatolaFile will already have the image file mtime,
...
thumbnail_thread_func (GTask        *task,
                       ...
                       GCancellable *cancellable)
{
    ScatolaThumbnailInfo *info = NULL;
    GdkPixbuf *pixbuf;
    ...
    for (;;)
    {
        ...
        */
        ...
        pixbuf = gde2_desktop_thumbnail_factory_generate_thumbnail (thumbnail_factory,
                 info->image_uri,
                 info->mime_type);
        ...
        if (pixbuf)
        {
			...
			g_message ("(Thumbnail Thread) Saving thumbnail: %s\n",
				   info->image_uri);
#endif
            gde2_desktop_thumbnail_factory_save_thumbnail (thumbnail_factory,
                    pixbuf,
                    info->image_uri,
                    current_orig_mtime);
        ...
        else
        {
			...
			g_message ("(Thumbnail Thread) Thumbnail failed: %s\n",
				   info->image_uri);
#endif
            gde2_desktop_thumbnail_factory_create_failed_thumbnail (thumbnail_factory,
                    info->image_uri,
                    current_orig_mtime);
        }
        /* We need to call scatola_file_changed(), but I don't think that is
