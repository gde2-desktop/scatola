/* -*- Mode: C; indent-tabs-mode: t; c-basic-offset: 8; tab-width: 8 -*-
   ...
   scatola-directory.c: Scatola directory model.
...
static void               scatola_directory_finalize         (GObject                *object);
static void               scatola_directory_init             (gpointer                object,
        gpointer                klass);
static void               scatola_directory_class_init (ScatolaDirectoryClass *klass);
static ScatolaDirectory *scatola_directory_new              (GFile                  *location);
static char *             real_get_name_for_self_as_new_file  (ScatolaDirectory      *directory);
static GList *            real_get_file_list                  (ScatolaDirectory      *directory);
static gboolean		  real_is_editable                    (ScatolaDirectory      *directory);
static void               set_directory_location              (ScatolaDirectory      *directory,
        GFile                  *location);

EEL_CLASS_BOILERPLATE (ScatolaDirectory,
                       scatola_directory,
                       G_TYPE_OBJECT)

static void
scatola_directory_class_init (ScatolaDirectoryClass *klass)
{
    GObjectClass *object_class;

    object_class = G_OBJECT_CLASS (klass);

    object_class->finalize = scatola_directory_finalize;

    signals[FILES_ADDED] =
        g_signal_new ("files_added",
                      G_TYPE_FROM_CLASS (object_class),
                      G_SIGNAL_RUN_LAST,
                      G_STRUCT_OFFSET (ScatolaDirectoryClass, files_added),
                      NULL, NULL,
    ...
    signals[FILES_CHANGED] =
        g_signal_new ("files_changed",
                      G_TYPE_FROM_CLASS (object_class),
                      G_SIGNAL_RUN_LAST,
                      G_STRUCT_OFFSET (ScatolaDirectoryClass, files_changed),
                      NULL, NULL,
                      g_cclosure_marshal_VOID__POINTER,
                      G_TYPE_NONE, 1, G_TYPE_POINTER);
    signals[DONE_LOADING] =
        g_signal_new ("done_loading",
                      G_TYPE_FROM_CLASS (object_class),
                      G_SIGNAL_RUN_LAST,
                      G_STRUCT_OFFSET (ScatolaDirectoryClass, done_loading),
                      NULL, NULL,
    ...
    signals[LOAD_ERROR] =
        g_signal_new ("load_error",
                      G_TYPE_FROM_CLASS (object_class),
                      G_SIGNAL_RUN_LAST,
                      G_STRUCT_OFFSET (ScatolaDirectoryClass, load_error),
                      NULL, NULL,
                      g_cclosure_marshal_VOID__POINTER,
                      G_TYPE_NONE, 1, G_TYPE_POINTER);
    ...
    g_type_class_add_private (klass, sizeof (ScatolaDirectoryDetails));
}

static void
scatola_directory_init (gpointer object, gpointer klass)
{
    ScatolaDirectory *directory;

    directory = SCATOLA_DIRECTORY(object);

    directory->details = G_TYPE_INSTANCE_GET_PRIVATE ((directory), SCATOLA_TYPE_DIRECTORY, ScatolaDirectoryDetails);
    directory->details->file_hash = g_hash_table_new (g_str_hash, g_str_equal);
    directory->details->high_priority_queue = scatola_file_queue_new ();
    directory->details->low_priority_queue = scatola_file_queue_new ();
    directory->details->extension_queue = scatola_file_queue_new ();
    directory->details->free_space = (guint64)-1;
}

ScatolaDirectory *
scatola_directory_ref (ScatolaDirectory *directory)
{
    if (directory == NULL)
    {
        return directory;
...
void
scatola_directory_unref (ScatolaDirectory *directory)
{
    if (directory == NULL)
    {
        return;
    ...
    g_return_if_fail (SCATOLA_IS_DIRECTORY (directory));

    g_object_unref (directory);
}

static void
scatola_directory_finalize (GObject *object)
{
    ScatolaDirectory *directory;
    ...
    scatola_directory_cancel (directory);
    g_assert (directory->details->count_in_progress == NULL);
    g_assert (directory->details->top_left_read_state == NULL);

    if (directory->details->monitor_list != NULL)
    {
        g_warning ("destroying a ScatolaDirectory while it's being monitored");
    ...
    scatola_file_queue_destroy (directory->details->high_priority_queue);
    scatola_file_queue_destroy (directory->details->low_priority_queue);
    scatola_file_queue_destroy (directory->details->extension_queue);
...
invalidate_one_count (gpointer key, gpointer value, gpointer user_data)
{
    ...
    g_assert (key != NULL);
    g_assert (SCATOLA_IS_DIRECTORY (value));
    g_assert (user_data == NULL);

    directory = SCATOLA_DIRECTORY (value);
...
void
emit_change_signals_for_all_files (ScatolaDirectory *directory)
{
    GList *files;
    ...
    scatola_file_list_ref (files);
    scatola_directory_emit_change_signals (directory, files);
...
collect_all_directories (gpointer key, gpointer value, gpointer callback_data)
{
    ...
    *dirs = g_list_prepend (*dirs, scatola_directory_ref (directory));
}
...
emit_change_signals_for_all_files_in_all_directories (void)
{
    ...
    {
        directory = SCATOLA_DIRECTORY (l->data);
...
add_preferences_callbacks (void)
{
    ...
    g_signal_connect_swapped (scatola_preferences,
                              ...
                              NULL);
    g_signal_connect_swapped (scatola_preferences,
                              "changed::" SCATOLA_PREFERENCES_DATE_FORMAT,
                              G_CALLBACK(async_data_preference_changed_callback),
                              NULL);
}

/**
 * scatola_directory_get_by_uri:
 ...
 */
ScatolaDirectory *
scatola_directory_get_internal (GFile *location, gboolean create)
{
    ScatolaDirectory *directory;
    ...
    else if (create)
    {
        ...
        directory = scatola_directory_new (location);
        if (directory == NULL)
        {
            return NULL;
        }
...
ScatolaDirectory *
scatola_directory_get (GFile *location)
{
    if (location == NULL)
    {
        return NULL;
    ...
    return scatola_directory_get_internal (location, TRUE);
}

ScatolaDirectory *
scatola_directory_get_existing (GFile *location)
{
    if (location == NULL)
    {
        return NULL;
    ...
    return scatola_directory_get_internal (location, FALSE);
}


ScatolaDirectory *
scatola_directory_get_by_uri (const char *uri)
{
    ScatolaDirectory *directory;
    GFile *location;
    ...
    directory = scatola_directory_get_internal (location, TRUE);
    g_object_unref (location);
    return directory;
}

ScatolaDirectory *
scatola_directory_get_for_file (ScatolaFile *file)
{
    char *uri;
    ScatolaDirectory *directory;
    ...
    directory = scatola_directory_get_by_uri (uri);
    g_free (uri);
    return directory;
}

/* Returns a reffed ScatolaFile object for this directory.
 */
ScatolaFile *
scatola_directory_get_corresponding_file (ScatolaDirectory *directory)
{
    ScatolaFile *file;
    char *uri;
    ...
    if (file == NULL)
    {
        ...
        file = scatola_file_get_by_uri (uri);
        g_free (uri);
    }
...
/* Returns a reffed ScatolaFile object for this directory, but only if the
 * ScatolaFile object has already been created.
 */
ScatolaFile *
scatola_directory_get_existing_corresponding_file (ScatolaDirectory *directory)
{
    ScatolaFile *file;
    char *uri;
...
/* scatola_directory_get_name_for_self_as_new_file:
 *
 * Get a name to display for the file representing this
 * directory. This is called only when there's no VFS
 * directory for this ScatolaDirectory.
 */
char *
scatola_directory_get_name_for_self_as_new_file (ScatolaDirectory *directory)
{
    g_return_val_if_fail (SCATOLA_IS_DIRECTORY (directory), NULL);

    return EEL_CALL_METHOD_WITH_RETURN_VALUE
           (SCATOLA_DIRECTORY_CLASS, directory,
            get_name_for_self_as_new_file, (directory));
}

static char *
real_get_name_for_self_as_new_file (ScatolaDirectory *directory)
{
    char *directory_uri;
    char *name, *colon;

    directory_uri = scatola_directory_get_uri (directory);
...
char *
scatola_directory_get_uri (ScatolaDirectory *directory)
{
    g_return_val_if_fail (SCATOLA_IS_DIRECTORY (directory), NULL);
...
GFile *
scatola_directory_get_location (ScatolaDirectory  *directory)
{
    g_return_val_if_fail (SCATOLA_IS_DIRECTORY (directory), NULL);
...
static ScatolaDirectory *
scatola_directory_new (GFile *location)
{
    ScatolaDirectory *directory;
    char *uri;
    ...
    {
        directory = SCATOLA_DIRECTORY (g_object_new (SCATOLA_TYPE_DESKTOP_DIRECTORY, NULL));
    }
    else if (eel_uri_is_search (uri))
    {
        directory = SCATOLA_DIRECTORY (g_object_new (SCATOLA_TYPE_SEARCH_DIRECTORY, NULL));
    }
    else if (g_str_has_suffix (uri, SCATOLA_SAVED_SEARCH_EXTENSION))
    {
        directory = SCATOLA_DIRECTORY (scatola_search_directory_new_from_saved_search (uri));
    }
    else
    {
        directory = SCATOLA_DIRECTORY (g_object_new (SCATOLA_TYPE_VFS_DIRECTORY, NULL));
    }
...
gboolean
scatola_directory_is_local (ScatolaDirectory *directory)
{
    g_return_val_if_fail (SCATOLA_IS_DIRECTORY (directory), FALSE);
    ...
    return scatola_directory_is_in_trash (directory) ||
           g_file_is_native (directory->details->location);
}

gboolean
scatola_directory_is_in_trash (ScatolaDirectory *directory)
{
...
gboolean
scatola_directory_are_all_files_seen (ScatolaDirectory *directory)
{
    g_return_val_if_fail (SCATOLA_IS_DIRECTORY (directory), FALSE);
...
static void
add_to_hash_table (ScatolaDirectory *directory, ScatolaFile *file, GList *node)
{
    const char *name;
...
static GList *
extract_from_hash_table (ScatolaDirectory *directory, ScatolaFile *file)
{
    const char *name;
    GList *node;
...
void
scatola_directory_add_file (ScatolaDirectory *directory, ScatolaFile *file)
{
    GList *node;
    ...
    else if (scatola_directory_has_active_request_for_file (directory, file))
    {
...
void
scatola_directory_remove_file (ScatolaDirectory *directory, ScatolaFile *file)
{
    GList *node;
...
GList *
scatola_directory_begin_file_name_change (ScatolaDirectory *directory,
                                       ScatolaFile *file)
{
    /* Find the list node in the hash table. */
...
void
scatola_directory_end_file_name_change (ScatolaDirectory *directory,
                                     ScatolaFile *file,
                                     GList *node)
{
...
ScatolaFile *
scatola_directory_find_file_by_name (ScatolaDirectory *directory,
                                  const char *name)
{
    GList *node;
    ...
    node = g_hash_table_lookup (directory->details->file_hash,
                                name);
    return node == NULL ? NULL : SCATOLA_FILE (node->data);
}

void
scatola_directory_emit_files_added (ScatolaDirectory *directory,
                                 GList *added_files)
{
    if (added_files != NULL)
    {
...
void
scatola_directory_emit_files_changed (ScatolaDirectory *directory,
                                   GList *changed_files)
{
...
void
scatola_directory_emit_change_signals (ScatolaDirectory *directory,
                                    GList *changed_files)
{
    GList *p;
    ...
    {
        scatola_file_emit_changed (p->data);
    }
    scatola_directory_emit_files_changed (directory, changed_files);
}

void
scatola_directory_emit_done_loading (ScatolaDirectory *directory)
{
    g_signal_emit (directory,
...
void
scatola_directory_emit_load_error (ScatolaDirectory *directory,
                                GError *error)
{
    g_signal_emit (directory,
...
get_parent_directory (GFile *location)
{
    ...
    if (parent)
    {
        directory = scatola_directory_get_internal (parent, TRUE);
...
get_parent_directory_if_exists (GFile *location)
{
    ...
    if (parent)
    {
        directory = scatola_directory_get_internal (parent, FALSE);
        g_object_unref (parent);
        return directory;
    }
    return NULL;
...
{
    g_assert (SCATOLA_IS_DIRECTORY (key));
    g_assert (value != NULL);
    g_assert (user_data == NULL);
...
static void
call_files_changed_common (ScatolaDirectory *directory, GList *file_list)
{
    GList *node;
    ScatolaFile *file;
    ...
    for (node = file_list; node != NULL; node = node->next)
    {
        ...
        {
            scatola_directory_add_file_to_work_queue (directory,
                                                   file);
        }
    }
    scatola_directory_async_state_changed (directory);
    scatola_directory_emit_change_signals (directory, file_list);
}
...
call_files_changed_free_list (gpointer key, gpointer value, gpointer user_data)
{
    ...
    call_files_changed_common (SCATOLA_DIRECTORY (key), value);
    g_list_free (value);
}
...
call_get_file_info_free_list (gpointer key, gpointer value, gpointer user_data)
{
    ...
    scatola_directory_get_info_for_new_files (directory, files);
...
invalidate_count_and_unref (gpointer key, gpointer value, gpointer user_data)
{
    ...
    scatola_directory_invalidate_count_and_mime_list (key);
    scatola_directory_unref (key);
}

static void
collect_parent_directories (GHashTable *hash_table, ScatolaDirectory *directory)
{
...
void
scatola_directory_notify_files_added (GList *files)
{
    ...
    for (p = files; p != NULL; p = p->next)
    {
        ...
        if (directory == NULL)
        {
            ...
            if (parent)
            {
                file = scatola_file_get_existing (parent);
                g_object_unref (parent);
            }
        ...
        file = scatola_file_get_existing (location);
        /* We check is_added here, because the file could have been added
         * to the directory by a scatola_file_get() but not gotten
         * files_added emitted
         */
    ...
    /* Now get file info for the new files. This creates ScatolaFile
...
void
scatola_directory_notify_files_changed (GList *files)
{
...
void
scatola_directory_notify_files_removed (GList *files)
{
    GHashTable *changed_lists;
    GList *p;
    ScatolaDirectory *directory;
    ...
    for (p = files; p != NULL; p = p->next)
    {
        ...
        /* Find the file. */
        file = scatola_file_get_existing (location);
        if (file != NULL && !scatola_file_rename_in_progress (file))
        {
...
static void
set_directory_location (ScatolaDirectory *directory,
                        GFile *location)
{
    if (directory->details->location)
    {
...
static void
change_directory_location (ScatolaDirectory *directory,
                           GFile *new_location)
{
...
static GList *
scatola_directory_moved_internal (GFile *old_location,
                               GFile *new_location)
{
    ...
    {
        directory = SCATOLA_DIRECTORY (node->data);
        ...
        if (new_directory_location)
        {
            ...
            if (directory->details->as_file != NULL)
            {
                affected_files = g_list_prepend
                                 (affected_files,
                                  scatola_file_ref (directory->details->as_file));
            }
...
void
scatola_directory_moved (const char *old_uri,
                      const char *new_uri)
{
    GList *list, *node;
    GHashTable *hash;
    ...
    list = scatola_directory_moved_internal (old_location, new_location);
...
void
scatola_directory_notify_files_moved (GList *file_pairs)
{
    GList *p, *affected_files, *node;
    GFilePair *pair;
    ScatolaFile *file;
    ScatolaDirectory *old_directory, *new_directory;
    GHashTable *parent_directories;
    GList *new_files_list, *unref_list;
    GHashTable *added_lists, *changed_lists;
    char *name;
    ScatolaFileAttributes cancel_attributes;
    ...
    cancel_attributes = scatola_file_get_all_attributes ();
    ...
    for (p = file_pairs; p != NULL; p = p->next)
    {
        ...
        file = scatola_file_get_existing (to_location);
        if (file != NULL)
        {
            /* Mark it gone and prepare to send the changed signal. */
            scatola_file_mark_gone (file);
        ...
        affected_files = scatola_directory_moved_internal (from_location,
                         to_location);
        ...
        file = scatola_file_get_existing (from_location);
        if (file == NULL)
        {
        ...
        else
        {
            ...
            scatola_directory_cancel_loading_file_attributes
             ...
             * anyway.
             */
            scatola_directory_unref (new_directory);

            /* Update the file's name and directory. */
            name = g_file_get_basename (to_location);
            scatola_file_update_name_and_directory
            (file, name, new_directory);
            g_free (name);

            /* Update file attributes */
            scatola_file_invalidate_attributes (file, SCATOLA_FILE_ATTRIBUTE_INFO);
            ...
            /* Unref each file once to balance out scatola_file_get_by_uri. */
    ...
    scatola_file_list_free (unref_list);
    ...
    scatola_directory_notify_files_added (new_files_list);
    g_list_free (new_files_list);
}

void
scatola_directory_schedule_position_set (GList *position_setting_list)
{
    GList *p;
    const ScatolaFileChangesQueuePosition *item;
    ScatolaFile *file;
    char str[64];
    time_t now;
    ...
    {
        item = (ScatolaFileChangesQueuePosition *) p->data;

        file = scatola_file_get (item->location);
        ...
        if (item->set)
        {
            scatola_file_set_time_metadata
            (file,
...
gboolean
scatola_directory_contains_file (ScatolaDirectory *directory,
                              ScatolaFile *file)
{
    g_return_val_if_fail (SCATOLA_IS_DIRECTORY (directory), FALSE);
...
void
scatola_directory_call_when_ready (ScatolaDirectory *directory,
...
void
scatola_directory_cancel_callback (ScatolaDirectory *directory,
                                ScatolaDirectoryCallback callback,
                                gpointer callback_data)
{
    g_return_if_fail (SCATOLA_IS_DIRECTORY (directory));
...
void
scatola_directory_file_monitor_add (ScatolaDirectory *directory,
...
void
scatola_directory_file_monitor_remove (ScatolaDirectory *directory,
                                    gconstpointer client)
{
    g_return_if_fail (SCATOLA_IS_DIRECTORY (directory));
    g_return_if_fail (client != NULL);
...
void
scatola_directory_force_reload (ScatolaDirectory *directory)
{
    g_return_if_fail (SCATOLA_IS_DIRECTORY (directory));
...
gboolean
scatola_directory_is_not_empty (ScatolaDirectory *directory)
{
    g_return_val_if_fail (SCATOLA_IS_DIRECTORY (directory), FALSE);
...
GList *
scatola_directory_get_file_list (ScatolaDirectory *directory)
{
    return EEL_CALL_METHOD_WITH_RETURN_VALUE
           (SCATOLA_DIRECTORY_CLASS, directory,
...
static GList *
real_get_file_list (ScatolaDirectory *directory)
{
    ...
    scatola_file_list_ref (non_tentative_files);
    return non_tentative_files;
}

static gboolean
real_is_editable (ScatolaDirectory *directory)
{
    return TRUE;
}

gboolean
scatola_directory_is_editable (ScatolaDirectory *directory)
{
    return EEL_CALL_METHOD_WITH_RETURN_VALUE
           (SCATOLA_DIRECTORY_CLASS, directory,
...
GList *
scatola_directory_match_pattern (ScatolaDirectory *directory, const char *pattern)
{
    GList *files, *l, *ret;
    GPatternSpec *spec;
    ...
    for (l = files; l; l = l->next)
    {
        ...
        {
            ret = g_list_prepend(ret, scatola_file_ref (file));
        }
...
/**
 * scatola_directory_list_ref
 *
 * Ref all the directories in a list.
 * @list: GList of directories.
 **/
GList *
scatola_directory_list_ref (GList *list)
{
    g_list_foreach (list, (GFunc) scatola_directory_ref, NULL);
    return list;
}

/**
 * scatola_directory_list_unref
 *
 * Unref all the directories in a list.
 * @list: GList of directories.
 **/
void
scatola_directory_list_unref (GList *list)
{
    g_list_foreach (list, (GFunc) scatola_directory_unref, NULL);
}

/**
 * scatola_directory_list_free
 *
 * Free a list of directories after unrefing them.
 * @list: GList of directories.
 **/
void
scatola_directory_list_free (GList *list)
{
    scatola_directory_list_unref (list);
    g_list_free (list);
}

/**
 * scatola_directory_list_copy
 *
 * Copy the list of directories, making a new ref of each,
 * @list: GList of directories.
 **/
GList *
scatola_directory_list_copy (GList *list)
{
    return g_list_copy (scatola_directory_list_ref (list));
}

static int
compare_by_uri (ScatolaDirectory *a, ScatolaDirectory *b)
{
    char *uri_a, *uri_b;
    int res;
...
{
    return compare_by_uri (SCATOLA_DIRECTORY (a), SCATOLA_DIRECTORY (b));
}

/**
 * scatola_directory_list_sort_by_uri
 *
 * Sort the list of directories by directory uri.
 * @list: GList of directories.
 **/
GList *
scatola_directory_list_sort_by_uri (GList *list)
{
    return g_list_sort (list, compare_by_uri_cover);
}

gboolean
scatola_directory_is_desktop_directory (ScatolaDirectory   *directory)
{
    if (directory->details->location == NULL)
    {
        return FALSE;
    ...
    return scatola_is_desktop_directory (directory->details->location);
}
...
static void
got_files_callback (ScatolaDirectory *directory, GList *files, gpointer callback_data)
{
    g_assert (SCATOLA_IS_DIRECTORY (directory));
...
/* Return the number of extant ScatolaDirectories */
int
scatola_directory_number_outstanding (void)
{
    return directories ? g_hash_table_size (directories) : 0;
}

void
scatola_self_check_directory (void)
{
    ScatolaDirectory *directory;
    ScatolaFile *file;

    directory = scatola_directory_get_by_uri ("file:///etc");
    file = scatola_file_get_by_uri ("file:///etc/passwd");
    ...
    scatola_file_set_metadata (file, "test", "default", "value");
    EEL_CHECK_STRING_RESULT (scatola_file_get_metadata (file, "test", "default"), "value");

    scatola_file_set_boolean_metadata (file, "test_boolean", TRUE, TRUE);
    EEL_CHECK_BOOLEAN_RESULT (scatola_file_get_boolean_metadata (file, "test_boolean", TRUE), TRUE);
    scatola_file_set_boolean_metadata (file, "test_boolean", TRUE, FALSE);
    EEL_CHECK_BOOLEAN_RESULT (scatola_file_get_boolean_metadata (file, "test_boolean", TRUE), FALSE);
    EEL_CHECK_BOOLEAN_RESULT (scatola_file_get_boolean_metadata (NULL, "test_boolean", TRUE), TRUE);

    scatola_file_set_integer_metadata (file, "test_integer", 0, 17);
    EEL_CHECK_INTEGER_RESULT (scatola_file_get_integer_metadata (file, "test_integer", 0), 17);
    scatola_file_set_integer_metadata (file, "test_integer", 0, -1);
    EEL_CHECK_INTEGER_RESULT (scatola_file_get_integer_metadata (file, "test_integer", 0), -1);
    scatola_file_set_integer_metadata (file, "test_integer", 42, 42);
    EEL_CHECK_INTEGER_RESULT (scatola_file_get_integer_metadata (file, "test_integer", 42), 42);
    EEL_CHECK_INTEGER_RESULT (scatola_file_get_integer_metadata (NULL, "test_integer", 42), 42);
    EEL_CHECK_INTEGER_RESULT (scatola_file_get_integer_metadata (file, "nonexistent_key", 42), 42);
    */

    EEL_CHECK_BOOLEAN_RESULT (scatola_directory_get_by_uri ("file:///etc") == directory, TRUE);
    scatola_directory_unref (directory);
    ...
    scatola_directory_file_monitor_remove (directory, &data_dummy);
    ...
    scatola_directory_call_when_ready (directory,
