...
/* scatola-tree-view-drag-dest.c: Handles drag and drop for treeviews which
 *                                 contain a hierarchy of files
 */

#include <config.h>
#include "scatola-tree-view-drag-dest.h"
...
struct _ScatolaTreeViewDragDestDetails
{
...
static void scatola_tree_view_drag_dest_init (ScatolaTreeViewDragDest      *dest);
static void scatola_tree_view_drag_dest_class_init    (ScatolaTreeViewDragDestClass *class);

static guint signals[LAST_SIGNAL] = { 0 };

G_DEFINE_TYPE (ScatolaTreeViewDragDest, scatola_tree_view_drag_dest,
               G_TYPE_OBJECT);
#define parent_class scatola_tree_view_drag_dest_parent_class
...
static void
remove_scroll_timeout (ScatolaTreeViewDragDest *dest)
{
...
static void
remove_expand_timeout (ScatolaTreeViewDragDest *dest)
{
...
static void
set_widget_highlight (ScatolaTreeViewDragDest *dest, gboolean highlight)
{
...
static void
set_drag_dest_row (ScatolaTreeViewDragDest *dest,
                   GtkTreePath *path)
{
    if (path)
    {
...
static void
clear_drag_dest_row (ScatolaTreeViewDragDest *dest)
{
...
static gboolean
get_drag_data (ScatolaTreeViewDragDest *dest,
               ...
               guint32 time)
{
    ...
    if (target == gdk_atom_intern (SCATOLA_ICON_DND_XDNDDIRECTSAVE_TYPE, FALSE) &&
            !dest->details->drop_occurred)
    {
        dest->details->drag_type = SCATOLA_ICON_DND_XDNDDIRECTSAVE;
...
static void
free_drag_data (ScatolaTreeViewDragDest *dest)
{
    ...
    {
        scatola_drag_destroy_selection_list (dest->details->drag_list);
...
static char *
get_root_uri (ScatolaTreeViewDragDest *dest)
{
    char *uri;
...
static ScatolaFile *
file_for_path (ScatolaTreeViewDragDest *dest, GtkTreePath *path)
{
    ScatolaFile *file;
    char *uri;
...
static GtkTreePath *
get_drop_path (ScatolaTreeViewDragDest *dest,
               GtkTreePath *path)
{
    ScatolaFile *file;
    GtkTreePath *ret;
    ...
    while (file == NULL /* dummy row */ ||
            !scatola_drag_can_accept_info (file,
...
static char *
get_drop_target_uri_for_path (ScatolaTreeViewDragDest *dest,
                              GtkTreePath *path)
{
    ScatolaFile *file;
    char *target;
    ...
    target = scatola_file_get_drop_target_uri (file);
    scatola_file_unref (file);
...
static guint
get_drop_action (ScatolaTreeViewDragDest *dest,
                 GdkDragContext *context,
                 GtkTreePath *path)
{
    char *drop_target;
    ...
    if (!dest->details->have_drag_data ||
            (dest->details->drag_type == SCATOLA_ICON_DND_GDE2_ICON_LIST &&
             dest->details->drag_list == NULL))
    {
        return 0;
    }
    ...
    case SCATOLA_ICON_DND_GDE2_ICON_LIST :
        ...
        scatola_drag_default_drop_action_for_icons
        (context,
         drop_target,
         dest->details->drag_list,
         &action);
                      ...
                      gpointer data)
{
    ...
    dest = SCATOLA_TREE_VIEW_DRAG_DEST (data);
...
static char *
get_drop_target_uri_at_pos (ScatolaTreeViewDragDest *dest, int x, int y)
{
    char *drop_target;
    GtkTreePath *path;
    GtkTreePath *drop_path;
...
static void
receive_uris (ScatolaTreeViewDragDest *dest,
              GdkDragContext *context,
              GList *source_uris,
              int x, int y)
{
    ...
    {
        if (scatola_drag_selection_includes_special_link (dest->details->drag_list))
        {
        ...
        real_action = scatola_drag_drop_action_ask
                      (GTK_WIDGET (dest->details->tree_view), action);
    }

    /* We only want to copy external uris */
    if (dest->details->drag_type == SCATOLA_ICON_DND_URI_LIST)
    {
    ...
    if (real_action > 0)
    {
        if (!scatola_drag_uris_local (drop_target, source_uris)
...
static void
receive_dropped_icons (ScatolaTreeViewDragDest *dest,
                       GdkDragContext *context,
                       int x, int y)
{
    GList *source_uris;
    ...
    for (l = dest->details->drag_list; l != NULL; l = l->next)
    {
        source_uris = g_list_prepend (source_uris,
                                      ((ScatolaDragSelectionItem *)l->data)->uri);
    }
...
static void
receive_dropped_uri_list (ScatolaTreeViewDragDest *dest,
                          GdkDragContext *context,
                          int x, int y)
{
    char *drop_target;
...
static void
receive_dropped_text (ScatolaTreeViewDragDest *dest,
                      GdkDragContext *context,
                      int x, int y)
{
    char *drop_target;
...
static void
receive_dropped_raw (ScatolaTreeViewDragDest *dest,
...
static void
receive_dropped_netscape_url (ScatolaTreeViewDragDest *dest,
                              GdkDragContext *context,
                              int x, int y)
{
    char *drop_target;
...
static void
receive_dropped_keyword (ScatolaTreeViewDragDest *dest,
                         GdkDragContext *context,
                         int x, int y)
{
    char *drop_target_uri;
    ScatolaFile *drop_target_file;
    ...
    drop_target_file = scatola_file_get_by_uri (drop_target_uri);

    if (drop_target_file != NULL)
    {
        scatola_drag_file_receive_dropped_keyword (drop_target_file,
                                                (char *) gtk_selection_data_get_data (dest->details->drag_data));
        scatola_file_unref (drop_target_file);
    }
...
static gboolean
receive_xds (ScatolaTreeViewDragDest *dest,
             GtkWidget *widget,
             guint32 time,
             GdkDragContext *context,
             int x, int y)
                             ...
                             gpointer data)
{
    ...
    if (!dest->details->have_drag_data)
    {
        ...
        {
            dest->details->drag_list =
                scatola_drag_build_selection_list (selection_data);
        }
    }
...
static gboolean
set_direct_save_uri (ScatolaTreeViewDragDest *dest,
                     GdkDragContext *context,
                     int x, int y)
{
    GFile *base, *child;
                       ...
                       GObject *object)
{
    ...
    dest = SCATOLA_TREE_VIEW_DRAG_DEST (user_data);
...
static void
scatola_tree_view_drag_dest_dispose (GObject *object)
{
    ScatolaTreeViewDragDest *dest;

    dest = SCATOLA_TREE_VIEW_DRAG_DEST (object);
...
static void
scatola_tree_view_drag_dest_finalize (GObject *object)
{
    ScatolaTreeViewDragDest *dest;
...
static void
scatola_tree_view_drag_dest_init (ScatolaTreeViewDragDest *dest)
{
    dest->details = g_new0 (ScatolaTreeViewDragDestDetails, 1);
}

static void
scatola_tree_view_drag_dest_class_init (ScatolaTreeViewDragDestClass *class)
{
    GObjectClass *gobject_class;

    gobject_class = G_OBJECT_CLASS (class);

    gobject_class->dispose = scatola_tree_view_drag_dest_dispose;
    gobject_class->finalize = scatola_tree_view_drag_dest_finalize;

    signals[GET_ROOT_URI] =
        g_signal_new ("get_root_uri",
                      G_TYPE_FROM_CLASS (class),
                      G_SIGNAL_RUN_LAST,
                      G_STRUCT_OFFSET (ScatolaTreeViewDragDestClass,
                                       get_root_uri),
                      NULL, NULL,
    ...
    signals[MOVE_COPY_ITEMS] =
        g_signal_new ("move_copy_items",
                      ...
                      scatola_marshal_VOID__POINTER_STRING_ENUM_INT_INT,
                      G_TYPE_NONE, 5,
                      G_TYPE_POINTER,
                      G_TYPE_STRING,
...
ScatolaTreeViewDragDest *
scatola_tree_view_drag_dest_new (GtkTreeView *tree_view)
{
    ScatolaTreeViewDragDest *dest;
    GtkTargetList *targets;

    dest = g_object_new (SCATOLA_TYPE_TREE_VIEW_DRAG_DEST, NULL);
