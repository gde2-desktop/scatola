...
struct _ScatolaFileConflictDialogDetails
{
    /* conflicting objects */
    ScatolaFile *source;
    ScatolaFile *destination;
    ScatolaFile *dest_dir;
...
G_DEFINE_TYPE (ScatolaFileConflictDialog,
               scatola_file_conflict_dialog,
               GTK_TYPE_DIALOG);

#define SCATOLA_FILE_CONFLICT_DIALOG_GET_PRIVATE(object)		\
	(G_TYPE_INSTANCE_GET_PRIVATE ((object), SCATOLA_TYPE_FILE_CONFLICT_DIALOG, \
				      ScatolaFileConflictDialogDetails))
...
static void
file_icons_changed (ScatolaFile *file,
                    ScatolaFileConflictDialog *fcd)
{
    GdkPixbuf *pixbuf;

    pixbuf = scatola_file_get_icon_pixbuf (fcd->details->destination,
                                        SCATOLA_ICON_SIZE_LARGE,
                                        TRUE,
    ...
    pixbuf = scatola_file_get_icon_pixbuf (fcd->details->source,
                                        SCATOLA_ICON_SIZE_LARGE,
                                        TRUE,
                                        SCATOLA_FILE_ICON_FLAGS_USE_THUMBNAILS);
...
file_list_ready_cb (GList *files,
                    gpointer user_data)
{
    ScatolaFileConflictDialog *fcd = user_data;
    ScatolaFile *src, *dest, *dest_dir;
    ...
    src_mtime = scatola_file_get_mtime (src);
    dest_mtime = scatola_file_get_mtime (dest);

    dest_name = scatola_file_get_display_name (dest);
    dest_dir_name = scatola_file_get_display_name (dest_dir);

    source_is_dir = scatola_file_is_directory (src);
    dest_is_dir = scatola_file_is_directory (dest);

    type = scatola_file_get_mime_type (dest);
    should_show_type = !scatola_file_is_mime_type (src, type);
    ...
    /* Set up file icons */
    pixbuf = scatola_file_get_icon_pixbuf (dest,
                                        SCATOLA_ICON_SIZE_LARGE,
                                        TRUE,
                                        SCATOLA_FILE_ICON_FLAGS_USE_THUMBNAILS);
    ...
    pixbuf = scatola_file_get_icon_pixbuf (src,
                                        SCATOLA_ICON_SIZE_LARGE,
                                        TRUE,
                                        SCATOLA_FILE_ICON_FLAGS_USE_THUMBNAILS);
    ...
    date = scatola_file_get_string_attribute (dest,
                                           "date_modified");
    size = scatola_file_get_string_attribute (dest, "size");

    if (should_show_type)
    {
        type = scatola_file_get_string_attribute (dest, "type");
    }
    ...
    /* Second label */
    label = gtk_label_new (NULL);
    date = scatola_file_get_string_attribute (src,
                                           "date_modified");
    size = scatola_file_get_string_attribute (src, "size");

    if (should_show_type)
    {
        type = scatola_file_get_string_attribute (src, "type");
    }
    ...
    /* Populate the entry */
    edit_name = scatola_file_get_edit_name (dest);
    ...
    if (!source_is_dir && !dest_is_dir)
    {
        ...
        if (meld_found) {
            ...
            src_is_binary = scatola_file_is_binary (details->source);
            dest_is_binary = scatola_file_is_binary (details->destination);
    ...
    scatola_file_monitor_add (src, fcd, SCATOLA_FILE_ATTRIBUTES_FOR_ICON);
    scatola_file_monitor_add (dest, fcd, SCATOLA_FILE_ATTRIBUTES_FOR_ICON);
...
static void
build_dialog_appearance (ScatolaFileConflictDialog *fcd)
{
    GList *files = NULL;
    ScatolaFileConflictDialogDetails *details = fcd->details;
    ...
    scatola_file_list_call_when_ready (files,
...
set_source_and_destination (GtkWidget *w,
                            ...
                            GFile *dest_dir)
{
    ScatolaFileConflictDialog *dialog;
    ScatolaFileConflictDialogDetails *details;

    dialog = SCATOLA_FILE_CONFLICT_DIALOG (w);
    details = dialog->details;

    details->source = scatola_file_get (source);
    details->destination = scatola_file_get (destination);
    details->dest_dir = scatola_file_get (dest_dir);
                        ...
                        ScatolaFileConflictDialog *dialog)
{
    ...
    if (command)
    {
        argv = g_new (char *, 4);
        argv[0] = command;
        argv[1] = g_file_get_path (scatola_file_get_location (details->source));
        argv[2] = g_file_get_path (scatola_file_get_location (details->destination));
        argv[3] = NULL;
...
static void
scatola_file_conflict_dialog_init (ScatolaFileConflictDialog *fcd)
{
#if GTK_CHECK_VERSION (3, 0, 0)
    ...
    details = fcd->details = SCATOLA_FILE_CONFLICT_DIALOG_GET_PRIVATE (fcd);
...
{
    ScatolaFileConflictDialogDetails *details =
        SCATOLA_FILE_CONFLICT_DIALOG (self)->details;

    g_free (details->conflict_name);

    if (details->handle != NULL)
    {
        scatola_file_list_cancel_call_when_ready (details->handle);
    }

    if (details->src_handler_id)
    {
        g_signal_handler_disconnect (details->source, details->src_handler_id);
        scatola_file_monitor_remove (details->source, self);
    }

    if (details->dest_handler_id)
    {
        g_signal_handler_disconnect (details->destination, details->dest_handler_id);
        scatola_file_monitor_remove (details->destination, self);
    }

    scatola_file_unref (details->source);
    scatola_file_unref (details->destination);
    scatola_file_unref (details->dest_dir);

    G_OBJECT_CLASS (scatola_file_conflict_dialog_parent_class)->finalize (self);
}

static void
scatola_file_conflict_dialog_class_init (ScatolaFileConflictDialogClass *klass)
{
    G_OBJECT_CLASS (klass)->finalize = do_finalize;

    g_type_class_add_private (klass, sizeof (ScatolaFileConflictDialogDetails));
}

char *
scatola_file_conflict_dialog_get_new_name (ScatolaFileConflictDialog *dialog)
{
...
gboolean
scatola_file_conflict_dialog_get_apply_to_all (ScatolaFileConflictDialog *dialog)
{
...
GtkWidget *
scatola_file_conflict_dialog_new (GtkWindow *parent,
                               GFile *source,
                               GFile *destination,
                               GFile *dest_dir)
{
    ...
    dialog = GTK_WIDGET (g_object_new (SCATOLA_TYPE_FILE_CONFLICT_DIALOG,
