/* -*- Mode: C; indent-tabs-mode: t; c-basic-offset: 8; tab-width: 8 -*-
   ...
   scatola-file.c: Scatola file model.
...
#include "scatola-lib-self-check-functions.h"
#include "scatola-link.h"
#include "scatola-metadata.h"
#include "scatola-module.h"
#include "scatola-search-directory.h"
#include "scatola-search-directory-file.h"
#include "scatola-thumbnails.h"
#include "scatola-ui-utilities.h"
#include "scatola-vfs-file.h"
#include "scatola-saved-search-file.h"
...
#include <libscatola-extension/scatola-extension-private.h>
...
#undef SCATOLA_FILE_DEBUG_REF
#undef SCATOLA_FILE_DEBUG_REF_VALGRIND

#ifdef SCATOLA_FILE_DEBUG_REF_VALGRIND
...
/* Name of Scatola trash directories */
...
typedef void (* ModifyListFunction) (GList **list, ScatolaFile *file);
...
static void     scatola_file_info_iface_init                (ScatolaFileInfoIface *iface);
static char *   scatola_file_get_owner_as_string            (ScatolaFile          *file,
							      gboolean               include_real_name);
static char *   scatola_file_get_type_as_string             (ScatolaFile          *file);
static gboolean update_info_and_name                         (ScatolaFile          *file,
							      GFileInfo             *info);
static const char * scatola_file_peek_display_name (ScatolaFile *file);
static const char * scatola_file_peek_display_name_collation_key (ScatolaFile *file);
static void file_mount_unmounted (GMount *mount,  gpointer data);
static void metadata_hash_free (GHashTable *hash);

G_DEFINE_TYPE_WITH_CODE (ScatolaFile, scatola_file, G_TYPE_OBJECT,
			 G_IMPLEMENT_INTERFACE (SCATOLA_TYPE_FILE_INFO,
						scatola_file_info_iface_init));

static void
scatola_file_init (ScatolaFile *file)
{
	file->details = G_TYPE_INSTANCE_GET_PRIVATE ((file), SCATOLA_TYPE_FILE, ScatolaFileDetails);

	scatola_file_clear_info (file);
	scatola_file_invalidate_extension_info_internal (file);
}

static GObject*
scatola_file_constructor (GType                  type,
			   ...
			   GObjectConstructParam *construct_params)
{
  ...
  object = (* G_OBJECT_CLASS (scatola_file_parent_class)->constructor) (type,
									 n_construct_properties,
									 construct_params);

  file = SCATOLA_FILE (object);

  /* Set to default type after full construction */
  if (SCATOLA_FILE_GET_CLASS (file)->default_file_type != G_FILE_TYPE_UNKNOWN) {
	  file->details->type = SCATOLA_FILE_GET_CLASS (file)->default_file_type;
  }
...
gboolean
scatola_file_set_display_name (ScatolaFile *file,
				...
				gboolean custom)
{
	...
	if (custom && display_name == NULL) {
		...
		if (file->details->got_custom_display_name) {
			...
			scatola_file_invalidate_attributes (file,
...
static void
scatola_file_clear_display_name (ScatolaFile *file)
{
	eel_ref_str_unref (file->details->display_name);
...
static void
clear_metadata (ScatolaFile *file)
{
...
get_metadata_from_info (GFileInfo *info)
{
	...
	for (i = 0; attrs[i] != NULL; i++) {
		id = scatola_metadata_get_id (attrs[i] + strlen ("metadata::"));
		if (id == 0) {
			continue;
		}
...
gboolean
scatola_file_update_metadata_from_info (ScatolaFile *file,
					 GFileInfo *info)
{
...
void
scatola_file_clear_info (ScatolaFile *file)
{
	...
	if (!file->details->got_custom_display_name) {
		scatola_file_clear_display_name (file);
	}
...
static ScatolaFile *
scatola_file_new_from_filename (ScatolaDirectory *directory,
				 const char *filename,
				 gboolean self_owned)
{
	ScatolaFile *file;
	...
	if (SCATOLA_IS_DESKTOP_DIRECTORY (directory)) {
		if (self_owned) {
			file = SCATOLA_FILE (g_object_new (SCATOLA_TYPE_DESKTOP_DIRECTORY_FILE, NULL));
		} else {
			/* This doesn't normally happen, unless the user somehow types in a uri
			 * that references a file like this. (See #349840) */
			file = SCATOLA_FILE (g_object_new (SCATOLA_TYPE_VFS_FILE, NULL));
		}
	} else if (SCATOLA_IS_SEARCH_DIRECTORY (directory)) {
		if (self_owned) {
			file = SCATOLA_FILE (g_object_new (SCATOLA_TYPE_SEARCH_DIRECTORY_FILE, NULL));
		} else {
			/* This doesn't normally happen, unless the user somehow types in a uri
			 * that references a file like this. (See #349840) */
			file = SCATOLA_FILE (g_object_new (SCATOLA_TYPE_VFS_FILE, NULL));
		}
	} else if (g_str_has_suffix (filename, SCATOLA_SAVED_SEARCH_EXTENSION)) {
		file = SCATOLA_FILE (g_object_new (SCATOLA_TYPE_SAVED_SEARCH_FILE, NULL));
	} else {
		file = SCATOLA_FILE (g_object_new (SCATOLA_TYPE_VFS_FILE, NULL));
	}

	file->details->directory = scatola_directory_ref (directory);
...
static void
modify_link_hash_table (ScatolaFile *file,
			ModifyListFunction modify_function)
{
	...
	target_uri = scatola_file_get_symbolic_link_target_uri (file);
...
static void
add_to_link_hash_table_list (GList **list, ScatolaFile *file)
{
...
static void
add_to_link_hash_table (ScatolaFile *file)
{
	modify_link_hash_table (file, add_to_link_hash_table_list);
}

static void
remove_from_link_hash_table_list (GList **list, ScatolaFile *file)
{
	if (g_list_find (*list, file) != NULL) {
...
static void
remove_from_link_hash_table (ScatolaFile *file)
{
	modify_link_hash_table (file, remove_from_link_hash_table_list);
}

ScatolaFile *
scatola_file_new_from_info (ScatolaDirectory *directory,
			     GFileInfo *info)
{
	ScatolaFile *file;
	const char *mime_type;

	g_return_val_if_fail (SCATOLA_IS_DIRECTORY (directory), NULL);
	g_return_val_if_fail (info != NULL, NULL);

	mime_type = g_file_info_get_content_type (info);
	if (mime_type &&
	    strcmp (mime_type, SCATOLA_SAVED_SEARCH_MIMETYPE) == 0) {
...
static ScatolaFile *
scatola_file_get_internal (GFile *location, gboolean create)
{
	gboolean self_owned;
	...
	directory = scatola_directory_get_internal (parent, create);

	g_object_unref (parent);

	/* Get the name for the file. */
	if (self_owned && directory != NULL) {
		basename = scatola_directory_get_name_for_self_as_new_file (directory);
	} else {
	...
	if (directory == NULL) {
		...
		file = scatola_directory_find_file_by_name (directory, basename);
	}
	...
	if (file != NULL) {
		...
		file = scatola_file_new_from_filename (directory, basename, self_owned);
		if (self_owned) {
...
ScatolaFile *
scatola_file_get (GFile *location)
{
	return scatola_file_get_internal (location, TRUE);
}

ScatolaFile *
scatola_file_get_existing (GFile *location)
{
	return scatola_file_get_internal (location, FALSE);
}

ScatolaFile *
scatola_file_get_existing_by_uri (const char *uri)
{
	GFile *location;
	ScatolaFile *file;

	location = g_file_new_for_uri (uri);
	file = scatola_file_get_internal (location, FALSE);
...
ScatolaFile *
scatola_file_get_by_uri (const char *uri)
{
	GFile *location;
	ScatolaFile *file;

	location = g_file_new_for_uri (uri);
	file = scatola_file_get_internal (location, TRUE);
	g_object_unref (location);
...
gboolean
scatola_file_is_self_owned (ScatolaFile *file)
{
...
finalize (GObject *object)
{
	...
	if (file->details->is_thumbnailing) {
		...
		scatola_thumbnail_remove_from_queue (uri);
		g_free (uri);
	}

	scatola_async_destroying_file (file);

	remove_from_link_hash_table (file);

	directory = file->details->directory;

	if (scatola_file_is_self_owned (file)) {
		directory->details->as_file = NULL;
	} else {
		if (!file->details->is_gone) {
			scatola_directory_remove_file (directory, file);
		}
	}
	...
	G_OBJECT_CLASS (scatola_file_parent_class)->finalize (object);
}

ScatolaFile *
scatola_file_ref (ScatolaFile *file)
{
	if (file == NULL) {
		return NULL;
	}
	g_return_val_if_fail (SCATOLA_IS_FILE (file), NULL);
...
void
scatola_file_unref (ScatolaFile *file)
{
	if (file == NULL) {
		return;
	}
...
/**
 * scatola_file_get_parent_uri_for_display:
 *
 ...
 */
char *
scatola_file_get_parent_uri_for_display (ScatolaFile *file)
{
	GFile *parent;
	char *result;
	...
	parent = scatola_file_get_parent_location (file);
	if (parent) {
 ...
 * Use scatola_file_get_parent_uri_for_display instead if the
 * result is to be displayed on-screen.
 * If the parent is NULL, returns the empty string.
 */
char *
scatola_file_get_parent_uri (ScatolaFile *file)
{
	g_assert (SCATOLA_IS_FILE (file));
	...
	return scatola_directory_get_uri (file->details->directory);
}

GFile *
scatola_file_get_parent_location (ScatolaFile *file)
{
	g_assert (SCATOLA_IS_FILE (file));
	...
	return scatola_directory_get_location (file->details->directory);
}

ScatolaFile *
scatola_file_get_parent (ScatolaFile *file)
{
	g_assert (SCATOLA_IS_FILE (file));
	...
	return scatola_directory_get_corresponding_file (file->details->directory);
}

/**
 * scatola_file_can_read:
 *
 ...
 */
gboolean
scatola_file_can_read (ScatolaFile *file)
{
	g_return_val_if_fail (SCATOLA_IS_FILE (file), FALSE);

	return file->details->can_read;
}

/**
 * scatola_file_can_write:
 *
 ...
 */
gboolean
scatola_file_can_write (ScatolaFile *file)
{
	g_return_val_if_fail (SCATOLA_IS_FILE (file), FALSE);
 ...
 */
gboolean
scatola_file_can_execute (ScatolaFile *file)
{
	g_return_val_if_fail (SCATOLA_IS_FILE (file), FALSE);
...
gboolean
scatola_file_can_mount (ScatolaFile *file)
{
	g_return_val_if_fail (SCATOLA_IS_FILE (file), FALSE);
...
gboolean
scatola_file_can_unmount (ScatolaFile *file)
{
	g_return_val_if_fail (SCATOLA_IS_FILE (file), FALSE);
...
gboolean
scatola_file_can_eject (ScatolaFile *file)
{
	g_return_val_if_fail (SCATOLA_IS_FILE (file), FALSE);
...
gboolean
scatola_file_can_start (ScatolaFile *file)
{
	gboolean ret;
	GDrive *drive;
...
gboolean
scatola_file_can_start_degraded (ScatolaFile *file)
{
	gboolean ret;
	GDrive *drive;
...
gboolean
scatola_file_can_poll_for_media (ScatolaFile *file)
{
	gboolean ret;
	GDrive *drive;
...
gboolean
scatola_file_is_media_check_automatic (ScatolaFile *file)
{
	gboolean ret;
	GDrive *drive;
...
gboolean
scatola_file_can_stop (ScatolaFile *file)
{
	gboolean ret;
	GDrive *drive;
...
GDriveStartStopType
scatola_file_get_start_stop_type (ScatolaFile *file)
{
...
void
scatola_file_mount (ScatolaFile                   *file,
		     ...
		     gpointer                        callback_data)
{
	...
	if (SCATOLA_FILE_GET_CLASS (file)->mount == NULL) {
		if (callback) {
			error = NULL;
		...
		SCATOLA_FILE_GET_CLASS (file)->mount (file, mount_op, cancellable, callback, callback_data);
	}
}
...
unmount_done (void *callback_data)
{
	...
	scatola_file_unref (data->file);
	g_free (data);
}

void
scatola_file_unmount (ScatolaFile                   *file,
		       GMountOperation                *mount_op,
		       GCancellable                   *cancellable,
		       ScatolaFileOperationCallback   callback,
		       gpointer                        callback_data)
{
	...
	if (file->details->can_unmount) {
		if (SCATOLA_FILE_GET_CLASS (file)->unmount != NULL) {
			SCATOLA_FILE_GET_CLASS (file)->unmount (file, mount_op, cancellable, callback, callback_data);
		} else {
			if (callback) {
				error = NULL;
				g_set_error_literal (&error, G_IO_ERROR, G_IO_ERROR_NOT_SUPPORTED,
		...
		data->file = scatola_file_ref (file);
		data->callback = callback;
		data->callback_data = callback_data;
		scatola_file_operations_unmount_mount_full (NULL, file->details->mount, FALSE, TRUE, unmount_done, data);
	} else if (callback) {
...
void
scatola_file_eject (ScatolaFile                   *file,
		     GMountOperation                *mount_op,
		     GCancellable                   *cancellable,
		     ScatolaFileOperationCallback   callback,
		     gpointer                        callback_data)
{
	...
	if (file->details->can_eject) {
		if (SCATOLA_FILE_GET_CLASS (file)->eject != NULL) {
			SCATOLA_FILE_GET_CLASS (file)->eject (file, mount_op, cancellable, callback, callback_data);
		} else {
			if (callback) {
				error = NULL;
				g_set_error_literal (&error, G_IO_ERROR, G_IO_ERROR_NOT_SUPPORTED,
		...
		data->callback = callback;
		data->callback_data = callback_data;
		scatola_file_operations_unmount_mount_full (NULL, file->details->mount, TRUE, TRUE, unmount_done, data);
	} else if (callback) {
		callback (file, NULL, NULL, callback_data);
	}
}
...
void
scatola_file_start (ScatolaFile                   *file,
		     ...
		     gpointer                        callback_data)
{
	...
	if ((file->details->can_start || file->details->can_start_degraded) &&
	    SCATOLA_FILE_GET_CLASS (file)->start != NULL) {
		SCATOLA_FILE_GET_CLASS (file)->start (file, start_op, cancellable, callback, callback_data);
	} else {
		if (callback) {
			error = NULL;
			g_set_error_literal (&error, G_IO_ERROR, G_IO_ERROR_NOT_SUPPORTED,
		    ...
		    gpointer callback_data)
{
	...
	scatola_file_operation_complete (op, NULL, error);
	if (error) {
		g_error_free (error);
	}
}
...
void
scatola_file_stop (ScatolaFile                   *file,
		    GMountOperation                *mount_op,
		    GCancellable                   *cancellable,
		    ScatolaFileOperationCallback   callback,
		    gpointer                        callback_data)
{
	...
	if (SCATOLA_FILE_GET_CLASS (file)->stop != NULL) {
		if (file->details->can_stop) {
			SCATOLA_FILE_GET_CLASS (file)->stop (file, mount_op, cancellable, callback, callback_data);
		} else {
			if (callback) {
				error = NULL;
				g_set_error_literal (&error, G_IO_ERROR, G_IO_ERROR_NOT_SUPPORTED,
...
void
scatola_file_poll_for_media (ScatolaFile *file)
{
	if (file->details->can_poll_for_media) {
		if (SCATOLA_FILE_GET_CLASS (file)->stop != NULL) {
			SCATOLA_FILE_GET_CLASS (file)->poll_for_media (file);
		}
	} else if (file->details->mount != NULL) {
		GDrive *drive;
		drive = g_mount_get_drive (file->details->mount);
...
/**
 * scatola_file_is_desktop_directory:
 *
 ...
 */
gboolean
scatola_file_is_desktop_directory (ScatolaFile *file)
{
	GFile *dir;
	...
	return scatola_is_desktop_directory_file (dir, eel_ref_str_peek (file->details->name));
}

static gboolean
is_desktop_file (ScatolaFile *file)
{
	return scatola_file_is_mime_type (file, "application/x-desktop");
}

static gboolean
can_rename_desktop_file (ScatolaFile *file)
{
	GFile *location;
	gboolean res;
 ...
 */
gboolean
scatola_file_can_rename (ScatolaFile *file)
{
	gboolean can_rename;
	...
	if (SCATOLA_IS_DESKTOP_ICON_FILE (file)) {
		ScatolaDesktopLink *link;

		link = scatola_desktop_icon_file_get_link (SCATOLA_DESKTOP_ICON_FILE (file));

		if (link != NULL) {
			can_rename = scatola_desktop_link_can_rename (link);
			g_object_unref (link);
		}
	}
...
gboolean
scatola_file_can_delete (ScatolaFile *file)
{
	g_return_val_if_fail (SCATOLA_IS_FILE (file), FALSE);
...
gboolean
scatola_file_can_trash (ScatolaFile *file)
{
	g_return_val_if_fail (SCATOLA_IS_FILE (file), FALSE);
...
GFile *
scatola_file_get_location (ScatolaFile *file)
{
	GFile *dir;
...
char *
scatola_file_get_uri (ScatolaFile *file)
{
	char *uri;
	GFile *loc;
	...
	loc = scatola_file_get_location (file);
...
char *
scatola_file_get_uri_scheme (ScatolaFile *file)
{
	GFile *loc;
	char *scheme;
	...
	loc = scatola_directory_get_location (file->details->directory);
...
scatola_file_operation_new (ScatolaFile *file,
			     ScatolaFileOperationCallback callback,
			     gpointer callback_data)
{
	ScatolaFileOperation *op;

	op = g_new0 (ScatolaFileOperation, 1);
	op->file = scatola_file_ref (file);
...
static void
scatola_file_operation_remove (ScatolaFileOperation *op)
{
...
void
scatola_file_operation_free (ScatolaFileOperation *op)
{
	scatola_file_operation_remove (op);
	scatola_file_unref (op->file);
	g_object_unref (op->cancellable);
	if (op->free_data) {
		op->free_data (op->data);
	}
	// Start UNDO-REDO
	scatola_undostack_manager_add_action (scatola_undostack_manager_instance(),
										   op->undo_redo_data);
	// End UNDO-REDO
	g_free (op);
}
...
void
scatola_file_operation_complete (ScatolaFileOperation *op, GFile *result_file, GError *error)
{
	 ...
	 */
	scatola_file_operation_remove (op);
	scatola_file_changed (op->file);
	if (op->callback) {
		(* op->callback) (op->file, result_file, error, op->callback_data);
	}
	scatola_file_operation_free (op);
}

void
scatola_file_operation_cancel (ScatolaFileOperation *op)
{
			  ...
			  gpointer callback_data)
{
	...
	if (new_info != NULL) {
		 ...
		 */
		existing_file = scatola_directory_find_file_by_name (directory, new_name);
		if (existing_file != NULL) {
			scatola_file_mark_gone (existing_file);
			scatola_file_changed (existing_file);
		}

		old_uri = scatola_file_get_uri (op->file);
		...
		new_uri = scatola_file_get_uri (op->file);
		scatola_directory_moved (old_uri, new_uri);
		g_free (new_uri);
		g_free (old_uri);
		...
		if (op->file->details->got_custom_display_name) {
			scatola_file_invalidate_attributes (op->file,
							     SCATOLA_FILE_ATTRIBUTE_INFO |
							     SCATOLA_FILE_ATTRIBUTE_LINK_INFO);
		}
		 ...
		 gpointer callback_data)
{
	...
	if (new_file != NULL) {
		// Start UNDO-REDO
		scatola_undostack_manager_data_set_rename_information(op->undo_redo_data, G_FILE (source_object), new_file);
		// End UNDO-REDO
...
static gboolean
name_is (ScatolaFile *file, const char *new_name)
{
	const char *old_name;
...
void
scatola_file_rename (ScatolaFile *file,
		      const char *new_name,
		      ScatolaFileOperationCallback callback,
		      gpointer callback_data)
{
	...
	if (SCATOLA_IS_DESKTOP_ICON_FILE (file)) {
		...
		link = scatola_desktop_icon_file_get_link (SCATOLA_DESKTOP_ICON_FILE (file));
		old_name = scatola_file_get_display_name (file);

		if ((old_name != NULL && strcmp (new_name, old_name) == 0)) {
			success = TRUE;
		} else {
			success = (link != NULL && scatola_desktop_link_rename (link, new_name));
		}
	...
	if (is_renameable_desktop_file) {
		 ...
		 */
		uri = scatola_file_get_uri (file);
		old_name = scatola_link_local_get_text (uri);
		if (old_name != NULL && strcmp (new_name, old_name) == 0) {
			success = TRUE;
			name_changed = FALSE;
		} else {
			success = scatola_link_local_set_text (uri, new_name);
			name_changed = TRUE;
		}
		g_free (old_name);
		g_free (uri);
	...
	// Start UNDO-REDO
	if (!scatola_undostack_manager_is_undo_redo(scatola_undostack_manager_instance())) {
		op->undo_redo_data = scatola_undostack_manager_data_new (SCATOLA_UNDOSTACK_RENAME, 1);
	}
	// End UNDO-REDO
...
gboolean
scatola_file_rename_in_progress (ScatolaFile *file)
{
	GList *node;
	ScatolaFileOperation *op;
...
void
scatola_file_cancel (ScatolaFile *file,
		      ScatolaFileOperationCallback callback,
		      gpointer callback_data)
{
	GList *node, *next;
	...
	for (node = file->details->operations_in_progress; node != NULL; node = next) {
		...
		if (op->callback == callback && op->callback_data == callback_data) {
			scatola_file_operation_cancel (op);
		}
	}
}

gboolean
scatola_file_matches_uri (ScatolaFile *file, const char *match_uri)
{
...
int
scatola_file_compare_location (ScatolaFile *file_1,
                                ScatolaFile *file_2)
{
	GFile *loc_a, *loc_b;
	gboolean res;

	loc_a = scatola_file_get_location (file_1);
	loc_b = scatola_file_get_location (file_2);
...
gboolean
scatola_file_is_local (ScatolaFile *file)
{
	g_return_val_if_fail (SCATOLA_IS_FILE (file), FALSE);

	return scatola_directory_is_local (file->details->directory);
}

static void
update_link (ScatolaFile *link_file, ScatolaFile *target_file)
{
	g_assert (SCATOLA_IS_FILE (link_file));
	g_assert (SCATOLA_IS_FILE (target_file));
...
static GList *
get_link_files (ScatolaFile *target_file)
{
	char *uri;
	GList **link_files;
	...
	if (symbolic_links == NULL) {
		...
		uri = scatola_file_get_uri (target_file);
		link_files = g_hash_table_lookup (symbolic_links, uri);
		g_free (uri);
	}
	if (link_files) {
		return scatola_file_list_copy (*link_files);
	}
	return NULL;
}

static void
update_links_if_target (ScatolaFile *target_file)
{
	GList *link_files, *p;

	link_files = get_link_files (target_file);
	for (p = link_files; p != NULL; p = p->next) {
		update_link (SCATOLA_FILE (p->data), target_file);
	}
	scatola_file_list_free (link_files);
}

static gboolean
update_info_internal (ScatolaFile *file,
		      GFileInfo *info,
		      gboolean update_name)
{
	...
	changed |= scatola_file_set_display_name (file,
	...
	changed |=
		scatola_file_update_metadata_from_info (file, info);
	...
	if (update_name) {
		...
		if (file->details->name == NULL ||
			...
			node = scatola_directory_begin_file_name_change
			...
			if (!file->details->got_custom_display_name &&
				 ...
				 * scatola_file_set_display_name() did
				 * not unset the display name.
				 */
				scatola_file_clear_display_name (file);
			}

			scatola_directory_end_file_name_change
...
static gboolean
update_info_and_name (ScatolaFile *file,
		      GFileInfo *info)
{
...
gboolean
scatola_file_update_info (ScatolaFile *file,
			   GFileInfo *info)
{
...
static gboolean
update_name_internal (ScatolaFile *file,
		      const char *name,
...
gboolean
scatola_file_update_name (ScatolaFile *file, const char *name)
{
	gboolean ret;
...
gboolean
scatola_file_update_name_and_directory (ScatolaFile *file,
					 const char *name,
					 ScatolaDirectory *new_directory)
{
	ScatolaDirectory *old_directory;
	FileMonitors *monitors;

	g_return_val_if_fail (SCATOLA_IS_FILE (file), FALSE);
	g_return_val_if_fail (SCATOLA_IS_DIRECTORY (file->details->directory), FALSE);
	g_return_val_if_fail (!file->details->is_gone, FALSE);
	g_return_val_if_fail (!scatola_file_is_self_owned (file), FALSE);
	g_return_val_if_fail (SCATOLA_IS_DIRECTORY (new_directory), FALSE);
	...
	monitors = scatola_directory_remove_file_monitors (old_directory, file);
	scatola_directory_remove_file (old_directory, file);

	file->details->directory = scatola_directory_ref (new_directory);
	scatola_directory_unref (old_directory);

	if (name) {
		update_name_internal (file, name, FALSE);
	}

	scatola_directory_add_file (new_directory, file);
	scatola_directory_add_file_monitors (new_directory, file, monitors);
...
void
scatola_file_set_directory (ScatolaFile *file,
			     ScatolaDirectory *new_directory)
{
	scatola_file_update_name_and_directory (file, NULL, new_directory);
}

static Knowledge
get_item_count (ScatolaFile *file,
		guint *count)
{
	gboolean known, unreadable;

	known = scatola_file_get_directory_item_count
	  ...
	  goffset *size)
{
	 ...
	 * such thing as a size as far as gde2-vfs is concerned,
	 * so "unknowable".
	 */
	  ...
	  ScatolaDateType type)
{
	...
	*time_out = time;
	 ...
	 * gde2-vfs is concerned, so "unknowable".
	 */
	if (time == 0) {
		return UNKNOWABLE;
	}
...
static int
compare_directories_by_count (ScatolaFile *file_1, ScatolaFile *file_2)
{
	/* Sort order:
...
static int
compare_files_by_size (ScatolaFile *file_1, ScatolaFile *file_2)
{
	/* Sort order:
...
static int
compare_by_size (ScatolaFile *file_1, ScatolaFile *file_2)
{
	/* Sort order:
	 *   Directories with n items
	 *   Directories with 0 items
	...
	is_directory_1 = scatola_file_is_directory (file_1);
	is_directory_2 = scatola_file_is_directory (file_2);
...
static int
compare_by_display_name (ScatolaFile *file_1, ScatolaFile *file_2)
{
	...
	name_1 = scatola_file_peek_display_name (file_1);
	name_2 = scatola_file_peek_display_name (file_2);
	...
	if (sort_last_1 && !sort_last_2) {
		...
		key_1 = scatola_file_peek_display_name_collation_key (file_1);
		key_2 = scatola_file_peek_display_name_collation_key (file_2);
...
static int
compare_by_directory_name (ScatolaFile *file_1, ScatolaFile *file_2)
{
	...
	directory_1 = scatola_file_get_parent_uri_for_display (file_1);
	directory_2 = scatola_file_get_parent_uri_for_display (file_2);
...
static gboolean
file_has_note (ScatolaFile *file)
{
	char *note;
	gboolean res;

	note = scatola_file_get_metadata (file, SCATOLA_METADATA_KEY_ANNOTATION, NULL);
...
static GList *
prepend_automatic_keywords (ScatolaFile *file,
			    GList *names)
{
	/* Prepend in reverse order. */
	ScatolaFile *parent;

	parent = scatola_file_get_parent (file);

#ifdef TRASH_IS_FAST_ENOUGH
	if (scatola_file_is_in_trash (file)) {
		names = g_list_prepend
			(names, g_strdup (SCATOLA_FILE_EMBLEM_NAME_TRASH));
	}
#endif
	if (file_has_note (file)) {
		names = g_list_prepend
			(names, g_strdup (SCATOLA_FILE_EMBLEM_NAME_NOTE));
	}

	/* Trash files are assumed to be read-only,
	 * so we want to ignore them here. */
	if (!scatola_file_can_write (file) &&
	    !scatola_file_is_in_trash (file) &&
	    (parent == NULL || scatola_file_can_write (parent))) {
		names = g_list_prepend
			(names, g_strdup (SCATOLA_FILE_EMBLEM_NAME_CANT_WRITE));
	}
	if (!scatola_file_can_read (file)) {
		names = g_list_prepend
			(names, g_strdup (SCATOLA_FILE_EMBLEM_NAME_CANT_READ));
	}
	if (scatola_file_is_symbolic_link (file)) {
		names = g_list_prepend
			(names, g_strdup (SCATOLA_FILE_EMBLEM_NAME_SYMBOLIC_LINK));
	}

	if (parent) {
		scatola_file_unref (parent);
	}
...
static void
fill_emblem_cache_if_needed (ScatolaFile *file)
{
	GList *node, *keywords;
	char *scanner;
	size_t length;
	...
	file->details->compare_by_emblem_cache = g_malloc (sizeof(ScatolaFileSortByEmblemCache) + length);
...
static int
compare_by_emblems (ScatolaFile *file_1, ScatolaFile *file_2)
{
...
static int
compare_by_type (ScatolaFile *file_1, ScatolaFile *file_2)
{
	...
	type_string_1 = scatola_file_get_type_as_string (file_1);
	type_string_2 = scatola_file_get_type_as_string (file_2);
...
static int
compare_by_time (ScatolaFile *file_1, ScatolaFile *file_2, ScatolaDateType type)
{
	/* Sort order:
...
static int
compare_by_full_path (ScatolaFile *file_1, ScatolaFile *file_2)
{
	int compare;
...
static int
scatola_file_compare_for_sort_internal (ScatolaFile *file_1,
					 ScatolaFile *file_2,
 ...
 **/
int
scatola_file_compare_for_sort (ScatolaFile *file_1,
				ScatolaFile *file_2,
				...
				gboolean reversed)
{
	...
	result = scatola_file_compare_for_sort_internal (file_1, file_2, directories_first, reversed);

	if (result == 0) {
		switch (sort_type) {
		case SCATOLA_FILE_SORT_BY_DISPLAY_NAME:
			result = compare_by_display_name (file_1, file_2);
			if (result == 0) {
				result = compare_by_directory_name (file_1, file_2);
			}
			break;
		case SCATOLA_FILE_SORT_BY_DIRECTORY:
			result = compare_by_full_path (file_1, file_2);
			break;
		case SCATOLA_FILE_SORT_BY_SIZE:
			/* Compare directory sizes ourselves, then if necessary
			 * use Gde2VFS to compare file sizes.
			 */
			...
			break;
		case SCATOLA_FILE_SORT_BY_TYPE:
			/* Gde2VFS doesn't know about our special text for certain
			...
			break;
		case SCATOLA_FILE_SORT_BY_MTIME:
			result = compare_by_time (file_1, file_2, SCATOLA_DATE_TYPE_MODIFIED);
			if (result == 0) {
				result = compare_by_full_path (file_1, file_2);
			}
			break;
		case SCATOLA_FILE_SORT_BY_ATIME:
			result = compare_by_time (file_1, file_2, SCATOLA_DATE_TYPE_ACCESSED);
			if (result == 0) {
				result = compare_by_full_path (file_1, file_2);
			}
			break;
		case SCATOLA_FILE_SORT_BY_TRASHED_TIME:
			result = compare_by_time (file_1, file_2, SCATOLA_DATE_TYPE_TRASHED);
			if (result == 0) {
				result = compare_by_full_path (file_1, file_2);
			}
			break;
		case SCATOLA_FILE_SORT_BY_EMBLEMS:
			/* Gde2VFS doesn't know squat about our emblems, so
...
int
scatola_file_compare_for_sort_by_attribute_q   (ScatolaFile                   *file_1,
						 ScatolaFile                   *file_2,
						 GQuark                          attribute,
						 gboolean                        directories_first,
						 gboolean                        reversed)
{
	...
	/* Convert certain attributes into ScatolaFileSortTypes and use
	 * scatola_file_compare_for_sort()
	 */
	if (attribute == 0 || attribute == attribute_name_q) {
		return scatola_file_compare_for_sort (file_1, file_2,
	...
	if (result == 0) {
		...
		value_1 = scatola_file_get_string_attribute_q (file_1,
								attribute);
		value_2 = scatola_file_get_string_attribute_q (file_2,
								attribute);
...
int
scatola_file_compare_for_sort_by_attribute     (ScatolaFile                   *file_1,
						 ScatolaFile                   *file_2,
						 const char                     *attribute,
						 gboolean                        directories_first,
						 gboolean                        reversed)
{
	return scatola_file_compare_for_sort_by_attribute_q (file_1, file_2,
 ...
 **/
int
scatola_file_compare_display_name (ScatolaFile *file,
				    const char *pattern)
{
	const char *name;
	int result;
	...
	name = scatola_file_peek_display_name (file);
...
gboolean
scatola_file_is_hidden_file (ScatolaFile *file)
{
	return file->details->is_hidden;
}

static gboolean
is_file_hidden (ScatolaFile *file)
{
			   ...
			   gboolean show_foreign)
{
	...
	if (scatola_file_is_in_trash (file)) {
		...
		return (show_hidden || (!scatola_file_is_hidden_file (file) && !is_file_hidden (file))) &&
			(show_foreign || !(scatola_file_is_in_desktop (file) && scatola_file_is_foreign_link (file)));
	}
}

gboolean
scatola_file_is_home (ScatolaFile *file)
{
	GFile *dir;
	...
	return scatola_is_home_directory_file (dir,
						eel_ref_str_peek (file->details->name));
}

gboolean
scatola_file_is_in_desktop (ScatolaFile *file)
{
	if (file->details->directory->details->location) {
		return scatola_is_desktop_directory (file->details->directory->details->location);
	}
	return FALSE;
					     ...
					     gpointer callback_data)
{
	...
	file = SCATOLA_FILE (data);
	options = GPOINTER_TO_INT (callback_data);

	return scatola_file_should_show (file,
					  options & SHOW_HIDDEN,
					  TRUE);
}

GList *
scatola_file_list_filter_hidden (GList    *files,
					     gboolean  show_hidden)
{
	...
	filtered_files = scatola_file_list_copy (files);
	filtered_files = eel_g_list_partition (filtered_files,
					       ...
					       &removed_files);
	scatola_file_list_free (removed_files);

	return filtered_files;
}

char *
scatola_file_get_metadata (ScatolaFile *file,
			    const char *key,
			    const char *default_metadata)
{
	...
	g_return_val_if_fail (SCATOLA_IS_FILE (file), g_strdup (default_metadata));

	id = scatola_metadata_get_id (key);
...
GList *
scatola_file_get_metadata_list (ScatolaFile *file,
				 const char *key)
{
	GList *res;
	guint id;
...
void
scatola_file_set_metadata (ScatolaFile *file,
			    const char *key,
			    const char *default_metadata,
			    const char *metadata)
{
	...
	EEL_CALL_METHOD
		(SCATOLA_FILE_CLASS, file,
		 set_metadata, (file, key, val));
}

void
scatola_file_set_metadata_list (ScatolaFile *file,
				 const char *key,
				 GList *list)
{
	char **val;
...
gboolean
scatola_file_get_boolean_metadata (ScatolaFile *file,
				    const char   *key,
				    gboolean      default_metadata)
{
	...
	g_return_val_if_fail (SCATOLA_IS_FILE (file), default_metadata);

	result_as_string = scatola_file_get_metadata
...
int
scatola_file_get_integer_metadata (ScatolaFile *file,
				    const char   *key,
...
time_t
scatola_file_get_time_metadata (ScatolaFile *file,
				 const char   *key)
{
	time_t time;
	char *time_string;

	time_string = scatola_file_get_metadata (file, key, NULL);
...
void
scatola_file_set_time_metadata (ScatolaFile *file,
				 const char   *key,
				 time_t        time)
{
	char time_str[21];
	...
	scatola_file_set_metadata (file, key, NULL, metadata);
}


void
scatola_file_set_boolean_metadata (ScatolaFile *file,
				    const char   *key,
				    gboolean      default_metadata,
				    gboolean      metadata)
{
	...
	scatola_file_set_metadata (file, key,
				    default_metadata ? "true" : "false",
				    metadata ? "true" : "false");
}

void
scatola_file_set_integer_metadata (ScatolaFile *file,
				    const char   *key,
				    int           default_metadata,
				    int           metadata)
{
...
static const char *
scatola_file_peek_display_name_collation_key (ScatolaFile *file)
{
	const char *res;
...
static const char *
scatola_file_peek_display_name (ScatolaFile *file)
{
	const char *name;
	char *escaped_name;
	...
	*/
	if (file == NULL || scatola_file_is_gone (file))
		return "";
...
char *
scatola_file_get_display_name (ScatolaFile *file)
{
	return g_strdup (scatola_file_peek_display_name (file));
}

char *
scatola_file_get_edit_name (ScatolaFile *file)
{
	const char *res;
...
char *
scatola_file_get_name (ScatolaFile *file)
{
	return g_strdup (eel_ref_str_peek (file->details->name));
}

/**
 * scatola_file_get_description:
 * @file: a #ScatolaFile.
 *
 ...
 */
char *
scatola_file_get_description (ScatolaFile *file)
{
	return g_strdup (file->details->description);
}

void
scatola_file_monitor_add (ScatolaFile *file,
...
void
scatola_file_monitor_remove (ScatolaFile *file,
			      gconstpointer client)
{
	g_return_if_fail (SCATOLA_IS_FILE (file));
	g_return_if_fail (client != NULL);
...
gboolean
scatola_file_is_launcher (ScatolaFile *file)
{
	return file->details->is_launcher;
}

gboolean
scatola_file_is_foreign_link (ScatolaFile *file)
{
	return file->details->is_foreign_link;
}

gboolean
scatola_file_is_trusted_link (ScatolaFile *file)
{
	return file->details->is_trusted_link;
}

gboolean
scatola_file_has_activation_uri (ScatolaFile *file)
{
 ...
 * the actual uri if the file is an desktop file or a scatola
 * xml link file.
 */
char *
scatola_file_get_activation_uri (ScatolaFile *file)
{
	g_return_val_if_fail (SCATOLA_IS_FILE (file), NULL);
	...
	return scatola_file_get_uri (file);
}

GFile *
scatola_file_get_activation_location (ScatolaFile *file)
{
	g_return_val_if_fail (SCATOLA_IS_FILE (file), NULL);
	...
	return scatola_file_get_location (file);
}


char *
scatola_file_get_drop_target_uri (ScatolaFile *file)
{
	char *uri, *target_uri;
	GFile *location;
	ScatolaDesktopLink *link;
	...
	if (SCATOLA_IS_DESKTOP_ICON_FILE (file)) {
		...
		if (link != NULL) {
			location = scatola_desktop_link_get_activation_location (link);
			g_object_unref (link);
			if (location != NULL) {
	...
	/* Check for Scatola link */
	if (scatola_file_is_scatola_link (file)) {
		location = scatola_file_get_location (file);
		/* FIXME bugzilla.gnome.org 43020: This does sync. I/O and works only locally. */
		if (g_file_is_native (location)) {
			target_uri = scatola_link_local_get_link_uri (uri);
			if (target_uri != NULL) {
				g_free (uri);
				uri = target_uri;
			}
...
static char *
get_custom_icon_metadata_uri (ScatolaFile *file)
{
	char *custom_icon_uri;
	char *uri;
	char *dir_uri;

	uri = scatola_file_get_metadata (file, SCATOLA_METADATA_KEY_CUSTOM_ICON, NULL);
	if (uri != NULL &&
	    scatola_file_is_directory (file) &&
	    is_uri_relative (uri)) {
		dir_uri = scatola_file_get_uri (file);
...
static GIcon *
get_custom_icon (ScatolaFile *file)
{
	char *custom_icon_uri;
	GFile *icon_file;
	GIcon *icon;
...
scatola_file_get_filesystem_use_preview (ScatolaFile *file)
{
...
gboolean
scatola_file_should_show_thumbnail (ScatolaFile *file)
{
	const char *mime_type;
	GFilesystemPreviewType use_preview;

	use_preview = scatola_file_get_filesystem_use_preview (file);
	 ...
	 */
	if (scatola_thumbnail_is_mimetype_limited_by_size (mime_type) &&
	    file->details->thumbnail_path == NULL &&
	    scatola_file_get_size (file) > cached_thumbnail_limit) {
		return FALSE;
	}

	if (show_image_thumbs == SCATOLA_SPEED_TRADEOFF_ALWAYS) {
		if (use_preview == G_FILESYSTEM_PREVIEW_TYPE_NEVER) {
			return FALSE;
		} else {
			return TRUE;
		}
	} else if (show_image_thumbs == SCATOLA_SPEED_TRADEOFF_NEVER) {
		return FALSE;
	} else {
		if (use_preview == G_FILESYSTEM_PREVIEW_TYPE_NEVER) {
			...
			/* only local files */
			return scatola_file_is_local (file);
		}
	}
...
GIcon *
scatola_file_get_gicon (ScatolaFile *file,
			 ScatolaFileIconFlags flags)
{
	...
	if (file->details->icon) {
		...
		if (flags & SCATOLA_FILE_ICON_FLAGS_USE_MOUNT_ICON ||
		    flags & SCATOLA_FILE_ICON_FLAGS_USE_MOUNT_ICON_AS_EMBLEM) {
			mount = scatola_file_get_mount (file);
		...
		if (((flags & SCATOLA_FILE_ICON_FLAGS_EMBEDDING_TEXT) ||
		     (flags & SCATOLA_FILE_ICON_FLAGS_FOR_DRAG_ACCEPT) ||
		     (flags & SCATOLA_FILE_ICON_FLAGS_FOR_OPEN_FOLDER) ||
		     (flags & SCATOLA_FILE_ICON_FLAGS_USE_MOUNT_ICON) ||
		     (flags & SCATOLA_FILE_ICON_FLAGS_USE_MOUNT_ICON_AS_EMBLEM) ||
		     ((flags & SCATOLA_FILE_ICON_FLAGS_IGNORE_VISITING) == 0 &&
		      scatola_file_has_open_window (file))) &&
			...
			for (i = 0; names[i] != NULL; i++) {
				...
				if (strcmp (name, "text-x-generic") == 0 &&
				    (flags & SCATOLA_FILE_ICON_FLAGS_EMBEDDING_TEXT)) {
					is_preview = TRUE;
				}
			}
			...
			if (is_folder && (flags & SCATOLA_FILE_ICON_FLAGS_FOR_OPEN_FOLDER)) {
		...
		if ((flags & SCATOLA_FILE_ICON_FLAGS_USE_MOUNT_ICON) &&
			...
			g_object_unref (icon);
			icon = mount_icon;
		} else if ((flags & SCATOLA_FILE_ICON_FLAGS_USE_MOUNT_ICON_AS_EMBLEM) &&
...
static GIcon *
get_default_file_icon (ScatolaFileIconFlags flags)
{
...
ScatolaIconInfo *
scatola_file_get_icon (ScatolaFile *file,
			int size,
			ScatolaFileIconFlags flags)
{
	...
	if (gicon) {
		...
		icon = scatola_icon_info_lookup (gicon, size);
		g_object_unref (gicon);

		pixbuf = scatola_icon_info_get_pixbuf (icon);
		if (pixbuf != NULL) {
			if (!file->details->is_launcher && !gdk_pixbuf_get_has_alpha (pixbuf)) {
				scatola_ui_frame_image (&pixbuf);
			}
			g_object_unref (icon);

			icon = scatola_icon_info_new_for_pixbuf (pixbuf);
	...
	if (flags & SCATOLA_FILE_ICON_FLAGS_FORCE_THUMBNAIL_SIZE) {
		modified_size = size;
	} else {
		modified_size = size * cached_thumbnail_size / SCATOLA_ICON_SIZE_STANDARD;
	}
	if (flags & SCATOLA_FILE_ICON_FLAGS_USE_THUMBNAILS &&
	    scatola_file_should_show_thumbnail (file)) {
		if (file->details->thumbnail) {
			int w, h, s;
			double scale;
			...
			if (s <= cached_thumbnail_size) {
				scale = (double)size / SCATOLA_ICON_SIZE_STANDARD;
			}
			else {
				scale = (double)modified_size / s;
			}
			/* Make sure that icons don't get smaller than SCATOLA_ICON_SIZE_SMALLEST */
			if (s*scale <= SCATOLA_ICON_SIZE_SMALLEST) {
				scale = (double) SCATOLA_ICON_SIZE_SMALLEST / s;
			}
			...
			gboolean is_image = file->details->mime_type &&
				...
				if (!is_image ||
					...
					scatola_ui_frame_image (&scaled_pixbuf);
				}
			...
			if (modified_size > 128*1.25 &&
			    ...
			    scatola_can_thumbnail_internally (file)) {
				/* Invalidate if we resize upward */
				file->details->thumbnail_wants_original = TRUE;
				scatola_file_invalidate_attributes (file, SCATOLA_FILE_ATTRIBUTE_THUMBNAIL);
			}

			icon = scatola_icon_info_new_for_pixbuf (scaled_pixbuf);
			...
			if (scatola_can_thumbnail (file)) {
				scatola_create_thumbnail (file);
			}
		}
	}

	if (file->details->is_thumbnailing &&
	    flags & SCATOLA_FILE_ICON_FLAGS_USE_THUMBNAILS)
		gicon = g_themed_icon_new (ICON_NAME_THUMBNAIL_LOADING);
	else
		gicon = scatola_file_get_gicon (file, flags);

	if (gicon) {
		icon = scatola_icon_info_lookup (gicon, size);
		if (scatola_icon_info_is_fallback (icon)) {
			g_object_unref (icon);
			icon = scatola_icon_info_lookup (get_default_file_icon (flags), size);
		}
		g_object_unref (gicon);
		return icon;
	} else {
		return scatola_icon_info_lookup (get_default_file_icon (flags), size);
	}
}

GdkPixbuf *
scatola_file_get_icon_pixbuf (ScatolaFile *file,
			       int size,
			       gboolean force_size,
			       ScatolaFileIconFlags flags)
{
	...
	info = scatola_file_get_icon (file, size, flags);
	if (force_size) {
		pixbuf =  scatola_icon_info_get_pixbuf_at_size (info, size);
	} else {
		pixbuf = scatola_icon_info_get_pixbuf (info);
	}
	g_object_unref (info);
...
char *
scatola_file_get_custom_icon (ScatolaFile *file)
{
	char *custom_icon;
...
gboolean
scatola_file_get_date (ScatolaFile *file,
			...
			time_t *date)
{
	...
	g_return_val_if_fail (date_type == SCATOLA_DATE_TYPE_CHANGED
			      || date_type == SCATOLA_DATE_TYPE_ACCESSED
			      || date_type == SCATOLA_DATE_TYPE_MODIFIED
			      || date_type == SCATOLA_DATE_TYPE_TRASHED
			      || date_type == SCATOLA_DATE_TYPE_PERMISSIONS_CHANGED, FALSE);
...
static char *
scatola_file_get_where_string (ScatolaFile *file)
{
	if (file == NULL) {
		return NULL;
	}
...
static const char *TODAY_TIME_FORMATS [] = {
	 ...
	 * in the Scatola version of strftime that can be
...
static char *
scatola_file_fit_date_as_string (ScatolaFile *file,
				  ScatolaDateType date_type,
				  int width,
				  ...
				  void *measure_context)
{
	...
	if (!scatola_file_get_date (file, date_type, &file_time_raw)) {
		return NULL;
	}

	file_time = localtime (&file_time_raw);

	if (date_format_pref == SCATOLA_DATE_FORMAT_LOCALE) {
		return eel_strdup_strftime ("%c", file_time);
	} else if (date_format_pref == SCATOLA_DATE_FORMAT_ISO) {
...
/**
 * scatola_file_fit_modified_date_as_string:
 *
 ...
 * @file: ScatolaFile representing the file in question.
 ...
 **/
char *
scatola_file_fit_modified_date_as_string (ScatolaFile *file,
					   int width,
					   ScatolaWidthMeasureCallback measure_callback,
					   ScatolaTruncateCallback truncate_callback,
					   void *measure_context)
{
	return scatola_file_fit_date_as_string (file, SCATOLA_DATE_TYPE_MODIFIED,
		width, measure_callback, truncate_callback, measure_context);
}

static char *
scatola_file_get_trash_original_file_parent_as_string (ScatolaFile *file)
{
	ScatolaFile *orig_file, *parent;
	GFile *location;
	char *filename;

	if (file->details->trash_orig_path != NULL) {
		orig_file = scatola_file_get_trash_original_file (file);
		parent = scatola_file_get_parent (orig_file);
		location = scatola_file_get_location (parent);

		filename = g_file_get_parse_name (location);

		g_object_unref (location);
		scatola_file_unref (parent);
		scatola_file_unref (orig_file);
...
/**
 * scatola_file_get_date_as_string:
 *
 * Get a user-displayable string representing a file modification date.
 ...
 **/
static char *
scatola_file_get_date_as_string (ScatolaFile *file, ScatolaDateType date_type)
{
	return scatola_file_fit_date_as_string (file, date_type,
		0, NULL, NULL, NULL);
}

static ScatolaSpeedTradeoffValue show_directory_item_count;
static ScatolaSpeedTradeoffValue show_text_in_icons;

static void
show_text_in_icons_changed_callback (gpointer callback_data)
{
	show_text_in_icons = g_settings_get_enum (scatola_preferences, SCATOLA_PREFERENCES_SHOW_TEXT_IN_ICONS);
}

static void
show_directory_item_count_changed_callback (gpointer callback_data)
{
	show_directory_item_count = g_settings_get_enum (scatola_preferences, SCATOLA_PREFERENCES_SHOW_DIRECTORY_ITEM_COUNTS);
}

static gboolean
get_speed_tradeoff_preference_for_file (ScatolaFile *file, ScatolaSpeedTradeoffValue value)
{
	GFilesystemPreviewType use_preview;
	...
	if (value == SCATOLA_SPEED_TRADEOFF_ALWAYS) {
		if (use_preview == G_FILESYSTEM_PREVIEW_TYPE_NEVER) {
			return FALSE;
		} else {
			return TRUE;
	...
	if (value == SCATOLA_SPEED_TRADEOFF_NEVER) {
		return FALSE;
	}

	g_assert (value == SCATOLA_SPEED_TRADEOFF_LOCAL_ONLY);
...
gboolean
scatola_file_should_show_directory_item_count (ScatolaFile *file)
{
	...
	if (!show_directory_item_count_callback_added) {
		g_signal_connect_swapped (scatola_preferences,
								  "changed::" SCATOLA_PREFERENCES_SHOW_DIRECTORY_ITEM_COUNTS,
...
gboolean
scatola_file_should_show_type (ScatolaFile *file)
{
	char *uri;
	gboolean ret;
...
gboolean
scatola_file_should_get_top_left_text (ScatolaFile *file)
{
	...
	if (!show_text_in_icons_callback_added) {
		g_signal_connect_swapped (scatola_preferences,
								  "changed::" SCATOLA_PREFERENCES_SHOW_TEXT_IN_ICONS,
	...
	if (show_text_in_icons == SCATOLA_SPEED_TRADEOFF_ALWAYS) {
		return TRUE;
	}

	if (show_text_in_icons == SCATOLA_SPEED_TRADEOFF_NEVER) {
		return FALSE;
	}
...
/**
 * scatola_file_get_directory_item_count
 *
 * Get the number of items in a directory.
 * @file: ScatolaFile representing a directory.
 ...
 **/
gboolean
scatola_file_get_directory_item_count (ScatolaFile *file,
					guint *count,
					gboolean *count_unreadable)
{
	...
	if (!scatola_file_should_show_directory_item_count (file)) {
		return FALSE;
	}
...
/**
 * scatola_file_get_deep_counts
 *
 * Get the statistics about items inside a directory.
 * @file: ScatolaFile representing a directory or file.
 ...
 * scatola_file_should_show_directory_item_count returns FALSE
 * for this file.
 *
...
scatola_file_get_deep_counts (ScatolaFile *file,
			       ...
			       gboolean force)
{
	...
	g_return_val_if_fail (SCATOLA_IS_FILE (file), SCATOLA_REQUEST_DONE);

	if (!force && !scatola_file_should_show_directory_item_count (file)) {
		/* Set field so an existing value isn't treated as up-to-date
		 * when preference changes later.
		 */
		file->details->deep_counts_status = SCATOLA_REQUEST_NOT_STARTED;
...
void
scatola_file_recompute_deep_counts (ScatolaFile *file)
{
	if (file->details->deep_counts_status != SCATOLA_REQUEST_IN_PROGRESS) {
		file->details->deep_counts_status = SCATOLA_REQUEST_NOT_STARTED;
		if (file->details->directory != NULL) {
			scatola_directory_add_file_to_work_queue (file->details->directory, file);
			scatola_directory_async_state_changed (file->details->directory);
		}
	}
}

gboolean
scatola_file_can_get_size (ScatolaFile *file)
{
 ...
 **/
goffset
scatola_file_get_size (ScatolaFile *file)
{
...
time_t
scatola_file_get_mtime (ScatolaFile *file)
{
				  ...
				  gpointer callback_data)
{
	...
	new_info = g_file_query_info_finish (G_FILE (source_object), res, &error);
	if (new_info != NULL) {
		if (scatola_file_update_info (op->file, new_info)) {
			scatola_file_changed (op->file);
		}
		g_object_unref (new_info);
	}
...
void
scatola_file_set_attributes (ScatolaFile *file,
...
/**
 * scatola_file_can_get_permissions:
 *
 ...
 */
gboolean
scatola_file_can_get_permissions (ScatolaFile *file)
{
	return file->details->has_permissions;
}

/**
 * scatola_file_can_set_permissions:
 *
 ...
 */
gboolean
scatola_file_can_set_permissions (ScatolaFile *file)
{
	uid_t user_id;

	if (file->details->uid != -1 &&
	    scatola_file_is_local (file)) {
		/* Check the user. */
		user_id = geteuid();
...
guint
scatola_file_get_permissions (ScatolaFile *file)
{
	g_return_val_if_fail (scatola_file_can_get_permissions (file), 0);

	return file->details->permissions;
}

/**
 * scatola_file_set_permissions:
 *
 * Change a file's permissions. This should only be called if
 * scatola_file_can_set_permissions returned TRUE.
 *
 * @file: ScatolaFile representing the file in question.
 ...
 **/
void
scatola_file_set_permissions (ScatolaFile *file,
			       ...
			       gpointer callback_data)
{
	...
	if (!scatola_file_can_set_permissions (file)) {
	...
	// Start UNDO-REDO
	if (!scatola_undostack_manager_is_undo_redo(scatola_undostack_manager_instance())) {
		ScatolaUndoStackActionData* undo_redo_data = scatola_undostack_manager_data_new (SCATOLA_UNDOSTACK_SETPERMISSIONS, 1);
		scatola_undostack_manager_data_set_file_permissions(undo_redo_data, scatola_file_get_uri(file), file->details->permissions, new_permissions);
		scatola_undostack_manager_add_action (scatola_undostack_manager_instance(),
										   undo_redo_data);
	}
	// End UNDO-REDO
	...
	scatola_file_set_attributes (file, info, callback, callback_data);
	g_object_unref (info);
}

/**
 * scatola_file_can_get_selinux_context:
 *
 ...
 */
gboolean
scatola_file_can_get_selinux_context (ScatolaFile *file)
{
...
/**
 * scatola_file_get_selinux_context:
 *
 ...
 **/
char *
scatola_file_get_selinux_context (ScatolaFile *file)
{
	char *translated;
	char *raw;
	...
	if (!scatola_file_can_get_selinux_context (file)) {
		return NULL;
	}
 ...
 */
gboolean
scatola_file_can_get_owner (ScatolaFile *file)
{
 ...
 */
char *
scatola_file_get_owner_name (ScatolaFile *file)
{
	return scatola_file_get_owner_as_string (file, FALSE);
}

/**
 * scatola_file_can_set_owner:
 *
 * Check whether the current user is allowed to change
 * the owner of a file.
 *
...
/**
 * scatola_file_set_owner:
 *
 * Set the owner of a file. This will only have any effect if
 * scatola_file_can_set_owner returns TRUE.
 *
 * @file: The file in question.
 ...
 */
void
scatola_file_set_owner (ScatolaFile *file,
			 ...
			 gpointer callback_data)
{
	...
	if (!scatola_file_can_set_owner (file)) {
    ...
    // Start UNDO-REDO
	if (!scatola_undostack_manager_is_undo_redo(scatola_undostack_manager_instance())) {
		char* current_owner = scatola_file_get_owner_as_string (file, FALSE);
		ScatolaUndoStackActionData* undo_redo_data = scatola_undostack_manager_data_new (SCATOLA_UNDOSTACK_CHANGEOWNER, 1);
		scatola_undostack_manager_data_set_owner_change_information(undo_redo_data, scatola_file_get_uri(file), current_owner, user_name_or_id);
		scatola_undostack_manager_add_action (scatola_undostack_manager_instance(),
										   undo_redo_data);
 ...
 */
gboolean
scatola_file_can_get_group (ScatolaFile *file)
{
 ...
 **/
char *
scatola_file_get_group_name (ScatolaFile *file)
{
	return g_strdup (eel_ref_str_peek (file->details->group));
}

/**
 * scatola_file_can_set_group:
 *
 * Check whether the current user is allowed to change
 * the group of a file.
 *
 ...
 */
gboolean
scatola_file_can_set_group (ScatolaFile *file)
{
	uid_t user_id;
	 ...
	 * systems.
	 */
	if (!scatola_file_can_get_group (file)) {
		return FALSE;
	}
 ...
 */
static GList *
scatola_get_group_names_for_user (void)
{
	GList *list;
	struct group *group;
	int count, i;
...
/**
 * scatola_get_group_names:
 *
 * Get a list of all group names.
 */
GList *
scatola_get_all_group_names (void)
{
	GList *list;
	struct group *group;
...
/**
 * scatola_file_get_settable_group_names:
 *
 * Get a list of all group names that the current user
 * can set the group of a specific file to.
 *
 * @file: The ScatolaFile in question.
 */
GList *
scatola_file_get_settable_group_names (ScatolaFile *file)
{
	uid_t user_id;
	GList *result;

	if (!scatola_file_can_set_group (file)) {
		return NULL;
	}
	...
	if (user_id == 0) {
		...
		result = scatola_get_all_group_names ();
	} else if (user_id == (uid_t) file->details->uid) {
		/* Owner is allowed to set group to any that owner is member of. */
		result = scatola_get_group_names_for_user ();
	} else {
		g_warning ("unhandled case in scatola_get_settable_group_names");
		result = NULL;
	}
...
/**
 * scatola_file_set_group:
 *
 * Set the group of a file. This will only have any effect if
 * scatola_file_can_set_group returns TRUE.
 *
 * @file: The file in question.
 ...
 */
void
scatola_file_set_group (ScatolaFile *file,
			 ...
			 gpointer callback_data)
{
	...
	// Start UNDO-REDO
	if (!scatola_undostack_manager_is_undo_redo(scatola_undostack_manager_instance())) {
		char* current_group = scatola_file_get_group_name (file);
		ScatolaUndoStackActionData* undo_redo_data = scatola_undostack_manager_data_new (SCATOLA_UNDOSTACK_CHANGEGROUP, 1);
		scatola_undostack_manager_data_set_group_change_information(undo_redo_data, scatola_file_get_uri(file), current_group, group_name_or_id);
		scatola_undostack_manager_add_action (scatola_undostack_manager_instance(),
											undo_redo_data);
...
/**
 * scatola_file_get_octal_permissions_as_string:
 *
 ...
 **/
static char *
scatola_file_get_octal_permissions_as_string (ScatolaFile *file)
{
	guint32 permissions;

	g_assert (SCATOLA_IS_FILE (file));

	if (!scatola_file_can_get_permissions (file)) {
		return NULL;
	}
...
/**
 * scatola_file_get_permissions_as_string:
 *
 ...
 **/
static char *
scatola_file_get_permissions_as_string (ScatolaFile *file)
{
	guint32 permissions;
	...
	permissions = file->details->permissions;
	is_directory = scatola_file_is_directory (file);
	is_link = scatola_file_is_symbolic_link (file);
...
/**
 * scatola_file_get_owner_as_string:
 *
 ...
 **/
static char *
scatola_file_get_owner_as_string (ScatolaFile *file, gboolean include_real_name)
{
	char *user_name;
...
/**
 * scatola_file_get_size_as_string:
 *
 ...
 **/
static char *
scatola_file_get_size_as_string (ScatolaFile *file)
{
	guint item_count;
	gboolean count_unreadable;
	...
	if (scatola_file_is_directory (file)) {
		if (!scatola_file_get_directory_item_count (file, &item_count, &count_unreadable)) {
			return NULL;
		}
	...
	if (g_settings_get_boolean (scatola_preferences, SCATOLA_PREFERENCES_USE_IEC_UNITS))
...
/**
 * scatola_file_get_size_as_string_with_real_size:
 *
 * Get a user-displayable string representing a file size. The caller
 * is responsible for g_free-ing this string. The string is an item
 * count for directories.
 ...
 **/
static char *
scatola_file_get_size_as_string_with_real_size (ScatolaFile *file)
{
	guint item_count;
	gboolean count_unreadable;
	char * formatted;
...
static char *
scatola_file_get_deep_count_as_string_internal (ScatolaFile *file,
						 ...
						 gboolean report_file_count)
{
	...
	g_assert (SCATOLA_IS_FILE (file));
	g_assert (scatola_file_is_directory (file));

	status = scatola_file_get_deep_counts
		(file, &directory_count, &file_count, &unreadable_count, &total_size, FALSE);

	/* Check whether any info is available. */
	if (status == SCATOLA_REQUEST_NOT_STARTED) {
		return NULL;
	}
	 ...
	 * Callers can distinguish them using scatola_file_get_deep_counts
	 * directly if desired.
	 */
	if (report_size)
	{
...
/**
 * scatola_file_get_deep_size_as_string:
 *
 ...
 **/
static char *
scatola_file_get_deep_size_as_string (ScatolaFile *file)
{
	return scatola_file_get_deep_count_as_string_internal (file, TRUE, FALSE, FALSE);
}

/**
 * scatola_file_get_deep_total_count_as_string:
 *
 ...
 **/
static char *
scatola_file_get_deep_total_count_as_string (ScatolaFile *file)
{
	return scatola_file_get_deep_count_as_string_internal (file, FALSE, TRUE, TRUE);
}

/**
 * scatola_file_get_deep_file_count_as_string:
 *
 * Get a user-displayable string representing the count of all contained
 ...
 **/
static char *
scatola_file_get_deep_file_count_as_string (ScatolaFile *file)
{
	return scatola_file_get_deep_count_as_string_internal (file, FALSE, FALSE, TRUE);
}

/**
 * scatola_file_get_deep_directory_count_as_string:
 *
 * Get a user-displayable string representing the count of all contained
 ...
 **/
static char *
scatola_file_get_deep_directory_count_as_string (ScatolaFile *file)
{
	return scatola_file_get_deep_count_as_string_internal (file, FALSE, TRUE, FALSE);
}

/**
 * scatola_file_get_string_attribute:
 *
 ...
 * scatola_file_get_string_attribute_with_default if you want a non-NULL
 * default.
 *
 * @file: ScatolaFile representing the file in question.
 ...
 **/
char *
scatola_file_get_string_attribute_q (ScatolaFile *file, GQuark attribute_q)
{
	char *extension_attribute;

	if (attribute_q == attribute_name_q) {
		return scatola_file_get_display_name (file);
	}
	if (attribute_q == attribute_type_q) {
		return scatola_file_get_type_as_string (file);
	}
	if (attribute_q == attribute_mime_type_q) {
		return scatola_file_get_mime_type (file);
	}
	if (attribute_q == attribute_size_q) {
		return scatola_file_get_size_as_string (file);
	}
	if (attribute_q == attribute_size_detail_q) {
		return scatola_file_get_size_as_string_with_real_size (file);
	}
	if (attribute_q == attribute_deep_size_q) {
		return scatola_file_get_deep_size_as_string (file);
	}
	if (attribute_q == attribute_deep_file_count_q) {
		return scatola_file_get_deep_file_count_as_string (file);
	}
	if (attribute_q == attribute_deep_directory_count_q) {
		return scatola_file_get_deep_directory_count_as_string (file);
	}
	if (attribute_q == attribute_deep_total_count_q) {
		return scatola_file_get_deep_total_count_as_string (file);
	}
	if (attribute_q == attribute_trash_orig_path_q) {
		return scatola_file_get_trash_original_file_parent_as_string (file);
	}
	if (attribute_q == attribute_date_modified_q) {
		return scatola_file_get_date_as_string (file,
	...
	if (attribute_q == attribute_permissions_q) {
		return scatola_file_get_permissions_as_string (file);
	}
	if (attribute_q == attribute_selinux_context_q) {
		return scatola_file_get_selinux_context (file);
	}
	if (attribute_q == attribute_octal_permissions_q) {
		return scatola_file_get_octal_permissions_as_string (file);
	}
	if (attribute_q == attribute_owner_q) {
		return scatola_file_get_owner_as_string (file, TRUE);
	}
	if (attribute_q == attribute_group_q) {
		return scatola_file_get_group_name (file);
	}
	if (attribute_q == attribute_uri_q) {
		return scatola_file_get_uri (file);
	}
	if (attribute_q == attribute_where_q) {
		return scatola_file_get_where_string (file);
	}
	if (attribute_q == attribute_link_target_q) {
		return scatola_file_get_symbolic_link_target_path (file);
	}
	if (attribute_q == attribute_volume_q) {
		return scatola_file_get_volume_name (file);
	}
	if (attribute_q == attribute_free_space_q) {
		return scatola_file_get_volume_free_space (file);
	}
...
char *
scatola_file_get_string_attribute (ScatolaFile *file, const char *attribute_name)
{
	return scatola_file_get_string_attribute_q (file, g_quark_from_string (attribute_name));
}


/**
 * scatola_file_get_string_attribute_with_default:
 *
 * Get a user-displayable string from a named attribute. Use g_free to
 ...
 * scatola_file_get_string_attribute if you want NULL instead of a default
 * result.
 *
 * @file: ScatolaFile representing the file in question.
 * @attribute_name: The name of the desired attribute. See the description of
 * scatola_file_get_string for the set of available attributes.
 *
 ...
 **/
char *
scatola_file_get_string_attribute_with_default_q (ScatolaFile *file, GQuark attribute_q)
{
	char *result;
	guint item_count;
	gboolean count_unreadable;
	...
	result = scatola_file_get_string_attribute_q (file, attribute_q);
	if (result != NULL) {
		return result;
	}
	...
	if (attribute_q == attribute_size_q) {
		...
		count_unreadable = FALSE;
		if (scatola_file_is_directory (file)) {
			scatola_file_get_directory_item_count (file, &item_count, &count_unreadable);
		}
		return g_strdup (count_unreadable ? _("? items") : "...");
	}
	if (attribute_q == attribute_deep_size_q) {
		status = scatola_file_get_deep_counts (file, NULL, NULL, NULL, NULL, FALSE);
		if (status == SCATOLA_REQUEST_DONE) {
...
char *
scatola_file_get_string_attribute_with_default (ScatolaFile *file, const char *attribute_name)
{
	return scatola_file_get_string_attribute_with_default_q (file, g_quark_from_string (attribute_name));
}

gboolean
scatola_file_is_date_sort_attribute_q (GQuark attribute_q)
{
 ...
 **/
static char *
get_description (ScatolaFile *file)
{
	const char *mime_type;
	char *description;
	...
	if (g_content_type_is_unknown (mime_type) &&
	    scatola_file_is_executable (file)) {
...
static char *
update_description_for_link (ScatolaFile *file, char *string)
{
	char *res;

	if (scatola_file_is_symbolic_link (file)) {
		g_assert (!scatola_file_is_broken_symbolic_link (file));
		if (string == NULL) {
...
static char *
scatola_file_get_type_as_string (ScatolaFile *file)
{
	if (file == NULL) {
		return NULL;
	}

	if (scatola_file_is_broken_symbolic_link (file)) {
 ...
 **/
GFileType
scatola_file_get_file_type (ScatolaFile *file)
{
	if (file == NULL) {
 ...
 **/
char *
scatola_file_get_mime_type (ScatolaFile *file)
{
	if (file != NULL) {
		g_return_val_if_fail (SCATOLA_IS_FILE (file), NULL);
 ...
 **/
gboolean
scatola_file_is_mime_type (ScatolaFile *file, const char *mime_type)
{
	g_return_val_if_fail (SCATOLA_IS_FILE (file), FALSE);
...
gboolean
scatola_file_is_launchable (ScatolaFile *file)
{
	...
	return type_can_be_executable &&
		scatola_file_can_get_permissions (file) &&
		scatola_file_can_execute (file) &&
		scatola_file_is_executable (file) &&
		!scatola_file_is_directory (file);
}
 ...
 **/
GList *
scatola_file_get_emblem_icons (ScatolaFile *file,
				char **exclude)
{
	GList *keywords, *l;
	GList *icons;
...
#ifdef TRASH_IS_FAST_ENOUGH
		if (strcmp (keyword, SCATOLA_FILE_EMBLEM_NAME_TRASH) == 0) {
			char *uri;
...
GList *
scatola_file_get_emblem_pixbufs (ScatolaFile *file,
				  int size,
				  gboolean force_size,
				  char **exclude)
{
	...
	icons = scatola_file_get_emblem_icons (file, exclude);
	pixbufs = NULL;

	for (l = icons; l != NULL; l = l->next) {
		icon = l->data;

		icon_info = scatola_icon_info_lookup (icon, size);
		if (force_size) {
			pixbuf = scatola_icon_info_get_pixbuf_nodefault_at_size (icon_info, size);
		} else {
			pixbuf = scatola_icon_info_get_pixbuf_nodefault (icon_info);
		}
 ...
 **/
GList *
scatola_file_get_keywords (ScatolaFile *file)
{
	GList *keywords;
	...
	keywords = scatola_file_get_metadata_list
		(file, SCATOLA_METADATA_KEY_EMBLEMS);
 ...
 **/
void
scatola_file_set_keywords (ScatolaFile *file, GList *keywords)
{
	...
	scatola_file_set_metadata_list
		(file, SCATOLA_METADATA_KEY_EMBLEMS, canonical_keywords);
	g_list_free (canonical_keywords);
}

/**
 * scatola_file_is_symbolic_link
 *
 ...
 **/
gboolean
scatola_file_is_symbolic_link (ScatolaFile *file)
{
	return file->details->is_symlink;
}

gboolean
scatola_file_is_mountpoint (ScatolaFile *file)
{
	return file->details->is_mountpoint;
}

GMount *
scatola_file_get_mount (ScatolaFile *file)
{
	if (file->details->mount) {
		      ...
		      gpointer data)
{
	...
	scatola_file_invalidate_attributes (file, SCATOLA_FILE_ATTRIBUTE_MOUNT);
}

void
scatola_file_set_mount (ScatolaFile *file,
			 GMount *mount)
{
	if (file->details->mount) {
		g_signal_handlers_disconnect_by_func (file->details->mount, file_mount_unmounted, file);
...
/**
 * scatola_file_is_broken_symbolic_link
 *
 ...
 **/
gboolean
scatola_file_is_broken_symbolic_link (ScatolaFile *file)
{
	if (file == NULL) {
		return FALSE;
	}
	...
	return scatola_file_get_file_type (file) == G_FILE_TYPE_SYMBOLIC_LINK;
}
		...
		gpointer user_data)
{
	...
	directory = SCATOLA_DIRECTORY (user_data);
	...
	if (directory->details->free_space != free_space) {
		...
		file = scatola_directory_get_existing_corresponding_file (directory);
		if (file) {
...
/**
 * scatola_file_get_volume_free_space
 ...
 */
char *
scatola_file_get_volume_free_space (ScatolaFile *file)
{
	ScatolaDirectory *directory;
	GFile *location;
	char *res;
	...
	directory = scatola_directory_get_for_file (file);
 ...
 */
char *
scatola_file_get_volume_name (ScatolaFile *file)
{
	GFile *location;
	char *res;
	GMount *mount;
...
/**
 * scatola_file_get_symbolic_link_target_path
 *
 ...
 * @file: ScatolaFile representing the symbolic link in question.
 *
 * Returns: newly-allocated copy of the file path of the target of the symbolic link.
 */
char *
scatola_file_get_symbolic_link_target_path (ScatolaFile *file)
{
	if (!scatola_file_is_symbolic_link (file)) {
...
/**
 * scatola_file_get_symbolic_link_target_uri
 *
 ...
 */
char *
scatola_file_get_symbolic_link_target_uri (ScatolaFile *file)
{
...
/**
 * scatola_file_is_scatola_link
 *
 * Check if this file is a "scatola link", meaning a historical
 * scatola xml link file or a desktop file.
 * @file: ScatolaFile representing the file in question.
 *
 * Returns: True if the file is a scatola link.
 *
 **/
gboolean
scatola_file_is_scatola_link (ScatolaFile *file)
{
    if (file->details->mime_type == NULL) {
        return FALSE;
        }
 ...
 **/
gboolean
scatola_file_is_directory (ScatolaFile *file)
{
	return scatola_file_get_file_type (file) == G_FILE_TYPE_DIRECTORY;
}

/**
 * scatola_file_is_user_special_directory
 *
 ...
 */
gboolean
scatola_file_is_user_special_directory (ScatolaFile *file,
...
gboolean
scatola_file_is_archive (ScatolaFile *file)
{
	char *mime_type;
	int i;
	...
	mime_type = scatola_file_get_mime_type (file);
 ...
 **/
gboolean
scatola_file_is_in_trash (ScatolaFile *file)
{
	g_assert (SCATOLA_IS_FILE (file));

	return scatola_directory_is_in_trash (file->details->directory);
}

GError *
scatola_file_get_file_info_error (ScatolaFile *file)
{
	if (!file->details->get_info_failed) {
		return NULL;
	}
 ...
 **/
gboolean
scatola_file_contains_text (ScatolaFile *file)
{
	if (file == NULL) {
		return FALSE;
	}
	...
	return scatola_file_is_mime_type (file, "text/plain");
}

/**
 * scatola_file_is_binary
 *
 ...
 **/
gboolean
scatola_file_is_binary (ScatolaFile *file)
{
	if (!scatola_file_can_read(file))
	{
		return FALSE;
	...
	fp = g_fopen (g_file_get_path (scatola_file_get_location (file)), "r");
	if (fp == NULL)
	{
		return FALSE;
	}
 ...
 **/
gboolean
scatola_file_is_executable (ScatolaFile *file)
{
...
/**
 * scatola_file_peek_top_left_text
 *
 ...
 **/
char *
scatola_file_peek_top_left_text (ScatolaFile *file,
				  gboolean  need_large_text,
				  gboolean *needs_loading)
{
	g_return_val_if_fail (SCATOLA_IS_FILE (file), NULL);

	if (!scatola_file_should_get_top_left_text (file)) {
		if (needs_loading) {
			*needs_loading = FALSE;
		}
		return NULL;
	...
	if (!file->details->got_top_left_text) {
		...
		if (scatola_file_contains_text (file)) {
			return " ...";
		}
		return NULL;
	}
...
/**
 * scatola_file_get_top_left_text
 *
 ...
 **/
char *
scatola_file_get_top_left_text (ScatolaFile *file)
{
	return g_strdup (scatola_file_peek_top_left_text (file, FALSE, NULL));
}

char *
scatola_file_get_filesystem_id (ScatolaFile *file)
{
	return g_strdup (eel_ref_str_peek (file->details->filesystem_id));
}

ScatolaFile *
scatola_file_get_trash_original_file (ScatolaFile *file)
{
	GFile *location;
	ScatolaFile *original_file;

	original_file = NULL;

	if (file->details->trash_orig_path != NULL) {
		location = g_file_new_for_path (file->details->trash_orig_path);
		original_file = scatola_file_get (location);
		g_object_unref (location);
	}
...
void
scatola_file_mark_gone (ScatolaFile *file)
{
	ScatolaDirectory *directory;
...
/**
 * scatola_file_changed
 *
 * Notify the user that this file has changed.
 * @file: ScatolaFile representing the file in question.
 **/
void
scatola_file_changed (ScatolaFile *file)
{
	GList fake_list;
	...
	if (scatola_file_is_self_owned (file)) {
		...
		fake_list.next = NULL;
		fake_list.prev = NULL;
		scatola_directory_emit_change_signals
			(file->details->directory, &fake_list);
	}
}

/**
 * scatola_file_updated_deep_count_in_progress
 *
 * Notify clients that a newer deep count is available for
 * the directory in question.
 */
void
scatola_file_updated_deep_count_in_progress (ScatolaFile *file) {
	...
	for (node = link_files; node != NULL; node = node->next) {
		scatola_file_updated_deep_count_in_progress (SCATOLA_FILE (node->data));
	}
	scatola_file_list_free (link_files);
}

/**
 * scatola_file_emit_changed
 *
 ...
 **/
void
scatola_file_emit_changed (ScatolaFile *file)
{
	GList *link_files, *p;
	...
	for (p = link_files; p != NULL; p = p->next) {
		if (p->data != file) {
			scatola_file_changed (SCATOLA_FILE (p->data));
		}
	}
	scatola_file_list_free (link_files);
}
 ...
 **/
gboolean
scatola_file_is_gone (ScatolaFile *file)
{
	g_return_val_if_fail (SCATOLA_IS_FILE (file), FALSE);
...
/**
 * scatola_file_is_not_yet_confirmed
 *
 ...
 **/
gboolean
scatola_file_is_not_yet_confirmed (ScatolaFile *file)
{
	g_return_val_if_fail (SCATOLA_IS_FILE (file), FALSE);
 ...
 */
gboolean
scatola_file_check_if_ready (ScatolaFile *file,
...
void
scatola_file_call_when_ready (ScatolaFile *file,
			       ScatolaFileAttributes file_attributes,
...
void
scatola_file_cancel_call_when_ready (ScatolaFile *file,
				      ScatolaFileCallback callback,
				      gpointer callback_data)
{
	g_return_if_fail (callback != NULL);
...
static void
invalidate_directory_count (ScatolaFile *file)
{
	file->details->directory_count_is_up_to_date = FALSE;
}

static void
invalidate_deep_counts (ScatolaFile *file)
{
	file->details->deep_counts_status = SCATOLA_REQUEST_NOT_STARTED;
}

static void
invalidate_mime_list (ScatolaFile *file)
{
	file->details->mime_list_is_up_to_date = FALSE;
}

static void
invalidate_top_left_text (ScatolaFile *file)
{
	file->details->top_left_text_is_up_to_date = FALSE;
}

static void
invalidate_file_info (ScatolaFile *file)
{
	file->details->file_info_is_up_to_date = FALSE;
}

static void
invalidate_link_info (ScatolaFile *file)
{
	file->details->link_info_is_up_to_date = FALSE;
}

static void
invalidate_thumbnail (ScatolaFile *file)
{
	file->details->thumbnail_is_up_to_date = FALSE;
}

static void
invalidate_mount (ScatolaFile *file)
{
	file->details->mount_is_up_to_date = FALSE;
}

void
scatola_file_invalidate_extension_info_internal (ScatolaFile *file)
{
	if (file->details->pending_info_providers)
		g_list_free_full (file->details->pending_info_providers, g_object_unref);

	file->details->pending_info_providers =
		scatola_extensions_get_for_type (SCATOLA_TYPE_INFO_PROVIDER);
}

void
scatola_file_invalidate_attributes_internal (ScatolaFile *file,
					      ScatolaFileAttributes file_attributes)
{
	Request request;
	...
	request = scatola_directory_set_up_request (file_attributes);
...
gboolean
scatola_file_has_open_window (ScatolaFile *file)
{
	return file->details->has_open_window;
}

void
scatola_file_set_has_open_window (ScatolaFile *file,
...
gboolean
scatola_file_is_thumbnailing (ScatolaFile *file)
{
	g_return_val_if_fail (SCATOLA_IS_FILE (file), FALSE);
...
void
scatola_file_set_is_thumbnailing (ScatolaFile *file,
...
/**
 * scatola_file_invalidate_attributes
 *
...
void
scatola_file_invalidate_attributes (ScatolaFile *file,
				     ScatolaFileAttributes file_attributes)
{
	/* Cancel possible in-progress loads of any of these attributes */
	scatola_directory_cancel_loading_file_attributes (file->details->directory,
							   file,
							   file_attributes);

	/* Actually invalidate the values */
	scatola_file_invalidate_attributes_internal (file, file_attributes);
...
scatola_file_get_all_attributes (void)
{
	return  SCATOLA_FILE_ATTRIBUTE_INFO |
		SCATOLA_FILE_ATTRIBUTE_LINK_INFO |
		SCATOLA_FILE_ATTRIBUTE_DEEP_COUNTS |
		SCATOLA_FILE_ATTRIBUTE_DIRECTORY_ITEM_COUNT |
		SCATOLA_FILE_ATTRIBUTE_DIRECTORY_ITEM_MIME_TYPES |
		SCATOLA_FILE_ATTRIBUTE_TOP_LEFT_TEXT |
		SCATOLA_FILE_ATTRIBUTE_LARGE_TOP_LEFT_TEXT |
		SCATOLA_FILE_ATTRIBUTE_EXTENSION_INFO |
		SCATOLA_FILE_ATTRIBUTE_THUMBNAIL |
		SCATOLA_FILE_ATTRIBUTE_MOUNT;
}

void
scatola_file_invalidate_all_attributes (ScatolaFile *file)
{
	ScatolaFileAttributes all_attributes;

	all_attributes = scatola_file_get_all_attributes ();
	scatola_file_invalidate_attributes (file, all_attributes);
}
 ...
 **/
void
scatola_file_dump (ScatolaFile *file)
{
...
/**
 * scatola_file_list_ref
 *
 * Ref all the files in a list.
 * @list: GList of files.
 **/
GList *
scatola_file_list_ref (GList *list)
{
	g_list_foreach (list, (GFunc) scatola_file_ref, NULL);
	return list;
}

/**
 * scatola_file_list_unref
 *
 * Unref all the files in a list.
 * @list: GList of files.
 **/
void
scatola_file_list_unref (GList *list)
{
	g_list_foreach (list, (GFunc) scatola_file_unref, NULL);
}

/**
 * scatola_file_list_free
 *
 * Free a list of files after unrefing them.
 * @list: GList of files.
 **/
void
scatola_file_list_free (GList *list)
{
	scatola_file_list_unref (list);
	g_list_free (list);
}

/**
 * scatola_file_list_copy
 *
 * Copy the list of files, making a new ref of each,
 * @list: GList of files.
 **/
GList *
scatola_file_list_copy (GList *list)
{
	return g_list_copy (scatola_file_list_ref (list));
}

static gboolean
get_attributes_for_default_sort_type (ScatolaFile *file,
				      ...
				      gboolean *is_trash)
{
	...
	*is_trash = FALSE;
	...
	if (file && scatola_file_is_directory (file)) {
		is_download_dir =
			scatola_file_is_user_special_directory (file, G_USER_DIRECTORY_DOWNLOAD);
		is_desktop_dir =
			scatola_file_is_user_special_directory (file, G_USER_DIRECTORY_DESKTOP);
		is_trash_dir =
			scatola_file_is_in_trash (file);
...
ScatolaFileSortType
scatola_file_get_default_sort_type (ScatolaFile *file,
				     gboolean *reversed)
{
	ScatolaFileSortType retval;
	gboolean is_download, is_trash, res;

	retval = SCATOLA_FILE_SORT_NONE;
	is_download = is_trash = FALSE;
	res = get_attributes_for_default_sort_type (file, &is_download, &is_trash);

	if (res) {
		if (is_download) {
			retval = SCATOLA_FILE_SORT_BY_MTIME;
		} else if (is_trash) {
			retval = SCATOLA_FILE_SORT_BY_TRASHED_TIME;
		}
...
const gchar *
scatola_file_get_default_sort_attribute (ScatolaFile *file,
					  gboolean *reversed)
{
	const gchar *retval;
	gboolean is_download, is_trash, res;
...
{
	return compare_by_display_name (SCATOLA_FILE (a), SCATOLA_FILE (b));
}

/**
 * scatola_file_list_sort_by_display_name
 *
 * Sort the list of files by file name.
 * @list: GList of files.
 **/
GList *
scatola_file_list_sort_by_display_name (GList *list)
{
...
file_list_ready_data_free (FileListReadyData *data)
{
	...
	if (l != NULL) {
		...
		scatola_file_list_free (data->file_list);
			  ...
			  gpointer callback_data)
{
	...
	data->file_list = scatola_file_list_copy (file_list);
...
static void
file_list_file_ready_callback (ScatolaFile *file,
			       gpointer user_data)
{
	FileListReadyData *data;
...
void
scatola_file_list_call_when_ready (GList *file_list,
				    ScatolaFileAttributes attributes,
				    ...
				    gpointer callback_data)
{
	...
	if (handle) {
		*handle = (ScatolaFileListHandle *) data;
	}


	l = file_list;
	while (l != NULL) {
		file = SCATOLA_FILE (l->data);
...
void
scatola_file_list_cancel_call_when_ready (ScatolaFileListHandle *handle)
{
	GList *l;
	ScatolaFile *file;
	FileListReadyData *data;
...
char *
scatola_extract_top_left_text (const char *text,
				gboolean large,
				int length)
{
        GString* buffer;
	...
	if (large) {
		max_bytes = SCATOLA_FILE_LARGE_TOP_LEFT_TEXT_MAXIMUM_BYTES;
		max_lines = SCATOLA_FILE_LARGE_TOP_LEFT_TEXT_MAXIMUM_LINES;
		max_cols = SCATOLA_FILE_LARGE_TOP_LEFT_TEXT_MAXIMUM_CHARACTERS_PER_LINE;
	} else {
		max_bytes = SCATOLA_FILE_TOP_LEFT_TEXT_MAXIMUM_BYTES;
		max_lines = SCATOLA_FILE_TOP_LEFT_TEXT_MAXIMUM_LINES;
		max_cols = SCATOLA_FILE_TOP_LEFT_TEXT_MAXIMUM_CHARACTERS_PER_LINE;
	}
...
{
	g_settings_get (scatola_preferences,
					SCATOLA_PREFERENCES_IMAGE_FILE_THUMBNAIL_LIMIT,
...
{
	cached_thumbnail_size = g_settings_get_int (scatola_icon_view_preferences, SCATOLA_PREFERENCES_ICON_VIEW_THUMBNAIL_SIZE);
...
{
	show_image_thumbs = g_settings_get_enum (scatola_preferences, SCATOLA_PREFERENCES_SHOW_IMAGE_FILE_THUMBNAILS);
			     ...
			     gpointer user_data)
{
	...
	scatola_icon_info_clear_caches ();
...
static void
scatola_file_class_init (ScatolaFileClass *class)
{
	GtkIconTheme *icon_theme;

	scatola_file_info_getter = scatola_file_get_internal;
	...
	G_OBJECT_CLASS (class)->constructor = scatola_file_constructor;

	signals[CHANGED] =
		g_signal_new ("changed",
		              G_TYPE_FROM_CLASS (class),
		              G_SIGNAL_RUN_LAST,
		              G_STRUCT_OFFSET (ScatolaFileClass, changed),
		              NULL, NULL,
	...
	signals[UPDATED_DEEP_COUNT_IN_PROGRESS] =
		g_signal_new ("updated_deep_count_in_progress",
		              G_TYPE_FROM_CLASS (class),
		              G_SIGNAL_RUN_LAST,
		              G_STRUCT_OFFSET (ScatolaFileClass, updated_deep_count_in_progress),
		              NULL, NULL,
			      g_cclosure_marshal_VOID__VOID,
		              G_TYPE_NONE, 0);

	g_type_class_add_private (class, sizeof (ScatolaFileDetails));

	eel_g_settings_add_auto_enum (scatola_preferences,
				                  SCATOLA_PREFERENCES_DATE_FORMAT,
				                  &date_format_pref);

	thumbnail_limit_changed_callback (NULL);
	g_signal_connect_swapped (scatola_preferences,
							  "changed::" SCATOLA_PREFERENCES_IMAGE_FILE_THUMBNAIL_LIMIT,
							  G_CALLBACK (thumbnail_limit_changed_callback),
							  NULL);
	thumbnail_size_changed_callback (NULL);
	g_signal_connect_swapped (scatola_icon_view_preferences,
							  "changed::" SCATOLA_PREFERENCES_ICON_VIEW_THUMBNAIL_SIZE,
							  G_CALLBACK (thumbnail_size_changed_callback),
							  NULL);
	show_thumbnails_changed_callback (NULL);
	g_signal_connect_swapped (scatola_preferences,
							  "changed::" SCATOLA_PREFERENCES_SHOW_IMAGE_FILE_THUMBNAILS,
	...
	g_signal_connect (scatola_signaller_get_current (),
			  "mime_data_changed",
...
static void
scatola_file_add_emblem (ScatolaFile *file,
...
static void
scatola_file_add_string_attribute (ScatolaFile *file,
...
static void
scatola_file_invalidate_extension_info (ScatolaFile *file)
{
	scatola_file_invalidate_attributes (file, SCATOLA_FILE_ATTRIBUTE_EXTENSION_INFO);
}

void
scatola_file_info_providers_done (ScatolaFile *file)
{
	g_list_free_full (file->details->extension_emblems, g_free);
...
static void
scatola_file_info_iface_init (ScatolaFileInfoIface *iface)
{
	iface->is_gone = scatola_file_is_gone;
	iface->get_name = scatola_file_get_name;
	iface->get_file_type = scatola_file_get_file_type;
	iface->get_location = scatola_file_get_location;
	iface->get_uri = scatola_file_get_uri;
	iface->get_parent_location = scatola_file_get_parent_location;
	iface->get_parent_uri = scatola_file_get_parent_uri;
	iface->get_parent_info = scatola_file_get_parent;
	iface->get_mount = scatola_file_get_mount;
	iface->get_uri_scheme = scatola_file_get_uri_scheme;
	iface->get_activation_uri = scatola_file_get_activation_uri;
	iface->get_mime_type = scatola_file_get_mime_type;
	iface->is_mime_type = scatola_file_is_mime_type;
	iface->is_directory = scatola_file_is_directory;
	iface->can_write = scatola_file_can_write;
	iface->add_emblem = scatola_file_add_emblem;
	iface->get_string_attribute = scatola_file_get_string_attribute;
	iface->add_string_attribute = scatola_file_add_string_attribute;
	iface->invalidate_extension_info = scatola_file_invalidate_extension_info;
}

#if !defined (SCATOLA_OMIT_SELF_CHECK)

void
scatola_self_check_file (void)
{
	ScatolaFile *file_1;
	ScatolaFile *file_2;
	GList *list;
        ...
        EEL_CHECK_INTEGER_RESULT (scatola_directory_number_outstanding (), 0);

	file_1 = scatola_file_get_by_uri ("file:///home/");
	...
	EEL_CHECK_INTEGER_RESULT (G_OBJECT (file_1->details->directory)->ref_count, 1);
        EEL_CHECK_INTEGER_RESULT (scatola_directory_number_outstanding (), 1);

	scatola_file_unref (file_1);

        EEL_CHECK_INTEGER_RESULT (scatola_directory_number_outstanding (), 0);

	file_1 = scatola_file_get_by_uri ("file:///etc");
	file_2 = scatola_file_get_by_uri ("file:///usr");

        list = NULL;
        list = g_list_prepend (list, file_1);
        list = g_list_prepend (list, file_2);

        scatola_file_list_ref (list);
	...
	EEL_CHECK_STRING_RESULT (scatola_file_get_name (file_1), "home");

	EEL_CHECK_BOOLEAN_RESULT (scatola_file_get_by_uri ("file:///home/") == file_1, TRUE);
	scatola_file_unref (file_1);
	 ...
	 * in real life */
	file_1 = scatola_file_get_by_uri (":");
	EEL_CHECK_STRING_RESULT (scatola_file_get_name (file_1), ":");
	scatola_file_unref (file_1);
#endif

	file_1 = scatola_file_get_by_uri ("eazel:");
	EEL_CHECK_STRING_RESULT (scatola_file_get_name (file_1), "eazel");
	scatola_file_unref (file_1);
	...
	EEL_CHECK_BOOLEAN_RESULT (scatola_file_compare_for_sort (file_1, file_2, SCATOLA_FILE_SORT_BY_DISPLAY_NAME, FALSE, FALSE) < 0, TRUE);
	EEL_CHECK_BOOLEAN_RESULT (scatola_file_compare_for_sort (file_1, file_2, SCATOLA_FILE_SORT_BY_DISPLAY_NAME, FALSE, TRUE) > 0, TRUE);
	EEL_CHECK_BOOLEAN_RESULT (scatola_file_compare_for_sort (file_1, file_1, SCATOLA_FILE_SORT_BY_DISPLAY_NAME, FALSE, FALSE) == 0, TRUE);
	EEL_CHECK_BOOLEAN_RESULT (scatola_file_compare_for_sort (file_1, file_1, SCATOLA_FILE_SORT_BY_DISPLAY_NAME, TRUE, FALSE) == 0, TRUE);
	EEL_CHECK_BOOLEAN_RESULT (scatola_file_compare_for_sort (file_1, file_1, SCATOLA_FILE_SORT_BY_DISPLAY_NAME, FALSE, TRUE) == 0, TRUE);
	EEL_CHECK_BOOLEAN_RESULT (scatola_file_compare_for_sort (file_1, file_1, SCATOLA_FILE_SORT_BY_DISPLAY_NAME, TRUE, TRUE) == 0, TRUE);

	scatola_file_unref (file_1);
	scatola_file_unref (file_2);
}
