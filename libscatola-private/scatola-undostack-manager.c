...
/* ScatolaUndoStackManager - Manages undo of file operations (implementation)
 *
...
#include "scatola-undostack-manager.h"
...
struct _ScatolaUndoStackActionData
{
  /* Common stuff */
  ScatolaUndoStackActionType type;
  gboolean isValid;
  ...
  ScatolaUndoStackManager *manager;    /* Pointer to the manager */
...
struct _ScatolaUndoStackManagerPrivate
{
  /* Private fields */
  GQueue *stack;
  guint undo_levels;
...
#define SCATOLA_UNDOSTACK_MANAGER_GET_PRIVATE(o)  \
   (G_TYPE_INSTANCE_GET_PRIVATE ((o), TYPE_SCATOLA_UNDOSTACK_MANAGER, ScatolaUndoStackManagerPrivate))
...
static void scatola_undostack_manager_set_property (GObject * object,
    guint prop_id, const GValue * value, GParamSpec * pspec);

static void scatola_undostack_manager_get_property (GObject * object,
    guint prop_id, GValue * value, GParamSpec * pspec);

/* *****************************************************************
 Destructors prototypes
 ***************************************************************** */
static void scatola_undostack_manager_finalize (GObject * object);

static void scatola_undostack_manager_dispose (GObject * object);

/* *****************************************************************
 Type definition
 ***************************************************************** */
G_DEFINE_TYPE (ScatolaUndoStackManager, scatola_undostack_manager,
    G_TYPE_OBJECT);
...
static void stack_fix_size (ScatolaUndoStackManagerPrivate * priv);

static gboolean can_undo (ScatolaUndoStackManagerPrivate * priv);

static gboolean can_redo (ScatolaUndoStackManagerPrivate * priv);

static void stack_push_action (ScatolaUndoStackManagerPrivate * priv,
    ScatolaUndoStackActionData * action);

static ScatolaUndoStackActionData
    * stack_scroll_left (ScatolaUndoStackManagerPrivate * priv);

static ScatolaUndoStackActionData
    * stack_scroll_right (ScatolaUndoStackManagerPrivate * priv);

static ScatolaUndoStackActionData
    * get_next_redo_action (ScatolaUndoStackManagerPrivate * priv);

static ScatolaUndoStackActionData
    * get_next_undo_action (ScatolaUndoStackManagerPrivate * priv);

static gchar *get_undo_label (ScatolaUndoStackActionData * action);

static gchar *get_undo_description (ScatolaUndoStackActionData * action);

static gchar *get_redo_label (ScatolaUndoStackActionData * action);

static gchar *get_redo_description (ScatolaUndoStackActionData * action);

static void do_menu_update (ScatolaUndoStackManager * manager);
...
static void undo_redo_done_rename_callback (ScatolaFile * file,
...
static void clear_redo_actions (ScatolaUndoStackManagerPrivate * priv);

static gchar *get_first_target_short_name (ScatolaUndoStackActionData *
    action);
 ...
 ***************************************************************** */
static void
scatola_undostack_manager_class_init (ScatolaUndoStackManagerClass * klass)
{
  ...
  g_type_class_add_private (klass, sizeof (ScatolaUndoStackManagerPrivate));
  ...
  g_object_class->set_property = scatola_undostack_manager_set_property;
  g_object_class->get_property = scatola_undostack_manager_get_property;
  ...
  /* Hook deconstructors */
  g_object_class->dispose = scatola_undostack_manager_dispose;
  g_object_class->finalize = scatola_undostack_manager_finalize;
}

static void
scatola_undostack_manager_init (ScatolaUndoStackManager * self)
{
  ScatolaUndoStackManagerPrivate *priv;

  priv = SCATOLA_UNDOSTACK_MANAGER_GET_PRIVATE (self);
...
static void
scatola_undostack_manager_dispose (GObject * object)
{
  ScatolaUndoStackManager *self = SCATOLA_UNDOSTACK_MANAGER (object);
  ScatolaUndoStackManagerPrivate *priv = self->priv;
  ...
  G_OBJECT_CLASS (scatola_undostack_manager_parent_class)->dispose (object);
}

static void
scatola_undostack_manager_finalize (GObject * object)
{
  G_OBJECT_CLASS (scatola_undostack_manager_parent_class)->finalize (object);
}
 ...
 ***************************************************************** */
static void
scatola_undostack_manager_set_property (GObject * object, guint prop_id,
    const GValue * value, GParamSpec * pspec)
{
  g_return_if_fail (IS_SCATOLA_UNDOSTACK_MANAGER (object));

  ScatolaUndoStackManager *manager = SCATOLA_UNDOSTACK_MANAGER (object);
  ScatolaUndoStackManagerPrivate *priv = manager->priv;
...
static void
scatola_undostack_manager_get_property (GObject * object, guint prop_id,
...
{
  static ScatolaUndoStackManager *manager = NULL;

  if (manager == NULL) {
    manager =
        g_object_new (TYPE_SCATOLA_UNDOSTACK_MANAGER, "undo-levels", 32, NULL);
  }
...
void
scatola_undostack_manager_request_menu_update (ScatolaUndoStackManager *
    manager)
{
  do_menu_update (manager);
}
    ...
    GtkWidget * parent_view, ScatolaUndostackFinishCallback cb)
{
  ...
  ScatolaUndoStackActionData *action = stack_scroll_left (priv);
  ...
  if (action != NULL) {
    ...
    switch (action->type) {
      ...
      case SCATOLA_UNDOSTACK_CREATEFILEFROMTEMPLATE:
        ...
        g_free (new_name);
        break;
      case SCATOLA_UNDOSTACK_DUPLICATE:
        uris = construct_gfile_list (action->sources, action->src_dir);
        scatola_file_operations_duplicate (uris, NULL, NULL,
            undo_redo_done_transfer_callback, action);
    	g_list_free_full (uris, g_object_unref);
        break;
      case SCATOLA_UNDOSTACK_RESTOREFROMTRASH:
      case SCATOLA_UNDOSTACK_MOVE:
        uris = construct_gfile_list (action->sources, action->src_dir);
        scatola_file_operations_move (uris, NULL,
            action->dest_dir, NULL, undo_redo_done_transfer_callback, action);
    	g_list_free_full (uris, g_object_unref);
        break;
      case SCATOLA_UNDOSTACK_RENAME:
        new_name = get_uri_basename (action->new_uri);
        file = scatola_file_get_by_uri (action->old_uri);
        scatola_file_rename (file, new_name,
            undo_redo_done_rename_callback, action);
        g_object_unref (file);
        g_free (new_name);
        break;
      case SCATOLA_UNDOSTACK_CREATEEMPTYFILE:
        puri = get_uri_parent (action->target_uri);
        new_name = get_uri_basename (action->target_uri);
        scatola_file_operations_new_file (NULL, NULL, puri,
            new_name,
            action->template,
        ...
        g_free (new_name);
        break;
      case SCATOLA_UNDOSTACK_CREATEFOLDER:
        puri = get_uri_parent (action->target_uri);
        scatola_file_operations_new_folder (NULL, NULL, puri,
            undo_redo_done_create_callback, action);
        g_free (puri);
        break;
      case SCATOLA_UNDOSTACK_MOVETOTRASH:
        if (g_hash_table_size (action->trashed) > 0) {
          ...
          priv->undo_redo_flag = TRUE;
          scatola_file_operations_trash_or_delete
              (uris, NULL, undo_redo_done_delete_callback, action);
          g_list_free (uri_to_trash);
    	  g_list_free_full (uris, g_object_unref);
        }
        break;
      case SCATOLA_UNDOSTACK_CREATELINK:
        uris = construct_gfile_list (action->sources, action->src_dir);
        scatola_file_operations_link (uris, NULL,
            action->dest_dir, NULL, undo_redo_done_transfer_callback, action);
    	g_list_free_full (uris, g_object_unref);
        break;
      case SCATOLA_UNDOSTACK_SETPERMISSIONS:
        file = scatola_file_get_by_uri (action->target_uri);
        scatola_file_set_permissions (file,
            action->new_permissions, undo_redo_done_rename_callback, action);
        g_object_unref (file);
        break;
      case SCATOLA_UNDOSTACK_RECURSIVESETPERMISSIONS:
        puri = g_file_get_uri (action->dest_dir);
        scatola_file_set_permissions_recursive (puri,
        ...
        g_free (puri);
        break;
      case SCATOLA_UNDOSTACK_CHANGEGROUP:
        file = scatola_file_get_by_uri (action->target_uri);
        scatola_file_set_group (file,
            action->new_group_name_or_id,
            undo_redo_done_rename_callback, action);
        g_object_unref (file);
        break;
      case SCATOLA_UNDOSTACK_CHANGEOWNER:
        file = scatola_file_get_by_uri (action->target_uri);
        scatola_file_set_owner (file,
            action->new_user_name_or_id,
            undo_redo_done_rename_callback, action);
        g_object_unref (file);
        break;
      case SCATOLA_UNDOSTACK_DELETE:
      default:
        priv->undo_redo_flag = FALSE;
    ...
    GtkWidget * parent_view, ScatolaUndostackFinishCallback cb)
{
  ...
  ScatolaUndoStackActionData *action = stack_scroll_right (priv);
    ...
    switch (action->type) {
      ...
      case SCATOLA_UNDOSTACK_CREATELINK:
        ...
        if (priv->confirm_delete) {
          scatola_file_operations_delete (uris, NULL,
      ...
      case SCATOLA_UNDOSTACK_RESTOREFROMTRASH:
        uris = construct_gfile_list (action->destinations, action->dest_dir);
        scatola_file_operations_trash_or_delete (uris, NULL,
            undo_redo_done_delete_callback, action);
    	g_list_free_full (uris, g_object_unref);
        break;
      case SCATOLA_UNDOSTACK_MOVETOTRASH:
      ...
      case SCATOLA_UNDOSTACK_RENAME:
        ...
        file = scatola_file_get_by_uri (action->new_uri);
        scatola_file_rename (file, new_name,
            undo_redo_done_rename_callback, action);
        g_object_unref (file);
        g_free (new_name);
...
{
  ScatolaUndoStackActionData *action = NULL;
  GList *trash = NULL;
  GList *l;
  GQueue *tmp_stack = g_queue_copy(stack);

  while ((action = (ScatolaUndoStackActionData *) g_queue_pop_tail (tmp_stack)) != NULL)
    if (action->trashed)
 ...
 ** ****************************************************************/
void
scatola_undostack_manager_trash_has_emptied (ScatolaUndoStackManager *
    manager)
{
  ScatolaUndoStackManagerPrivate *priv = manager->priv;
  ...
  while ((action = (ScatolaUndoStackActionData *) g_queue_pop_tail (tmp_stack)) != NULL)
  {
    ...
    if (action->type == SCATOLA_UNDOSTACK_MOVETOTRASH) {
        g_queue_remove (priv->stack, action);
    }
  }
...
scatola_undostack_manager_data_new (ScatolaUndoStackActionType type,
    gint items_count)
{
  ScatolaUndoStackActionData *data =
      g_slice_new0 (ScatolaUndoStackActionData);
  data->type = type;
  data->count = items_count;

  if (type == SCATOLA_UNDOSTACK_MOVETOTRASH) {
    data->trashed =
        g_hash_table_new_full (g_str_hash, g_str_equal, g_free, g_free);
  } else if (type == SCATOLA_UNDOSTACK_RECURSIVESETPERMISSIONS) {
 ...
 ** ****************************************************************/
void
scatola_undostack_manager_data_set_src_dir (ScatolaUndoStackActionData *
    data, GFile * src)
{
  if (!data)
    return;
 ...
 ** ****************************************************************/
void
scatola_undostack_manager_data_set_dest_dir (ScatolaUndoStackActionData *
    data, GFile * dest)
{
  if (!data)
    return;
 ...
 ** ****************************************************************/
void scatola_undostack_manager_data_add_origin_target_pair
 ...
 ** ****************************************************************/
void
scatola_undostack_manager_data_add_trashed_file (ScatolaUndoStackActionData
 ...
 ** ****************************************************************/
void scatola_undostack_manager_data_add_file_permissions
 ...
 ** ****************************************************************/
void scatola_undostack_manager_data_set_file_permissions
    (ScatolaUndoStackActionData * data, char *uri,
 ...
 ** ****************************************************************/
void scatola_undostack_manager_data_set_owner_change_information
    (ScatolaUndoStackActionData * data, char *uri,
 ...
 ** ****************************************************************/
void scatola_undostack_manager_data_set_group_change_information
    (ScatolaUndoStackActionData * data, char *uri,
 ...
 ** ****************************************************************/
void scatola_undostack_manager_data_set_recursive_permissions
    (ScatolaUndoStackActionData * data, guint32 file_permissions,
 ...
 ** ****************************************************************/
void
scatola_undostack_manager_data_set_create_data (ScatolaUndoStackActionData *
 ...
 ** ****************************************************************/
void scatola_undostack_manager_data_set_rename_information
...
stack_clear_n_oldest (GQueue * stack, guint n)
{
  ...
  for (i = 0; i < n; i++) {
    if ((action = (ScatolaUndoStackActionData *) g_queue_pop_tail (stack)) == NULL)
        break;
    if (action->locked) {
      action->freed = TRUE;
    } else {
...
/** ---------------------------------------------------------------- */
static void
stack_fix_size (ScatolaUndoStackManagerPrivate * priv)
{
...
/** ---------------------------------------------------------------- */
static void
clear_redo_actions (ScatolaUndoStackManagerPrivate * priv)
{
  while (priv->index > 0) {
    ScatolaUndoStackActionData *head = (ScatolaUndoStackActionData *)
...
/** ---------------------------------------------------------------- */
static void
stack_push_action (ScatolaUndoStackManagerPrivate * priv,
    ScatolaUndoStackActionData * action)
{
  guint length;
...
/** ---------------------------------------------------------------- */
static gchar *
get_first_target_short_name (ScatolaUndoStackActionData * action)
{
  GList *targets_first;
  gchar *file_name;
...
/** ---------------------------------------------------------------- */
static gchar *
get_undo_description (ScatolaUndoStackActionData * action)
{
...
/** ---------------------------------------------------------------- */
static gchar *
get_redo_description (ScatolaUndoStackActionData * action)
{
  gchar *description = NULL;
...
/** ---------------------------------------------------------------- */
static gchar *
get_undo_label (ScatolaUndoStackActionData * action)
{
  gchar *label = NULL;
  guint count;
...
/** ---------------------------------------------------------------- */
static gchar *
get_redo_label (ScatolaUndoStackActionData * action)
{
  gchar *label = NULL;
  guint count;
...
undo_redo_done_transfer_callback (GHashTable * debuting_uris, gpointer data)
{
  ...
  action = (ScatolaUndoStackActionData *) data;
  ...
  ScatolaUndoStackManager *manager = action->manager;
...
/** ---------------------------------------------------------------- */
static void
undo_redo_done_rename_callback (ScatolaFile * file,
    GFile * result_location, GError * error, gpointer callback_data)
{
  undo_redo_done_transfer_callback (NULL, callback_data);
}
...
{
  ScatolaUndoStackActionData *action = (ScatolaUndoStackActionData *) data;
  ...
  if (action)
    g_slice_free (ScatolaUndoStackActionData, action);
}
...
/** ---------------------------------------------------------------- */
static gboolean
can_undo (ScatolaUndoStackManagerPrivate * priv)
{
...
/** ---------------------------------------------------------------- */
static gboolean
can_redo (ScatolaUndoStackManagerPrivate * priv)
{
...
get_next_redo_action (ScatolaUndoStackManagerPrivate * priv)
{
  ...
  ScatolaUndoStackActionData *action = g_queue_peek_nth (priv->stack,
      priv->index - 1);
...
/** ---------------------------------------------------------------- */
static void
do_menu_update (ScatolaUndoStackManager * manager)
{
  ...
  ScatolaUndoStackActionData *action;
  ScatolaUndoStackManagerPrivate *priv = manager->priv;
  ScatolaUndoStackMenuData *data = g_slice_new0 (ScatolaUndoStackMenuData);
  ...
  g_slice_free (ScatolaUndoStackMenuData, data);
}
