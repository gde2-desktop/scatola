...
get_builtin_columns (void)
{
    ...
    columns = g_list_append (NULL,
                             g_object_new (SCATOLA_TYPE_COLUMN,
                                           "name", "name",
                                           "attribute", "name",
                                           "label", _("Name"),
...
get_extension_columns (void)
{
    ...
    providers = scatola_extensions_get_for_type (SCATOLA_TYPE_COLUMN_PROVIDER);

    columns = NULL;

    for (l = providers; l != NULL; l = l->next)
    {
        ScatolaColumnProvider *provider;
        GList *provider_columns;

        provider = SCATOLA_COLUMN_PROVIDER (l->data);
        provider_columns = scatola_column_provider_get_columns (provider);
...
get_trash_columns (void)
{
    ...
    return scatola_column_list_copy (columns);
}

GList *
scatola_get_common_columns (void)
{
    static GList *columns = NULL;
...
GList *
scatola_get_all_columns (void)
{
    GList *columns = NULL;

    columns = g_list_concat (scatola_get_common_columns (),
                             get_trash_columns ());
...
GList *
scatola_get_columns_for_file (ScatolaFile *file)
{
    GList *columns;
...
GList *
scatola_column_list_copy (GList *columns)
{
    GList *ret;
    GList *l;
...
void
scatola_column_list_free (GList *columns)
{
    GList *l;
...
static int
column_compare (ScatolaColumn *a, ScatolaColumn *b, char **column_order)
{
    int index_a;
    int index_b;
    char *name;
...
GList *
scatola_sort_columns (GList  *columns,
                   char  **column_order)
{
    if (!column_order)
    {
