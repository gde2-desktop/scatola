...
/* scatola-file-utilities.c - implementation of file manipulation routines.
...
#define DEFAULT_SCATOLA_DIRECTORY_MODE (0755)
...
char *
scatola_compute_title_for_location (GFile *location)
{
    ScatolaFile *file;
    char *title;
    ...
    if (location)
    {
        ...
        title = scatola_file_get_description (file);
        if (title == NULL)
        {
            title = scatola_file_get_display_name (file);
        }
...
/**
 * scatola_get_user_directory:
 *
 * Get the path for the directory containing scatola settings.
 *
...
char* scatola_get_user_directory(void)
{
	 ...
	 * Note that scatola_application_startup will refuse to launch if this
	 * directory doesn't get created, so that case is OK. But the directory
	 * could be deleted after Scatola was launched, and perhaps
	 ...
	 */
	char* user_directory = g_build_filename(g_get_user_config_dir(), "scatola", NULL);
	/* Se necesita que esta dirección sea una carpeta, con los permisos
	 * DEFAULT_SCATOLA_DIRECTORY_MODE. Pero si es un archivo, el programa intentará
	 * eliminar el archivo silenciosamente. */
	if (g_file_test(user_directory, G_FILE_TEST_IS_DIR) == FALSE ||
		g_access(user_directory, DEFAULT_SCATOLA_DIRECTORY_MODE) == -1)
	{
		...
		if (g_file_test(user_directory, G_FILE_TEST_IS_SYMLINK) == TRUE)
		{
			...
			if (link)
			{
				...
				if (g_file_test(link, G_FILE_TEST_IS_DIR) != TRUE ||
					g_chmod(link, DEFAULT_SCATOLA_DIRECTORY_MODE) != 0)
				{
					/* podemos borrar el enlace y crear la carpeta */
					g_unlink(user_directory);
					g_mkdir(user_directory, DEFAULT_SCATOLA_DIRECTORY_MODE);
				}
		...
		{
			g_chmod(user_directory, DEFAULT_SCATOLA_DIRECTORY_MODE);
		}
		...
		else
		{
			...
			g_mkdir_with_parents(user_directory, DEFAULT_SCATOLA_DIRECTORY_MODE);
		}

		/* Faltan permisos */
		if (g_chmod(user_directory, DEFAULT_SCATOLA_DIRECTORY_MODE) != 0)
		{
			GtkWidget* dialog = gtk_message_dialog_new(
				...
				GTK_MESSAGE_ERROR,
				GTK_BUTTONS_CLOSE,
				"The path for the directory containing scatola settings need read and write permissions: %s",
				user_directory);
...
/**
 * scatola_get_accel_map_file:
 *
 * Get the path for the filename containing scatola accelerator map.
...
char* scatola_get_accel_map_file(void)
{
	...
	override = g_getenv ("GDE222_USER_DIR");

	if (override)
	{
		return g_build_filename(override, "accels", "scatola", NULL);
	}
	else
	{
		return g_build_filename(g_get_user_config_dir(), "scatola", "accels", NULL);
	}
}
...
static void
xdg_dir_changed (ScatolaFile *file,
                 XdgDirEntry *dir)
{
...
free_xdg_dir_cache (void)
{
    ...
    if (cached_xdg_dirs != NULL)
    {
        for (i = 0; cached_xdg_dirs[i].type != NULL; i++)
        {
            ...
            {
                scatola_file_monitor_remove (cached_xdg_dirs[i].file,
                                          &cached_xdg_dirs[i]);
                g_signal_handlers_disconnect_by_func (cached_xdg_dirs[i].file,
                                                      G_CALLBACK (xdg_dir_changed),
                                                      &cached_xdg_dirs[i]);
                scatola_file_unref (cached_xdg_dirs[i].file);
            }
...
update_xdg_dir_cache (void)
{
    ...
    for (i = 0 ; cached_xdg_dirs[i].type != NULL; i++)
    {
        ...
        if (strcmp (cached_xdg_dirs[i].path, g_get_home_dir ()) != 0)
        {
            ...
            cached_xdg_dirs[i].file = scatola_file_get_by_uri (uri);
            scatola_file_monitor_add (cached_xdg_dirs[i].file,
                                   &cached_xdg_dirs[i],
...
char *
scatola_get_xdg_dir (const char *type)
{
    int i;
...
{
    if (g_settings_get_boolean (scatola_preferences, SCATOLA_PREFERENCES_DESKTOP_IS_HOME_DIR))
    {
        return g_strdup (g_get_home_dir());
    }
    else
    {
        return scatola_get_xdg_dir ("DESKTOP");
    }
}

/**
 * scatola_get_desktop_directory:
 *
 ...
 **/
char *
scatola_get_desktop_directory (void)
{
...
GFile *
scatola_get_desktop_location (void)
{
    char *desktop_directory;
    GFile *res;
...
/**
 * scatola_get_desktop_directory_uri:
 *
 ...
 **/
char *
scatola_get_desktop_directory_uri (void)
{
    char *desktop_path;
    char *desktop_uri;
...
char *
scatola_get_home_directory_uri (void)
{
...
gboolean
scatola_should_use_templates_directory (void)
{
    char *dir;
    gboolean res;

    dir = scatola_get_xdg_dir ("TEMPLATES");
...
char *
scatola_get_templates_directory (void)
{
    return scatola_get_xdg_dir ("TEMPLATES");
}

void
scatola_create_templates_directory (void)
{
    char *dir;

    dir = scatola_get_templates_directory ();
    if (!g_file_test (dir, G_FILE_TEST_EXISTS))
    {
        g_mkdir (dir, DEFAULT_SCATOLA_DIRECTORY_MODE);
    }
    g_free (dir);
}

char *
scatola_get_templates_directory_uri (void)
{
    char *directory, *uri;

    directory = scatola_get_templates_directory ();
...
gboolean
scatola_is_home_directory_file (GFile *dir,
                             const char *filename)
{
    char *dirname;
...
gboolean
scatola_is_home_directory (GFile *dir)
{
...
gboolean
scatola_is_root_directory (GFile *dir)
{
...
gboolean
scatola_is_desktop_directory_file (GFile *dir,
                                const char *file)
{
...
gboolean
scatola_is_desktop_directory (GFile *dir)
{
...
/**
 * scatola_get_pixmap_directory
 *
 * Get the path for the directory containing Scatola pixmaps.
 *
 * Return value: the directory path.
 **/
char *
scatola_get_pixmap_directory (void)
{
    return g_strdup (DATADIR "/pixmaps/scatola");
}
 ...
 */
char *
scatola_pixmap_file (const char *partial_path)
{
    char *path;

    path = g_build_filename (DATADIR "/pixmaps/scatola", partial_path, NULL);
    ...
    {
        char *tmp;
        tmp = scatola_get_pixmap_directory ();
        g_debug ("Failed to locate \"%s\" in Scatola pixmap path \"%s\". Incomplete installation?", partial_path, tmp);
        g_free (tmp);
    }
    g_free (path);
    return NULL;
...
char *
scatola_get_data_file_path (const char *partial_path)
{
    char *path;
    char *user_directory;
    ...
    path = g_build_filename (SCATOLA_DATADIR, partial_path, NULL);
    if (g_file_test (path, G_FILE_TEST_EXISTS))
    {
        return path;
    }
...
char *
scatola_ensure_unique_file_name (const char *directory_uri,
...
GFile *
scatola_find_existing_uri_in_hierarchy (GFile *location)
{
    GFileInfo *info;
    GFile *tmp;
...
gboolean
scatola_is_engrampa_installed (void)
{
 ...
 * (e.g. whenever Scatola is doing file operations).
 *
 * Returns: an integer cookie, which must be passed to
 *    scatola_uninhibit_power_manager() to resume
 *    normal power management.
 */
int
scatola_inhibit_power_manager (const char *message)
{
...
/**
 * scatola_uninhibit_power_manager:
 * @cookie: the cookie value returned by scatola_inhibit_power_manager()
 *
 ...
 */
void
scatola_uninhibit_power_manager (gint cookie)
{
    GDBusConnection *connection;
    GVariant *result;
    GError *error = NULL;
...
/* Returns TRUE if the file is in XDG_DATA_DIRS or
   in "~/.gde22/". This is used for deciding
   if a desktop file is "trusted" based on the path */
gboolean
scatola_is_in_system_dir (GFile *file)
{
    ...
    if (!res)
    {
        ...
        gde22 = g_build_filename(g_get_home_dir(), ".gde22", NULL);

        if (g_str_has_prefix (path, gde22))
        {
            res = TRUE;
        }
...
GHashTable *
scatola_trashed_files_get_original_directories (GList *files,
        GList **unhandled_files)
{
    GHashTable *directories;
    ScatolaFile *file, *original_file, *original_dir;
    GList *l, *m;
    ...
    for (l = files; l != NULL; l = l->next)
    {
        ...
        original_file = scatola_file_get_trash_original_file (file);

        original_dir = NULL;
        if (original_file != NULL)
        {
            original_dir = scatola_file_get_parent (original_file);
        }
        ...
        if (original_dir != NULL)
        {
            if (directories == NULL)
            {
                directories = g_hash_table_new_full (g_direct_hash, g_direct_equal,
                                                     ...
                                                     (GDestroyNotify) scatola_file_list_unref);
            }
            scatola_file_ref (original_dir);
            ...
            m = g_list_append (m, scatola_file_ref (file));
            g_hash_table_insert (directories, original_dir, m);
        }
        else if (unhandled_files != NULL)
        {
            *unhandled_files = g_list_append (*unhandled_files, scatola_file_ref (file));
        }
...
locations_from_file_list (GList *file_list)
{
    ...
    {
        file = SCATOLA_FILE (l->data);
        ret = g_list_prepend (ret, scatola_file_get_location (file));
    }
...
void
scatola_restore_files_from_trash (GList *files,
                               GtkWindow *parent_window)
{
    ScatolaFile *file, *original_dir;
    ...
    original_dirs_hash = scatola_trashed_files_get_original_directories (files, &unhandled_files);
    ...
    if (original_dirs_hash != NULL)
    {
        ...
        {
            original_dir = SCATOLA_FILE (l->data);
            original_dir_location = scatola_file_get_location (original_dir);

            files = g_hash_table_lookup (original_dirs_hash, original_dir);
            locations = locations_from_file_list (files);

            scatola_file_operations_move
            (locations, NULL,
    ...
    scatola_file_list_unref (unhandled_files);
}

#if !defined (SCATOLA_OMIT_SELF_CHECK)

void
scatola_self_check_file_utilities (void)
{
}
