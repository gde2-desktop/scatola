...
} ScatolaFileChangeKind;

typedef struct
{
    ScatolaFileChangeKind kind;
    GFile *from;
    GFile *to;
    GdkPoint point;
    int screen;
...
typedef struct
{
    ...
    GList *tail;
    GMutex mutex;
} ScatolaFileChangesQueue;

static ScatolaFileChangesQueue *
scatola_file_changes_queue_new (void)
{
    ScatolaFileChangesQueue *result;

    result = g_new0 (ScatolaFileChangesQueue, 1);
...
static ScatolaFileChangesQueue *
scatola_file_changes_queue_get (void)
{
    static ScatolaFileChangesQueue *file_changes_queue;

    if (file_changes_queue == NULL)
    {
        file_changes_queue = scatola_file_changes_queue_new ();
    }
...
static void
scatola_file_changes_queue_add_common (ScatolaFileChangesQueue *queue,
                                    ScatolaFileChange *new_item)
{
...
void
scatola_file_changes_queue_file_added (GFile *location)
{
    ScatolaFileChange *new_item;
    ScatolaFileChangesQueue *queue;

    queue = scatola_file_changes_queue_get();

    new_item = g_new0 (ScatolaFileChange, 1);
    new_item->kind = CHANGE_FILE_ADDED;
    new_item->from = g_object_ref (location);
    scatola_file_changes_queue_add_common (queue, new_item);
}

void
scatola_file_changes_queue_file_changed (GFile *location)
{
    ScatolaFileChange *new_item;
    ScatolaFileChangesQueue *queue;
...
void
scatola_file_changes_queue_file_removed (GFile *location)
{
    ScatolaFileChange *new_item;
    ScatolaFileChangesQueue *queue;
...
void
scatola_file_changes_queue_file_moved (GFile *from,
                                    GFile *to)
{
    ScatolaFileChange *new_item;
    ScatolaFileChangesQueue *queue;
    ...
    new_item = g_new (ScatolaFileChange, 1);
...
void
scatola_file_changes_queue_schedule_position_set (GFile *location,
        GdkPoint point,
        int screen)
{
    ScatolaFileChange *new_item;
...
void
scatola_file_changes_queue_schedule_position_remove (GFile *location)
{
    ScatolaFileChange *new_item;
    ScatolaFileChangesQueue *queue;
...
static ScatolaFileChange *
scatola_file_changes_queue_get_change (ScatolaFileChangesQueue *queue)
{
    GList *new_tail;
    ScatolaFileChange *result;
 ...
 * in a list to the different scatola_directory_notify calls
 */
void
scatola_file_changes_consume_changes (gboolean consume_all)
{
    ScatolaFileChange *change;
    GList *additions, *changes, *deletions, *moves;
    GList *position_set_requests;
    GFilePair *pair;
    ScatolaFileChangesQueuePosition *position_set;
    guint chunk_count;
    ScatolaFileChangesQueue *queue;
    ...
    {
        change = scatola_file_changes_queue_get_change (queue);
        ...
        if (flush_needed)
        {
            ...
            if (deletions != NULL)
            {
                ...
                scatola_directory_notify_files_removed (deletions);
            ...
            if (moves != NULL)
            {
                ...
                scatola_directory_notify_files_moved (moves);
            ...
            if (additions != NULL)
            {
                ...
                scatola_directory_notify_files_added (additions);
            ...
            if (changes != NULL)
            {
                ...
                scatola_directory_notify_files_changed (changes);
            ...
            if (position_set_requests != NULL)
            {
                ...
                scatola_directory_schedule_position_set (position_set_requests);
        ...
        case CHANGE_POSITION_SET:
            position_set = g_new (ScatolaFileChangesQueuePosition, 1);
