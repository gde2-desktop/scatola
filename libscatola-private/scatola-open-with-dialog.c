...
struct _ScatolaOpenWithDialogDetails
{
...
G_DEFINE_TYPE (ScatolaOpenWithDialog, scatola_open_with_dialog, GTK_TYPE_DIALOG);

static void
scatola_open_with_dialog_finalize (GObject *object)
{
    ScatolaOpenWithDialog *dialog;

    dialog = SCATOLA_OPEN_WITH_DIALOG (object);
    ...
    G_OBJECT_CLASS (scatola_open_with_dialog_parent_class)->finalize (object);
}
 ...
 */
static gboolean
check_application (ScatolaOpenWithDialog *dialog)
{
    char *command;
    char *path = NULL;
    char **argv = NULL;
 ...
 */
static GAppInfo *
add_or_find_application (ScatolaOpenWithDialog *dialog)
{
    GAppInfo *app;
    char *app_name;
...
static void
emit_application_selected (ScatolaOpenWithDialog *dialog,
...
static void
response_cb (ScatolaOpenWithDialog *dialog,
             int response_id,
             gpointer data)
{
    GAppInfo *application;
...
static void
scatola_open_with_dialog_class_init (ScatolaOpenWithDialogClass *class)
{
    GObjectClass *gobject_class;

    gobject_class = G_OBJECT_CLASS (class);
    gobject_class->finalize = scatola_open_with_dialog_finalize;

    signals[APPLICATION_SELECTED] =
        g_signal_new ("application_selected",
                      G_TYPE_FROM_CLASS (class),
                      G_SIGNAL_RUN_LAST,
                      G_STRUCT_OFFSET (ScatolaOpenWithDialogClass,
                                       application_selected),
                      NULL, NULL,
                     ...
                     gpointer user_data)
{
    ...
    dialog = SCATOLA_OPEN_WITH_DIALOG (user_data);
...
static gboolean
scatola_open_with_dialog_add_icon_idle (ScatolaOpenWithDialog *dialog)
{
    GtkTreeIter   iter;
    GtkTreePath  *path;
    GdkPixbuf    *pixbuf;
...
static gboolean
scatola_open_with_search_equal_func (GtkTreeModel *model,
                                  int column,
                                  const char *key,
                                  GtkTreeIter *iter,
                                  gpointer user_data)
...
static gboolean
scatola_open_with_dialog_add_items_idle (ScatolaOpenWithDialog *dialog)
{
    ...
    gtk_tree_view_set_search_equal_func (GTK_TREE_VIEW (dialog->details->program_list),
                                         scatola_open_with_search_equal_func,
                                         NULL, NULL);
    ...
    if (!dialog->details->add_icons_idle_id)
    {
        dialog->details->add_icons_idle_id =
            g_idle_add_full (G_PRIORITY_DEFAULT_IDLE, (GSourceFunc) scatola_open_with_dialog_add_icon_idle,
                             dialog, NULL);
    }
...
static void
expander_toggled (GtkWidget *expander, ScatolaOpenWithDialog *dialog)
{
...
static void
scatola_open_with_dialog_init (ScatolaOpenWithDialog *dialog)
{
    GtkWidget *hbox;
    GtkWidget *vbox;
    GtkWidget *vbox2;
    ...
    dialog->details = g_new0 (ScatolaOpenWithDialogDetails, 1);
    ...
    dialog->details->add_items_idle_id = g_idle_add_full (G_PRIORITY_DEFAULT_IDLE,
                                         (GSourceFunc) scatola_open_with_dialog_add_items_idle,
                                         dialog, NULL);
...
static void
set_uri_and_type (ScatolaOpenWithDialog *dialog,
                  const char *uri,
...
static GtkWidget *
real_scatola_open_with_dialog_new (const char *uri,
                                const char *mime_type,
                                ...
                                gboolean add_mode)
{
    ...
    dialog = gtk_widget_new (SCATOLA_TYPE_OPEN_WITH_DIALOG, NULL);

    set_uri_and_type (SCATOLA_OPEN_WITH_DIALOG (dialog), uri, mime_type, extension, add_mode);

    return dialog;
}

GtkWidget *
scatola_open_with_dialog_new (const char *uri,
                           const char *mime_type,
                           const char *extension)
{
    return real_scatola_open_with_dialog_new (uri, mime_type, extension, FALSE);
}

GtkWidget *
scatola_add_application_dialog_new (const char *uri,
                                 const char *mime_type)
{
    ScatolaOpenWithDialog *dialog;

    dialog = SCATOLA_OPEN_WITH_DIALOG (real_scatola_open_with_dialog_new (uri, mime_type, NULL, TRUE));

    return GTK_WIDGET (dialog);
}

GtkWidget *
scatola_add_application_dialog_new_for_multiple_files (const char *extension,
        const char *mime_type)
{
    ScatolaOpenWithDialog *dialog;

    dialog = SCATOLA_OPEN_WITH_DIALOG (real_scatola_open_with_dialog_new (NULL, mime_type, extension, TRUE));
