/* -*- Mode: C; indent-tabs-mode: t; c-basic-offset: 8; tab-width: 8 -*-
   ...
   scatola-directory-async.c: Scatola directory model state machine.
...
struct DirectoryLoadState
{
    ...
    ScatolaFile *load_directory_file;
    int load_file_count;
};
...
typedef struct
{
    ScatolaFile *file; /* Which file, NULL means all. */
    union
    {
        ScatolaDirectoryCallback directory;
        ScatolaFileCallback file;
    } callback;
...
typedef gboolean (* FileCheck) (ScatolaFile *);
...
static void     deep_count_load                               (DeepCountState         *state,
        GFile                  *location);
static gboolean request_is_satisfied                          (ScatolaDirectory      *directory,
        ScatolaFile           *file,
        Request                 request);
static void     cancel_loading_attributes                     (ScatolaDirectory      *directory,
        ScatolaFileAttributes  file_attributes);
static void     add_all_files_to_work_queue                   (ScatolaDirectory      *directory);
static void     link_info_done                                (ScatolaDirectory      *directory,
        ScatolaFile           *file,
        const char             *uri,
        const char             *name,
        const char             *icon,
        ...
        gboolean                is_foreign);
static void     move_file_to_low_priority_queue               (ScatolaDirectory      *directory,
        ScatolaFile           *file);
static void     move_file_to_extension_queue                  (ScatolaDirectory      *directory,
        ScatolaFile           *file);
static void     scatola_directory_invalidate_file_attributes (ScatolaDirectory      *directory,
        ScatolaFileAttributes  file_attributes);

void
scatola_set_kde_trash_name (const char *trash_dir)
{
 ...
 * Move to scatola-glib-extensions?
 */
...
#if 0
static void
scatola_directory_verify_request_counts (ScatolaDirectory *directory)
{
    GList *l;
 ...
 */
static gboolean
async_job_start (ScatolaDirectory *directory,
                 const char *job)
{
#ifdef DEBUG_ASYNC_JOBS
    char *key;
    ...
    if (async_job_count >= MAX_ASYNC_JOBS)
    {
        if (waiting_directories == NULL)
        {
            waiting_directories = eel_g_hash_table_new_free_at_exit
                                  (NULL, NULL,
                                   "scatola-directory-async.c: waiting_directories");
        }
        ...
        if (async_jobs == NULL)
        {
            async_jobs = eel_g_hash_table_new_free_at_exit
                         (g_str_hash, g_str_equal,
                          "scatola-directory-async.c: async_jobs");
        }
...
/* End a job. */
static void
async_job_end (ScatolaDirectory *directory,
               const char *job)
{
#ifdef DEBUG_ASYNC_JOBS
    char *key;
...
async_job_wake_up (void)
{
    ...
    while (async_job_count < MAX_ASYNC_JOBS)
    {
        ...
        scatola_directory_async_state_changed
        (SCATOLA_DIRECTORY (value));
    }
    already_waking_up = FALSE;
}

static void
directory_count_cancel (ScatolaDirectory *directory)
{
...
static void
deep_count_cancel (ScatolaDirectory *directory)
{
    if (directory->details->deep_count_in_progress != NULL)
    {
        g_assert (SCATOLA_IS_FILE (directory->details->deep_count_file));

        g_cancellable_cancel (directory->details->deep_count_in_progress->cancellable);

        directory->details->deep_count_file->details->deep_counts_status = SCATOLA_REQUEST_NOT_STARTED;
...
static void
mime_list_cancel (ScatolaDirectory *directory)
{
...
static void
top_left_cancel (ScatolaDirectory *directory)
{
...
static void
link_info_cancel (ScatolaDirectory *directory)
{
...
static void
thumbnail_cancel (ScatolaDirectory *directory)
{
...
static void
mount_cancel (ScatolaDirectory *directory)
{
...
static void
file_info_cancel (ScatolaDirectory *directory)
{
...
static void
new_files_cancel (ScatolaDirectory *directory)
{
    GList *l;
    NewFilesState *state;
...
static GList *
find_monitor (ScatolaDirectory *directory,
              ScatolaFile *file,
              gconstpointer client)
{
    Monitor monitor;
...
static void
remove_monitor_link (ScatolaDirectory *directory,
                     GList *link)
{
    Monitor *monitor;
...
static void
remove_monitor (ScatolaDirectory *directory,
                ScatolaFile *file,
                gconstpointer client)
{
...
Request
scatola_directory_set_up_request (ScatolaFileAttributes file_attributes)
{
    Request request;

    request = 0;

    if ((file_attributes & SCATOLA_FILE_ATTRIBUTE_DIRECTORY_ITEM_COUNT) != 0)
    {
        REQUEST_SET_TYPE (request, REQUEST_DIRECTORY_COUNT);
    }

    if ((file_attributes & SCATOLA_FILE_ATTRIBUTE_DEEP_COUNTS) != 0)
    {
        REQUEST_SET_TYPE (request, REQUEST_DEEP_COUNT);
    }

    if ((file_attributes & SCATOLA_FILE_ATTRIBUTE_DIRECTORY_ITEM_MIME_TYPES) != 0)
    {
        REQUEST_SET_TYPE (request, REQUEST_MIME_LIST);
    }
    if ((file_attributes & SCATOLA_FILE_ATTRIBUTE_INFO) != 0)
    {
        REQUEST_SET_TYPE (request, REQUEST_FILE_INFO);
    }

    if (file_attributes & SCATOLA_FILE_ATTRIBUTE_LINK_INFO)
    {
        REQUEST_SET_TYPE (request, REQUEST_FILE_INFO);
        REQUEST_SET_TYPE (request, REQUEST_LINK_INFO);
    }

    if (file_attributes & SCATOLA_FILE_ATTRIBUTE_TOP_LEFT_TEXT)
    {
        REQUEST_SET_TYPE (request, REQUEST_TOP_LEFT_TEXT);
        REQUEST_SET_TYPE (request, REQUEST_FILE_INFO);
    }

    if (file_attributes & SCATOLA_FILE_ATTRIBUTE_LARGE_TOP_LEFT_TEXT)
    {
        REQUEST_SET_TYPE (request, REQUEST_LARGE_TOP_LEFT_TEXT);
        REQUEST_SET_TYPE (request, REQUEST_FILE_INFO);
    }

    if ((file_attributes & SCATOLA_FILE_ATTRIBUTE_EXTENSION_INFO) != 0)
    {
        REQUEST_SET_TYPE (request, REQUEST_EXTENSION_INFO);
    }

    if (file_attributes & SCATOLA_FILE_ATTRIBUTE_THUMBNAIL)
    {
        REQUEST_SET_TYPE (request, REQUEST_THUMBNAIL);
        REQUEST_SET_TYPE (request, REQUEST_FILE_INFO);
    }

    if (file_attributes & SCATOLA_FILE_ATTRIBUTE_MOUNT)
    {
        REQUEST_SET_TYPE (request, REQUEST_MOUNT);
        REQUEST_SET_TYPE (request, REQUEST_FILE_INFO);
    }

    if (file_attributes & SCATOLA_FILE_ATTRIBUTE_FILESYSTEM_INFO)
    {
...
static void
mime_db_changed_callback (GObject *ignore, ScatolaDirectory *dir)
{
    ScatolaFileAttributes attrs;

    g_assert (dir != NULL);
    g_assert (dir->details != NULL);

    attrs = SCATOLA_FILE_ATTRIBUTE_INFO |
            SCATOLA_FILE_ATTRIBUTE_LINK_INFO |
            SCATOLA_FILE_ATTRIBUTE_DIRECTORY_ITEM_MIME_TYPES;

    scatola_directory_force_reload_internal (dir, attrs);
}

void
scatola_directory_monitor_add_internal (ScatolaDirectory *directory,
                                     ScatolaFile *file,
                                     gconstpointer client,
                                     ...
                                     gpointer callback_data)
{
    ...
    monitor->client = client;
    monitor->request = scatola_directory_set_up_request (file_attributes);
     ...
     * scatola almost always shows the whole directory anyway, and
     * it allows us to avoid one file monitor per file in a directory.
     */
    if (directory->details->monitor == NULL)
    {
        directory->details->monitor = scatola_monitor_directory (directory->details->location);
    }
    ...
    if (file != NULL)
    {
        scatola_directory_add_file_to_work_queue (directory, file);
    }
    else
    {
    ...
    /* Kick off I/O. */
    scatola_directory_async_state_changed (directory);
}

static void
set_file_unconfirmed (ScatolaFile *file, gboolean unconfirmed)
{
    ScatolaDirectory *directory;
...
{
    show_hidden_files = g_settings_get_boolean (scatola_preferences, SCATOLA_PREFERENCES_SHOW_HIDDEN_FILES);
}

static gboolean
should_skip_file (ScatolaDirectory *directory, GFileInfo *info)
{
    ...
    if (!show_hidden_files_changed_callback_installed)
    {
        g_signal_connect_swapped (scatola_preferences,
                                  "changed::" SCATOLA_PREFERENCES_SHOW_HIDDEN_FILES,
...
dequeue_pending_idle_callback (gpointer callback_data)
{
    ...
    directory = SCATOLA_DIRECTORY (callback_data);
    ...
    if (!scatola_directory_is_file_list_monitored (directory))
    {
        scatola_directory_async_state_changed (directory);
        goto drain;
    }
    ...
    /* Build a list of ScatolaFile objects. */
    for (node = pending_file_info; node != NULL; node = node->next)
    {
        ...
        file = scatola_directory_find_file_by_name (directory, name);
        if (file != NULL)
        {
            ...
            if (!file->details->is_added)
            {
                 ...
                 * This can happen if someone called scatola_file_get_by_uri()
            ...
            else if (scatola_file_update_info (file, file_info))
            {
        ...
        {
            /* new file, create a scatola file object and add it to the list */
            file = scatola_file_new_from_info (directory, file_info);
    ...
    scatola_directory_emit_change_signals (directory, changed_files);
    scatola_file_list_free (changed_files);
    scatola_directory_emit_files_added (directory, added_files);
    scatola_file_list_free (added_files);

    if (directory->details->directory_loaded &&
            !directory->details->directory_loaded_sent_notification)
    {
        /* Send the done_loading signal. */
        scatola_directory_emit_done_loading (directory);
...
void
scatola_directory_schedule_dequeue_pending (ScatolaDirectory *directory)
{
...
static void
directory_load_one (ScatolaDirectory *directory,
                    GFileInfo *info)
{
    if (info == NULL)
    {
    ...
    scatola_directory_schedule_dequeue_pending (directory);
}

static void
directory_load_cancel (ScatolaDirectory *directory)
{
    ScatolaFile *file;
...
static void
file_list_cancel (ScatolaDirectory *directory)
{
...
static void
directory_load_done (ScatolaDirectory *directory,
                     GError *error)
{
    GList *node;
    ...
    if (error != NULL)
    {
        ...
        {
            set_file_unconfirmed (SCATOLA_FILE (node->data), FALSE);
        }

        scatola_directory_emit_load_error (directory, error);
    }
...
void
scatola_directory_monitor_remove_internal (ScatolaDirectory *directory,
                                        ScatolaFile *file,
                                        gconstpointer client)
{
    g_assert (SCATOLA_IS_DIRECTORY (directory));
    g_assert (file == NULL || SCATOLA_IS_FILE (file));
    ...
    {
        scatola_monitor_cancel (directory->details->monitor);
...
FileMonitors *
scatola_directory_remove_file_monitors (ScatolaDirectory *directory,
                                     ScatolaFile *file)
{
...
void
scatola_directory_add_file_monitors (ScatolaDirectory *directory,
                                  ScatolaFile *file,
...
static void
ready_callback_call (ScatolaDirectory *directory,
...
void
scatola_directory_call_when_ready_internal (ScatolaDirectory *directory,
        ScatolaFile *file,
        ScatolaFileAttributes file_attributes,
        gboolean wait_for_file_list,
        ScatolaDirectoryCallback directory_callback,
        ScatolaFileCallback file_callback,
        gpointer callback_data)
{
    ReadyCallback callback;

    g_assert (directory == NULL || SCATOLA_IS_DIRECTORY (directory));
    g_assert (file == NULL || SCATOLA_IS_FILE (file));
    ...
    callback.request = scatola_directory_set_up_request (file_attributes);
...
gboolean
scatola_directory_check_if_ready_internal (ScatolaDirectory *directory,
                                        ScatolaFile *file,
                                        ScatolaFileAttributes file_attributes)
{
    Request request;
...
static void
remove_callback_link_keep_data (ScatolaDirectory *directory,
                                GList *link)
{
    ReadyCallback *callback;
...
static void
remove_callback_link (ScatolaDirectory *directory,
                      GList *link)
{
    ReadyCallback *callback;
...
void
scatola_directory_cancel_callback_internal (ScatolaDirectory *directory,
        ScatolaFile *file,
        ScatolaDirectoryCallback directory_callback,
        ScatolaFileCallback file_callback,
        gpointer callback_data)
                    ...
                    gpointer user_data)
{
    ...
    directory = scatola_directory_ref (state->directory);
...
void
scatola_directory_get_info_for_new_files (ScatolaDirectory *directory,
                                       GList *location_list)
{
    NewFilesState *state;
    GFile *location;
...
void
scatola_async_destroying_file (ScatolaFile *file)
{
    ScatolaDirectory *directory;
    gboolean changed;
    GList *node, *next;
...
static gboolean
lacks_directory_count (ScatolaFile *file)
{
    return !file->details->directory_count_is_up_to_date
           && scatola_file_should_show_directory_item_count (file);
}

static gboolean
should_get_directory_count_now (ScatolaFile *file)
{
...
static gboolean
lacks_top_left (ScatolaFile *file)
{
    return file->details->file_info_is_up_to_date &&
           !file->details->top_left_text_is_up_to_date
           && scatola_file_should_get_top_left_text (file);
}

static gboolean
lacks_large_top_left (ScatolaFile *file)
{
    return file->details->file_info_is_up_to_date &&
           (!file->details->top_left_text_is_up_to_date ||
...
static gboolean
lacks_filesystem_info (ScatolaFile *file)
{
    return !file->details->filesystem_info_is_up_to_date;
}

static gboolean
lacks_deep_count (ScatolaFile *file)
{
    return file->details->deep_counts_status != SCATOLA_REQUEST_DONE;
}

static gboolean
lacks_mime_list (ScatolaFile *file)
{
    return !file->details->mime_list_is_up_to_date;
}

static gboolean
should_get_mime_list (ScatolaFile *file)
{
...
static gboolean
lacks_link_info (ScatolaFile *file)
{
...
static gboolean
lacks_extension_info (ScatolaFile *file)
{
    return file->details->pending_info_providers != NULL;
}

static gboolean
lacks_thumbnail (ScatolaFile *file)
{
    return scatola_file_should_show_thumbnail (file) &&
           file->details->thumbnail_path != NULL &&
           !file->details->thumbnail_is_up_to_date;
}

static gboolean
lacks_mount (ScatolaFile *file)
{
    return (!file->details->mount_is_up_to_date &&
            (
                ...
                (file->details->type == G_FILE_TYPE_DIRECTORY &&
                 scatola_file_is_self_owned (file)) ||
...
static gboolean
has_problem (ScatolaDirectory *directory, ScatolaFile *file, FileCheck problem)
{
    GList *node;
...
static gboolean
request_is_satisfied (ScatolaDirectory *directory,
                      ScatolaFile *file,
                      Request request)
{
...
static void
schedule_call_ready_callbacks (ScatolaDirectory *directory)
{
 ...
 * before then */
static gboolean
call_ready_callbacks (ScatolaDirectory *directory)
{
    gboolean found_any;
    GList *node, *next;
    ReadyCallback *callback;
...
gboolean
scatola_directory_has_active_request_for_file (ScatolaDirectory *directory,
        ScatolaFile *file)
{
    GList *node;
    ReadyCallback *callback;
...
gboolean
scatola_directory_is_anyone_monitoring_file_list (ScatolaDirectory *directory)
{
...
gboolean
scatola_directory_is_file_list_monitored (ScatolaDirectory *directory)
{
    return directory->details->file_list_monitored;
}

static void
mark_all_files_unconfirmed (ScatolaDirectory *directory)
{
    GList *node;
    ScatolaFile *file;
...
static void
read_dot_hidden_file (ScatolaDirectory *directory)
{
    gsize file_size;
    char *file_contents;
    GFile *child;
...
directory_load_state_free (DirectoryLoadState *state)
{
    ...
    scatola_file_unref (state->load_directory_file);
    g_object_unref (state->cancellable);
    g_free (state);
}
...
static void
start_monitoring_file_list (ScatolaDirectory *directory)
{
    DirectoryLoadState *state;
    ...
    if (!directory->details->file_list_monitored)
    {
        ...
        scatola_file_list_ref (directory->details->file_list);
    }
    ...
    if (kde_trash_dir_name != NULL && scatola_directory_is_desktop_directory (directory))
    {
        char *fn;
...
void
scatola_directory_stop_monitoring_file_list (ScatolaDirectory *directory)
{
    if (!directory->details->file_list_monitored)
    {
        g_assert (directory->details->directory_load_in_progress == NULL);
    ...
    scatola_file_list_unref (directory->details->file_list);
    directory->details->directory_loaded = FALSE;
}

static void
file_list_start_or_stop (ScatolaDirectory *directory)
{
    if (scatola_directory_is_anyone_monitoring_file_list (directory))
    {
        start_monitoring_file_list (directory);
    }
    else
    {
        scatola_directory_stop_monitoring_file_list (directory);
    }
}

void
scatola_file_invalidate_count_and_mime_list (ScatolaFile *file)
{
    ScatolaFileAttributes attributes;

    attributes = SCATOLA_FILE_ATTRIBUTE_DIRECTORY_ITEM_COUNT |
                 SCATOLA_FILE_ATTRIBUTE_DIRECTORY_ITEM_MIME_TYPES;

    scatola_file_invalidate_attributes (file, attributes);
}
 ...
 */
void
scatola_directory_invalidate_count_and_mime_list (ScatolaDirectory *directory)
{
    ScatolaFile *file;
    ...
    if (file != NULL)
    {
        scatola_file_invalidate_count_and_mime_list (file);
    }
...
static void
scatola_directory_invalidate_file_attributes (ScatolaDirectory      *directory,
        ScatolaFileAttributes  file_attributes)
{
    GList *node;
    ...
    for (node = directory->details->file_list; node != NULL; node = node->next)
    {
        scatola_file_invalidate_attributes_internal (SCATOLA_FILE (node->data),
                file_attributes);
    }

    if (directory->details->as_file != NULL)
    {
        scatola_file_invalidate_attributes_internal (directory->details->as_file,
                file_attributes);
    }
}

void
scatola_directory_force_reload_internal (ScatolaDirectory     *directory,
                                      ScatolaFileAttributes file_attributes)
{
    /* invalidate attributes that are getting reloaded for all files */
    scatola_directory_invalidate_file_attributes (directory, file_attributes);
    ...
    scatola_directory_invalidate_count_and_mime_list (directory);
...
static void
directory_count_stop (ScatolaDirectory *directory)
{
    ScatolaFile *file;
...
static void
count_children_done (ScatolaDirectory *directory,
                     ScatolaFile *count_file,
                     gboolean succeeded,
                     int count)
{
    g_assert (SCATOLA_IS_FILE (count_file));
     ...
     */
    scatola_file_changed (count_file);
...
directory_count_state_free (DirectoryCountState *state)
{
    ...
    g_object_unref (state->cancellable);
    scatola_directory_unref (state->directory);
    g_free (state);
}
...
static void
directory_count_start (ScatolaDirectory *directory,
                       ScatolaFile *file,
                       gboolean *doing_io)
{
    DirectoryCountState *state;
    ...
    *doing_io = TRUE;
    ...
    state->directory = scatola_directory_ref (directory);
    state->cancellable = g_cancellable_new ();
...
deep_count_next_dir (DeepCountState *state)
{
    ...
    {
        file->details->deep_counts_status = SCATOLA_REQUEST_DONE;
        directory->details->deep_count_file = NULL;
        directory->details->deep_count_in_progress = NULL;
    ...
    scatola_file_updated_deep_count_in_progress (file);
...
static void
deep_count_stop (ScatolaDirectory *directory)
{
    ScatolaFile *file;
...
static void
deep_count_start (ScatolaDirectory *directory,
                  ScatolaFile *file,
                  gboolean *doing_io)
{
    GFile *location;
    ...
    *doing_io = TRUE;
    ...
    /* Start counting. */
    file->details->deep_counts_status = SCATOLA_REQUEST_IN_PROGRESS;
...
static void
mime_list_stop (ScatolaDirectory *directory)
{
    ScatolaFile *file;
...
static void
mime_list_start (ScatolaDirectory *directory,
                 ScatolaFile *file,
                 gboolean *doing_io)
{
    MimeListState *state;
...
static void
top_left_stop (ScatolaDirectory *directory)
{
    ScatolaFile *file;
                        ...
                        gpointer callback_data)
{
    ...
    ScatolaDirectory *directory;
    ScatolaFileDetails *file_details;
    gsize file_size;
    char *file_contents;
    ...
    if (g_file_load_partial_contents_finish (G_FILE (source_object),
            ...
            NULL, NULL))
    {
        file_details->top_left_text = scatola_extract_top_left_text (file_contents, state->large, file_size);
    ...
    scatola_file_changed (state->file);
                             ...
                             gpointer callback_data)
{
    ...
    if (state->large)
    {
        return bytes_read < SCATOLA_FILE_LARGE_TOP_LEFT_TEXT_MAXIMUM_BYTES &&
               count_lines (file_contents, bytes_read) <= SCATOLA_FILE_LARGE_TOP_LEFT_TEXT_MAXIMUM_LINES;
    }
    else
    {
        return bytes_read < SCATOLA_FILE_TOP_LEFT_TEXT_MAXIMUM_BYTES &&
               count_lines (file_contents, bytes_read) <= SCATOLA_FILE_TOP_LEFT_TEXT_MAXIMUM_LINES;
    }
}

static void
top_left_start (ScatolaDirectory *directory,
                ScatolaFile *file,
                gboolean *doing_io)
{
    GFile *location;
                     ...
                     gpointer user_data)
{
    ...
    g_assert (SCATOLA_IS_FILE (get_info_file));
     ...
     */
    scatola_file_ref (get_info_file);
    ...
    if (info == NULL)
    {
        ...
        {
            /* mark file as gone */
            scatola_file_mark_gone (get_info_file);
        }
        get_info_file->details->file_info_is_up_to_date = TRUE;
        scatola_file_clear_info (get_info_file);
        get_info_file->details->get_info_failed = TRUE;
        get_info_file->details->get_info_error = error;
    }
    else
    {
        scatola_file_update_info (get_info_file, info);
        g_object_unref (info);
    }

    scatola_file_changed (get_info_file);
    scatola_file_unref (get_info_file);
...
static void
file_info_stop (ScatolaDirectory *directory)
{
    ScatolaFile *file;
...
static void
file_info_start (ScatolaDirectory *directory,
                 ScatolaFile *file,
                 gboolean *doing_io)
{
    GFile *location;
...
static gboolean
is_link_trusted (ScatolaFile *file,
                 gboolean is_launcher)
{
    GFile *location;
    gboolean res;
    ...
    if (scatola_file_can_execute (file))
    {
        return TRUE;
    }
    ...
    {
        location = scatola_file_get_location (file);
        res = scatola_is_in_system_dir (location);
        g_object_unref (location);
    }
...
static void
link_info_done (ScatolaDirectory *directory,
                ScatolaFile *file,
                const char *uri,
                const char *name,
                const char *icon,
                ...
                gboolean is_foreign)
{
    ...
    if (is_trusted)
    {
        scatola_file_set_display_name (file, name, name, TRUE);
    }
    else
    {
        scatola_file_set_display_name (file, NULL, NULL, TRUE);
    }
...
static gboolean
should_read_link_info_sync (ScatolaFile *file)
{
#ifdef READ_LOCAL_LINKS_SYNC
    return (scatola_file_is_local (file) && !scatola_file_is_directory (file));
#else
    return FALSE;
#endif
}
...
static void
link_info_stop (ScatolaDirectory *directory)
{
    ScatolaFile *file;
...
static void
link_info_got_data (ScatolaDirectory *directory,
                    ScatolaFile *file,
                    gboolean result,
                    goffset bytes_read,
                    char *file_contents)
{
    ...
    /* Handle the case where we read the Scatola link. */
    if (result)
    {
        link_uri = scatola_file_get_uri (file);
        scatola_link_get_link_info_given_file_contents (file_contents, bytes_read, link_uri,
...
static void
link_info_scatola_link_read_callback (GObject *source_object,
                                   GAsyncResult *res,
                                   gpointer user_data)
{
...
static void
link_info_start (ScatolaDirectory *directory,
                 ScatolaFile *file,
                 gboolean *doing_io)
{
    GFile *location;
    gboolean scatola_style_link;
    gsize file_size;
    char *file_contents;
    gboolean result;
    LinkInfoReadState *state;
    ...
    *doing_io = TRUE;
    ...
    scatola_style_link = scatola_file_is_scatola_link (file);
    location = scatola_file_get_location (file);

    /* If it's not a link we are done. If it is, we need to read it. */
    if (!scatola_style_link)
    {
    ...
    else
    {
        ...
        g_file_load_contents_async (location,
                                    state->cancellable,
                                    link_info_scatola_link_read_callback,
                                    state);
    }
    g_object_unref (location);
}
...
static void
thumbnail_done (ScatolaDirectory *directory,
                ScatolaFile *file,
                GdkPixbuf *pixbuf,
                gboolean tried_original)
{
...
static void
thumbnail_stop (ScatolaDirectory *directory)
{
    ScatolaFile *file;
...
static void
thumbnail_got_pixbuf (ScatolaDirectory *directory,
                      ScatolaFile *file,
                      GdkPixbuf *pixbuf,
                      gboolean tried_original)
{
                                ...
                                gpointer user_data)
{
    ...
    /* cf. scatola_file_get_icon() */
    max_thumbnail_size = SCATOLA_ICON_SIZE_LARGEST * cached_thumbnail_size / SCATOLA_ICON_SIZE_STANDARD;
...
static void
thumbnail_start (ScatolaDirectory *directory,
                 ScatolaFile *file,
                 gboolean *doing_io)
{
    GFile *location;
...
static void
mount_stop (ScatolaDirectory *directory)
{
    ScatolaFile *file;
...
got_mount (MountState *state, GMount *mount)
{
    ...
    file = scatola_file_ref (state->file);

    file->details->mount_is_up_to_date = TRUE;
    scatola_file_set_mount (file, mount);
                               ...
                               gpointer user_data)
{
    ...
    if (mount)
    {
        ...
        location = scatola_file_get_location (state->file);
...
static void
mount_start (ScatolaDirectory *directory,
             ScatolaFile *file,
             gboolean *doing_io)
{
    GFile *location;
    ...
    *doing_io = TRUE;
    ...
    if (file->details->type == G_FILE_TYPE_MOUNTABLE)
    {
        ...
        mount = NULL;
        target = scatola_file_get_activation_location (file);
        if (target != NULL)
        {
...
static void
filesystem_info_cancel (ScatolaDirectory *directory)
{
    if (directory->details->filesystem_info_state != NULL)
    {
...
static void
filesystem_info_stop (ScatolaDirectory *directory)
{
    ScatolaFile *file;
...
static void
filesystem_info_start (ScatolaDirectory *directory,
                       ScatolaFile *file,
                       gboolean *doing_io)
{
    GFile *location;
...
static void
extension_info_cancel (ScatolaDirectory *directory)
{
    if (directory->details->extension_info_in_progress != NULL)
    {
        ...
        {
            scatola_info_provider_cancel_update
...
static void
extension_info_stop (ScatolaDirectory *directory)
{
    if (directory->details->extension_info_in_progress != NULL)
    {
        ScatolaFile *file;
...
static void
finish_info_provider (ScatolaDirectory *directory,
                      ScatolaFile *file,
                      ScatolaInfoProvider *provider)
{
    ...
    {
        scatola_file_info_providers_done (file);
    }
}
...
info_provider_idle_callback (gpointer user_data)
{
    ...
    {
        g_warning ("Unexpected plugin response.  This probably indicates a bug in a Scatola extension: handle=%p", response->handle);
    }
    else
    {
        ScatolaFile *file;
...
static void
info_provider_callback (ScatolaInfoProvider *provider,
                        ScatolaOperationHandle *handle,
                        ScatolaOperationResult result,
                        gpointer user_data)
{
    ...
    response->directory = SCATOLA_DIRECTORY (user_data);
...
static void
extension_info_start (ScatolaDirectory *directory,
                      ScatolaFile *file,
                      gboolean *doing_io)
{
    ScatolaInfoProvider *provider;
    ...
    *doing_io = TRUE;
    ...
    g_closure_set_marshal (update_complete,
                           scatola_marshal_VOID__POINTER_ENUM);

    result = scatola_info_provider_update_file_info
             (provider,
              SCATOLA_FILE_INFO (file),
              update_complete,
              &handle);

    g_closure_unref (update_complete);

    if (result == SCATOLA_OPERATION_COMPLETE ||
            result == SCATOLA_OPERATION_FAILED)
    {
...
static void
start_or_stop_io (ScatolaDirectory *directory)
{
    ScatolaFile *file;
    gboolean doing_io;
    ...
    while (!scatola_file_queue_is_empty (directory->details->high_priority_queue))
    {
        file = scatola_file_queue_head (directory->details->high_priority_queue);
    ...
    while (!scatola_file_queue_is_empty (directory->details->low_priority_queue))
    {
        file = scatola_file_queue_head (directory->details->low_priority_queue);
    ...
    while (!scatola_file_queue_is_empty (directory->details->extension_queue))
    {
        file = scatola_file_queue_head (directory->details->extension_queue);
        ...
        scatola_directory_remove_file_from_work_queue (directory, file);
    }
}
 ...
 */
void
scatola_directory_async_state_changed (ScatolaDirectory *directory)
{
...
void
scatola_directory_cancel (ScatolaDirectory *directory)
{
...
static void
cancel_directory_count_for_file (ScatolaDirectory *directory,
                                 ScatolaFile      *file)
{
    if (directory->details->count_in_progress != NULL &&
            directory->details->count_in_progress->count_file == file)
...
static void
cancel_deep_counts_for_file (ScatolaDirectory *directory,
                             ScatolaFile      *file)
{
    if (directory->details->deep_count_file == file)
    {
...
static void
cancel_mime_list_for_file (ScatolaDirectory *directory,
                           ScatolaFile      *file)
{
    if (directory->details->mime_list_in_progress != NULL &&
            directory->details->mime_list_in_progress->mime_list_file == file)
...
static void
cancel_top_left_text_for_file (ScatolaDirectory *directory,
                               ScatolaFile      *file)
{
    if (directory->details->top_left_read_state != NULL &&
            directory->details->top_left_read_state->file == file)
...
static void
cancel_file_info_for_file (ScatolaDirectory *directory,
                           ScatolaFile      *file)
{
    if (directory->details->get_info_file == file)
    {
...
static void
cancel_thumbnail_for_file (ScatolaDirectory *directory,
                           ScatolaFile      *file)
{
    if (directory->details->thumbnail_state != NULL &&
            directory->details->thumbnail_state->file == file)
...
static void
cancel_mount_for_file (ScatolaDirectory *directory,
                       ScatolaFile      *file)
{
    if (directory->details->mount_state != NULL &&
            directory->details->mount_state->file == file)
...
static void
cancel_filesystem_info_for_file (ScatolaDirectory *directory,
                                 ScatolaFile      *file)
{
    if (directory->details->filesystem_info_state != NULL &&
            directory->details->filesystem_info_state->file == file)
...
static void
cancel_link_info_for_file (ScatolaDirectory *directory,
                           ScatolaFile      *file)
{
    if (directory->details->link_info_read_state != NULL &&
            directory->details->link_info_read_state->file == file)
...
static void
cancel_loading_attributes (ScatolaDirectory *directory,
                           ScatolaFileAttributes file_attributes)
{
    Request request;
...
void
scatola_directory_cancel_loading_file_attributes (ScatolaDirectory      *directory,
        ScatolaFile           *file,
        ScatolaFileAttributes  file_attributes)
{
    Request request;
...
void
scatola_directory_add_file_to_work_queue (ScatolaDirectory *directory,
                                       ScatolaFile *file)
{
    g_return_if_fail (file->details->directory == directory);

    scatola_file_queue_enqueue (directory->details->high_priority_queue,
                             file);
}


static void
add_all_files_to_work_queue (ScatolaDirectory *directory)
{
    GList *node;
    ScatolaFile *file;
...
void
scatola_directory_remove_file_from_work_queue (ScatolaDirectory *directory,
        ScatolaFile *file)
{
    scatola_file_queue_remove (directory->details->high_priority_queue,
                            file);
    scatola_file_queue_remove (directory->details->low_priority_queue,
                            file);
    scatola_file_queue_remove (directory->details->extension_queue,
                            file);
}


static void
move_file_to_low_priority_queue (ScatolaDirectory *directory,
                                 ScatolaFile *file)
{
    /* Must add before removing to avoid ref underflow */
    scatola_file_queue_enqueue (directory->details->low_priority_queue,
                             file);
    scatola_file_queue_remove (directory->details->high_priority_queue,
                            file);
}
...
static void
move_file_to_extension_queue (ScatolaDirectory *directory,
                              ScatolaFile *file)
{
    /* Must add before removing to avoid ref underflow */
    scatola_file_queue_enqueue (directory->details->extension_queue,
                             file);
    scatola_file_queue_remove (directory->details->low_priority_queue,
                            file);
}
