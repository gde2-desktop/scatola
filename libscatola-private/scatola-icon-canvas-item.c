...
#include <config.h>
#include <math.h>
#include "scatola-icon-canvas-item.h"
...
/* Private part of the ScatolaIconCanvasItem structure. */
struct ScatolaIconCanvasItemDetails
{
    /* The image, text, font. */
    double x, y;
    GdkPixbuf *pixbuf;
...
typedef struct
{
    ScatolaIconCanvasItem *icon_item;
    EelIRect icon_rect;
    RectangleSide side;
    int position;
    int index;
...
static void scatola_icon_canvas_item_text_interface_init (EelAccessibleTextIface *iface);
static GType scatola_icon_canvas_item_accessible_factory_get_type (void);

G_DEFINE_TYPE_WITH_CODE (ScatolaIconCanvasItem, scatola_icon_canvas_item, EEL_TYPE_CANVAS_ITEM,
                         G_IMPLEMENT_INTERFACE (EEL_TYPE_ACCESSIBLE_TEXT,
                                 scatola_icon_canvas_item_text_interface_init));

/* private */
static void     draw_label_text                      (ScatolaIconCanvasItem        *item,
    						      ...
    						      EelIRect                  icon_rect);
static void     measure_label_text                   (ScatolaIconCanvasItem        *item);
static void     get_icon_canvas_rectangle            (ScatolaIconCanvasItem        *item,
    						      EelIRect                  *rect);
...
static PangoLayout *get_label_layout                 (PangoLayout               **layout,
    						      ScatolaIconCanvasItem        *item,
    						      const char                *text);
#if !GTK_CHECK_VERSION(3,0,0)
static void     draw_label_layout                    (ScatolaIconCanvasItem        *item,
    						      GdkDrawable               *drawable,
    						      PangoLayout               *layout,
    						      gboolean                  highlight,
    						      ...
    						      int                       y);
#endif
static gboolean hit_test_stretch_handle              (ScatolaIconCanvasItem        *item,
    						      EelIRect                  canvas_rect,
    						      GtkCornerType *corner);
static void      draw_embedded_text                  (ScatolaIconCanvasItem        *icon_item,
#if GTK_CHECK_VERSION(3,0,0)
    						      cairo_t                   *cr,
#else
    						      GdkDrawable               *drawable,
...
static void       scatola_icon_canvas_item_ensure_bounds_up_to_date (ScatolaIconCanvasItem *icon_item);


/* Object initialization function for the icon item. */
static void
scatola_icon_canvas_item_init (ScatolaIconCanvasItem *icon_item)
{
    ...
    if (!setup_auto_enums)
    {
        eel_g_settings_add_auto_enum
             (scatola_preferences,
             SCATOLA_PREFERENCES_CLICK_POLICY,
             &click_policy_auto_value);
        setup_auto_enums = TRUE;
    }

    icon_item->details = G_TYPE_INSTANCE_GET_PRIVATE ((icon_item), SCATOLA_TYPE_ICON_CANVAS_ITEM, ScatolaIconCanvasItemDetails);
    scatola_icon_canvas_item_invalidate_label_size (icon_item);
}

static void
scatola_icon_canvas_item_finalize (GObject *object)
{
    ScatolaIconCanvasItemDetails *details;

    g_assert (SCATOLA_IS_ICON_CANVAS_ITEM (object));

    details = SCATOLA_ICON_CANVAS_ITEM (object)->details;
    ...
    G_OBJECT_CLASS (scatola_icon_canvas_item_parent_class)->finalize (object);
}
...
static void
scatola_icon_canvas_item_invalidate_bounds_cache (ScatolaIconCanvasItem *item)
{
...
void
scatola_icon_canvas_item_invalidate_label_size (ScatolaIconCanvasItem *item)
{
    ...
    scatola_icon_canvas_item_invalidate_bounds_cache (item);
...
static void
scatola_icon_canvas_item_set_property (GObject        *object,
                                    guint           property_id,
                                    const GValue   *value,
                                    GParamSpec     *pspec)
{
    ...
    item = SCATOLA_ICON_CANVAS_ITEM (object);
    ...
    case PROP_EDITABLE_TEXT:
        ...
        scatola_icon_canvas_item_invalidate_label_size (item);
    ...
    default:
        g_warning ("scatola_icons_view_item_item_set_arg on unknown argument");
        return;
    }
...
static void
scatola_icon_canvas_item_get_property (GObject        *object,
                                    guint           property_id,
                                    GValue         *value,
                                    GParamSpec     *pspec)
{
...
#if GTK_CHECK_VERSION(3,0,0)
cairo_surface_t *
scatola_icon_canvas_item_get_drag_surface (ScatolaIconCanvasItem *item)
#else
GdkPixmap *
scatola_icon_canvas_item_get_image (ScatolaIconCanvasItem *item,
                                 GdkBitmap **mask,
...
#endif
{
    ...
    g_return_val_if_fail (SCATOLA_IS_ICON_CANVAS_ITEM (item), NULL);
    ...
    gtk_style_context_add_class (context, "scatola-canvas-item");
#else
    ...
    is_rtl = scatola_icon_container_is_layout_rtl (SCATOLA_ICON_CONTAINER (canvas));
...
void
scatola_icon_canvas_item_set_image (ScatolaIconCanvasItem *item,
                                 GdkPixbuf *image)
{
    ScatolaIconCanvasItemDetails *details;

    g_return_if_fail (SCATOLA_IS_ICON_CANVAS_ITEM (item));
...
void
scatola_icon_canvas_item_set_emblems (ScatolaIconCanvasItem *item,
                                   GList *emblem_pixbufs)
{
    GList *p;
...
void
scatola_icon_canvas_item_set_attach_points (ScatolaIconCanvasItem *item,
        GdkPoint *attach_points,
        int n_attach_points)
{
...
void
scatola_icon_canvas_item_set_embedded_text_rect (ScatolaIconCanvasItem       *item,
...
void
scatola_icon_canvas_item_set_embedded_text (ScatolaIconCanvasItem       *item,
        const char                   *text)
{
 ...
 */
static void
recompute_bounding_box (ScatolaIconCanvasItem *icon_item,
...
static EelIRect
compute_text_rectangle (const ScatolaIconCanvasItem *item,
                        ...
                        ScatolaIconCanvasItemBoundsUsage usage)
{
    ...
    if (SCATOLA_ICON_CONTAINER (EEL_CANVAS_ITEM (item)->canvas)->details->label_position == SCATOLA_ICON_LABEL_POSITION_BESIDE)
    {
        if (!scatola_icon_container_is_layout_rtl (SCATOLA_ICON_CONTAINER (EEL_CANVAS_ITEM (item)->canvas)))
        {
...
void
scatola_icon_canvas_item_update_bounds (ScatolaIconCanvasItem *item,
                                     double i2w_dx, double i2w_dy)
{
    ...
    is_rtl = scatola_icon_container_is_layout_rtl (SCATOLA_ICON_CONTAINER (canvas_item->canvas));
...
static void
scatola_icon_canvas_item_update (EelCanvasItem *item,
                              double i2w_dx, double i2w_dy,
                              gint flags)
{
    scatola_icon_canvas_item_update_bounds (SCATOLA_ICON_CANVAS_ITEM (item), i2w_dx, i2w_dy);

    eel_canvas_item_request_redraw (EEL_CANVAS_ITEM (item));

    EEL_CANVAS_ITEM_CLASS (scatola_icon_canvas_item_parent_class)->update (item, i2w_dx, i2w_dy, flags);
}
...
{
    return click_policy_auto_value == SCATOLA_CLICK_POLICY_SINGLE;
}
...
static void
draw_frame (ScatolaIconCanvasItem *item,
            GdkDrawable *drawable,
            guint color,
            gboolean create_mask,
            int x,
...
#define IS_COMPACT_VIEW(container) \
        ((container->details->layout_mode == SCATOLA_ICON_LAYOUT_T_B_L_R || \
...
static void
prepare_pango_layout_width (ScatolaIconCanvasItem *item,
                            PangoLayout *layout)
{
    if (scatola_icon_canvas_item_get_max_text_width (item) < 0)
    {
        pango_layout_set_width (layout, -1);
    }
    else
    {
        pango_layout_set_width (layout, floor (scatola_icon_canvas_item_get_max_text_width (item)) * PANGO_SCALE);
        pango_layout_set_ellipsize (layout, PANGO_ELLIPSIZE_END);
    }
}

static void
prepare_pango_layout_for_measure_entire_text (ScatolaIconCanvasItem *item,
        PangoLayout *layout)
{
    ...
    container = SCATOLA_ICON_CONTAINER (EEL_CANVAS_ITEM (item)->canvas);
...
static void
prepare_pango_layout_for_draw (ScatolaIconCanvasItem *item,
                               PangoLayout *layout)
{
    ScatolaIconCanvasItemDetails *details;
    ScatolaIconContainer *container;
    ...
    else
    {
        ...
        pango_layout_set_height (layout,
                                 scatola_icon_container_get_max_layout_lines_for_pango (container));
    }
}

static void
measure_label_text (ScatolaIconCanvasItem *item)
{
    ScatolaIconCanvasItemDetails *details;
    ScatolaIconContainer *container;
    ...
    if (have_editable)
    {
        ...
        layout_get_size_for_layout (editable_layout,
                                    scatola_icon_container_get_max_layout_lines (container),
                                    editable_height_for_entire_text,
...
static void
draw_label_text (ScatolaIconCanvasItem *item,
#if GTK_CHECK_VERSION(3,0,0)
                 cairo_t *cr,
                 gboolean create_mask,
                 EelIRect icon_rect)
{
    ...
    needs_highlight = details->is_highlighted_for_selection || details->is_highlighted_for_drop;
    is_rtl_label_beside = scatola_icon_container_is_layout_rtl (container) &&
                          container->details->label_position == SCATOLA_ICON_LABEL_POSITION_BESIDE;
    ...
    max_text_width = floor (scatola_icon_canvas_item_get_max_text_width (item));
                 ...
                 EelIRect icon_rect)
{
    ...
    if (have_editable)
    {
        ...
        scatola_icon_container_get_label_color
             (SCATOLA_ICON_CONTAINER (canvas_item->canvas),
...
void
scatola_icon_canvas_item_set_is_visible (ScatolaIconCanvasItem       *item,
                                      gboolean                      visible)
{
    ...
    if (!visible)
    {
        scatola_icon_canvas_item_invalidate_label (item);
    }
}

void
scatola_icon_canvas_item_invalidate_label (ScatolaIconCanvasItem     *item)
{
    scatola_icon_canvas_item_invalidate_label_size (item);
...
get_knob_pixbuf (void)
{
    ...
    if (!knob_pixbuf)
    {
        knob_filename = scatola_pixmap_file ("knob.png");
...
static void
draw_stretch_handles (ScatolaIconCanvasItem *item,
#if GTK_CHECK_VERSION(3,0,0)
                      cairo_t *cr,
#else
                      GdkDrawable *drawable,
...
static void
emblem_layout_reset (EmblemLayout *layout, ScatolaIconCanvasItem *icon_item, EelIRect icon_rect, gboolean is_rtl)
{
...
static GdkPixbuf *
real_map_pixbuf (ScatolaIconCanvasItem *icon_item)
{
    EelCanvas *canvas;
    char *audio_filename;
    ScatolaIconContainer *container;
    ...
    container = SCATOLA_ICON_CONTAINER (canvas);
            ...
            icon_item->details->is_highlighted_for_clipboard)
    {
        ...
        {
            emblem_size = scatola_icon_get_emblem_size_for_icon_size (gdk_pixbuf_get_width (temp_pixbuf));
            /* Load the audio symbol. */
            audio_filename = scatola_pixmap_file ("audio.svg");
            ...
            || icon_item->details->is_highlighted_for_drop)
    {
        ...
        color =  gtk_widget_has_focus (GTK_WIDGET (canvas)) ? SCATOLA_ICON_CONTAINER (canvas)->details->highlight_color_rgba : SCATOLA_ICON_CONTAINER (canvas)->details->active_color_rgba;
...
static GdkPixbuf *
map_pixbuf (ScatolaIconCanvasItem *icon_item)
{
...
static void
draw_embedded_text (ScatolaIconCanvasItem *item,
#if GTK_CHECK_VERSION(3,0,0)
                    cairo_t *cr,
#else
                    GdkDrawable *drawable,
...
static void
#if GTK_CHECK_VERSION(3,0,0)
scatola_icon_canvas_item_draw (EelCanvasItem *item,
                            cairo_t *cr,
                            cairo_region_t *region)
{
    ScatolaIconContainer *container;
#else
scatola_icon_canvas_item_draw (EelCanvasItem *item, GdkDrawable *drawable,
                            GdkEventExpose *expose)
{
    ...
    container = SCATOLA_ICON_CONTAINER (item->canvas);
    gboolean is_rtl;
#else
...
#endif
    icon_item = SCATOLA_ICON_CANVAS_ITEM (item);
    ...
    is_rtl = scatola_icon_container_is_layout_rtl (SCATOLA_ICON_CONTAINER (item->canvas));
...
static PangoLayout *
create_label_layout (ScatolaIconCanvasItem *item,
                     const char *text)
{
    PangoLayout *layout;
    PangoContext *context;
    ...
    container = SCATOLA_ICON_CONTAINER (canvas_item->canvas);
...
#if !GTK_CHECK_VERSION(3,0,0)
static void
draw_label_layout (ScatolaIconCanvasItem *item,
                   GdkDrawable *drawable,
                   PangoLayout *layout,
                   gboolean highlight,
                   GdkColor *label_color,
                   ...
                   int y)
{
    ...
    if (!highlight && (SCATOLA_ICON_CONTAINER (EEL_CANVAS_ITEM (item)->canvas)->details->use_drop_shadows))
    {
        /* draw a drop shadow */
...
static int
scatola_icon_canvas_item_event (EelCanvasItem *item, GdkEvent *event)
{
    ScatolaIconCanvasItem *icon_item;
    GdkCursor *cursor;
    GdkWindow *cursor_window;
    ...
    case GDK_ENTER_NOTIFY:
        if (!icon_item->details->is_prelit)
        {
            ...
            eel_canvas_item_send_behind (item,
                                         SCATOLA_ICON_CONTAINER (item->canvas)->details->rubberband_info.selection_rectangle);
             ...
             */
            icon_item->details->is_active = scatola_icon_container_emit_preview_signal
                                            (SCATOLA_ICON_CONTAINER (item->canvas),
                                             SCATOLA_ICON_CANVAS_ITEM (item)->user_data,
                                             TRUE);
        }
        return TRUE;
    ...
    case GDK_LEAVE_NOTIFY:
                ...
                || icon_item->details->is_highlighted_for_drop)
        {
             ...
             * does not have a return value.
             */
            scatola_icon_container_emit_preview_signal
            (SCATOLA_ICON_CONTAINER (item->canvas),
             SCATOLA_ICON_CANVAS_ITEM (item)->user_data,
             FALSE);
...
static gboolean
hit_test (ScatolaIconCanvasItem *icon_item, EelIRect canvas_rect)
{
    ScatolaIconCanvasItemDetails *details;
    EelIRect emblem_rect;
    EmblemLayout emblem_layout;
    ...
    is_rtl = scatola_icon_container_is_layout_rtl (SCATOLA_ICON_CONTAINER (EEL_CANVAS_ITEM (icon_item)->canvas));
...
static double
scatola_icon_canvas_item_point (EelCanvasItem *item, double x, double y, int cx, int cy,
                             EelCanvasItem **actual_item)
{
    ...
    *actual_item = item;
    ...
    canvas_rect.x1 = cx + 1;
    canvas_rect.y1 = cy + 1;
    if (hit_test (SCATOLA_ICON_CANVAS_ITEM (item), canvas_rect))
    {
        return 0.0;
    }
    else
...
static void
scatola_icon_canvas_item_translate (EelCanvasItem *item, double dx, double dy)
{
    ScatolaIconCanvasItem *icon_item;
    ScatolaIconCanvasItemDetails *details;
...
void
scatola_icon_canvas_item_get_bounds_for_layout (ScatolaIconCanvasItem *icon_item,
        double *x1, double *y1, double *x2, double *y2)
{
    ...
    scatola_icon_canvas_item_ensure_bounds_up_to_date (icon_item);
...
void
scatola_icon_canvas_item_get_bounds_for_entire_item (ScatolaIconCanvasItem *icon_item,
        double *x1, double *y1, double *x2, double *y2)
{
    ScatolaIconCanvasItemDetails *details;
    EelIRect *total_rect;
...
static void
scatola_icon_canvas_item_bounds (EelCanvasItem *item,
                              double *x1, double *y1, double *x2, double *y2)
{
    ScatolaIconCanvasItem *icon_item;
    ScatolaIconCanvasItemDetails *details;
...
static void
scatola_icon_canvas_item_ensure_bounds_up_to_date (ScatolaIconCanvasItem *icon_item)
{
    ScatolaIconCanvasItemDetails *details;
...
EelDRect
scatola_icon_canvas_item_get_icon_rectangle (const ScatolaIconCanvasItem *item)
{
    EelDRect rectangle;
    double pixels_per_unit;
    GdkPixbuf *pixbuf;

    g_return_val_if_fail (SCATOLA_IS_ICON_CANVAS_ITEM (item), eel_drect_empty);
...
EelDRect
scatola_icon_canvas_item_get_text_rectangle (ScatolaIconCanvasItem *item,
        gboolean for_layout)
{
    /* FIXME */
    EelIRect icon_rectangle;
...
static void
get_icon_canvas_rectangle (ScatolaIconCanvasItem *item,
                           EelIRect *rect)
{
    GdkPixbuf *pixbuf;

    g_assert (SCATOLA_IS_ICON_CANVAS_ITEM (item));
    g_assert (rect != NULL);
...
void
scatola_icon_canvas_item_set_show_stretch_handles (ScatolaIconCanvasItem *item,
...
static gboolean
hit_test_stretch_handle (ScatolaIconCanvasItem *item,
...
gboolean
scatola_icon_canvas_item_hit_test_stretch_handles (ScatolaIconCanvasItem *item,
        EelDPoint world_point,
        GtkCornerType *corner)
{
    EelIRect canvas_rect;

    g_return_val_if_fail (SCATOLA_IS_ICON_CANVAS_ITEM (item), FALSE);
...
/* scatola_icon_canvas_item_hit_test_rectangle
 *
 * Check and see if there is an intersection between the item and the
 * canvas rect.
 */
gboolean
scatola_icon_canvas_item_hit_test_rectangle (ScatolaIconCanvasItem *item, EelIRect canvas_rect)
{
    g_return_val_if_fail (SCATOLA_IS_ICON_CANVAS_ITEM (item), FALSE);
...
const char *
scatola_icon_canvas_item_get_editable_text (ScatolaIconCanvasItem *icon_item)
{
    g_return_val_if_fail (SCATOLA_IS_ICON_CANVAS_ITEM (icon_item), NULL);

    return icon_item->details->editable_text;
}

void
scatola_icon_canvas_item_set_renaming (ScatolaIconCanvasItem *item, gboolean state)
{
    g_return_if_fail (SCATOLA_IS_ICON_CANVAS_ITEM (item));
...
double
scatola_icon_canvas_item_get_max_text_width (ScatolaIconCanvasItem *item)
{
    EelCanvasItem *canvas_item;
    ScatolaIconContainer *container;
    ...
    if (scatola_icon_container_is_tighter_layout (container))
    {
    ...
    else
    {
        ...
        if (container->details->label_position == SCATOLA_ICON_LABEL_POSITION_BESIDE)
        {
            if (container->details->layout_mode == SCATOLA_ICON_LAYOUT_T_B_L_R ||
                    container->details->layout_mode == SCATOLA_ICON_LAYOUT_T_B_R_L)
            {
...
void
scatola_icon_canvas_item_set_entire_text (ScatolaIconCanvasItem       *item,
...
static void
scatola_icon_canvas_item_class_init (ScatolaIconCanvasItemClass *class)
{
	...
	object_class->finalize = scatola_icon_canvas_item_finalize;
	object_class->set_property = scatola_icon_canvas_item_set_property;
	object_class->get_property = scatola_icon_canvas_item_get_property;
	...
	item_class->update = scatola_icon_canvas_item_update;
	item_class->draw = scatola_icon_canvas_item_draw;
	item_class->point = scatola_icon_canvas_item_point;
	item_class->translate = scatola_icon_canvas_item_translate;
	item_class->bounds = scatola_icon_canvas_item_bounds;
	item_class->event = scatola_icon_canvas_item_event;

	atk_registry_set_factory_type (atk_get_default_registry (),
				       SCATOLA_TYPE_ICON_CANVAS_ITEM,
				       scatola_icon_canvas_item_accessible_factory_get_type ());

	g_type_class_add_private (class, sizeof (ScatolaIconCanvasItemDetails));
}

static GailTextUtil *
scatola_icon_canvas_item_get_text (GObject *text)
{
	return SCATOLA_ICON_CANVAS_ITEM (text)->details->text_util;
}

static void
scatola_icon_canvas_item_text_interface_init (EelAccessibleTextIface *iface)
{
	iface->get_text = scatola_icon_canvas_item_get_text;
}

/* ============================= a11y interfaces =========================== */

static const char *scatola_icon_canvas_item_accessible_action_names[] = {
        "open",
        "menu",
        NULL
};

static const char *scatola_icon_canvas_item_accessible_action_descriptions[] = {
        "Open item",
        "Popup context menu",
        NULL
};
...
typedef struct {
	...
	char *description;
} ScatolaIconCanvasItemAccessiblePrivate;

typedef struct {
	ScatolaIconCanvasItem *item;
	gint action_number;
} ScatolaIconCanvasItemAccessibleActionContext;

static GType scatola_icon_canvas_item_accessible_get_type (void);

#define GET_PRIV(o) G_TYPE_INSTANCE_GET_PRIVATE(o, scatola_icon_canvas_item_accessible_get_type (), ScatolaIconCanvasItemAccessiblePrivate);

/* accessible AtkAction interface */

static gboolean
scatola_icon_canvas_item_accessible_idle_do_action (gpointer data)
{
    ScatolaIconCanvasItem *item;
    ScatolaIconCanvasItemAccessibleActionContext *ctx;
    ScatolaIcon *icon;
    ScatolaIconContainer *container;
    GList* selection;
    GList file_list;
...
static gboolean
scatola_icon_canvas_item_accessible_do_action (AtkAction *accessible, int i)
{
    ScatolaIconCanvasItem *item;
    ScatolaIconCanvasItemAccessibleActionContext *ctx;
    ScatolaIconContainer *container;
    ...
    item = SCATOLA_ICON_CANVAS_ITEM (atk_gobject_accessible_get_object (ATK_GOBJECT_ACCESSIBLE (accessible)));
    if (!item)
    {
        return FALSE;
    }
    ...
    case ACTION_MENU:
        ...
        ctx = g_new (ScatolaIconCanvasItemAccessibleActionContext, 1);
        ctx->action_number = i;
        ctx->item = item;
        g_queue_push_head (container->details->a11y_item_action_queue, ctx);
        if (container->details->a11y_item_action_idle_handler == 0)
        {
            container->details->a11y_item_action_idle_handler = g_idle_add (scatola_icon_canvas_item_accessible_idle_do_action, container);
        }
        break;
    default :
        g_warning ("Invalid action passed to ScatolaIconCanvasItemAccessible::do_action");
        return FALSE;
    }
...
static int
scatola_icon_canvas_item_accessible_get_n_actions (AtkAction *accessible)
{
    return LAST_ACTION;
}

static const char *
scatola_icon_canvas_item_accessible_action_get_description (AtkAction *accessible,
        int i)
{
    ScatolaIconCanvasItemAccessiblePrivate *priv;
    ...
    {
        return scatola_icon_canvas_item_accessible_action_descriptions[i];
    }
}

static const char *
scatola_icon_canvas_item_accessible_action_get_name (AtkAction *accessible, int i)
{
    g_assert (i < LAST_ACTION);

    return scatola_icon_canvas_item_accessible_action_names[i];
}

static const char *
scatola_icon_canvas_item_accessible_action_get_keybinding (AtkAction *accessible,
        int i)
{
    g_assert (i < LAST_ACTION);
...
static gboolean
scatola_icon_canvas_item_accessible_action_set_description (AtkAction *accessible,
        int i,
...
static void
scatola_icon_canvas_item_accessible_action_interface_init (AtkActionIface *iface)
{
    iface->do_action = scatola_icon_canvas_item_accessible_do_action;
    iface->get_n_actions = scatola_icon_canvas_item_accessible_get_n_actions;
    iface->get_description = scatola_icon_canvas_item_accessible_action_get_description;
    iface->get_keybinding = scatola_icon_canvas_item_accessible_action_get_keybinding;
    iface->get_name = scatola_icon_canvas_item_accessible_action_get_name;
    iface->set_description = scatola_icon_canvas_item_accessible_action_set_description;
}

static const gchar* scatola_icon_canvas_item_accessible_get_name(AtkObject* accessible)
{
    ScatolaIconCanvasItem* item;
...
static const gchar* scatola_icon_canvas_item_accessible_get_description(AtkObject* accessible)
{
    ScatolaIconCanvasItem* item;
...
static AtkObject *
scatola_icon_canvas_item_accessible_get_parent (AtkObject *accessible)
{
    ScatolaIconCanvasItem *item;
...
static int
scatola_icon_canvas_item_accessible_get_index_in_parent (AtkObject *accessible)
{
    ScatolaIconCanvasItem *item;
    ScatolaIconContainer *container;
    GList *l;
...
static const gchar* scatola_icon_canvas_item_accessible_get_image_description(AtkImage* image)
{
    ScatolaIconCanvasItemAccessiblePrivate* priv;
    ScatolaIconCanvasItem* item;
    ScatolaIcon* icon;
    ...
    {
        item = SCATOLA_ICON_CANVAS_ITEM (atk_gobject_accessible_get_object (ATK_GOBJECT_ACCESSIBLE (image)));
        ...
        icon = item->user_data;
        container = SCATOLA_ICON_CONTAINER(EEL_CANVAS_ITEM(item)->canvas);
        description = scatola_icon_container_get_icon_description(container, icon->data);
...
static void
scatola_icon_canvas_item_accessible_get_image_size
(AtkImage *image,
 gint     *width,
 gint     *height)
{
...
static void
scatola_icon_canvas_item_accessible_get_image_position
(AtkImage		 *image,
 gint                    *x,
 gint	                 *y,
...
static gboolean
scatola_icon_canvas_item_accessible_set_image_description
(AtkImage    *image,
...
static void
scatola_icon_canvas_item_accessible_image_interface_init (AtkImageIface *iface)
{
    iface->get_image_description = scatola_icon_canvas_item_accessible_get_image_description;
    iface->set_image_description = scatola_icon_canvas_item_accessible_set_image_description;
    iface->get_image_size        = scatola_icon_canvas_item_accessible_get_image_size;
    iface->get_image_position    = scatola_icon_canvas_item_accessible_get_image_position;
}

static gint
scatola_icon_canvas_item_accessible_get_offset_at_point (AtkText	 *text,
        gint           x,
        gint           y,
        AtkCoordType coords)
{
    ...
    item = SCATOLA_ICON_CANVAS_ITEM (atk_gobject_accessible_get_object (ATK_GOBJECT_ACCESSIBLE (text)));
...
static void
scatola_icon_canvas_item_accessible_get_character_extents (AtkText	   *text,
        gint	   offset,
        gint	   *x,
        gint	   *y,
        gint	   *width,
...
static void
scatola_icon_canvas_item_accessible_text_interface_init (AtkTextIface *iface)
{
    ...
    iface->get_character_extents   = scatola_icon_canvas_item_accessible_get_character_extents;
    iface->get_offset_at_point     = scatola_icon_canvas_item_accessible_get_offset_at_point;
}

typedef struct {
	EelCanvasItemAccessible parent;
} ScatolaIconCanvasItemAccessible;

typedef struct {
	EelCanvasItemAccessibleClass parent_class;
} ScatolaIconCanvasItemAccessibleClass;

G_DEFINE_TYPE_WITH_CODE (ScatolaIconCanvasItemAccessible,
			 scatola_icon_canvas_item_accessible,
			 eel_canvas_item_accessible_get_type (),
			 G_IMPLEMENT_INTERFACE (ATK_TYPE_IMAGE,
						scatola_icon_canvas_item_accessible_image_interface_init)
			 G_IMPLEMENT_INTERFACE (ATK_TYPE_TEXT,
						scatola_icon_canvas_item_accessible_text_interface_init)
			 G_IMPLEMENT_INTERFACE (ATK_TYPE_ACTION,
						scatola_icon_canvas_item_accessible_action_interface_init));

static AtkStateSet*
scatola_icon_canvas_item_accessible_ref_state_set (AtkObject *accessible)
{
	...
	state_set = ATK_OBJECT_CLASS (scatola_icon_canvas_item_accessible_parent_class)->ref_state_set (accessible);
...
static void
scatola_icon_canvas_item_accessible_finalize (GObject *object)
{
	ScatolaIconCanvasItemAccessiblePrivate *priv;
	int i;
	...
	g_free (priv->description);
        ...
        G_OBJECT_CLASS (scatola_icon_canvas_item_accessible_parent_class)->finalize (object);
}

static void
scatola_icon_canvas_item_accessible_initialize (AtkObject *accessible,
						 gpointer widget)
{
	ATK_OBJECT_CLASS (scatola_icon_canvas_item_accessible_parent_class)->initialize (accessible, widget);

	atk_object_set_role (accessible, ATK_ROLE_ICON);
}

static void
scatola_icon_canvas_item_accessible_class_init (ScatolaIconCanvasItemAccessibleClass *klass)
{
	AtkObjectClass *aclass = ATK_OBJECT_CLASS (klass);
	GObjectClass *oclass = G_OBJECT_CLASS (klass);

	oclass->finalize = scatola_icon_canvas_item_accessible_finalize;

	aclass->initialize = scatola_icon_canvas_item_accessible_initialize;

	aclass->get_name = scatola_icon_canvas_item_accessible_get_name;
	aclass->get_description = scatola_icon_canvas_item_accessible_get_description;
	aclass->get_parent = scatola_icon_canvas_item_accessible_get_parent;
	aclass->get_index_in_parent = scatola_icon_canvas_item_accessible_get_index_in_parent;
	aclass->ref_state_set = scatola_icon_canvas_item_accessible_ref_state_set;

	g_type_class_add_private (klass, sizeof (ScatolaIconCanvasItemAccessiblePrivate));
}

static void
scatola_icon_canvas_item_accessible_init (ScatolaIconCanvasItemAccessible *self)
{
}

/* dummy typedef */
typedef AtkObjectFactory      ScatolaIconCanvasItemAccessibleFactory;
typedef AtkObjectFactoryClass ScatolaIconCanvasItemAccessibleFactoryClass;

G_DEFINE_TYPE (ScatolaIconCanvasItemAccessibleFactory, scatola_icon_canvas_item_accessible_factory,
	       ATK_TYPE_OBJECT_FACTORY);


static AtkObject *
scatola_icon_canvas_item_accessible_factory_create_accessible (GObject *for_object)
{
    AtkObject *accessible;
    ScatolaIconCanvasItem *item;
    GString *item_text;

    item = SCATOLA_ICON_CANVAS_ITEM (for_object);
    g_assert (item != NULL);
    ...
    accessible = g_object_new (scatola_icon_canvas_item_accessible_get_type (), NULL);
    atk_object_initialize (accessible, for_object);
    return accessible;
}

static GType
scatola_icon_canvas_item_accessible_factory_get_accessible_type (void)
{
    return scatola_icon_canvas_item_accessible_get_type ();
}

static void
scatola_icon_canvas_item_accessible_factory_init (ScatolaIconCanvasItemAccessibleFactory *self)
{
}

static void
scatola_icon_canvas_item_accessible_factory_class_init (ScatolaIconCanvasItemAccessibleFactoryClass *klass)
{
	klass->create_accessible = scatola_icon_canvas_item_accessible_factory_create_accessible;
	klass->get_accessible_type = scatola_icon_canvas_item_accessible_factory_get_accessible_type;
}
