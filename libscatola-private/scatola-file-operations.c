...
/* scatola-file-operations.c - Scatola file operations.
...
#include "scatola-file-changes-queue.h"
#include "scatola-lib-self-check-functions.h"

#include "scatola-progress-info.h"
...
#include "scatola-trash-monitor.h"
#include "scatola-file-utilities.h"
#include "scatola-file-conflict-dialog.h"
...
typedef struct {
	...
	ScatolaCopyCallback  done_callback;
...
typedef struct {
	...
	ScatolaDeleteCallback done_callback;
	gpointer done_callback_data;
} DeleteJob;
...
typedef struct {
	...
	GFile *created_file;
	ScatolaCreateCallback done_callback;
	gpointer done_callback_data;
} CreateJob;
...
typedef struct {
	...
	ScatolaOpCallback done_callback;
	gpointer done_callback_data;
} EmptyTrashJob;
...
format_time (int seconds)
{
	...
	return g_strdup_printf (ngettext ("approxigde2ly %'d hour",
					  "approxigde2ly %'d hours",
					  hours), hours);
}
	     ...
	     GtkWindow *parent_window)
{
	...
	common->progress = scatola_progress_info_new ();
	common->cancellable = scatola_progress_info_get_cancellable (common->progress);
...
{
	scatola_progress_info_finish (common->progress);

	if (common->inhibit_cookie != -1) {
		scatola_uninhibit_power_manager (common->inhibit_cookie);
	}
...
can_delete_without_confirm (GFile *file)
{
	if (g_file_has_uri_scheme (file, "burn") ||
	    g_file_has_uri_scheme (file, "x-scatola-desktop")) {
		return TRUE;
	}
		      ...
		      va_list varargs)
{
	...
	scatola_progress_info_pause (job->progress);
	g_io_scheduler_job_send_to_mainloop (job->io_job,
					     do_run_simple_dialog,
					     data,
					     NULL);
	scatola_progress_info_resume (job->progress);
	res = data->result;
...
{
	job->inhibit_cookie = scatola_inhibit_power_manager (message);
}
...
should_confirm_trash (void)
{
	...
	prefs = g_settings_new ("org.gde2.scatola.preferences");
	confirm_trash = g_settings_get_boolean (prefs, SCATOLA_PREFERENCES_CONFIRM_TRASH);
			...
			TransferInfo *transfer_info)
{
	...
	scatola_progress_info_take_status (job->progress,
					    f (_("Deleting files")));

	elapsed = g_timer_elapsed (job->time, NULL);
	if (elapsed < SECONDS_NEEDED_FOR_RELIABLE_TRANSFER_RATE) {

		scatola_progress_info_set_details (job->progress, files_left_s);
	} else {
		...
		scatola_progress_info_take_details (job->progress, details);
	...
	if (source_info->num_files != 0) {
		scatola_progress_info_set_progress (job->progress, transfer_info->num_files, source_info->num_files);
	}
}
 ...
 retry:
	...
	if (!job_aborted (job) &&
		...
		skip:
			...
			scatola_file_changes_queue_file_removed (dir);
	     ...
	     gboolean toplevel)
{
	...
	if (g_file_delete (file, job->cancellable, &error)) {
		scatola_file_changes_queue_file_removed (file);
		transfer_info->num_files ++;
		report_delete_progress (job, source_info, transfer_info);
		return;
	}
		       ...
		       int total_files)
{
	...
	s = f (ngettext ("%'d file left to trash",
			 ...
			 files_left),
	       files_left);
	scatola_progress_info_take_details (job->progress, s);

	if (total_files != 0) {
		scatola_progress_info_set_progress (job->progress, files_trashed, total_files);
	}
}
...
trash_files (CommonJob *job, GList *files, int *files_skipped)
{
	...
	for (l = files;
		...
		mtime = scatola_undostack_manager_get_file_modification_time (file);
		...
		skip:
			...
			// Start UNDO-REDO
			scatola_undostack_manager_data_add_trashed_file (job->undo_redo_data, file, mtime);
			// End UNDO-REDO
	    ...
	    gpointer user_data)
{
	...
	scatola_progress_info_start (job->common.progress);
			  ...
			  gpointer                done_callback_data)
{
	...
	if (try_trash && !scatola_undostack_manager_is_undo_redo(scatola_undostack_manager_instance())) {
		job->common.undo_redo_data = scatola_undostack_manager_data_new (SCATOLA_UNDOSTACK_MOVETOTRASH, g_list_length(files));
		GFile* src_dir = g_file_get_parent (files->data);
		scatola_undostack_manager_data_set_src_dir (job->common.undo_redo_data, src_dir);
	}
	// End UNDO-REDO
...
void
scatola_file_operations_trash_or_delete (GList                  *files,
					  GtkWindow              *parent_window,
					  ScatolaDeleteCallback  done_callback,
					  gpointer                done_callback_data)
{
...
void
scatola_file_operations_delete (GList                  *files,
				 GtkWindow              *parent_window,
				 ScatolaDeleteCallback  done_callback,
				 gpointer                done_callback_data)
{
...
typedef struct {
	...
	GMount *mount;
	GtkWindow *parent_window;
	ScatolaUnmountCallback callback;
...
void
scatola_file_operations_unmount_mount_full (GtkWindow                      *parent_window,
					     GMount                         *mount,
					     gboolean                        eject,
					     ...
					     gpointer                        callback_data)
{
	...
	if (check_trash && has_trash_files (mount)) {
		...
		if (response == GTK_RESPONSE_ACCEPT) {
			...
			job->done_callback = (ScatolaOpCallback)do_unmount;
...
void
scatola_file_operations_unmount_mount (GtkWindow                      *parent_window,
					GMount                         *mount,
					gboolean                        eject,
					gboolean                        check_trash)
{
	scatola_file_operations_unmount_mount_full (parent_window, mount, eject,
...
volume_mount_cb (GObject *source_object,
		 ...
		 gpointer user_data)
{
	ScatolaMountCallback mount_callback;
	...
	error = NULL;
	scatola_allow_autorun_for_volume_finish (G_VOLUME (source_object));
	...
	mount_callback = (ScatolaMountCallback)
...
void
scatola_file_operations_mount_volume (GtkWindow *parent_window,
				       GVolume *volume,
				       gboolean allow_autorun)
{
	scatola_file_operations_mount_volume_full (parent_window, volume,
						    allow_autorun, NULL, NULL);
}

void
scatola_file_operations_mount_volume_full (GtkWindow *parent_window,
					    GVolume *volume,
					    gboolean allow_autorun,
					    ScatolaMountCallback mount_callback,
					    GObject *mount_callback_data_object)
		       ...
		       SourceInfo *source_info)
{
	...
	scatola_progress_info_take_details (job->progress, s);
	scatola_progress_info_pulse_progress (job->progress);
}
		      ...
		      TransferInfo *transfer_info)
{
	...
	scatola_progress_info_set_progress (job->progress, transfer_info->num_bytes, total_size);
}
		 ...
		 char **dest_fs_type)
{
 ...
 retry:
	...
	// Start UNDO-REDO
	scatola_undostack_manager_data_add_origin_target_pair (job->undo_redo_data, src, *dest);
	// End UNDO-REDO
	scatola_file_changes_queue_file_added (*dest);
...
do_run_conflict_dialog (gpointer _data)
{
	...
	dialog = scatola_file_conflict_dialog_new (data->parent,
						    data->src,
						    data->dest,
						    data->dest_dir);
	...
	if (response == CONFLICT_RESPONSE_RENAME) {
		data->resp_data->new_name =
			scatola_file_conflict_dialog_get_new_name (SCATOLA_FILE_CONFLICT_DIALOG (dialog));
	} else if (response != GTK_RESPONSE_CANCEL ||
		   response != GTK_RESPONSE_NONE) {
		   data->resp_data->apply_to_all =
			   scatola_file_conflict_dialog_get_apply_to_all
				(SCATOLA_FILE_CONFLICT_DIALOG (dialog));
	}
		...
		gboolean readonly_source_fs)
{
 ...
 retry:
	...
	if (res) {
		...
		if (debuting_files) {
			if (position) {
				scatola_file_changes_queue_schedule_position_set (dest, *position, job->screen_num);
			} else {
				scatola_file_changes_queue_schedule_position_remove (dest);
			}

			g_hash_table_replace (debuting_files, g_object_ref (dest), GINT_TO_POINTER (TRUE));
		}
		if (copy_job->is_move) {
			scatola_file_changes_queue_file_moved (src, dest);
		} else {
			scatola_file_changes_queue_file_added (dest);
		}
	...
	/* Needs to recurse */
		...
		if (overwrite && would_recurse) {
			...
			scatola_file_changes_queue_file_removed (dest);
		}
...
void
scatola_file_operations_copy (GList *files,
			       ...
			       gpointer done_callback_data)
{
	...
	job->desktop_location = scatola_get_desktop_location ();
	job->done_callback = done_callback;
	job->done_callback_data = done_callback_data;
	job->files = eel_g_object_list_copy (files);
	...
	if (!scatola_undostack_manager_is_undo_redo(scatola_undostack_manager_instance())) {
		job->common.undo_redo_data = scatola_undostack_manager_data_new (SCATOLA_UNDOSTACK_COPY, g_list_length(files));
		GFile* src_dir = g_file_get_parent (files->data);
		scatola_undostack_manager_data_set_src_dir (job->common.undo_redo_data, src_dir);
		g_object_ref (target_dir);
		scatola_undostack_manager_data_set_dest_dir (job->common.undo_redo_data, target_dir);
	}
	// End UNDO-REDO
...
report_move_progress (CopyMoveJob *move_job, int total, int left)
{
	...
	scatola_progress_info_take_details (job->progress,
...
void
scatola_file_operations_move (GList *files,
			       GArray *relative_item_points,
			       GFile *target_dir,
			       GtkWindow *parent_window,
			       ScatolaCopyCallback  done_callback,
			       gpointer done_callback_data)
{
	...
	if (!scatola_undostack_manager_is_undo_redo(scatola_undostack_manager_instance())) {
		if (g_file_has_uri_scheme (g_list_first(files)->data, "trash")) {
			job->common.undo_redo_data = scatola_undostack_manager_data_new (SCATOLA_UNDOSTACK_RESTOREFROMTRASH, g_list_length(files));
		} else {
			job->common.undo_redo_data = scatola_undostack_manager_data_new (SCATOLA_UNDOSTACK_MOVE, g_list_length(files));
		}
		GFile* src_dir = g_file_get_parent (files->data);
		scatola_undostack_manager_data_set_src_dir (job->common.undo_redo_data, src_dir);
		g_object_ref (target_dir);
...
report_link_progress (CopyMoveJob *link_job, int total, int left)
{
	...
	scatola_progress_info_set_progress (job->progress, left, total);
}
 ...
 retry:
	...
	if (path == NULL) {
		not_local = TRUE;
					      ...
					      &error)) {
		// Start UNDO-REDO
		scatola_undostack_manager_data_add_origin_target_pair (common->undo_redo_data, src, dest);
		// End UNDO-REDO
		g_free (path);
		if (debuting_files) {
			g_hash_table_replace (debuting_files, g_object_ref (dest), GINT_TO_POINTER (TRUE));
		...
		scatola_file_changes_queue_file_added (dest);
		if (position) {
			scatola_file_changes_queue_schedule_position_set (dest, *position, common->screen_num);
		} else {
			scatola_file_changes_queue_schedule_position_remove (dest);
		}
...
void
scatola_file_operations_link (GList *files,
			       GArray *relative_item_points,
			       GFile *target_dir,
			       GtkWindow *parent_window,
			       ScatolaCopyCallback  done_callback,
			       gpointer done_callback_data)
{
	...
	// Start UNDO-REDO
	if (!scatola_undostack_manager_is_undo_redo(scatola_undostack_manager_instance())) {
		job->common.undo_redo_data = scatola_undostack_manager_data_new (SCATOLA_UNDOSTACK_CREATELINK, g_list_length(files));
		GFile* src_dir = g_file_get_parent (files->data);
		scatola_undostack_manager_data_set_src_dir (job->common.undo_redo_data, src_dir);
		g_object_ref (target_dir);
		scatola_undostack_manager_data_set_dest_dir (job->common.undo_redo_data, target_dir);
...
void
scatola_file_operations_duplicate (GList *files,
				    GArray *relative_item_points,
				    GtkWindow *parent_window,
				    ScatolaCopyCallback  done_callback,
				    gpointer done_callback_data)
{
	...
	// Start UNDO-REDO
	if (!scatola_undostack_manager_is_undo_redo(scatola_undostack_manager_instance())) {
		job->common.undo_redo_data = scatola_undostack_manager_data_new (SCATOLA_UNDOSTACK_DUPLICATE, g_list_length(files));
		GFile* src_dir = g_file_get_parent (files->data);
		scatola_undostack_manager_data_set_src_dir (job->common.undo_redo_data, src_dir);
		g_object_ref (src_dir);
		scatola_undostack_manager_data_set_dest_dir (job->common.undo_redo_data, src_dir);
	}
	// End UNDO-REDO
		      ...
		      GFileInfo *info)
{
	...
	scatola_progress_info_pulse_progress (common->progress);
	...
	if (!job_aborted (common) &&
		...
		// Start UNDO-REDO
		scatola_undostack_manager_data_add_file_permissions(common->undo_redo_data, file, current);
		// End UNDO-REDO
		     ...
		     gpointer user_data)
{
	...
	scatola_progress_info_set_status (common->progress,
...
void
scatola_file_set_permissions_recursive (const char *directory,
					 guint32         file_permissions,
					 guint32         file_mask,
					 guint32         dir_permissions,
					 guint32         dir_mask,
					 ScatolaOpCallback  callback,
					 gpointer  callback_data)
{
	SetPermissionsJob *job;
	...
	// Start UNDO-REDO
	if (!scatola_undostack_manager_is_undo_redo(scatola_undostack_manager_instance())) {
		job->common.undo_redo_data = scatola_undostack_manager_data_new (SCATOLA_UNDOSTACK_RECURSIVESETPERMISSIONS, 1);
		g_object_ref (job->file);
		scatola_undostack_manager_data_set_dest_dir (job->common.undo_redo_data, job->file);
		scatola_undostack_manager_data_set_recursive_permissions(job->common.undo_redo_data, file_permissions, file_mask, dir_permissions, dir_mask);
	}
	// End UNDO-REDO
...
typedef struct {
	ScatolaCopyCallback real_callback;
	gpointer real_data;
} MoveTrashCBData;
...
void
scatola_file_operations_copy_move (const GList *item_uris,
				    GArray *relative_item_points,
				    ...
				    gpointer done_callback_data)
{
	...
	if (copy_action == GDK_ACTION_COPY) {
		...
		if (target_dir == NULL ||
			...
			scatola_file_operations_duplicate (locations,
							    relative_item_points,
							    parent_window,
							    done_callback, done_callback_data);
		} else {
			scatola_file_operations_copy (locations,
						       relative_item_points,
						       dest,
						       parent_window,
						       done_callback, done_callback_data);
		...
		if (g_file_has_uri_scheme (dest, "trash")) {
			...
			scatola_file_operations_trash_or_delete (locations,
								  parent_window,
								  (ScatolaDeleteCallback) callback_for_move_to_trash,
								  cb_data);
		} else {
			scatola_file_operations_move (locations,
						       relative_item_points,
						       dest,
						       parent_window,
						       done_callback, done_callback_data);
		...
		scatola_file_operations_link (locations,
					       relative_item_points,
					       dest,
					       parent_window,
					       done_callback, done_callback_data);
	    ...
	    gpointer user_data)
{
 ...
 retry:
	...
	if (job->make_dir) {
		...
		// Start UNDO-REDO
		if (res) {
			scatola_undostack_manager_data_set_create_data(common->undo_redo_data,
					g_file_get_uri(dest),
					NULL);
		}
		// End UNDO-REDO
	...
	if (res) {
		...
		scatola_file_changes_queue_file_added (dest);
		if (job->has_position) {
			scatola_file_changes_queue_schedule_position_set (dest, job->position, common->screen_num);
		} else {
			scatola_file_changes_queue_schedule_position_remove (dest);
		}
	} else {
...
void
scatola_file_operations_new_folder (GtkWidget *parent_view,
				     ...
				     gpointer done_callback_data)
{
	...
	// Start UNDO-REDO
	if (!scatola_undostack_manager_is_undo_redo(scatola_undostack_manager_instance())) {
		job->common.undo_redo_data = scatola_undostack_manager_data_new (SCATOLA_UNDOSTACK_CREATEFOLDER, 1);
	}
	// End UNDO-REDO
...
void
scatola_file_operations_new_file_from_template (GtkWidget *parent_view,
						 GdkPoint *target_point,
						 const char *parent_dir,
						 ...
						 gpointer done_callback_data)
{
	...
	// Start UNDO-REDO
	if (!scatola_undostack_manager_is_undo_redo(scatola_undostack_manager_instance())) {
		job->common.undo_redo_data = scatola_undostack_manager_data_new (SCATOLA_UNDOSTACK_CREATEFILEFROMTEMPLATE, 1);
	}
	// End UNDO-REDO
...
void
scatola_file_operations_new_file (GtkWidget *parent_view,
				   GdkPoint *target_point,
				   const char *parent_dir,
				   const char *target_filename,
				   ...
				   gpointer done_callback_data)
{
	...
	// Start UNDO-REDO
	if (!scatola_undostack_manager_is_undo_redo(scatola_undostack_manager_instance())) {
		job->common.undo_redo_data = scatola_undostack_manager_data_new (SCATOLA_UNDOSTACK_CREATEEMPTYFILE, 1);
	}
	// End UNDO-REDO
...
empty_trash_job_done (gpointer user_data)
{
	...
	scatola_undostack_manager_trash_has_emptied(scatola_undostack_manager_instance());
...
void
scatola_file_operations_empty_trash (GtkWidget *parent_view)
{
	EmptyTrashJob *job;
	GtkWindow *parent_window;
...
void
scatola_file_mark_desktop_file_trusted (GFile *file,
					 GtkWindow *parent_window,
					 gboolean interactive,
					 ScatolaOpCallback done_callback,
					 gpointer done_callback_data)
...
void
scatola_self_check_file_operations (void)
{
