...
struct ScatolaSearchDirectoryDetails
{
    ScatolaQuery *query;
...
G_DEFINE_TYPE (ScatolaSearchDirectory, scatola_search_directory,
               SCATOLA_TYPE_DIRECTORY);

static void search_engine_hits_added (ScatolaSearchEngine *engine, GList *hits, ScatolaSearchDirectory *search);
static void search_engine_hits_subtracted (ScatolaSearchEngine *engine, GList *hits, ScatolaSearchDirectory *search);
static void search_engine_finished (ScatolaSearchEngine *engine, ScatolaSearchDirectory *search);
static void search_engine_error (ScatolaSearchEngine *engine, const char *error, ScatolaSearchDirectory *search);
static void search_callback_file_ready_callback (ScatolaFile *file, gpointer data);
static void file_changed (ScatolaFile *file, ScatolaSearchDirectory *search);

static void
ensure_search_engine (ScatolaSearchDirectory *search)
{
    if (!search->details->engine)
    {
        search->details->engine = scatola_search_engine_new ();
...
static void
reset_file_list (ScatolaSearchDirectory *search)
{
    ...
    for (list = search->details->files; list != NULL; list = list->next)
    {
                ...
                monitor_list = monitor_list->next)
        {
            ...
            scatola_file_monitor_remove (file, monitor);
        }
    }

    scatola_file_list_free (search->details->files);
    search->details->files = NULL;
}

static void
start_or_stop_search_engine (ScatolaSearchDirectory *search, gboolean adding)
{
            ...
            !search->details->search_running)
    {
        ...
        scatola_search_engine_set_query (search->details->engine, search->details->query);

        reset_file_list (search);

        scatola_search_engine_start (search->details->engine);
    }
             ...
             search->details->search_running)
    {
        ...
        scatola_search_engine_stop (search->details->engine);
...
static void
file_changed (ScatolaFile *file, ScatolaSearchDirectory *search)
{
    GList list;
    ...
    scatola_directory_emit_files_changed (SCATOLA_DIRECTORY (search), &list);
}

static void
search_monitor_add (ScatolaDirectory *directory,
                    gconstpointer client,
                    gboolean monitor_hidden_files,
                    ...
                    gpointer callback_data)
{
    ...
    search = SCATOLA_SEARCH_DIRECTORY (directory);
    ...
    for (list = search->details->files; list != NULL; list = list->next)
    {
        ...
        /* Add monitors */
        scatola_file_monitor_add (file, monitor, file_attributes);
    }
...
static void
search_monitor_remove_file_monitors (SearchMonitor *monitor, ScatolaSearchDirectory *search)
{
    GList *list;
    ScatolaFile *file;
...
static void
search_monitor_destroy (SearchMonitor *monitor, ScatolaSearchDirectory *search)
{
...
static void
search_monitor_remove (ScatolaDirectory *directory,
                       gconstpointer client)
{
    ScatolaSearchDirectory *search;
    SearchMonitor *monitor;
...
cancel_call_when_ready (gpointer key, gpointer value, gpointer user_data)
{
    ...
    scatola_file_cancel_call_when_ready (file, search_callback_file_ready_callback,
                                      search_callback);
}
...
search_callback_destroy (SearchCallback *search_callback)
{
    ...
    scatola_file_list_free (search_callback->file_list);
...
{
    search_callback->callback (SCATOLA_DIRECTORY (search_callback->search_directory),
...
static void
search_callback_file_ready_callback (ScatolaFile *file, gpointer data)
{
...
search_callback_find (ScatolaSearchDirectory *search, ScatolaDirectoryCallback callback, gpointer callback_data)
{
    SearchCallback *search_callback;
    GList *list;
...
static SearchCallback *
search_callback_find_pending (ScatolaSearchDirectory *search, ScatolaDirectoryCallback callback, gpointer callback_data)
{
    SearchCallback *search_callback;
    GList *list;
...
static void
search_call_when_ready (ScatolaDirectory *directory,
                        ScatolaFileAttributes file_attributes,
                        gboolean wait_for_file_list,
                        ScatolaDirectoryCallback callback,
                        gpointer callback_data)
{
    ...
    {
        search_callback->file_list = scatola_file_list_copy (search->details->files);
...
static void
search_cancel_callback (ScatolaDirectory *directory,
                        ScatolaDirectoryCallback callback,
                        gpointer callback_data)
{
    ScatolaSearchDirectory *search;
...
static void
search_engine_hits_added (ScatolaSearchEngine *engine, GList *hits,
                          ScatolaSearchDirectory *search)
{
    GList *hit_list;
    GList *file_list;
    ...
    for (hit_list = hits; hit_list != NULL; hit_list = hit_list->next)
    {
        ...
        if (g_str_has_suffix (uri, SCATOLA_SAVED_SEARCH_EXTENSION))
        {
        ...
        for (monitor_list = search->details->monitor_list; monitor_list; monitor_list = monitor_list->next)
        {
            ...
            /* Add monitors */
            scatola_file_monitor_add (file, monitor, monitor->monitor_attributes);
        }
    ...
    scatola_directory_emit_files_added (SCATOLA_DIRECTORY (search), file_list);

    file = scatola_directory_get_corresponding_file (SCATOLA_DIRECTORY (search));
    scatola_file_emit_changed (file);
    scatola_file_unref (file);
}

static void
search_engine_hits_subtracted (ScatolaSearchEngine *engine, GList *hits,
                               ScatolaSearchDirectory *search)
{
    GList *hit_list;
    GList *monitor_list;
    ...
    scatola_directory_emit_files_changed (SCATOLA_DIRECTORY (search), file_list);
...
{
    callback->file_list = scatola_file_list_copy (callback->search_directory->details->files);
...
static void
search_engine_error (ScatolaSearchEngine *engine, const char *error_message, ScatolaSearchDirectory *search)
{
    GError *error;

    error = g_error_new_literal (G_IO_ERROR, G_IO_ERROR_FAILED,
                                 error_message);
    scatola_directory_emit_load_error (SCATOLA_DIRECTORY (search),
                                    error);
    g_error_free (error);
}

static void
search_engine_finished (ScatolaSearchEngine *engine, ScatolaSearchDirectory *search)
{
    search->details->search_finished = TRUE;

    scatola_directory_emit_done_loading (SCATOLA_DIRECTORY (search));
...
static void
search_force_reload (ScatolaDirectory *directory)
{
    ScatolaSearchDirectory *search;
...
static gboolean
search_are_all_files_seen (ScatolaDirectory *directory)
{
    ScatolaSearchDirectory *search;
...
static gboolean
search_contains_file (ScatolaDirectory *directory,
                      ScatolaFile *file)
{
    ScatolaSearchDirectory *search;
...
static GList *
search_get_file_list (ScatolaDirectory *directory)
{
    ScatolaSearchDirectory *search;

    search = SCATOLA_SEARCH_DIRECTORY (directory);

    return scatola_file_list_copy (search->details->files);
}


static gboolean
search_is_editable (ScatolaDirectory *directory)
{
    return FALSE;
}
...
search_dispose (GObject *object)
{
    ...
    search = SCATOLA_SEARCH_DIRECTORY (object);
    ...
    G_OBJECT_CLASS (scatola_search_directory_parent_class)->dispose (object);
}
...
search_finalize (GObject *object)
{
    ...
    G_OBJECT_CLASS (scatola_search_directory_parent_class)->finalize (object);
}

static void
scatola_search_directory_init (ScatolaSearchDirectory *search)
{
    search->details = g_new0 (ScatolaSearchDirectoryDetails, 1);
}

static void
scatola_search_directory_class_init (ScatolaSearchDirectoryClass *class)
{
...
char *
scatola_search_directory_generate_new_uri (void)
{
...
void
scatola_search_directory_set_query (ScatolaSearchDirectory *search,
                                 ScatolaQuery *query)
{
    ...
    dir = SCATOLA_DIRECTORY (search);
    as_file = dir->details->as_file;
    if (as_file != NULL)
    {
        scatola_search_directory_file_update_display_name (SCATOLA_SEARCH_DIRECTORY_FILE (as_file));
    }
}

ScatolaQuery *
scatola_search_directory_get_query (ScatolaSearchDirectory *search)
{
...
scatola_search_directory_new_from_saved_search (const char *uri)
{
    ScatolaSearchDirectory *search;
    ScatolaQuery *query;
    char *file;

    search = SCATOLA_SEARCH_DIRECTORY (g_object_new (SCATOLA_TYPE_SEARCH_DIRECTORY, NULL));
    ...
    if (file != NULL)
    {
        query = scatola_query_load (file);
        if (query != NULL)
        {
            scatola_search_directory_set_query (search, query);
...
gboolean
scatola_search_directory_is_saved_search (ScatolaSearchDirectory *search)
{
    return search->details->saved_search_uri != NULL;
}

gboolean
scatola_search_directory_is_modified (ScatolaSearchDirectory *search)
{
    return search->details->modified;
}

gboolean
scatola_search_directory_is_indexed (ScatolaSearchDirectory *search)
{
    ensure_search_engine (search);
    return scatola_search_engine_is_indexed (search->details->engine);
}


void
scatola_search_directory_save_to_file (ScatolaSearchDirectory *search,
                                    const char              *save_file_uri)
{
    ...
    if (search->details->query != NULL)
    {
        scatola_query_save (search->details->query, file);
    }
...
void
scatola_search_directory_save_search (ScatolaSearchDirectory *search)
{
    ...
    scatola_search_directory_save_to_file (search,
