...
/* scatola-dnd.c - Common Drag & drop handling code shared by the icon container
   and the list view.
...
/* FIXME: This should really be back in Scatola, not here in Eel. */
...
void
scatola_drag_init (ScatolaDragInfo     *drag_info,
...
void
scatola_drag_finalize (ScatolaDragInfo *drag_info)
{
    gtk_target_list_unref (drag_info->target_list);
    scatola_drag_destroy_selection_list (drag_info->selection_list);

    g_free (drag_info);
}


/* Functions to deal with ScatolaDragSelectionItems.  */

ScatolaDragSelectionItem *
scatola_drag_selection_item_new (void)
{
    return g_new0 (ScatolaDragSelectionItem, 1);
}

static void
drag_selection_item_destroy (ScatolaDragSelectionItem *item)
{
    g_free (item->uri);
    g_free (item);
}
...
void
scatola_drag_destroy_selection_list (GList *list)
{
    GList *p;
...
GList *
scatola_drag_uri_list_from_selection_list (const GList *selection_list)
{
    ScatolaDragSelectionItem *selection_item;
    GList *uri_list;
    const GList *l;
    ...
    {
        selection_item = (ScatolaDragSelectionItem *) l->data;
...
GList *
scatola_drag_uri_list_from_array (const char **uris)
{
    GList *uri_list;
    int i;
...
GList *
scatola_drag_build_selection_list (GtkSelectionData *data)
{
    GList *result;
    const guchar *p, *oldp;
    int size;
    ...
    while (size > 0)
    {
        ...
        if (*p == '\n' || *p == '\0')
        {
            ...
            if (p == 0)
            {
                g_warning ("Invalid x-special/gde2-icon-list data received: "
...
static gboolean
scatola_drag_file_local_internal (const char *target_uri_string,
...
gboolean
scatola_drag_uris_local (const char *target_uri,
                      const GList *source_uri_list)
{
    /* must have at least one item */
    g_assert (source_uri_list);

    return scatola_drag_file_local_internal (target_uri, source_uri_list->data);
}

gboolean
scatola_drag_items_local (const char *target_uri_string,
                       const GList *selection_list)
{
    /* must have at least one item */
    g_assert (selection_list);

    return scatola_drag_file_local_internal (target_uri_string,
                                          ((ScatolaDragSelectionItem *)selection_list->data)->uri);
}

gboolean
scatola_drag_items_on_desktop (const GList *selection_list)
{
    char *uri;
     ...
     * and currently dropped items can only be from the same container
     */
    uri = ((ScatolaDragSelectionItem *)selection_list->data)->uri;
    if (eel_uri_is_desktop (uri))
    {
        return TRUE;
    }

    desktop = scatola_get_desktop_location ();
...
GdkDragAction
scatola_drag_default_drop_action_for_netscape_url (GdkDragContext *context)
{
...
static gboolean
check_same_fs (ScatolaFile *file1,
               ScatolaFile *file2)
{
    char *id1, *id2;
    gboolean result;
    ...
    {
        id1 = scatola_file_get_filesystem_id (file1);
        id2 = scatola_file_get_filesystem_id (file2);
...
{
    ScatolaFile *naut_file;
    gboolean ret;

    /* if there's no a cached ScatolaFile, it returns NULL */
    naut_file = scatola_file_get_existing (file);
    if (naut_file == NULL)
    {
        return FALSE;
    }

    ret = scatola_file_can_delete (naut_file);
    scatola_file_unref (naut_file);

    return ret;
}

void
scatola_drag_default_drop_action_for_icons (GdkDragContext *context,
        ...
        int *action)
{
    ...
    ScatolaFile *dropped_file, *target_file;
    ...
    dropped_uri = ((ScatolaDragSelectionItem *)items->data)->uri;
    dropped_file = scatola_file_get_existing_by_uri (dropped_uri);
    target_file = scatola_file_get_existing_by_uri (target_uri_string);

    /*
     * Check for trash URI.  We do a find_directory for any Trash directory.
     * Passing 0 permissions as gde2-vfs would override the permissions
    ...
    if (eel_uri_is_trash (target_uri_string))
    {
        ...
        scatola_file_unref (dropped_file);
        scatola_file_unref (target_file);
        return;

    }
    else if (dropped_file != NULL && scatola_file_is_launcher (dropped_file))
    {
        if (actions & GDK_ACTION_MOVE)
        {
            *action = GDK_ACTION_MOVE;
    ...
    {
        target = scatola_get_desktop_location ();

        scatola_file_unref (target_file);
        target_file = scatola_file_get (target);
    ...
    else if (target_file != NULL && scatola_file_is_archive (target_file))
    {
...
GdkDragAction
scatola_drag_default_drop_action_for_uri_list (GdkDragContext *context,
...
static gboolean
is_path_that_gde2_uri_list_extract_filenames_can_parse (const char *path)
{
 ...
 * trying to make the old gde2_uri_list_extract_filenames function
...
add_one_compatible_uri (const char *uri, int x, int y, int w, int h, gpointer data)
{
     ...
     * gde2-libs function or nothing will be able to understand
     * it.
     */
    ...
    else
    {
         ...
         * gde2_uri_list_extract_filenames implementation, and just leave
         * out any paths with those in them.
         */
        if (is_path_that_gde2_uri_list_extract_filenames_can_parse (local_path))
        {
...
gboolean
scatola_drag_drag_data_get (GtkWidget *widget,
                         ...
                         ScatolaDragEachSelectedItemIterator each_selected_item_iterator)
{
    ...
    case SCATOLA_ICON_DND_GDE2_ICON_LIST:
        ...
        (* each_selected_item_iterator) (add_one_gde2_icon, container_context, result);
        break;
...
GdkDragAction
scatola_drag_drop_action_ask (GtkWidget *widget,
                           GdkDragAction actions)
{
    GtkWidget *menu;
    GtkWidget *menu_item;
    ...
    append_drop_action_menu_item (menu, _("Set as _Background"),
                                  SCATOLA_DND_ACTION_SET_AS_BACKGROUND,
                                  (actions & SCATOLA_DND_ACTION_SET_AS_BACKGROUND) != 0,
                                  &damd);
...
GdkDragAction
scatola_drag_drop_background_ask (GtkWidget *widget,
                               GdkDragAction actions)
{
    GtkWidget *menu;
    GtkWidget *menu_item;
    ...
    append_drop_action_menu_item (menu, _("Set as background for _all folders"),
                                  SCATOLA_DND_ACTION_SET_AS_GLOBAL_BACKGROUND,
                                  (actions & SCATOLA_DND_ACTION_SET_AS_GLOBAL_BACKGROUND) != 0,
                                  &damd);

    append_drop_action_menu_item (menu, _("Set as background for _this folder"),
                                  SCATOLA_DND_ACTION_SET_AS_FOLDER_BACKGROUND,
                                  (actions & SCATOLA_DND_ACTION_SET_AS_FOLDER_BACKGROUND) != 0,
                                  &damd);
...
gboolean
scatola_drag_autoscroll_in_scroll_region (GtkWidget *widget)
{
...
void
scatola_drag_autoscroll_calculate_delta (GtkWidget *widget, float *x_scroll_delta, float *y_scroll_delta)
{
...
void
scatola_drag_autoscroll_start (ScatolaDragInfo *drag_info,
                            GtkWidget        *widget,
                            GSourceFunc       callback,
                            gpointer          user_data)
{
    if (scatola_drag_autoscroll_in_scroll_region (widget))
    {
...
void
scatola_drag_autoscroll_stop (ScatolaDragInfo *drag_info)
{
    if (drag_info->auto_scroll_timeout_id != 0)
    {
        g_source_remove (drag_info->auto_scroll_timeout_id);
...
gboolean
scatola_drag_selection_includes_special_link (GList *selection_list)
{
    GList *node;
    char *uri;
    ...
    {
        uri = ((ScatolaDragSelectionItem *) node->data)->uri;
                        ...
                        gpointer            user_data)
{
    ...
    g_assert (SCATOLA_IS_WINDOW_INFO (window));
    ...
    else
    {
        ...
        {
            target_slot = scatola_window_info_get_active_slot (SCATOLA_WINDOW_INFO (window));
        }

        if (target_slot != NULL)
        {
            target_uri = scatola_window_slot_info_get_current_location (target_slot);
        }
    }
    ...
    {
        if (drag_info->info == SCATOLA_ICON_DND_GDE2_ICON_LIST)
        {
            scatola_drag_default_drop_action_for_icons (context, target_uri,
                    drag_info->data.selection_list,
                    &action);
        }
        else if (drag_info->info == SCATOLA_ICON_DND_URI_LIST)
        {
            action = scatola_drag_default_drop_action_for_uri_list (context, target_uri);
        }
        else if (drag_info->info == SCATOLA_ICON_DND_NETSCAPE_URL)
        {
...
static void
drag_info_clear (ScatolaDragSlotProxyInfo *drag_info)
{
    if (!drag_info->have_data)
    {
        goto out;
    ...
    if (drag_info->info == SCATOLA_ICON_DND_GDE2_ICON_LIST)
    {
        scatola_drag_destroy_selection_list (drag_info->data.selection_list);
    }
    else if (drag_info->info == SCATOLA_ICON_DND_URI_LIST)
    {
                        ...
                        ScatolaDragSlotProxyInfo *drag_info)
{
    ...
    if (target_slot != NULL)
    {
        target_view = scatola_window_slot_info_get_current_view (target_slot);
    }
    ...
    if (target_slot != NULL && target_view != NULL)
    {
        if (drag_info->info == SCATOLA_ICON_DND_GDE2_ICON_LIST)
        {
            uri_list = scatola_drag_uri_list_from_selection_list (drag_info->data.selection_list);
            g_assert (uri_list != NULL);

            scatola_view_drop_proxy_received_uris (target_view,
                                                uri_list,
                                                target_uri,
        ...
        if (drag_info->info == SCATOLA_ICON_DND_NETSCAPE_URL)
        {
            scatola_view_drop_proxy_received_netscape_url (target_view,
                               ...
                               gpointer            user_data)
{
    ...
    {
        drag_info->data.selection_list = scatola_drag_build_selection_list (data);
    ...
    else if (info == SCATOLA_ICON_DND_URI_LIST)
    {
        ...
        drag_info->data.uri_list = scatola_drag_uri_list_from_array ((const char **) uris);
        g_strfreev (uris);

        drag_info->have_valid_data = drag_info->data.uri_list != NULL;
    }
    else if (info == SCATOLA_ICON_DND_NETSCAPE_URL)
    {
...
void
scatola_drag_slot_proxy_init (GtkWidget *widget,
                           ScatolaDragSlotProxyInfo *drag_info)
{
