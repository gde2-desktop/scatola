...
/* scatola-column-chooser.h - A column chooser widget
...
#include <config.h>
#include "scatola-column-chooser.h"

#include <string.h>
#include <gtk/gtk.h>
#include <glib/gi18n.h>

#include "scatola-column-utilities.h"

#if GTK_CHECK_VERSION (3, 0, 0)
#define gtk_vbox_new(X,Y) gtk_box_new(GTK_ORIENTATION_VERTICAL,Y)
#endif

struct _ScatolaColumnChooserDetails
{
    GtkTreeView *view;
    GtkListStore *store;
...
#if GTK_CHECK_VERSION (3, 0, 0)
G_DEFINE_TYPE(ScatolaColumnChooser, scatola_column_chooser, GTK_TYPE_BOX);
#else
G_DEFINE_TYPE(ScatolaColumnChooser, scatola_column_chooser, GTK_TYPE_HBOX);
#endif

static void scatola_column_chooser_constructed (GObject *object);

static void
scatola_column_chooser_set_property (GObject *object,
                                  guint param_id,
                                  const GValue *value,
                                  GParamSpec *pspec)
{
    ScatolaColumnChooser *chooser;

    chooser = SCATOLA_COLUMN_CHOOSER (object);
...
static void
scatola_column_chooser_class_init (ScatolaColumnChooserClass *chooser_class)
{
    GObjectClass *oclass;

    oclass = G_OBJECT_CLASS (chooser_class);

    oclass->set_property = scatola_column_chooser_set_property;
    oclass->constructed = scatola_column_chooser_constructed;

    signals[CHANGED] = g_signal_new
                       ("changed",
                        G_TYPE_FROM_CLASS (chooser_class),
                        G_SIGNAL_RUN_LAST,
                        G_STRUCT_OFFSET (ScatolaColumnChooserClass,
                                         changed),
                        NULL, NULL,
    ...
    g_type_class_add_private (chooser_class, sizeof (ScatolaColumnChooserDetails));
}

static void
update_buttons (ScatolaColumnChooser *chooser)
{
...
static void
list_changed (ScatolaColumnChooser *chooser)
{
                          ...
                          gpointer user_data)
{
    ...
    chooser = SCATOLA_COLUMN_CHOOSER (user_data);
...
{
    update_buttons (SCATOLA_COLUMN_CHOOSER (user_data));
}
...
row_deleted_callback (GtkTreeModel *model,
                      ...
                      gpointer user_data)
{
    list_changed (SCATOLA_COLUMN_CHOOSER (user_data));
}

static void
add_tree_view (ScatolaColumnChooser *chooser)
{
    GtkWidget *scrolled;
    GtkWidget *view;
    GtkListStore *store;
...
{
    g_signal_emit (SCATOLA_COLUMN_CHOOSER (user_data),
...
static void
add_buttons (ScatolaColumnChooser *chooser)
{
    GtkWidget *box;
    GtkWidget *separator;
...
static void
populate_tree (ScatolaColumnChooser *chooser)
{
    GList *columns;
    GList *l;

    columns = scatola_get_columns_for_file (chooser->details->file);
...
static void
scatola_column_chooser_constructed (GObject *object)
{
    ScatolaColumnChooser *chooser;
...
static void
scatola_column_chooser_init (ScatolaColumnChooser *chooser)
{
    chooser->details = G_TYPE_INSTANCE_GET_PRIVATE ((chooser), SCATOLA_TYPE_COLUMN_CHOOSER, ScatolaColumnChooserDetails);
...
static void
set_visible_columns (ScatolaColumnChooser *chooser,
...
static char **
get_column_names (ScatolaColumnChooser *chooser, gboolean only_visible)
{
    GPtrArray *ret;
    GtkTreeIter iter;
...
static gboolean
get_column_iter (ScatolaColumnChooser *chooser,
                 ScatolaColumn *column,
                 GtkTreeIter *iter)
{
    char *column_name;

    g_object_get (SCATOLA_COLUMN (column), "name", &column_name, NULL);
...
static void
set_column_order (ScatolaColumnChooser *chooser,
                  char **column_order)

{
    ...
    columns = scatola_get_columns_for_file (chooser->details->file);
    columns = scatola_sort_columns (columns, column_order);
    ...
    for (l = columns; l != NULL; l = l->next)
    {
        ...
        if (get_column_iter (chooser, SCATOLA_COLUMN (l->data), &iter))
        {
            GtkTreeIter before;
            if (path)
            {
...
void
scatola_column_chooser_set_settings (ScatolaColumnChooser *chooser,
                                  char **visible_columns,
                                  char **column_order)
{
    g_return_if_fail (SCATOLA_IS_COLUMN_CHOOSER (chooser));
...
void
scatola_column_chooser_get_settings (ScatolaColumnChooser *chooser,
                                  char ***visible_columns,
                                  char ***column_order)
{
    g_return_if_fail (SCATOLA_IS_COLUMN_CHOOSER (chooser));
...
GtkWidget *
scatola_column_chooser_new (ScatolaFile *file)
{
    return g_object_new (SCATOLA_TYPE_COLUMN_CHOOSER, "file", file, NULL);
}
