...
struct _ScatolaProgressInfoClass
{
...
G_DEFINE_TYPE (ScatolaProgressInfo, scatola_progress_info, G_TYPE_OBJECT)

GList *
scatola_get_all_progress_info (void)
{
    GList *l;
...
static void
scatola_progress_info_finalize (GObject *object)
{
    ScatolaProgressInfo *info;

    info = SCATOLA_PROGRESS_INFO (object);

    g_free (info->status);
    g_free (info->details);
    g_object_unref (info->cancellable);

    if (G_OBJECT_CLASS (scatola_progress_info_parent_class)->finalize)
    {
        (*G_OBJECT_CLASS (scatola_progress_info_parent_class)->finalize) (object);
    }
}

static void
scatola_progress_info_dispose (GObject *object)
{
    ScatolaProgressInfo *info;
...
static void
scatola_progress_info_class_init (ScatolaProgressInfoClass *klass)
{
    GObjectClass *gobject_class = G_OBJECT_CLASS (klass);

    gobject_class->finalize = scatola_progress_info_finalize;
    gobject_class->dispose = scatola_progress_info_dispose;

    signals[CHANGED] =
        g_signal_new ("changed",
                      SCATOLA_TYPE_PROGRESS_INFO,
                      G_SIGNAL_RUN_LAST,
                      0,
                      NULL, NULL,
...
update_data (ProgressWidgetData *data)
{
    ...
    status = scatola_progress_info_get_status (data->info);
    gtk_label_set_text (data->status, status);
    g_free (status);

    details = scatola_progress_info_get_details (data->info);
...
update_progress (ProgressWidgetData *data)
{
    ...
    progress = scatola_progress_info_get_progress (data->info);
    if (progress < 0)
    {
...
{
    scatola_progress_info_cancel (data->info);
    gtk_widget_set_sensitive (button, FALSE);
}


static GtkWidget *
progress_widget_new (ScatolaProgressInfo *info)
{
    ProgressWidgetData *data;
...
static void
handle_new_progress_info (ScatolaProgressInfo *info)
{
...
static gboolean
new_op_started_timeout (ScatolaProgressInfo *info)
{
    if (scatola_progress_info_get_is_paused (info))
    {
        return TRUE;
    }
    if (!scatola_progress_info_get_is_finished (info))
    {
...
static void
new_op_started (ScatolaProgressInfo *info)
{
...
static void
scatola_progress_info_init (ScatolaProgressInfo *info)
{
...
ScatolaProgressInfo *
scatola_progress_info_new (void)
{
    ScatolaProgressInfo *info;

    info = g_object_new (SCATOLA_TYPE_PROGRESS_INFO, NULL);

    return info;
}

char *
scatola_progress_info_get_status (ScatolaProgressInfo *info)
{
    char *res;
...
char *
scatola_progress_info_get_details (ScatolaProgressInfo *info)
{
    char *res;
...
double
scatola_progress_info_get_progress (ScatolaProgressInfo *info)
{
    double res;
...
void
scatola_progress_info_cancel (ScatolaProgressInfo *info)
{
    G_LOCK (progress_info);
...
GCancellable *
scatola_progress_info_get_cancellable (ScatolaProgressInfo *info)
{
    GCancellable *c;
...
gboolean
scatola_progress_info_get_is_started (ScatolaProgressInfo *info)
{
    gboolean res;
...
gboolean
scatola_progress_info_get_is_finished (ScatolaProgressInfo *info)
{
    gboolean res;
...
gboolean
scatola_progress_info_get_is_paused (ScatolaProgressInfo *info)
{
    gboolean res;
...
static void
queue_idle (ScatolaProgressInfo *info, gboolean now)
{
...
void
scatola_progress_info_pause (ScatolaProgressInfo *info)
{
    G_LOCK (progress_info);
...
void
scatola_progress_info_resume (ScatolaProgressInfo *info)
{
    G_LOCK (progress_info);
...
void
scatola_progress_info_start (ScatolaProgressInfo *info)
{
    G_LOCK (progress_info);
...
void
scatola_progress_info_finish (ScatolaProgressInfo *info)
{
    G_LOCK (progress_info);
...
void
scatola_progress_info_take_status (ScatolaProgressInfo *info,
                                char *status)
{
    G_LOCK (progress_info);
...
void
scatola_progress_info_set_status (ScatolaProgressInfo *info,
                               const char *status)
{
    G_LOCK (progress_info);
...
void
scatola_progress_info_take_details (ScatolaProgressInfo *info,
                                 char           *details)
{
    G_LOCK (progress_info);
...
void
scatola_progress_info_set_details (ScatolaProgressInfo *info,
                                const char           *details)
{
    G_LOCK (progress_info);
...
void
scatola_progress_info_pulse_progress (ScatolaProgressInfo *info)
{
    G_LOCK (progress_info);
...
void
scatola_progress_info_set_progress (ScatolaProgressInfo *info,
                                 double                current,
                                 double                total)
{
