...
/* scatola-icon-container.c - Icon container widget.
...
#include <config.h>
#include <math.h>
#include "scatola-icon-container.h"

#include "scatola-debug-log.h"
#include "scatola-global-preferences.h"
#include "scatola-icon-private.h"
#include "scatola-lib-self-check-functions.h"
#include "scatola-marshal.h"
...
/* From scatola-icon-canvas-item.c */
...
/* Copied from ScatolaIconContainer */
#define SCATOLA_ICON_CONTAINER_SEARCH_DIALOG_TIMEOUT 5

/* Copied from ScatolaFile */
...
} ScatolaIconContainerAccessiblePrivate;

static GType         scatola_icon_container_accessible_get_type (void);

#if GTK_CHECK_VERSION(3, 0, 0)
typedef struct _ScatolaIconContainerAccessible ScatolaIconContainerAccessible;
typedef struct _ScatolaIconContainerAccessibleClass ScatolaIconContainerAccessibleClass;

struct _ScatolaIconContainerAccessible
{
    EelCanvasAccessible parent;
};

struct _ScatolaIconContainerAccessibleClass
{
    EelCanvasAccessibleClass parent_class;
};
#endif

static void          activate_selected_items                        (ScatolaIconContainer *container);
static void          activate_selected_items_alternate              (ScatolaIconContainer *container,
        ScatolaIcon          *icon);
#if !GTK_CHECK_VERSION(3, 0, 0)
static void          scatola_icon_container_theme_changed          (gpointer               user_data);
#endif
static void          compute_stretch                                (StretchState          *start,
        StretchState          *current);
static ScatolaIcon *get_first_selected_icon                        (ScatolaIconContainer *container);
static ScatolaIcon *get_nth_selected_icon                          (ScatolaIconContainer *container,
        int                    index);
static gboolean      has_multiple_selection                         (ScatolaIconContainer *container);
static gboolean      all_selected                                   (ScatolaIconContainer *container);
static gboolean      has_selection                                  (ScatolaIconContainer *container);
static void          icon_destroy                                   (ScatolaIconContainer *container,
        ScatolaIcon          *icon);
static void          end_renaming_mode                              (ScatolaIconContainer *container,
        gboolean               commit);
static ScatolaIcon *get_icon_being_renamed                         (ScatolaIconContainer *container);
static void          finish_adding_new_icons                        (ScatolaIconContainer *container);
#if !GTK_CHECK_VERSION(3,0,0)
static void          update_label_color                             (EelBackground         *background,
        ScatolaIconContainer *icon_container);
#endif
static inline void   icon_get_bounding_box                          (ScatolaIcon          *icon,
        int                   *x1_return,
        int                   *y1_return,
        int                   *x2_return,
        int                   *y2_return,
        ScatolaIconCanvasItemBoundsUsage usage);
static gboolean      is_renaming                                    (ScatolaIconContainer *container);
static gboolean      is_renaming_pending                            (ScatolaIconContainer *container);
static void          process_pending_icon_to_rename                 (ScatolaIconContainer *container);
#if !GTK_CHECK_VERSION(3, 0, 0)
static void          setup_label_gcs                                (ScatolaIconContainer *container);
#endif
static void          scatola_icon_container_stop_monitor_top_left  (ScatolaIconContainer *container,
        ScatolaIconData      *data,
        gconstpointer          client);
static void          scatola_icon_container_start_monitor_top_left (ScatolaIconContainer *container,
        ScatolaIconData      *data,
        gconstpointer          client,
        gboolean               large_text);
static void          handle_hadjustment_changed                     (GtkAdjustment         *adjustment,
        ScatolaIconContainer *container);
static void          handle_vadjustment_changed                     (GtkAdjustment         *adjustment,
        ScatolaIconContainer *container);
static GList *       scatola_icon_container_get_selected_icons (ScatolaIconContainer *container);
static void          scatola_icon_container_update_visible_icons   (ScatolaIconContainer *container);
static void          reveal_icon                                    (ScatolaIconContainer *container,
        ScatolaIcon *icon);

static void	     scatola_icon_container_set_rtl_positions (ScatolaIconContainer *container);
static double	     get_mirror_x_position                     (ScatolaIconContainer *container,
        ScatolaIcon *icon,
        double x);
static void         text_ellipsis_limit_changed_container_callback  (gpointer callback_data);

static int compare_icons_horizontal (ScatolaIconContainer *container,
                                     ScatolaIcon *icon_a,
                                     ScatolaIcon *icon_b);

static int compare_icons_vertical (ScatolaIconContainer *container,
                                   ScatolaIcon *icon_a,
                                   ScatolaIcon *icon_b);

static void store_layout_timestamps_now (ScatolaIconContainer *container);

static gpointer accessible_parent_class;

static GQuark accessible_private_data_quark = 0;

static const char *scatola_icon_container_accessible_action_names[] =
{
    "activate",
    "menu",
    NULL
...
static const char *scatola_icon_container_accessible_action_descriptions[] =
{
...
G_DEFINE_TYPE (ScatolaIconContainer, scatola_icon_container, EEL_TYPE_CANVAS);

/* The ScatolaIconContainer signals.  */
enum
{
    ACTIVATE,
    ACTIVATE_ALTERNATE,
...
/* Functions dealing with ScatolaIcons.  */

static void
icon_free (ScatolaIcon *icon)
{
...
static gboolean
icon_is_positioned (const ScatolaIcon *icon)
{
...
static void
icon_set_position (ScatolaIcon *icon,
                   double x, double y)
{
    ScatolaIconContainer *container;
    ...
    container = SCATOLA_ICON_CONTAINER (EEL_CANVAS_ITEM (icon->item)->canvas);
    ...
    if (scatola_icon_container_get_is_fixed_size (container))
    {
        ...
        */
        ...
        icon_bounds = scatola_icon_canvas_item_get_icon_rectangle (icon->item);
...
static void
icon_get_size (ScatolaIconContainer *container,
               ScatolaIcon *icon,
               guint *size)
{
    if (size != NULL)
    {
        *size = MAX (scatola_get_icon_size_for_zoom_level (container->details->zoom_level)
                     * icon->scale, SCATOLA_ICON_SIZE_SMALLEST);
    }
}
 ...
 */
static void
icon_set_size (ScatolaIconContainer *container,
               ScatolaIcon *icon,
               guint icon_size,
               gboolean snap,
               gboolean update_position)
{
    ...
    scale = (double) icon_size /
            scatola_get_icon_size_for_zoom_level
            (container->details->zoom_level);
    scatola_icon_container_move_icon (container, icon,
                                   icon->x, icon->y,
                                   scale, FALSE,
...
static void
icon_raise (ScatolaIcon *icon)
{
    EelCanvasItem *item, *band;

    item = EEL_CANVAS_ITEM (icon->item);
    band = SCATOLA_ICON_CONTAINER (item->canvas)->details->rubberband_info.selection_rectangle;

    eel_canvas_item_send_behind (item, band);
}

static void
emit_stretch_started (ScatolaIconContainer *container, ScatolaIcon *icon)
{
...
static void
emit_stretch_ended (ScatolaIconContainer *container, ScatolaIcon *icon)
{
    g_signal_emit (container,
...
static void
icon_toggle_selected (ScatolaIconContainer *container,
                      ScatolaIcon *icon)
{
    end_renaming_mode (container, TRUE);
    ...
    if (icon == container->details->stretch_icon)
    {
        ...
        scatola_icon_canvas_item_set_show_stretch_handles (icon->item, FALSE);
        /* snap the icon if necessary */
        if (container->details->keep_aligned)
        {
            scatola_icon_container_move_icon (container,
                                           icon,
                                           icon->x, icon->y,
                                           icon->scale,
                                           FALSE, TRUE, TRUE);
...
static gboolean
icon_set_selected (ScatolaIconContainer *container,
                   ScatolaIcon *icon,
                   gboolean select)
{
...
static inline void
icon_get_bounding_box (ScatolaIcon *icon,
                       ...
                       ScatolaIconCanvasItemBoundsUsage usage)
{
    ...
    {
        scatola_icon_canvas_item_get_bounds_for_layout (icon->item,
                &x1, &y1, &x2, &y2);
    }
    else if (usage == BOUNDS_USAGE_FOR_ENTIRE_ITEM)
    {
        scatola_icon_canvas_item_get_bounds_for_entire_item (icon->item,
                &x1, &y1, &x2, &y2);
    }
    else
    {
...
/* Utility functions for ScatolaIconContainer.  */

gboolean
scatola_icon_container_scroll (ScatolaIconContainer *container,
...
static void
pending_icon_to_reveal_destroy_callback (ScatolaIconCanvasItem *item,
        ScatolaIconContainer *container)
{
    g_assert (SCATOLA_IS_ICON_CONTAINER (container));
...
static ScatolaIcon*
get_pending_icon_to_reveal (ScatolaIconContainer *container)
{
    return container->details->pending_icon_to_reveal;
}

static void
set_pending_icon_to_reveal (ScatolaIconContainer *container, ScatolaIcon *icon)
{
    ScatolaIcon *old_icon;
...
static void
icon_get_row_and_column_bounds (ScatolaIconContainer *container,
                                ScatolaIcon *icon,
                                EelIRect *bounds,
                                gboolean safety_pad)
{
    GList *p;
    ScatolaIcon *one_icon;
    EelIRect one_bounds;
...
static void
reveal_icon (ScatolaIconContainer *container,
             ScatolaIcon *icon)
{
    ...
    if (scatola_icon_container_is_auto_layout (container)) {
...
static void
process_pending_icon_to_reveal (ScatolaIconContainer *container)
{
    ScatolaIcon *pending_icon_to_reveal;
...
keyboard_icon_reveal_timeout_callback (gpointer data)
{
    ...
    container = SCATOLA_ICON_CONTAINER (data);
...
static void
unschedule_keyboard_icon_reveal (ScatolaIconContainer *container)
{
...
static void
schedule_keyboard_icon_reveal (ScatolaIconContainer *container,
                               ScatolaIcon *icon)
{
    ScatolaIconContainerDetails *details;
...
static void
clear_keyboard_focus (ScatolaIconContainer *container)
{
...
static void inline
emit_atk_focus_state_change (ScatolaIcon *icon, gboolean focused)
{
...
static void
set_keyboard_focus (ScatolaIconContainer *container,
                    ScatolaIcon *icon)
{
    g_assert (icon != NULL);
...
static void
set_keyboard_rubberband_start (ScatolaIconContainer *container,
                               ScatolaIcon *icon)
{
...
static void
clear_keyboard_rubberband_start (ScatolaIconContainer *container)
{
    container->details->keyboard_rubberband_start = NULL;
}

/* carbon-copy of eel_canvas_group_bounds(), but
 * for ScatolaIconContainerItems it returns the
                                   ...
                                   ScatolaIconCanvasItemBoundsUsage usage)
{
    ...
    for (list = group->item_list; list; list = list->next)
    {
        ...
        {
            set = TRUE;
            if (!SCATOLA_IS_ICON_CANVAS_ITEM (child) ||
            ...
            {
                scatola_icon_canvas_item_get_bounds_for_layout (SCATOLA_ICON_CANVAS_ITEM (child),
                        &minx, &miny, &maxx, &maxy);
            }
            else if (usage == BOUNDS_USAGE_FOR_ENTIRE_ITEM)
            {
                scatola_icon_canvas_item_get_bounds_for_entire_item (SCATOLA_ICON_CANVAS_ITEM (child),
                        &minx, &miny, &maxx, &maxy);
            }
            else
            {
...
static void
get_all_icon_bounds (ScatolaIconContainer *container,
                     double *x1, double *y1,
                     double *x2, double *y2,
                     ScatolaIconCanvasItemBoundsUsage usage)
{
...
void
scatola_icon_container_reset_scroll_region (ScatolaIconContainer *container)
{
...
void
scatola_icon_container_update_scroll_region (ScatolaIconContainer *container)
{
    double x1, y1, x2, y2;
    double pixels_per_unit;
    GtkAdjustment *hadj, *vadj;
    ...
    reset_scroll_region = container->details->reset_scroll_region_trigger
                          || scatola_icon_container_is_empty (container)
                          || scatola_icon_container_is_auto_layout (container);
     ...
     */
    if (!scatola_icon_container_is_empty (container))
    {
     ...
     */
    if (scatola_icon_container_is_layout_vertical (container))
    {
        if (scatola_icon_container_is_layout_rtl (container))
        {
    ...
    step_increment = scatola_get_icon_size_for_zoom_level
...
{
    ScatolaIconContainerClass *klass;
    const ScatolaIcon *icon_a, *icon_b;

    icon_a = a;
    icon_b = b;
    klass  = SCATOLA_ICON_CONTAINER_GET_CLASS (icon_container);

    return klass->compare_icons (icon_container, icon_a->data, icon_b->data);
}

static void
sort_icons (ScatolaIconContainer *container,
            GList                **icons)
{
    ScatolaIconContainerClass *klass;

    klass = SCATOLA_ICON_CONTAINER_GET_CLASS (container);
...
static void
resort (ScatolaIconContainer *container)
{
...
#if 0
static double
get_grid_width (ScatolaIconContainer *container)
{
    if (container->details->label_position == SCATOLA_ICON_LABEL_POSITION_BESIDE)
    {
...
static void
lay_down_one_line (ScatolaIconContainer *container,
                   GList *line_start,
                   GList *line_end,
                   double y,
                   double max_height,
                   ...
                   gboolean whole_text)
{
    ...
    is_rtl = scatola_icon_container_is_layout_rtl (container);
    ...
    for (p = line_start; p != line_end; p = p->next)
    {
        ...
        (icon,
         ...
         y + y_offset);
        scatola_icon_canvas_item_set_entire_text (icon->item, whole_text);
...
static void
lay_down_one_column (ScatolaIconContainer *container,
                     GList *line_start,
                     GList *line_end,
                     double x,
                     double y_start,
...
static void
lay_down_icons_horizontal (ScatolaIconContainer *container,
                           GList *icons,
                           double start_y)
{
    GList *p, *line_start;
    ...
    if (container->details->label_position == SCATOLA_ICON_LABEL_POSITION_BESIDE)
    {
        ...
        for (p = icons; p != NULL; p = p->next)
        {
            ...
            text_bounds = scatola_icon_canvas_item_get_text_rectangle (icon->item, TRUE);
    ...
    gridded_layout = !scatola_icon_container_is_tighter_layout (container);

    line_width = container->details->label_position == SCATOLA_ICON_LABEL_POSITION_BESIDE ? ICON_PAD_LEFT : 0;
    line_start = icons;
...
static void
lay_down_icons_vertical (ScatolaIconContainer *container,
                         GList *icons,
                         double start_y)
{
    GList *p, *line_start;
    ...
    g_assert (SCATOLA_IS_ICON_CONTAINER (container));
    g_assert (container->details->label_position == SCATOLA_ICON_LABEL_POSITION_BESIDE);
...
static void
snap_position (ScatolaIconContainer *container,
               ScatolaIcon *icon,
               int *x, int *y)
{
    int center_x;
    ...
    icon_position = scatola_icon_canvas_item_get_icon_rectangle (icon->item);
...
{
    ScatolaIcon *icon_a, *icon_b;
    int x1, y1, x2, y2;
    int center_a;
    int center_b;

    icon_a = (ScatolaIcon*)a;
    icon_b = (ScatolaIcon*)b;
...
placement_grid_new (ScatolaIconContainer *container, gboolean tight)
{
    PlacementGrid *grid;
    int width, height;
    int num_columns;
...
static void
placement_grid_mark_icon (PlacementGrid *grid, ScatolaIcon *icon)
{
    EelIRect icon_pos;
    EelIRect grid_pos;
...
static void
find_empty_location (ScatolaIconContainer *container,
                     PlacementGrid *grid,
                     ScatolaIcon *icon,
                     int start_x,
                     int start_y,
                     ...
                     int *y)
{
    ...
    pixbuf_rect = scatola_icon_canvas_item_get_icon_rectangle (icon->item);
...
static void
align_icons (ScatolaIconContainer *container)
{
    ...
    if (scatola_icon_container_is_layout_rtl (container))
    {
        scatola_icon_container_set_rtl_positions (container);
    }
}

static double
get_mirror_x_position (ScatolaIconContainer *container, ScatolaIcon *icon, double x)
{
    EelDRect icon_bounds;
    GtkAllocation allocation;
...
static void
scatola_icon_container_set_rtl_positions (ScatolaIconContainer *container)
{
    GList *l;
    ScatolaIcon *icon;
    double x;
...
static void
lay_down_icons_vertical_desktop (ScatolaIconContainer *container, GList *icons)
{
    ...
    if (placed > 0)
    {
        ...
        if (grid)
        {
            ...
            for (p = unplaced_icons; p != NULL; p = p->next)
            {
                ...
                icon_rect = scatola_icon_canvas_item_get_icon_rectangle (icon->item);
     ...
     */
    scatola_icon_container_freeze_icon_positions (container);
}


static void
lay_down_icons (ScatolaIconContainer *container, GList *icons, double start_y)
{
    switch (container->details->layout_mode)
    {
    case SCATOLA_ICON_LAYOUT_L_R_T_B:
    case SCATOLA_ICON_LAYOUT_R_L_T_B:
        lay_down_icons_horizontal (container, icons, start_y);
        break;

    case SCATOLA_ICON_LAYOUT_T_B_L_R:
    case SCATOLA_ICON_LAYOUT_T_B_R_L:
        if (scatola_icon_container_get_is_desktop (container))
        {
...
static void
redo_layout_internal (ScatolaIconContainer *container)
{
    ...
    scatola_icon_container_update_scroll_region (container);

    process_pending_icon_to_reveal (container);
    process_pending_icon_to_rename (container);
    scatola_icon_container_update_visible_icons (container);
}
...
redo_layout_callback (gpointer callback_data)
{
    ...
    container = SCATOLA_ICON_CONTAINER (callback_data);
...
static void
unschedule_redo_layout (ScatolaIconContainer *container)
{
...
static void
schedule_redo_layout (ScatolaIconContainer *container)
{
    if (container->details->idle_id == 0
...
static void
redo_layout (ScatolaIconContainer *container)
{
...
static void
reload_icon_positions (ScatolaIconContainer *container)
{
...
static void
invalidate_label_sizes (ScatolaIconContainer *container)
{
    GList *p;
    ScatolaIcon *icon;
    ...
    for (p = container->details->icons; p != NULL; p = p->next)
    {
        ...
        scatola_icon_canvas_item_invalidate_label_size (icon->item);
    }
}

/* invalidate the entire labels (i.e. their attributes) for all the icons */
static void
invalidate_labels (ScatolaIconContainer *container)
{
    GList *p;
    ScatolaIcon *icon;
    ...
    for (p = container->details->icons; p != NULL; p = p->next)
    {
        ...
        scatola_icon_canvas_item_invalidate_label (icon->item);
    }
}

static gboolean
select_range (ScatolaIconContainer *container,
              ScatolaIcon *icon1,
              ScatolaIcon *icon2,
...
static gboolean
select_one_unselect_others (ScatolaIconContainer *container,
                            ScatolaIcon *icon_to_select)
{
    gboolean selection_changed;
    GList *p;
    ScatolaIcon *icon;
...
static gboolean
unselect_all (ScatolaIconContainer *container)
{
    return select_one_unselect_others (container, NULL);
}

void
scatola_icon_container_move_icon (ScatolaIconContainer *container,
                               ScatolaIcon *icon,
                               int x, int y,
                               double scale,
                               gboolean raise,
                               ...
                               gboolean update_position)
{
    ...
    {
        icon->scale = scale;
        scatola_icon_container_update_icon (container, icon);
        if (update_position)
        {
    ...
    if (!details->auto_layout)
    {
        ...
        icon->saved_ltr_x = scatola_icon_container_is_layout_rtl (container) ? get_mirror_x_position (container, icon, icon->x) : icon->x;
    }
...
static void
rubberband_select (ScatolaIconContainer *container,
                   ...
                   const EelDRect *current_rect)
{
    ...
    for (p = container->details->icons; p != NULL; p = p->next)
    {
        ...
        is_in = scatola_icon_canvas_item_hit_test_rectangle (icon->item, canvas_rect);
...
rubberband_timeout_callback (gpointer data)
{
    ScatolaIconContainer *container;
    GtkWidget *widget;
    ScatolaIconRubberbandInfo *band_info;
    int x, y;
    double x1, y1, x2, y2;
    ...
    scatola_icon_container_scroll (container, x_scroll, y_scroll);
...
#if GTK_CHECK_VERSION(3,0,0)
/*borrowed from Nemo, makes Scatola rubberbanding follow same selectors as Nemo and presumably Nautilus */
static void
start_rubberbanding (ScatolaIconContainer *container,
...
static void
#if GTK_CHECK_VERSION(3, 20, 0)
stop_rubberbanding (ScatolaIconContainer *container)
#else
stop_rubberbanding (ScatolaIconContainer *container,
                    guint32 time)
#endif
{
    ...
    icons = scatola_icon_container_get_selected_icons (container);
...
typedef gboolean (* IsBetterIconFunction) (ScatolaIconContainer *container,
        ScatolaIcon *start_icon,
        ScatolaIcon *best_so_far,
        ScatolaIcon *candidate,
        void *data);

static ScatolaIcon *
find_best_icon (ScatolaIconContainer *container,
                ScatolaIcon *start_icon,
                IsBetterIconFunction function,
                void *data)
{
    GList *p;
    ScatolaIcon *best, *candidate;
...
static ScatolaIcon *
find_best_selected_icon (ScatolaIconContainer *container,
                         ScatolaIcon *start_icon,
                         IsBetterIconFunction function,
                         void *data)
{
...
static int
compare_icons_by_uri (ScatolaIconContainer *container,
                      ScatolaIcon *icon_a,
                      ScatolaIcon *icon_b)
{
    char *uri_a, *uri_b;
    ...
    uri_a = scatola_icon_container_get_icon_uri (container, icon_a);
    uri_b = scatola_icon_container_get_icon_uri (container, icon_b);
...
static int
get_cmp_point_x (ScatolaIconContainer *container,
                 EelDRect icon_rect)
{
    if (container->details->label_position == SCATOLA_ICON_LABEL_POSITION_BESIDE)
    {
...
static int
get_cmp_point_y (ScatolaIconContainer *container,
                 EelDRect icon_rect)
{
    if (container->details->label_position == SCATOLA_ICON_LABEL_POSITION_BESIDE)
    {
...
static int
compare_icons_horizontal (ScatolaIconContainer *container,
                          ScatolaIcon *icon_a,
                          ScatolaIcon *icon_b)
{
    EelDRect world_rect;
    ...
    world_rect = scatola_icon_canvas_item_get_icon_rectangle (icon_a->item);
    eel_canvas_w2c
    (EEL_CANVAS (container),
     ...
     &ax,
     NULL);
    world_rect = scatola_icon_canvas_item_get_icon_rectangle (icon_b->item);
    eel_canvas_w2c
    (EEL_CANVAS (container),
     get_cmp_point_x (container, world_rect),
     get_cmp_point_y (container, world_rect),
...
static int
compare_icons_vertical (ScatolaIconContainer *container,
                        ScatolaIcon *icon_a,
                        ScatolaIcon *icon_b)
{
    EelDRect world_rect;
...
static int
compare_icons_horizontal_first (ScatolaIconContainer *container,
                                ScatolaIcon *icon_a,
                                ScatolaIcon *icon_b)
{
    EelDRect world_rect;
...
static int
compare_icons_vertical_first (ScatolaIconContainer *container,
                              ScatolaIcon *icon_a,
                              ScatolaIcon *icon_b)
{
    EelDRect world_rect;
...
static gboolean
leftmost_in_top_row (ScatolaIconContainer *container,
                     ScatolaIcon *start_icon,
                     ScatolaIcon *best_so_far,
                     ScatolaIcon *candidate,
                     void *data)
...
static gboolean
rightmost_in_top_row (ScatolaIconContainer *container,
                      ScatolaIcon *start_icon,
                      ScatolaIcon *best_so_far,
                      ScatolaIcon *candidate,
                      void *data)
...
static gboolean
rightmost_in_bottom_row (ScatolaIconContainer *container,
                         ScatolaIcon *start_icon,
                         ScatolaIcon *best_so_far,
                         ScatolaIcon *candidate,
                         void *data)
...
static int
compare_with_start_row (ScatolaIconContainer *container,
                        ScatolaIcon *icon)
{
    EelCanvasItem *item;
...
static int
compare_with_start_column (ScatolaIconContainer *container,
                           ScatolaIcon *icon)
{
    EelCanvasItem *item;
...
static gboolean
same_row_right_side_leftmost (ScatolaIconContainer *container,
                              ScatolaIcon *start_icon,
                              ScatolaIcon *best_so_far,
                              ScatolaIcon *candidate,
                              void *data)
...
static gboolean
same_row_left_side_rightmost (ScatolaIconContainer *container,
                              ScatolaIcon *start_icon,
                              ScatolaIcon *best_so_far,
                              ScatolaIcon *candidate,
                              void *data)
...
static gboolean
next_row_leftmost (ScatolaIconContainer *container,
                   ScatolaIcon *start_icon,
                   ScatolaIcon *best_so_far,
                   ScatolaIcon *candidate,
                   void *data)
...
static gboolean
next_row_rightmost (ScatolaIconContainer *container,
                    ScatolaIcon *start_icon,
                    ScatolaIcon *best_so_far,
                    ScatolaIcon *candidate,
                    void *data)
...
static gboolean
next_column_bottommost (ScatolaIconContainer *container,
                        ScatolaIcon *start_icon,
                        ScatolaIcon *best_so_far,
                        ScatolaIcon *candidate,
                        void *data)
...
static gboolean
previous_row_rightmost (ScatolaIconContainer *container,
                        ScatolaIcon *start_icon,
                        ScatolaIcon *best_so_far,
                        ScatolaIcon *candidate,
                        void *data)
...
static gboolean
same_column_above_lowest (ScatolaIconContainer *container,
                          ScatolaIcon *start_icon,
                          ScatolaIcon *best_so_far,
                          ScatolaIcon *candidate,
                          void *data)
...
static gboolean
same_column_below_highest (ScatolaIconContainer *container,
                           ScatolaIcon *start_icon,
                           ScatolaIcon *best_so_far,
                           ScatolaIcon *candidate,
                           void *data)
...
static gboolean
previous_column_highest (ScatolaIconContainer *container,
                         ScatolaIcon *start_icon,
                         ScatolaIcon *best_so_far,
                         ScatolaIcon *candidate,
                         void *data)
...
static gboolean
next_column_highest (ScatolaIconContainer *container,
                     ScatolaIcon *start_icon,
                     ScatolaIcon *best_so_far,
                     ScatolaIcon *candidate,
                     void *data)
...
static gboolean
previous_column_lowest (ScatolaIconContainer *container,
                        ScatolaIcon *start_icon,
                        ScatolaIcon *best_so_far,
                        ScatolaIcon *candidate,
                        void *data)
...
static gboolean
last_column_lowest (ScatolaIconContainer *container,
                    ScatolaIcon *start_icon,
                    ScatolaIcon *best_so_far,
                    ScatolaIcon *candidate,
                    void *data)
...
static gboolean
closest_in_90_degrees (ScatolaIconContainer *container,
                       ScatolaIcon *start_icon,
                       ScatolaIcon *best_so_far,
                       ScatolaIcon *candidate,
                       void *data)
{
    ...
    world_rect = scatola_icon_canvas_item_get_icon_rectangle (candidate->item);
    eel_canvas_w2c
    (EEL_CANVAS (container),
     get_cmp_point_x (container, world_rect),
     get_cmp_point_y (container, world_rect),
...
static EelDRect
get_rubberband (ScatolaIcon *icon1,
                ScatolaIcon *icon2)
{
    EelDRect rect1;
    EelDRect rect2;
...
static void
keyboard_move_to (ScatolaIconContainer *container,
                  ScatolaIcon *icon,
                  ScatolaIcon *from,
                  GdkEventKey *event)
{
...
static void
keyboard_home (ScatolaIconContainer *container,
               GdkEventKey *event)
{
    ScatolaIcon *from;
    ScatolaIcon *to;
...
static void
keyboard_end (ScatolaIconContainer *container,
              GdkEventKey *event)
{
    ScatolaIcon *to;
    ScatolaIcon *from;
    ...
    to = find_best_icon (container, NULL,
                         scatola_icon_container_is_layout_vertical (container) ?
                         last_column_lowest :
                         rightmost_in_bottom_row,
                         NULL);
...
static void
record_arrow_key_start (ScatolaIconContainer *container,
                        ScatolaIcon *icon,
                        GtkDirectionType direction)
{
    EelDRect world_rect;

    world_rect = scatola_icon_canvas_item_get_icon_rectangle (icon->item);
    eel_canvas_w2c
    (EEL_CANVAS (container),
     get_cmp_point_x (container, world_rect),
     get_cmp_point_y (container, world_rect),
...
static void
keyboard_arrow_key (ScatolaIconContainer *container,
                    GdkEventKey *event,
                    GtkDirectionType direction,
...
static void
keyboard_right (ScatolaIconContainer *container,
                GdkEventKey *event)
{
    ...
    keyboard_arrow_key (container,
                        ...
                        GTK_DIR_RIGHT,
                        rightmost_in_bottom_row,
                        scatola_icon_container_is_layout_rtl (container) ?
...
static void
keyboard_left (ScatolaIconContainer *container,
               GdkEventKey *event)
{
    IsBetterIconFunction fallback;
...
static void
keyboard_down (ScatolaIconContainer *container,
               GdkEventKey *event)
{
    IsBetterIconFunction fallback;
...
static void
keyboard_up (ScatolaIconContainer *container,
             GdkEventKey *event)
{
    IsBetterIconFunction fallback;
...
static void
keyboard_space (ScatolaIconContainer *container,
                GdkEventKey *event)
{
    ScatolaIcon *icon;
...
static void
select_previous_or_next_icon (ScatolaIconContainer *container,
                              gboolean next,
                              GdkEventKey *event)
{
    ScatolaIcon *icon;
...
#endif
{
    ...
    container = SCATOLA_ICON_CONTAINER (object);

    scatola_icon_container_clear (container);
...
#if GTK_CHECK_VERSION(3, 0, 0)
    GTK_WIDGET_CLASS (scatola_icon_container_parent_class)->destroy (object);
#else
    GTK_OBJECT_CLASS (scatola_icon_container_parent_class)->destroy (object);
#endif
}
...
finalize (GObject *object)
{
    ...
    details = SCATOLA_ICON_CONTAINER (object)->details;

    g_signal_handlers_disconnect_by_func (scatola_icon_view_preferences,
    ...
    G_OBJECT_CLASS (scatola_icon_container_parent_class)->finalize (object);
}
               ...
               GtkAllocation *allocation)
{
    ...
    container = SCATOLA_ICON_CONTAINER (widget);
    ...
    GTK_WIDGET_CLASS (scatola_icon_container_parent_class)->size_allocate (widget, allocation);
...
{
    if (!SCATOLA_ICON_CONTAINER (widget)->details->is_desktop)
    {
        eel_background_draw (widget, cr);
    }

    return GTK_WIDGET_CLASS (scatola_icon_container_parent_class)->draw (widget,
                                                                      cr);
}
#endif
...
realize (GtkWidget *widget)
{
    ...
    GTK_WIDGET_CLASS (scatola_icon_container_parent_class)->realize (widget);
...
#endif
    /* Set up DnD.  */
    scatola_icon_dnd_init (container);
...
unrealize (GtkWidget *widget)
{
    ...
    scatola_icon_dnd_fini (container);
    ...
    GTK_WIDGET_CLASS (scatola_icon_container_parent_class)->unrealize (widget);
}
...
style_updated (GtkWidget *widget)
{
    ...
    */
    if (!scatola_icon_container_get_is_desktop (container)) {
           GTK_WIDGET_CLASS (scatola_icon_container_parent_class)->style_updated (widget);
    }

    if (gtk_widget_get_realized (widget))
    {
        invalidate_labels (container);
        scatola_icon_container_request_update_all (container);
    }
#else
           ...
           GtkStyle  *previous_style)
{
    ...
    scatola_icon_container_theme_changed (SCATOLA_ICON_CONTAINER (widget));
                    ...
                    GdkEventButton *event)
{
    ...
    clicked_on_icon = GTK_WIDGET_CLASS (scatola_icon_container_parent_class)->button_press_event (widget, event);
...
static void
scatola_icon_container_did_not_drag (ScatolaIconContainer *container,
                                  GdkEventButton *event)
{
    ScatolaIconContainerDetails *details;
    gboolean selection_changed;
             ...
             event->button == MIDDLE_BUTTON))
    {
                ...
                ! button_event_modifies_selection (event))
        {
             ...
             * ScatolaList goes the other way because its "links" seem
             * much more link-like.
             */
            if (event->button == MIDDLE_BUTTON)
            {
...
static gboolean
clicked_within_double_click_interval (ScatolaIconContainer *container)
{
    static gint64 last_click_time = 0;
    static gint click_count = 0;
    gint double_click_time;
...
static void
clear_drag_state (ScatolaIconContainer *container)
{
...
static gboolean
#if GTK_CHECK_VERSION(3, 20, 0)
start_stretching (ScatolaIconContainer *container,
		  GdkEvent *event)
#else
start_stretching (ScatolaIconContainer *container)
#endif
{
    ...
    if (!scatola_icon_canvas_item_hit_test_stretch_handles (icon->item, world_point, &corner))
    {
        return FALSE;
    }
...
static gboolean
update_stretch_at_idle (ScatolaIconContainer *container)
{
    ScatolaIconContainerDetails *details;
    ScatolaIcon *icon;
    double world_x, world_y;
...
static void
continue_stretching (ScatolaIconContainer *container,
                     double world_x, double world_y)
{

    g_return_if_fail (SCATOLA_IS_ICON_CONTAINER (container));
...
static gboolean
keyboard_stretching (ScatolaIconContainer *container,
                     GdkEventKey           *event)
{
    ScatolaIcon *icon;
    guint size;
...
static void
ungrab_stretch_icon (ScatolaIconContainer *container)
{
#if GTK_CHECK_VERSION(3, 20, 0)
...
static void
end_stretching (ScatolaIconContainer *container,
                double world_x, double world_y)
{
    ScatolaIconPosition position;
    ScatolaIcon *icon;
...
static gboolean
undo_stretching (ScatolaIconContainer *container)
{
    ScatolaIcon *stretched_icon;
    ...
    scatola_icon_canvas_item_set_show_stretch_handles
                      ...
                      GdkEventButton *event)
{
    ...
    if (event->button == details->drag_button)
    {
        ...
        case DRAG_STATE_MOVE_OR_COPY:
            ...
            {
                scatola_icon_container_did_not_drag (container, event);
            }
            else
            {
                scatola_icon_dnd_end_drag (container);
                scatola_debug_log (FALSE, SCATOLA_DEBUG_LOG_DOMAIN_USER,
    ...
    return GTK_WIDGET_CLASS (scatola_icon_container_parent_class)->button_release_event (widget, event);
}
                     ...
                     GdkEventMotion *event)
{
    ...
    if (details->drag_button != 0)
    {
        ...
        case DRAG_STATE_MOVE_OR_COPY:
                                          ...
                                          world_y))
            {
                ...
                scatola_icon_dnd_begin_drag (container,
                                          actions,
                                          details->drag_button,
                                          event,
                                          canvas_x,
    ...
    return GTK_WIDGET_CLASS (scatola_icon_container_parent_class)->motion_notify_event (widget, event);
}

static void
scatola_icon_container_search_position_func (ScatolaIconContainer *container,
...
static gboolean
scatola_icon_container_real_search_enable_popdown (gpointer data)
{
    ScatolaIconContainer *container = (ScatolaIconContainer *)data;
...
static void
scatola_icon_container_search_enable_popdown (GtkWidget *widget,
        gpointer   data)
{
    ScatolaIconContainer *container = (ScatolaIconContainer *) data;

    g_object_ref (container);
    g_timeout_add (200, scatola_icon_container_real_search_enable_popdown, data);
}

static void
scatola_icon_container_search_disable_popdown (GtkEntry *entry,
        GtkMenu  *menu,
        gpointer  data)
{
    ScatolaIconContainer *container = (ScatolaIconContainer *) data;
    ...
    g_signal_connect (menu, "hide",
                      G_CALLBACK (scatola_icon_container_search_enable_popdown),
                      data);
}
...
static void
scatola_icon_container_search_dialog_hide (GtkWidget *search_dialog,
                                        ScatolaIconContainer *container)
{
...
static gboolean
scatola_icon_container_search_entry_flush_timeout (ScatolaIconContainer *container)
{
    scatola_icon_container_search_dialog_hide (container->details->search_window, container);
 ...
 */
static void
scatola_icon_container_search_preedit_changed (GtkEntry *entry,
        gchar *preedit,
        ScatolaIconContainer *container)
{
    ...
    if (container->details->typeselect_flush_timeout)
    {
        ...
        container->details->typeselect_flush_timeout =
            g_timeout_add_seconds (SCATOLA_ICON_CONTAINER_SEARCH_DIALOG_TIMEOUT,
                                   (GSourceFunc) scatola_icon_container_search_entry_flush_timeout,
                                   container);
    }
}

static void
scatola_icon_container_search_activate (GtkEntry *entry,
                                     ScatolaIconContainer *container)
{
    scatola_icon_container_search_dialog_hide (container->details->search_window,
                                            container);
...
static gboolean
scatola_icon_container_search_delete_event (GtkWidget *widget,
        GdkEventAny *event,
        ScatolaIconContainer *container)
{
    g_assert (GTK_IS_WIDGET (widget));

    scatola_icon_container_search_dialog_hide (widget, container);

    return TRUE;
}

static gboolean
scatola_icon_container_search_button_press_event (GtkWidget *widget,
        GdkEventButton *event,
        ScatolaIconContainer *container)
{
    g_assert (GTK_IS_WIDGET (widget));
...
static void
scatola_icon_container_get_icon_text (ScatolaIconContainer *container,
                                   ScatolaIconData      *data,
                                   char                 **editable_text,
...
static gboolean
scatola_icon_container_search_iter (ScatolaIconContainer *container,
                                 const char *key, gint n)
{
    ...
    {
        icon = p->data;
        scatola_icon_container_get_icon_text (container, icon->data, &name,
                                           NULL, TRUE);
...
static void
scatola_icon_container_search_move (GtkWidget *window,
                                 ScatolaIconContainer *container,
                                 gboolean up)
{
    gboolean ret;
    ...
    ret = scatola_icon_container_search_iter (container, text,
    ...
    {
        /* return to old iter */
        scatola_icon_container_search_iter (container, text,
                                         container->details->selected_iter);
    }
}

static gboolean
scatola_icon_container_search_scroll_event (GtkWidget *widget,
        ...
        ScatolaIconContainer *container)
{
    ...
    {
        scatola_icon_container_search_move (widget, container, TRUE);
        retval = TRUE;
    }
    else if (event->direction == GDK_SCROLL_DOWN)
    {
        scatola_icon_container_search_move (widget, container, FALSE);
        retval = TRUE;
    }
...
static gboolean
scatola_icon_container_search_key_press_event (GtkWidget *widget,
        GdkEventKey *event,
        ScatolaIconContainer *container)
{
    gboolean retval = FALSE;
    ...
    {
        scatola_icon_container_search_dialog_hide (widget,
                                                container);
...
static void
scatola_icon_container_search_init (GtkWidget   *entry,
                                 ScatolaIconContainer *container)
{
    gint ret;
    gint len;
    ...
    ret = scatola_icon_container_search_iter (container, text, 1);
...
static void
scatola_icon_container_ensure_interactive_directory (ScatolaIconContainer *container)
{
    ...
    g_signal_connect (container->details->search_window, "delete_event",
                      G_CALLBACK (scatola_icon_container_search_delete_event),
                      container);
    g_signal_connect (container->details->search_window, "key_press_event",
                      G_CALLBACK (scatola_icon_container_search_key_press_event),
                      container);
    g_signal_connect (container->details->search_window, "button_press_event",
                      G_CALLBACK (scatola_icon_container_search_button_press_event),
                      container);
    g_signal_connect (container->details->search_window, "scroll_event",
                      G_CALLBACK (scatola_icon_container_search_scroll_event),
                      container);
    ...
    g_signal_connect (container->details->search_entry, "populate_popup",
                      G_CALLBACK (scatola_icon_container_search_disable_popdown),
                      container);
    g_signal_connect (container->details->search_entry, "activate",
                      G_CALLBACK (scatola_icon_container_search_activate),
                      container);
    g_signal_connect (container->details->search_entry,
                      "preedit-changed",
                      G_CALLBACK (scatola_icon_container_search_preedit_changed),
                      container);
 ...
 */
static gboolean
scatola_icon_container_real_start_interactive_search (ScatolaIconContainer *container,
        gboolean keybinding)
{
    ...
    scatola_icon_container_ensure_interactive_directory (container);
    ...
    /* done, show it */
    scatola_icon_container_search_position_func (container, container->details->search_window);
    ...
    if (container->details->search_entry_changed_id == 0)
    {
        container->details->search_entry_changed_id =
            g_signal_connect (container->details->search_entry, "changed",
                              G_CALLBACK (scatola_icon_container_search_init),
                              container);
    }
    ...
    * really non-kosher. */
    ...
    scatola_icon_container_search_init (container->details->search_entry, container);

    return TRUE;
}

static gboolean
scatola_icon_container_start_interactive_search (ScatolaIconContainer *container)
{
    return scatola_icon_container_real_start_interactive_search (container, TRUE);
}

static gboolean
handle_popups (ScatolaIconContainer *container,
               GdkEventKey           *event,
               const char            *signal)
{
                 ...
                 GdkEventKey *event)
{
    ...
    if (!handled)
    {
        handled = GTK_WIDGET_CLASS (scatola_icon_container_parent_class)->key_press_event (widget, event);
    }
            ...
            event->keyval != GDK_KEY_Delete)
    {
        ...
        {
            if (scatola_icon_container_real_start_interactive_search (container, FALSE))
            {
...
get_accessible (GtkWidget *widget)
{
    ...
    accessible = g_object_new
                 (scatola_icon_container_accessible_get_type (), NULL);
...
static GObject*
scatola_icon_container_constructor (GType                  type,
                                 ...
                                 GObjectConstructParam *construct_params)
{
    ...
    object = G_OBJECT_CLASS (scatola_icon_container_parent_class)->constructor
             (type,
              n_construct_params,
              construct_params);
    ...
    if (scatola_icon_container_get_is_desktop (container))
    {
        g_signal_connect_swapped (scatola_desktop_preferences,
                                  "changed::" SCATOLA_PREFERENCES_DESKTOP_TEXT_ELLIPSIS_LIMIT,
    ...
    else
    {
        g_signal_connect_swapped (scatola_icon_view_preferences,
                                  "changed::" SCATOLA_PREFERENCES_ICON_VIEW_TEXT_ELLIPSIS_LIMIT,
                                  G_CALLBACK (text_ellipsis_limit_changed_container_callback),
                                  container);
    }
...
static void
scatola_icon_container_class_init (ScatolaIconContainerClass *class)
{
    GtkWidgetClass *widget_class;
    EelCanvasClass *canvas_class;
    GtkBindingSet *binding_set;

    G_OBJECT_CLASS (class)->constructor = scatola_icon_container_constructor;
    ...
    signals[SELECTION_CHANGED]
        = g_signal_new ("selection_changed",
                        ...
                        G_SIGNAL_RUN_LAST,
                        G_STRUCT_OFFSET (ScatolaIconContainerClass,
                                         selection_changed),
                        NULL, NULL,
    ...
    signals[BUTTON_PRESS]
        = g_signal_new ("button_press",
                        ...
                        G_STRUCT_OFFSET (ScatolaIconContainerClass,
                                         button_press),
                        NULL, NULL,
                        scatola_marshal_BOOLEAN__POINTER,
                        G_TYPE_BOOLEAN, 1,
                        GDK_TYPE_EVENT);
    signals[ACTIVATE]
    ...
    signals[ICON_POSITION_CHANGED]
        = g_signal_new ("icon_position_changed",
                        ...
                        G_STRUCT_OFFSET (ScatolaIconContainerClass,
                                         icon_position_changed),
                        NULL, NULL,
                        scatola_marshal_VOID__POINTER_POINTER,
                        G_TYPE_NONE, 2,
                        G_TYPE_POINTER,
                        G_TYPE_POINTER);
    signals[ICON_TEXT_CHANGED]
        = g_signal_new ("icon_text_changed",
                        ...
                        G_STRUCT_OFFSET (ScatolaIconContainerClass,
                                         icon_text_changed),
                        NULL, NULL,
                        scatola_marshal_VOID__POINTER_STRING,
                        G_TYPE_NONE, 2,
                        G_TYPE_POINTER,
                        G_TYPE_STRING);
    ...
    signals[GET_ICON_URI]
        = g_signal_new ("get_icon_uri",
                        ...
                        G_STRUCT_OFFSET (ScatolaIconContainerClass,
                                         get_icon_uri),
                        NULL, NULL,
		                scatola_marshal_STRING__POINTER,
                        G_TYPE_STRING, 1,
                        G_TYPE_POINTER);
    ...
    signals[MOVE_COPY_ITEMS]
        = g_signal_new ("move_copy_items",
                        ...
                        G_STRUCT_OFFSET (ScatolaIconContainerClass,
                                         move_copy_items),
                        NULL, NULL,
                        scatola_marshal_VOID__POINTER_POINTER_POINTER_ENUM_INT_INT,
                        G_TYPE_NONE, 6,
                        G_TYPE_POINTER,
                        G_TYPE_POINTER,
                        G_TYPE_POINTER,
    ...
    signals[HANDLE_NETSCAPE_URL]
        = g_signal_new ("handle_netscape_url",
                        ...
                        G_STRUCT_OFFSET (ScatolaIconContainerClass,
                                         handle_netscape_url),
                        NULL, NULL,
                        scatola_marshal_VOID__STRING_STRING_ENUM_INT_INT,
                        G_TYPE_NONE, 5,
                        G_TYPE_STRING,
                        G_TYPE_STRING,
                        GDK_TYPE_DRAG_ACTION,
    ...
    signals[HANDLE_RAW]
        = g_signal_new ("handle_raw",
                        ...
                        G_STRUCT_OFFSET (ScatolaIconContainerClass,
                                         handle_raw),
                        NULL, NULL,
                        scatola_marshal_VOID__POINTER_INT_STRING_STRING_ENUM_INT_INT,
                        G_TYPE_NONE, 7,
                        G_TYPE_POINTER,
                        G_TYPE_INT,
                        G_TYPE_STRING,
    ...
    signals[CAN_ACCEPT_ITEM]
        = g_signal_new ("can_accept_item",
                        ...
                        G_STRUCT_OFFSET (ScatolaIconContainerClass,
                                         can_accept_item),
                        NULL, NULL,
		                scatola_marshal_INT__POINTER_STRING,
                        G_TYPE_INT, 2,
                        G_TYPE_POINTER,
                        G_TYPE_STRING);
    signals[GET_STORED_ICON_POSITION]
        = g_signal_new ("get_stored_icon_position",
                        ...
                        G_STRUCT_OFFSET (ScatolaIconContainerClass,
                                         get_stored_icon_position),
                        NULL, NULL,
		                scatola_marshal_BOOLEAN__POINTER_POINTER,
                        G_TYPE_BOOLEAN, 2,
                        G_TYPE_POINTER,
                        G_TYPE_POINTER);
    ...
    signals[PREVIEW]
        = g_signal_new ("preview",
                        ...
                        G_STRUCT_OFFSET (ScatolaIconContainerClass,
                                         preview),
                        NULL, NULL,
                        scatola_marshal_INT__POINTER_BOOLEAN,
                        G_TYPE_INT, 2,
                        G_TYPE_POINTER,
                        G_TYPE_BOOLEAN);
    ...
    gtk_widget_class_set_accessible_type (widget_class, scatola_icon_container_accessible_get_type ());
#else
    ...
    class->start_interactive_search = scatola_icon_container_start_interactive_search;
...
static void
update_selected (ScatolaIconContainer *container)
{
    GList *node;
    ScatolaIcon *icon;
...
{
    update_selected (SCATOLA_ICON_CONTAINER (widget));
...
handle_focus_out_event (GtkWidget *widget, GdkEventFocus *event, gpointer user_data)
{
    ...
    end_renaming_mode (SCATOLA_ICON_CONTAINER (widget), TRUE);
    update_selected (SCATOLA_ICON_CONTAINER (widget));
...
static int text_ellipsis_limits[SCATOLA_ZOOM_LEVEL_N_ENTRIES];
...
text_ellipsis_limit_changed_callback (gpointer callback_data)
{
    ...
    pref = g_settings_get_strv (scatola_icon_view_preferences, SCATOLA_PREFERENCES_ICON_VIEW_TEXT_ELLIPSIS_LIMIT);

    /* set default */
    get_text_ellipsis_limit_for_zoom (pref, NULL, &one_limit);
    for (i = 0; i < SCATOLA_ZOOM_LEVEL_N_ENTRIES; i++)
    {
...
desktop_text_ellipsis_limit_changed_callback (gpointer callback_data)
{
    ...
    pref = g_settings_get_int (scatola_desktop_preferences, SCATOLA_PREFERENCES_DESKTOP_TEXT_ELLIPSIS_LIMIT);
    desktop_text_ellipsis_limit = pref;
}

static void
scatola_icon_container_init (ScatolaIconContainer *container)
{
    ScatolaIconContainerDetails *details;
    EelBackground *background;
    static gboolean setup_prefs = FALSE;

    details = g_new0 (ScatolaIconContainerDetails, 1);

    details->icon_set = g_hash_table_new (g_direct_hash, g_direct_equal);
    details->layout_timestamp = UNDEFINED_TIME;

    details->zoom_level = SCATOLA_ZOOM_LEVEL_STANDARD;

    details->font_size_table[SCATOLA_ZOOM_LEVEL_SMALLEST] = -2 * PANGO_SCALE;
    details->font_size_table[SCATOLA_ZOOM_LEVEL_SMALLER] = -2 * PANGO_SCALE;
    details->font_size_table[SCATOLA_ZOOM_LEVEL_SMALL] = -0 * PANGO_SCALE;
    details->font_size_table[SCATOLA_ZOOM_LEVEL_STANDARD] = 0 * PANGO_SCALE;
    details->font_size_table[SCATOLA_ZOOM_LEVEL_LARGE] = 0 * PANGO_SCALE;
    details->font_size_table[SCATOLA_ZOOM_LEVEL_LARGER] = 0 * PANGO_SCALE;
    details->font_size_table[SCATOLA_ZOOM_LEVEL_LARGEST] = 0 * PANGO_SCALE;
    ...
    scatola_icon_container_theme_changed (container);
#endif
...
static gboolean
handle_icon_double_click (ScatolaIconContainer *container,
                          ScatolaIcon *icon,
                          GdkEventButton *event)
{
    ScatolaIconContainerDetails *details;
...
static gboolean
handle_icon_button_press (ScatolaIconContainer *container,
                          ScatolaIcon *icon,
                          GdkEventButton *event)
{
    ScatolaIconContainerDetails *details;
                     ...
                     gpointer data)
{
    ...
    icon = SCATOLA_ICON_CANVAS_ITEM (item)->user_data;
    g_assert (icon != NULL);
...
GtkWidget *
scatola_icon_container_new (void)
{
    return gtk_widget_new (SCATOLA_TYPE_ICON_CONTAINER, NULL);
}

/* Clear all of the icons in the container. */
void
scatola_icon_container_clear (ScatolaIconContainer *container)
{
    ScatolaIconContainerDetails *details;
    ScatolaIcon *icon;
    GList *p;
    ...
    for (p = details->icons; p != NULL; p = p->next)
    {
        ...
        {
            scatola_icon_container_stop_monitor_top_left (container,
                    icon->data,
                    icon);
        }
        icon_free (p->data);
...
gboolean
scatola_icon_container_is_empty (ScatolaIconContainer *container)
{
    return container->details->icons == NULL;
}

ScatolaIconData *
scatola_icon_container_get_first_visible_icon (ScatolaIconContainer *container)
{
    GList *l;
    ScatolaIcon *icon, *best_icon;
    double x, y;
    double x1, y1, x2, y2;
    double *pos, best_pos;
...
void
scatola_icon_container_scroll_to_icon (ScatolaIconContainer  *container,
                                    ScatolaIconData       *data)
{
    GList *l;
    ScatolaIcon *icon;
    ...
    scatola_icon_container_layout_now (container);
...
void
scatola_icon_container_for_each (ScatolaIconContainer *container,
                              ScatolaIconCallback callback,
                              gpointer callback_data)
{
    CallbackAndData callback_and_data;
...
static void
icon_destroy (ScatolaIconContainer *container,
              ScatolaIcon *icon)
{
    ScatolaIconContainerDetails *details;
    gboolean was_selected;
    ScatolaIcon *icon_to_focus;
    GList *item;
...
static void
activate_selected_items (ScatolaIconContainer *container)
{
    GList *selection;

    g_return_if_fail (SCATOLA_IS_ICON_CONTAINER (container));

    selection = scatola_icon_container_get_selection (container);
    if (selection != NULL)
    {
        g_signal_emit (container,
                       signals[ACTIVATE], 0,
...
static void
activate_selected_items_alternate (ScatolaIconContainer *container,
                                   ScatolaIcon *icon)
{
    GList *selection;
...
static ScatolaIcon *
get_icon_being_renamed (ScatolaIconContainer *container)
{
    ScatolaIcon *rename_icon;
...
scatola_icon_container_get_icon_images (ScatolaIconContainer *container,
                                     ScatolaIconData      *data,
                                     int                    size,
...
static void
scatola_icon_container_freeze_updates (ScatolaIconContainer *container)
{
    ScatolaIconContainerClass *klass;
...
static void
scatola_icon_container_unfreeze_updates (ScatolaIconContainer *container)
{
    ScatolaIconContainerClass *klass;
...
static void
scatola_icon_container_start_monitor_top_left (ScatolaIconContainer *container,
        ScatolaIconData *data,
        gconstpointer client,
        gboolean large_text)
{
...
static void
scatola_icon_container_stop_monitor_top_left (ScatolaIconContainer *container,
        ScatolaIconData *data,
        gconstpointer client)
{
    ScatolaIconContainerClass *klass;
...
static void
scatola_icon_container_prioritize_thumbnailing (ScatolaIconContainer *container,
        ScatolaIcon *icon)
{
    ScatolaIconContainerClass *klass;
...
static void
scatola_icon_container_update_visible_icons (ScatolaIconContainer *container)
{
    GtkAdjustment *vadj, *hadj;
    double min_y, max_y;
    double min_x, max_x;
    ...
    for (node = g_list_last (container->details->icons); node != NULL; node = node->prev)
    {
        ...
        if (icon_is_positioned (icon))
        {
            ...
            if (visible)
            {
                scatola_icon_canvas_item_set_is_visible (icon->item, TRUE);
                scatola_icon_container_prioritize_thumbnailing (container,
                        icon);
            }
            else
            {
                scatola_icon_canvas_item_set_is_visible (icon->item, FALSE);
            }
        }
    }
}
...
void
scatola_icon_container_update_icon (ScatolaIconContainer *container,
                                 ScatolaIcon *icon)
{
    ScatolaIconContainerDetails *details;
    guint icon_size;
    ...
    max_image_size = MAX (MAXIMUM_IMAGE_SIZE * EEL_CANVAS (container)->pixels_per_unit, SCATOLA_ICON_MAXIMUM_SIZE);
    ...
    icon_info = scatola_icon_container_get_icon_images (container, icon->data, icon_size,
                &emblem_pixbufs,
                &embedded_text,
    ...
    if (container->details->forced_icon_size > 0)
        pixbuf = scatola_icon_info_get_pixbuf_at_size (icon_info, icon_size);
    else
        pixbuf = scatola_icon_info_get_pixbuf (icon_info);
    scatola_icon_info_get_attach_points (icon_info, &attach_points, &n_attach_points);
    has_embedded_text_rect = scatola_icon_info_get_embedded_rect (icon_info,
                             &embedded_text_rect);

    if (has_embedded_text_rect && embedded_text_needs_loading)
    {
        icon->is_monitored = TRUE;
        scatola_icon_container_start_monitor_top_left (container, icon->data, icon, large_embedded_text);
    }

    scatola_icon_container_get_icon_text (container,
                                       icon->data,
                                       &editable_text,
                                       &additional_text,
                                       FALSE);
    ...
    if (icon == get_icon_being_renamed (container) &&
            g_strcmp0 (editable_text,
                        scatola_icon_canvas_item_get_editable_text (icon->item)) != 0)
    {
        end_renaming_mode (container, FALSE);
    }
    ...
    scatola_icon_canvas_item_set_image (icon->item, pixbuf);
    scatola_icon_canvas_item_set_attach_points (icon->item, attach_points, n_attach_points);
    scatola_icon_canvas_item_set_emblems (icon->item, emblem_pixbufs);
    scatola_icon_canvas_item_set_embedded_text_rect (icon->item, &embedded_text_rect);
    scatola_icon_canvas_item_set_embedded_text (icon->item, embedded_text);
...
static gboolean
assign_icon_position (ScatolaIconContainer *container,
                      ScatolaIcon *icon)
{
    gboolean have_stored_position;
    ScatolaIconPosition position;
...
static void
finish_adding_icon (ScatolaIconContainer *container,
                    ScatolaIcon *icon)
{
    scatola_icon_container_update_icon (container, icon);
...
static void
finish_adding_new_icons (ScatolaIconContainer *container)
{
...
static gboolean
is_old_or_unknown_icon_data (ScatolaIconContainer *container,
                             ScatolaIconData *data)
{
    time_t timestamp;
    gboolean success;
...
/**
 * scatola_icon_container_add:
 * @container: A ScatolaIconContainer
 * @data: Icon data.
 *
 ...
 **/
gboolean
scatola_icon_container_add (ScatolaIconContainer *container,
                         ScatolaIconData *data)
{
    ScatolaIconContainerDetails *details;
    ScatolaIcon *icon;
    ...
    g_return_val_if_fail (SCATOLA_IS_ICON_CONTAINER (container), FALSE);
    ...
    icon->scale = 1.0;
    icon->item = SCATOLA_ICON_CANVAS_ITEM
                 (eel_canvas_item_new (EEL_CANVAS_GROUP (EEL_CANVAS (container)->root),
                                       scatola_icon_canvas_item_get_type (),
                                       "visible", FALSE,
                                       NULL));
...
void
scatola_icon_container_layout_now (ScatolaIconContainer *container)
{
    if (container->details->idle_id != 0)
    {
        unschedule_redo_layout (container);
 ...
 **/
gboolean
scatola_icon_container_remove (ScatolaIconContainer *container,
                            ScatolaIconData *data)
{
    ScatolaIcon *icon;
...
/**
 * scatola_icon_container_request_update:
 * @container: A ScatolaIconContainer.
 * @data: Icon data.
 *
 ...
 **/
void
scatola_icon_container_request_update (ScatolaIconContainer *container,
                                    ScatolaIconData *data)
{
    ScatolaIcon *icon;
...
ScatolaZoomLevel
scatola_icon_container_get_zoom_level (ScatolaIconContainer *container)
{
    return container->details->zoom_level;
}

void
scatola_icon_container_set_zoom_level (ScatolaIconContainer *container, int new_level)
{
    ScatolaIconContainerDetails *details;
    int pinned_level;
    double pixels_per_unit;
    ...
    if (pinned_level < SCATOLA_ZOOM_LEVEL_SMALLEST)
    {
        pinned_level = SCATOLA_ZOOM_LEVEL_SMALLEST;
    }
    else if (pinned_level > SCATOLA_ZOOM_LEVEL_LARGEST)
    {
        pinned_level = SCATOLA_ZOOM_LEVEL_LARGEST;
    }
    ...
    pixels_per_unit = (double) scatola_get_icon_size_for_zoom_level (pinned_level)
                      / SCATOLA_ICON_SIZE_STANDARD;
...
/**
 * scatola_icon_container_request_update_all:
 ...
 **/
void
scatola_icon_container_request_update_all (ScatolaIconContainer *container)
{
    GList *node;
    ScatolaIcon *icon;
...
/**
 * scatola_icon_container_reveal:
 * Change scroll position as necessary to reveal the specified item.
 */
void
scatola_icon_container_reveal (ScatolaIconContainer *container, ScatolaIconData *data)
{
    ScatolaIcon *icon;
...
/**
 * scatola_icon_container_get_selection:
 * @container: An icon container.
 *
 ...
 **/
GList *
scatola_icon_container_get_selection (ScatolaIconContainer *container)
{
    GList *list, *p;

    g_return_val_if_fail (SCATOLA_IS_ICON_CONTAINER (container), NULL);
...
static GList *
scatola_icon_container_get_selected_icons (ScatolaIconContainer *container)
{
    GList *list, *p;
...
/**
 * scatola_icon_container_invert_selection:
 * @container: An icon container.
 *
 ...
 **/
void
scatola_icon_container_invert_selection (ScatolaIconContainer *container)
{
    GList *p;
...
static GArray *
scatola_icon_container_get_icon_locations (ScatolaIconContainer *container,
                                        GList *icons)
{
    GArray *result;
    GList *node;
    ...
    for (index = 0, node = icons; node != NULL; index++, node = node->next)
    {
        g_array_index (result, GdkPoint, index).x =
            ((ScatolaIcon *)node->data)->x;
        g_array_index (result, GdkPoint, index).y =
            ((ScatolaIcon *)node->data)->y;
    }
...
/**
 * scatola_icon_container_get_selected_icon_locations:
 * @container: An icon container widget.
 *
 * Returns an array of GdkPoints of locations of the selected icons.
 **/
GArray *
scatola_icon_container_get_selected_icon_locations (ScatolaIconContainer *container)
{
    GArray *result;
    GList *icons;
    ...
    icons = scatola_icon_container_get_selected_icons (container);
    result = scatola_icon_container_get_icon_locations (container, icons);
    g_list_free (icons);
...
/**
 * scatola_icon_container_select_all:
 * @container: An icon container widget.
 *
 * Select all the icons in @container at once.
 **/
void
scatola_icon_container_select_all (ScatolaIconContainer *container)
{
    gboolean selection_changed;
    GList *p;
    ScatolaIcon *icon;
...
/**
 * scatola_icon_container_set_selection:
 * @container: An icon container widget.
 * @selection: A list of ScatolaIconData *.
 *
 * Set the selection to exactly the icons in @container which have
 * programmer data matching one of the items in @selection.
 **/
void
scatola_icon_container_set_selection (ScatolaIconContainer *container,
                                   GList *selection)
{
    gboolean selection_changed;
    GHashTable *hash;
...
/**
 * scatola_icon_container_select_list_unselect_others.
 * @container: An icon container widget.
 * @selection: A list of ScatolaIcon *.
 *
 * Set the selection to exactly the icons in @selection.
 **/
void
scatola_icon_container_select_list_unselect_others (ScatolaIconContainer *container,
        GList *selection)
{
    gboolean selection_changed;
    GHashTable *hash;
...
/**
 * scatola_icon_container_unselect_all:
 * @container: An icon container widget.
 *
 * Deselect all the icons in @container.
 **/
void
scatola_icon_container_unselect_all (ScatolaIconContainer *container)
{
...
/**
 * scatola_icon_container_get_icon_by_uri:
 * @container: An icon container widget.
 ...
 **/
ScatolaIcon *
scatola_icon_container_get_icon_by_uri (ScatolaIconContainer *container,
                                     const char *uri)
{
    ScatolaIconContainerDetails *details;
    GList *p;
    ...
    */
    ...
    for (p = details->icons; p != NULL; p = p->next)
    {
        ...
        icon_uri = scatola_icon_container_get_icon_uri
                   (container, icon);
...
static ScatolaIcon *
get_nth_selected_icon (ScatolaIconContainer *container, int index)
{
    GList *p;
    ScatolaIcon *icon;
    int selection_count;
...
static ScatolaIcon *
get_first_selected_icon (ScatolaIconContainer *container)
{
    return get_nth_selected_icon (container, 1);
}

static gboolean
has_multiple_selection (ScatolaIconContainer *container)
{
    return get_nth_selected_icon (container, 2) != NULL;
}

static gboolean
all_selected (ScatolaIconContainer *container)
{
    GList *p;
    ScatolaIcon *icon;
...
static gboolean
has_selection (ScatolaIconContainer *container)
{
    return get_nth_selected_icon (container, 1) != NULL;
}

/**
 * scatola_icon_container_show_stretch_handles:
 * @container: An icon container widget.
 *
 * Makes stretch handles visible on the first selected icon.
 **/
void
scatola_icon_container_show_stretch_handles (ScatolaIconContainer *container)
{
    ScatolaIconContainerDetails *details;
    ScatolaIcon *icon;
    guint initial_size;
    ...
    scatola_icon_canvas_item_set_show_stretch_handles (icon->item, TRUE);
...
/**
 * scatola_icon_container_has_stretch_handles
 * @container: An icon container widget.
 *
 * Returns true if the first selected item has stretch handles.
 **/
gboolean
scatola_icon_container_has_stretch_handles (ScatolaIconContainer *container)
{
    ScatolaIcon *icon;
...
/**
 * scatola_icon_container_is_stretched
 * @container: An icon container widget.
 *
 * Returns true if the any selected item is stretched to a size other than 1.0.
 **/
gboolean
scatola_icon_container_is_stretched (ScatolaIconContainer *container)
{
    GList *p;
    ScatolaIcon *icon;
...
/**
 * scatola_icon_container_unstretch
 * @container: An icon container widget.
 *
 * Gets rid of any icon stretching.
 **/
void
scatola_icon_container_unstretch (ScatolaIconContainer *container)
{
    GList *p;
    ScatolaIcon *icon;
                 ...
                 StretchState *current)
{
    ...
    current->icon_size = MAX ((int) start->icon_size + MIN (x_stretch, y_stretch),
                              (int) SCATOLA_ICON_SIZE_SMALLEST);
...
char *
scatola_icon_container_get_icon_uri (ScatolaIconContainer *container,
                                  ScatolaIcon *icon)
{
    char *uri;
...
char *
scatola_icon_container_get_icon_drop_target_uri (ScatolaIconContainer *container,
        ScatolaIcon *icon)
{
    char *uri;
 ...
 */
static void
reset_scroll_region_if_not_empty (ScatolaIconContainer *container)
{
    if (!scatola_icon_container_is_empty (container))
    {
        scatola_icon_container_reset_scroll_region (container);
    }
}
 ...
 */
void
scatola_icon_container_set_auto_layout (ScatolaIconContainer *container,
...
void
scatola_icon_container_set_tighter_layout (ScatolaIconContainer *container,
...
gboolean
scatola_icon_container_is_keep_aligned (ScatolaIconContainer *container)
{
...
static void
unschedule_align_icons (ScatolaIconContainer *container)
{
    if (container->details->align_idle_id != 0)
    {
        g_source_remove (container->details->align_idle_id);
...
static void
schedule_align_icons (ScatolaIconContainer *container)
{
    if (container->details->align_idle_id == 0
            && container->details->has_been_allocated)
    {
...
void
scatola_icon_container_set_keep_aligned (ScatolaIconContainer *container,
...
void
scatola_icon_container_set_layout_mode (ScatolaIconContainer *container,
...
void
scatola_icon_container_set_label_position (ScatolaIconContainer *container,
                                        ScatolaIconLabelPosition position)
{
    g_return_if_fail (SCATOLA_IS_ICON_CONTAINER (container));
 ...
 */
void
scatola_icon_container_freeze_icon_positions (ScatolaIconContainer *container)
{
    gboolean changed;
    GList *p;
    ScatolaIcon *icon;
...
void
scatola_icon_container_sort (ScatolaIconContainer *container)
{
    gboolean changed;
...
gboolean
scatola_icon_container_is_auto_layout (ScatolaIconContainer *container)
{
    g_return_val_if_fail (SCATOLA_IS_ICON_CONTAINER (container), FALSE);
...
gboolean
scatola_icon_container_is_tighter_layout (ScatolaIconContainer *container)
{
    g_return_val_if_fail (SCATOLA_IS_ICON_CONTAINER (container), FALSE);
...
static void
pending_icon_to_rename_destroy_callback (ScatolaIconCanvasItem *item, ScatolaIconContainer *container)
{
...
static ScatolaIcon*
get_pending_icon_to_rename (ScatolaIconContainer *container)
{
    return container->details->pending_icon_to_rename;
}

static void
set_pending_icon_to_rename (ScatolaIconContainer *container, ScatolaIcon *icon)
{
    ScatolaIcon *old_icon;
...
static void
process_pending_icon_to_rename (ScatolaIconContainer *container)
{
    ScatolaIcon *pending_icon_to_rename;
    ...
    if (pending_icon_to_rename != NULL)
    {
        ...
        {
            scatola_icon_container_start_renaming_selected_item (container, FALSE);
        }
        else
        {
...
static gboolean
is_renaming_pending (ScatolaIconContainer *container)
{
    return get_pending_icon_to_rename (container) != NULL;
}

static gboolean
is_renaming (ScatolaIconContainer *container)
{
    return container->details->renaming;
}

/**
 * scatola_icon_container_start_renaming_selected_item
 * @container: An icon container widget.
 ...
 **/
void
scatola_icon_container_start_renaming_selected_item (ScatolaIconContainer *container,
        gboolean select_all)
{
    ScatolaIconContainerDetails *details;
    ScatolaIcon *icon;
    ...
    editable_text = scatola_icon_canvas_item_get_editable_text (icon->item);
    ...
    scatola_icon_container_freeze_updates (container);
    ...
    if (details->rename_widget == NULL)
    {
        ...
        if (details->label_position != SCATOLA_ICON_LABEL_POSITION_BESIDE)
        {
    ...
    icon_rect = scatola_icon_canvas_item_get_icon_rectangle (icon->item);
    text_rect = scatola_icon_canvas_item_get_text_rectangle (icon->item, TRUE);

    if (scatola_icon_container_is_layout_vertical (container) &&
            container->details->label_position == SCATOLA_ICON_LABEL_POSITION_BESIDE)
    {
    ...
    {
        width = scatola_icon_canvas_item_get_max_text_width (icon->item);
    }
    ...
    scatola_icon_canvas_item_set_renaming (icon->item, TRUE);
}

static void
end_renaming_mode (ScatolaIconContainer *container, gboolean commit)
{
    ScatolaIcon *icon;
    ...
    scatola_icon_canvas_item_set_renaming (icon->item, FALSE);

    scatola_icon_container_unfreeze_updates (container);
...
gboolean
scatola_icon_container_emit_preview_signal (ScatolaIconContainer *icon_container,
        ScatolaIcon *icon,
        gboolean start_flag)
{
    gboolean result;

    g_return_val_if_fail (SCATOLA_IS_ICON_CONTAINER (icon_container), FALSE);
...
gboolean
scatola_icon_container_has_stored_icon_positions (ScatolaIconContainer *container)
{
    GList *p;
    ScatolaIcon *icon;
    gboolean have_stored_position;
...
void
scatola_icon_container_set_single_click_mode (ScatolaIconContainer *container,
...
void
scatola_icon_container_get_label_color (ScatolaIconContainer *container,
        GdkColor             **color,
        gboolean             is_name,
...
static void
setup_gc_with_fg (ScatolaIconContainer *container, int idx, guint32 color)
{
    container->details->label_colors [idx] = eel_gdk_rgb_to_color (color);
}

static void
setup_label_gcs (ScatolaIconContainer *container)
{
    EelBackground *background;
    GtkWidget *widget;
    ...
    /* If ScatolaIconContainer::frame_text is set, we can safely
...
gboolean
scatola_icon_container_get_is_fixed_size (ScatolaIconContainer *container)
{
    g_return_val_if_fail (SCATOLA_IS_ICON_CONTAINER (container), FALSE);
...
void
scatola_icon_container_set_is_fixed_size (ScatolaIconContainer *container,
...
gboolean
scatola_icon_container_get_is_desktop (ScatolaIconContainer *container)
{
    g_return_val_if_fail (SCATOLA_IS_ICON_CONTAINER (container), FALSE);
...
void
scatola_icon_container_set_is_desktop (ScatolaIconContainer *container,
                                    gboolean is_desktop)
{
    g_return_if_fail (SCATOLA_IS_ICON_CONTAINER (container));
    ...
    if (is_desktop) {
            ...
            context = gtk_widget_get_style_context (GTK_WIDGET (container));
            gtk_style_context_add_class (context, "scatola-desktop");
    }
#endif
}

void
scatola_icon_container_set_margins (ScatolaIconContainer *container,
                                 int left_margin,
                                 int right_margin,
                                 int top_margin,
                                 int bottom_margin)
...
void
scatola_icon_container_set_use_drop_shadows (ScatolaIconContainer  *container,
...
#if !GTK_CHECK_VERSION(3,0,0)
static void
scatola_icon_container_theme_changed (gpointer user_data)
{
    ScatolaIconContainer *container;
    GtkStyle *style;
    ...
    container = SCATOLA_ICON_CONTAINER (user_data);
...
void
scatola_icon_container_set_font (ScatolaIconContainer *container,
                              const char *font)
{
    g_return_if_fail (SCATOLA_IS_ICON_CONTAINER (container));
...
void
scatola_icon_container_set_font_size_table (ScatolaIconContainer *container,
        const int font_size_table[SCATOLA_ZOOM_LEVEL_LARGEST + 1])
{
    ...
    for (i = 0; i <= SCATOLA_ZOOM_LEVEL_LARGEST; i++)
    {
...
/**
 * scatola_icon_container_get_icon_description
 * @container: An icon container widget.
 * @data: Icon data
 *
 ...
 **/
char*
scatola_icon_container_get_icon_description (ScatolaIconContainer *container,
        ScatolaIconData      *data)
{
    ScatolaIconContainerClass *klass;
...
gboolean
scatola_icon_container_get_allow_moves (ScatolaIconContainer *container)
{
    g_return_val_if_fail (SCATOLA_IS_ICON_CONTAINER (container), FALSE);
...
void
scatola_icon_container_set_allow_moves	(ScatolaIconContainer *container,
...
void
scatola_icon_container_set_forced_icon_size (ScatolaIconContainer *container,
...
void
scatola_icon_container_set_all_columns_same_width (ScatolaIconContainer *container,
...
/**
 * scatola_icon_container_set_highlighted_for_clipboard
 * @container: An icon container widget.
 * @data: Icon Data associated with all icons that should be highlighted.
 *        Others will be unhighlighted.
 **/
void
scatola_icon_container_set_highlighted_for_clipboard (ScatolaIconContainer *container,
...
/* ScatolaIconContainerAccessible */

static ScatolaIconContainerAccessiblePrivate *
accessible_get_priv (AtkObject *accessible)
{
    ScatolaIconContainerAccessiblePrivate *priv;
...
static gboolean
scatola_icon_container_accessible_do_action (AtkAction *accessible, int i)
{
    GtkWidget *widget;
    ScatolaIconContainer *container;
    GList *selection;
    ...
    case ACTION_MENU :
        ...
        break;
    default :
        g_warning ("Invalid action passed to ScatolaIconContainerAccessible::do_action");
        return FALSE;
    }
    return TRUE;
}
...
static int
scatola_icon_container_accessible_get_n_actions (AtkAction *accessible)
{
    return LAST_ACTION;
}

static const char *
scatola_icon_container_accessible_action_get_description (AtkAction *accessible,
        int i)
{
    ScatolaIconContainerAccessiblePrivate *priv;
    ...
    {
        return scatola_icon_container_accessible_action_descriptions[i];
    }
}

static const char *
scatola_icon_container_accessible_action_get_name (AtkAction *accessible, int i)
{
    g_assert (i < LAST_ACTION);

    return scatola_icon_container_accessible_action_names[i];
}

static const char *
scatola_icon_container_accessible_action_get_keybinding (AtkAction *accessible,
        int i)
{
    g_assert (i < LAST_ACTION);
...
static gboolean
scatola_icon_container_accessible_action_set_description (AtkAction *accessible,
        int i,
...
static void
scatola_icon_container_accessible_action_interface_init (AtkActionIface *iface)
{
    iface->do_action = scatola_icon_container_accessible_do_action;
    iface->get_n_actions = scatola_icon_container_accessible_get_n_actions;
    iface->get_description = scatola_icon_container_accessible_action_get_description;
    iface->get_name = scatola_icon_container_accessible_action_get_name;
    iface->get_keybinding = scatola_icon_container_accessible_action_get_keybinding;
    iface->set_description = scatola_icon_container_accessible_action_set_description;
}

/* AtkSelection interface */

static void
scatola_icon_container_accessible_update_selection (AtkObject *accessible)
{
    ScatolaIconContainer *container;
    ScatolaIconContainerAccessiblePrivate *priv;
    GList *l;
    ...
    container = SCATOLA_ICON_CONTAINER (gtk_accessible_get_widget (GTK_ACCESSIBLE (accessible)));
...
static void
scatola_icon_container_accessible_selection_changed_cb (ScatolaIconContainer *container,
        gpointer data)
{
...
static void
scatola_icon_container_accessible_icon_added_cb (ScatolaIconContainer *container,
        ScatolaIconData *icon_data,
        gpointer data)
{
    ScatolaIcon *icon;
...
static void
scatola_icon_container_accessible_icon_removed_cb (ScatolaIconContainer *container,
        ScatolaIconData *icon_data,
        gpointer data)
{
    ScatolaIcon *icon;
...
static void
scatola_icon_container_accessible_cleared_cb (ScatolaIconContainer *container,
        gpointer data)
{
...
static gboolean
scatola_icon_container_accessible_add_selection (AtkSelection *accessible,
        int i)
{
    GtkWidget *widget;
    ScatolaIconContainer *container;
    ...
    if (icon)
    {
        ...
        selection = g_list_prepend (selection,
                                    icon->data);
        scatola_icon_container_set_selection (container, selection);
...
static gboolean
scatola_icon_container_accessible_clear_selection (AtkSelection *accessible)
{
    GtkWidget *widget;
    ScatolaIconContainer *container;
    ...
    scatola_icon_container_unselect_all (container);

    return TRUE;
}

static AtkObject *
scatola_icon_container_accessible_ref_selection (AtkSelection *accessible,
        int i)
{
    ...
    scatola_icon_container_accessible_update_selection (ATK_OBJECT (accessible));
    priv = accessible_get_priv (ATK_OBJECT (accessible));
...
static int
scatola_icon_container_accessible_get_selection_count (AtkSelection *accessible)
{
    int count;
    ScatolaIconContainerAccessiblePrivate *priv;
...
static gboolean
scatola_icon_container_accessible_is_child_selected (AtkSelection *accessible,
        int i)
{
    ScatolaIconContainer *container;
    ScatolaIcon *icon;
...
static gboolean
scatola_icon_container_accessible_remove_selection (AtkSelection *accessible,
        int i)
{
    ScatolaIconContainer *container;
    ScatolaIconContainerAccessiblePrivate *priv;
...
static gboolean
scatola_icon_container_accessible_select_all_selection (AtkSelection *accessible)
{
    ScatolaIconContainer *container;
    GtkWidget *widget;
    ...
    scatola_icon_container_select_all (container);

    return TRUE;
}

void
scatola_icon_container_widget_to_file_operation_position (ScatolaIconContainer *container,
        GdkPoint              *position)
{
    double x, y;
    ...
    position->x -= scatola_get_icon_size_for_zoom_level (container->details->zoom_level) / 2;
    position->y -= scatola_get_icon_size_for_zoom_level (container->details->zoom_level) / 2;
}

static void
scatola_icon_container_accessible_selection_interface_init (AtkSelectionIface *iface)
{
    iface->add_selection = scatola_icon_container_accessible_add_selection;
    iface->clear_selection = scatola_icon_container_accessible_clear_selection;
    iface->ref_selection = scatola_icon_container_accessible_ref_selection;
    iface->get_selection_count = scatola_icon_container_accessible_get_selection_count;
    iface->is_child_selected = scatola_icon_container_accessible_is_child_selected;
    iface->remove_selection = scatola_icon_container_accessible_remove_selection;
    iface->select_all_selection = scatola_icon_container_accessible_select_all_selection;
}


static gint
scatola_icon_container_accessible_get_n_children (AtkObject *accessible)
{
    ScatolaIconContainer *container;
    GtkWidget *widget;
    gint i;
...
static AtkObject*
scatola_icon_container_accessible_ref_child (AtkObject *accessible, int i)
{
    AtkObject *atk_object;
    ScatolaIconContainer *container;
    ScatolaIcon *icon;
...
static void
scatola_icon_container_accessible_initialize (AtkObject *accessible,
        gpointer data)
{
    ScatolaIconContainer *container;
    ScatolaIconContainerAccessiblePrivate *priv;
    ...
    priv = g_new0 (ScatolaIconContainerAccessiblePrivate, 1);
    ...
    {
        scatola_icon_container_accessible_update_selection
        (ATK_OBJECT (accessible));

        container = SCATOLA_ICON_CONTAINER (gtk_accessible_get_widget (GTK_ACCESSIBLE (accessible)));
        g_signal_connect (G_OBJECT (container), "selection_changed",
                          G_CALLBACK (scatola_icon_container_accessible_selection_changed_cb),
                          accessible);
        g_signal_connect (G_OBJECT (container), "icon_added",
                          G_CALLBACK (scatola_icon_container_accessible_icon_added_cb),
                          accessible);
        g_signal_connect (G_OBJECT (container), "icon_removed",
                          G_CALLBACK (scatola_icon_container_accessible_icon_removed_cb),
                          accessible);
        g_signal_connect (G_OBJECT (container), "cleared",
                          G_CALLBACK (scatola_icon_container_accessible_cleared_cb),
                          accessible);
    }
}

static void
scatola_icon_container_accessible_finalize (GObject *object)
{
    ScatolaIconContainerAccessiblePrivate *priv;
    int i;
...
#if GTK_CHECK_VERSION(3,2,0)
static void
scatola_icon_container_accessible_init (ScatolaIconContainerAccessible *accessible)
{
}

static void
scatola_icon_container_accessible_class_init (ScatolaIconContainerAccessibleClass *klass)
{
    ...
    gobject_class->finalize = scatola_icon_container_accessible_finalize;

    atk_class->get_n_children = scatola_icon_container_accessible_get_n_children;
    atk_class->ref_child = scatola_icon_container_accessible_ref_child;
    atk_class->initialize = scatola_icon_container_accessible_initialize;

    accessible_private_data_quark = g_quark_from_static_string ("icon-container-accessible-private-data");
}

G_DEFINE_TYPE_WITH_CODE (ScatolaIconContainerAccessible,
                         scatola_icon_container_accessible,
                         eel_canvas_accessible_get_type (),
                         G_IMPLEMENT_INTERFACE (ATK_TYPE_ACTION,
                                                scatola_icon_container_accessible_action_interface_init)
                         G_IMPLEMENT_INTERFACE (ATK_TYPE_SELECTION,
                                                scatola_icon_container_accessible_selection_interface_init));

#else
static void
scatola_icon_container_accessible_class_init (AtkObjectClass *klass)
{
    GObjectClass *gobject_class = G_OBJECT_CLASS (klass);
    ...
    klass->get_n_children = scatola_icon_container_accessible_get_n_children;
    klass->ref_child = scatola_icon_container_accessible_ref_child;
    klass->initialize = scatola_icon_container_accessible_initialize;

    accessible_private_data_quark = g_quark_from_static_string ("icon-container-accessible-private-data");
}

static GType
scatola_icon_container_accessible_get_type (void)
{
    static GType type = 0;
    ...
    if (!type)
    {
        ...
        {
            (GInterfaceInitFunc) scatola_icon_container_accessible_action_interface_init,
        ...
        {
            (GInterfaceInitFunc) scatola_icon_container_accessible_selection_interface_init,
            (GInterfaceFinalizeFunc) NULL,
            NULL
        };
        ...
        type = eel_accessibility_create_derived_type
               ("ScatolaIconContainerAccessible",
                EEL_TYPE_CANVAS,
                scatola_icon_container_accessible_class_init);
...
void
scatola_self_check_icon_container (void)
{
...
gboolean
scatola_icon_container_is_layout_rtl (ScatolaIconContainer *container)
{
    g_return_val_if_fail (SCATOLA_IS_ICON_CONTAINER (container), 0);

    return container->details->layout_mode == SCATOLA_ICON_LAYOUT_T_B_R_L ||
           container->details->layout_mode == SCATOLA_ICON_LAYOUT_R_L_T_B;
}

gboolean
scatola_icon_container_is_layout_vertical (ScatolaIconContainer *container)
{
    g_return_val_if_fail (SCATOLA_IS_ICON_CONTAINER (container), FALSE);

    return (container->details->layout_mode == SCATOLA_ICON_LAYOUT_T_B_L_R ||
            container->details->layout_mode == SCATOLA_ICON_LAYOUT_T_B_R_L);
}

int
scatola_icon_container_get_max_layout_lines_for_pango (ScatolaIconContainer  *container)
{
    int limit;
...
int
scatola_icon_container_get_max_layout_lines (ScatolaIconContainer  *container)
{
    int limit;
...
void
scatola_icon_container_begin_loading (ScatolaIconContainer *container)
{
    gboolean dummy;

    if (scatola_icon_container_get_store_layout_timestamps (container))
    {
...
static void
store_layout_timestamps_now (ScatolaIconContainer *container)
{
    ScatolaIcon *icon;
    GList *p;
    gboolean dummy;
...
void
scatola_icon_container_end_loading (ScatolaIconContainer *container,
                                 gboolean               all_icons_added)
{
    if (all_icons_added &&
            scatola_icon_container_get_store_layout_timestamps (container))
    {
...
gboolean
scatola_icon_container_get_store_layout_timestamps (ScatolaIconContainer *container)
{
...
void
scatola_icon_container_set_store_layout_timestamps (ScatolaIconContainer *container,
