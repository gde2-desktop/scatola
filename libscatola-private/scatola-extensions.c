...
#define SCATOLA_EXTENSION_GROUP "Scatola Extension"

static GList *scatola_extensions = NULL;
...
extension_new (gchar *filename, gboolean state, gboolean python, GObject *module)
{
    ...
    extension_filename = g_strdup_printf(SCATOLA_DATADIR "/extensions/%s.scatola-extension", filename);
    if (g_key_file_load_from_file (extension_file, extension_filename, G_KEY_FILE_NONE, NULL))
    {
        ext->name = g_key_file_get_locale_string (extension_file, SCATOLA_EXTENSION_GROUP, "Name", NULL, NULL);
        ext->description = g_key_file_get_locale_string (extension_file, SCATOLA_EXTENSION_GROUP, "Description", NULL, NULL);
        ext->icon = g_key_file_get_string (extension_file, SCATOLA_EXTENSION_GROUP, "Icon", NULL);
        ext->author = g_key_file_get_string_list (extension_file, SCATOLA_EXTENSION_GROUP, "Author", NULL, NULL);
        ext->copyright = g_key_file_get_string (extension_file, SCATOLA_EXTENSION_GROUP, "Copyright", NULL);
        ext->version = g_key_file_get_string (extension_file, SCATOLA_EXTENSION_GROUP, "Version", NULL);
        ext->website = g_key_file_get_string (extension_file, SCATOLA_EXTENSION_GROUP, "Website", NULL);
    }
    ...
    if (python)
    {
        ...
        ext->description = "Python-scatola extension";
    }
...
gsettings_key_has_value (const gchar *value)
{
    ...
    list = g_settings_get_strv (scatola_extension_preferences, 
                                SCATOLA_PREFERENCES_DISABLED_EXTENSIONS);
...
gsettings_append_to_list (const char *value)
{
    ...
    current = g_settings_get_strv (scatola_extension_preferences, 
                                   SCATOLA_PREFERENCES_DISABLED_EXTENSIONS);
    ...
    retval = g_settings_set_strv (scatola_extension_preferences, 
                                  SCATOLA_PREFERENCES_DISABLED_EXTENSIONS,
                                 (const gchar **) new);
...
static gboolean
scatola_extension_get_state (const gchar *extname)
{
    return !gsettings_key_has_value (extname);
}

GList *
scatola_extensions_get_for_type (GType type)
{
    GList *l;
    GList *ret = NULL;

    for (l = scatola_extensions; l != NULL; l = l->next)
    {
        Extension *ext = l->data;
        ext->state = scatola_extension_get_state (ext->filename);
...
GList *
scatola_extensions_get_list (void)
{
    return scatola_extensions;
}

void
scatola_extension_register (gchar *filename, GObject *module)
{
    ...
    ext_state = scatola_extension_get_state (ext_filename);
    ...
    scatola_extensions = g_list_append (scatola_extensions, ext);
}

gboolean
scatola_extension_set_state (Extension *ext, gboolean new_state)
{
    if (ext)
    {
